...
public class GtnReportingComparisonBreakdownMassUpdateAction  implements GtnUIFrameworkActionShareable, GtnUIFrameWorkAction, GtnUIFrameworkDynamicClass{
...
/*	         String sourceComponentId = GtnUIFrameworkGlobalUI.getVaadinViewComponentData(componentId)
	              ...
	              String comparisonBreakdownValues = GtnUIFrameworkGlobalUI.getVaadinBaseComponent(actionParameterList.get(1).toString())
	                      .getCaptionFromV8ComboBox();
	              String comparisonBreakdownComparisonComboBox = GtnUIFrameworkGlobalUI.getVaadinBaseComponent(actionParameterList.get(2).toString())
	                      .getStringCaptionFromV8ComboBox();
	              String comparisonBreakdownStartPeriod = GtnUIFrameworkGlobalUI.getVaadinBaseComponent(actionParameterList.get(3).toString())
	                      .getCaptionFromV8ComboBox();
	              String comparisonBreakdownEndPeriod = GtnUIFrameworkGlobalUI.getVaadinBaseComponent(actionParameterList.get(4).toString())
	                      .getCaptionFromV8ComboBox();
	              ...
	              for (int i = 1; i < grid.getHeaderRowCount(); i++) {
	                  ...
	                  if (projectionNames.getValue().equalsIgnoreCase(comparisonBreakdownComparisonComboBox)) {
	                      for (int k=0;k<gridColumnIdSubList.size();k++) {
	                    	  int size = gridColumnIdSubList.size();
