...
public class SampleMongoTest {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// MongoCollection<GtnWsMongodbConnectionBean> ins =
		// instance.getCollection("testcustom",
	...
	public void insert(MongoCollection<Document> instance) {
		...
		add.append("projectiondetails",
				new Document("periodSid", 601).append("exfactoryactuals", 10000).append("exfactoryprojection", 20000));
		instance.insertOne(add);
	...
	// public void searchAllTest(MongoCollection<GtnWsMongodbConnectionBean>
	// instance) {
	// FindIterable itr = instance.find();
	// MongoCursor cr = itr.iterator();
	// while (cr.hasNext()) {
	...
	public void aggregation(MongoCollection<Document> instance) {
		...
		groupFields.put("_id", new Document("Semi Annual", "$projectionDetailsValues.semiAnnual").append("Year",
				"$projectionDetailsValues.year"));
		groupFields.put("contractSalesActuals", new Document("$sum", "$projectionDetailsValues.contractSalesActuals"));
		Document group = new Document("$group", groupFields);
		Document project = new Document("$project", new Document("_id", 1).append("Total",
				new Document("$add", "$contractSalesActuals,$contractSalesProjection")));
