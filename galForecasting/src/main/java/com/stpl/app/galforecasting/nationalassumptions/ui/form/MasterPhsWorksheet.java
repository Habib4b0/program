...
import com.stpl.app.galforecasting.nationalassumptions.logic.tablelogic.PhsWorksheetTableLogic;
...
public class MasterPhsWorksheet extends Window {
     ...
     * @param projSelection
     */
    public MasterPhsWorksheet(ProjectionSelectionDTO projSelection) {
        super("Master PHS Worksheet");
        LOGGER.info("MasterPhsWorksheet Constructor initiated ");
        ...
        init();
        LOGGER.info("MasterPhsWorksheet Constructor Ended ");
    }
    ...
    @UiHandler("generate")
    public void generate(Button.ClickEvent event) throws Exception {
        if (checkSelection()) {
            ...
            if (formatFlag) {
                ...
                if (!adjustedValues.isEmpty() || !editedNotes.isEmpty()) {
                    if (!adjustedValues.isEmpty()) {
                        queryUtil.saveNotes(adjustedValues, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId());
                    ...
                    if (!editedNotes.isEmpty()) {
                        queryUtil.saveNotes(editedNotes, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId());
                        editedNotes.clear();
                    }
                    loadResultTable();
                }
    ...
    void loadNdc() {
        ...
        ndcDdlb.markAsDirty();
//            ndcContainer = new LazyContainer(HelperDTO.class, new NdcContainer(), new NdcCriteria());
    ...
    public void excelBtn(final Button.ClickEvent event) {
        ...
        ExcelExport exp = new ExcelExport(new ExtCustomTableHolder(exceltable), "Master PHS Worksheet", "Master PHS Worksheet", "Master PHS Worksheet.xls", false);
        exp.export();
    ...
    private void loadExcelResultTable() {
        ...
        List<TableDTO> resultList = phsResLogic.getPHSWorksheet(projectionDTO);
    ...
    public void addLowerLevelsForExport(TableDTO id) {
        ...
        List<TableDTO> resultList = phsResLogic.getPhsWorksheetChild(projectionDTO, id.getItemMasterSid());
    ...
    private void callAdjustmentProcedure() {
        ...
        try {
            ...
            medLogic.workSheetSetupCook(projectionDTO.getNdcSid().getId(), priceType, "PHS WORKSHEET", StringUtils.EMPTY);
        } catch (Exception ex) {
            LOGGER.error(ex.getMessage());
        }
    }
    ...
    public void submit(Button.ClickEvent event) {
        if (submitMsg) {
            new AbstractNotificationUtils() {
                ...
                public void yesMethod() {
                    try {
                        fcpQueryUtil.updateAdjustment(projectionDTO.getNdcSid().getId(), "getPhsAdjSumbitUpdate");
                        submitFlag = true;
                        submitMsg = false;
                    } catch (Exception ex) {
                        LOGGER.error(ex.getMessage());
    ...
    public void closeLogic() {
        try {
            fcpQueryUtil.updateAdjustment(projectionDTO.getNdcSid().getId(), "getPhsAdjCloseUpdate");
        } catch (Exception ex) {
            LOGGER.error(ex.getMessage());
        }
    }
