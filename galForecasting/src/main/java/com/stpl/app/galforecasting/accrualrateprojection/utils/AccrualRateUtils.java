 ...
 */
package com.stpl.app.galforecasting.accrualrateprojection.utils;
...
public class AccrualRateUtils {
     ...
     */
    public static void createHeadersForDetails(final int historyStartindex, final int filterStartindex, int filterEndIndex, final CustomTableHeaderDTO tableHeaderDTO, final AccrualRateSelectionDTO accrualRateSelectionDTO) {
        ...
        for (int i = historyStartindex; i < 0; i++) {
            ...
            periodBasisQuarter = generatePeriodBasis(accrualRateSelectionDTO.getPeriodBasis(), (month + 1) % 3, ((month) / 3 + 1));
            tableHeaderDTO.addSingleColumn(column, Q + (periodBasisQuarter == 5 ? 1 : periodBasisQuarter) + SPACE + (periodBasisQuarter == 5 ? year + 1 : year), Object.class);
    ...
    private static int generatePeriodBasis(String periodBasis, final int reminder, final int quarter) {
        ...
        switch (periodBasis) {
            ...
            case AccrualRateUtils.MONTH_3_FORWARD_QUARTER:
                ...
                break;
            default:
                LOGGER.info("Invalid Period Basis: " + periodBasis);
                value = quarter + 1;
                break;
        }
        return value;
    ...
    public static void createHeadersForSales(final int historyStartindex, int filterStartindex, int filterEndIndex, final CustomTableHeaderDTO tableHeaderDTO, final AccrualRateSelectionDTO accrualRateSelectionDTO, List<String> selectedValues) {
     ...
     * @param accrualRateSelectionDTO
     */
    public static void getRightHeadersForAccrual(final CustomTableHeaderDTO tableHeaderDTO, final Object historyValue, final AccrualRateSelectionDTO accrualRateSelectionDTO, final String screenName, final List<String> selectedValues) {
        int historyStartPeriod = historyValue == null || historyValue.equals(Constant.SELECT_ONE) ? -1 : Integer.valueOf(DASH + (historyValue.toString().substring(0, 1)));
        ...
        switch (screenName) {
            case RATES:
                createHeadersForRates(historyStartPeriod, filterStartindex, filterEndIndex, tableHeaderDTO, accrualRateSelectionDTO);
                break;
            case SALES:
                createHeadersForSales(historyStartPeriod, filterStartindex, filterEndIndex, tableHeaderDTO, accrualRateSelectionDTO, selectedValues);
                break;
            case DETAILS:
                createHeadersForDetails(historyStartPeriod, filterStartindex, filterEndIndex, tableHeaderDTO, accrualRateSelectionDTO);
                break;
        }
    }
     ...
     */
    public static void createHeadersForRates(final int historyStartindex, final int filterStartindex, int filterEndIndex, final CustomTableHeaderDTO tableHeaderDTO, final AccrualRateSelectionDTO accrualRateSelectionDTO) {
        DateFormatSymbols dateFormatSymbols = new DateFormatSymbols();
        String[] months = dateFormatSymbols.getShortMonths();
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.MONTH, historyStartindex);
    ...
    public static int getMonthDifference(final Calendar calendar) {
        ...
        int diffMonth = diffYear * 12 + calendar.get(Calendar.MONTH) - currentCalendar.get(Calendar.MONTH);
        return diffMonth;
    }
