...
public class UserGrpBsnsRoleLogic extends BeanItemContainer<BusinessroleMasterDTO>
   ...
   UserGrpBsnsRoleLogicDAO dao=new UserGrpBsnsRoleLogicDAOImpl();
    ...
    public UserGrpBsnsRoleDTO getBsnsRoles(UserGrpBsnsRoleDTO usrDto)
    {
    	...
    	ugbrmDynamicQuery.add(RestrictionsFactoryUtil.like("usergroupId", Integer.valueOf(String.valueOf(userGroupId))));
    	...
    	while (iterate1.hasNext()) {
	    	...
	    	if(ugbrm.size()>0)
	    	{
		    	...
		    	brmDynamicQuery.add(RestrictionsFactoryUtil.in("businessroleMasterSid", brmIdList));
    ...
    public boolean saveLogic(UserGrpBsnsRoleDTO usrDto)
    {
    	...
    	deleteDynamicQuery.add(RestrictionsFactoryUtil.eq("usergroupId", selUsrGrpId));
    	Criterion cri = RestrictionsFactoryUtil.in("businessroleMasterSid", idList);
    	...
    	for (Iterator iterator = idList.iterator(); iterator.hasNext();) {
    		...
    		checkDynamicQuery.add(RestrictionsFactoryUtil.eq("usergroupId", selUsrGrpId));
    		checkDynamicQuery.add(RestrictionsFactoryUtil.eq("businessroleMasterSid", brId));
