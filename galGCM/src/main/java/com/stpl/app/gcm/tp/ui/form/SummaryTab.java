...
public class SummaryTab extends VerticalLayout {
    ...
    public GridLayout selCriteriaGrid = new GridLayout(10, 1);
    ...
    public void configureFields() {
        ...
        if (ADD_TRADING_PARTNER.getConstant().equalsIgnoreCase(session.getModuleName())) {
            ...
            if (TRADING_PARTNER_REMOVE.getConstant().equals(session.getModuleName()) || TRADING_PARTNER_UPDATE.getConstant().equals(session.getModuleName())) {
                ...
                tabSheet.addTab(rebateTab, REBATES_PROJECTION.getConstant(), null, 2);
                tabSheet.addTab(salesRebateTab, SALES_AND_REBATES_PROJECTION.getConstant(), null, 3);
                    ...
                    public void buttonClicked(final ButtonId buttonId) {
                        ...
                        if (buttonId.name().equals(idString)) {
                            ...
                            try {
                                ...
                                boolean tpFlag = true;
                                if (ttpForm != null) {
                                    tpFlag = ttpForm.isSalesCopyFlag();
                                }
                                
                                if (projectionId != 0 && tpFlag) {
                                    ...
                                    if (session.getModuleName().equalsIgnoreCase(TRADING_PARTNER_REMOVE.getConstant()) || session.getModuleName().equalsIgnoreCase(TRADING_PARTNER_UPDATE.getConstant())) {
                                        tempList = common.createNewProjection(projectionId, session.getCompanyMasterSids(), true, session);
                                        if (tempList != null && !tempList.isEmpty()) {
                                            projectionCreationFlag = true;
                                        }

                                    } else if (session.getModuleName().equalsIgnoreCase(ADD_TRADING_PARTNER.getConstant())) {
                                        tempList = common.generateNewProjection(session.getUserId(), session.getSessionId(), projectionId, session.getCompanyMasterSids(), true, true);
                                        if (tempList != null && !tempList.isEmpty()) {
                                            projectionCreationFlag = true;
                                        }
                                        ...
                                        tempList = common.copyTempProjection(fromProjectionId, false, null, null, null);
                                        if (tempList != null && !tempList.isEmpty()) {
                                            int copyFromProjectionId = Integer.parseInt(tempList.get(2));
                                            ...
                                            if (toProjectionId != 0) {
                                                if (session.getModuleName().equalsIgnoreCase(TRANSFER_TRADING_PARTNER.getConstant())) {
                                                    tempTransferList = common.generateNewProjection(session.getUserId(), session.getSessionId(), toProjectionId, session.getCompanyMasterSids(), true, false);
                                                } else {
                                                    tempTransferList = common.copyProjection(toProjectionId, false, null, null, null);
                                                }
                                                copyToProjectionId = Integer.parseInt(tempTransferList.get(2));
                                            } else {
                                            }
                                            ...
                                            CommonLogic.insertInputsBeforeTranfer(fromProjectionId, copyFromProjectionId, toProjectionId, copyToProjectionId, sourceContract,
                                                    destinationContract, companies, fromCustomerEndDate, toCustomerStartDate, ttpForm.isSalesRemoveFlag(), session.getSessionId());
