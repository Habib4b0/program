...
FROM (
	...
	JOIN RS_CONTRACT RM ON RM.RS_MODEL_SID = NMDP.RS_MODEL_SID
...
LEFT JOIN #TEMP_HIER_SALES TH ON TH.HIERARCHY_NO = FD.HIERARCHY_NO
LEFT OUTER JOIN #TEMP_HIER_DISC THd ON THD.HIERARCHY_NO = FD.HIERARCHY_NO
...
LEFT OUTER JOIN #TEMP_HIER_DISC TH ON TH.HIERARCHY_NO = FD.HIERARCHY_NO
WHERE EXISTS (
		SELECT 1
...
LEFT JOIN #TEMP_HIER_DISC TD ON TD.HIERARCHY_NO = FD.HIERARCHY_NO;
...
--			 JOIN (SELECT DISTINCT CFF_MASTER_SID,ITEM_MASTER_SID,HIERARCHY_NO
--			,LEVEL_NAME
--			,PARENT_HIERARCHY_NO FROM #TEMP_CCP) B
...
IF Object_id('TEMPDB..#PRODUCT_FILE_DATA') IS NOT NULL
	...
	SELECT P.YEAR
			...
			INTO #PRODUCT_FILE_DATA
		FROM CFF_PRODUCT_LEVEL_FILES_DATA CF
			 JOIN (SELECT DISTINCT a.CFF_MASTER_SID,a.ITEM_MASTER_SID,a.HIERARCHY_NO
			,a.LEVEL_NAME
			,a.PARENT_HIERARCHY_NO,b.DISCOUNT
	FROM #TEMP_CCP A
	JOIN #CURRENT_RS_DATA_PERIOD B ON A.CFF_MASTER_SID = B.CFF_MASTER_SID
		AND A.HIERARCHY_NO = B.HIERARCHY_NO
		AND A.LEVEL_NAME = B.LEVEL_NAME
...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN #TEMP_HIER_SALES TS ON TS.HIERARCHY_NO = RD.HIERARCHY_NO
		LEFT JOIN #TEMP_HIER_DISC TD ON TD.HIERARCHY_NO = ND.HIERARCHY_NO
		...
		LEFT JOIN #TEMP_HIER_SALES TS ON TS.HIERARCHY_NO = RD.HIERARCHY_NO
		LEFT JOIN #TEMP_HIER_DISC TD ON TD.HIERARCHY_NO = PD.HIERARCHY_NO
	END
	ELSE
	BEGIN
		INSERT INTO #RESULT (
...
BEGIN
	...
	LEFT JOIN #TEMP_HIER_SALES TS ON TS.HIERARCHY_NO = MD.HIERARCHY_NO
	LEFT JOIN #TEMP_HIER_DISC TD ON TD.HIERARCHY_NO = MD.HIERARCHY_NO
	...
	LEFT JOIN #TEMP_HIER_SALES TS ON TS.HIERARCHY_NO = sd.HIERARCHY_NO
	LEFT JOIN #TEMP_HIER_DISC TD ON TD.HIERARCHY_NO = SD.HIERARCHY_NO
END
...
WHERE  EXISTS (
	...
	JOIN NM_SALES_PROJECTION NSP ON NMDP.PROJECTION_DETAILS_SID = NSP.PROJECTION_DETAILS_SID
		...
		AND PRD.PERIOD_SID BETWEEN @PROJ_START_PERIOD_SID
			AND @PROJ_END_PERIOD_SID
	JOIN #PRIOR_DISC_INFORMATION A ON A.CFF_MASTER_SID = PRD.CFF_MASTER_SID
		AND PRD.PROJECTION_DETAILS_SID = A.PROJECTION_DETAILS_SID
		AND PRD.DISCOUNT = A.DISCOUNT
	JOIN CCP_DETAILS CD ON CD.CCP_DETAILS_SID = a.CCP_DETAILS_SID
	LEFT JOIN #ITEM_PRICING U ON CD.ITEM_MASTER_SID = U.ITEM_MASTER_SID
	...
	LEFT JOIN #TEMP_PRIOR_HIER_SALES TS ON TS.CFF_MASTER_SID = FD.CFF_MASTER_SID
		AND TS.HIERARCHY_NO = FD.HIERARCHY_NO
	LEFT OUTER JOIN #TEMP_PRIOR_HIER_DISC TD ON TD.HIERARCHY_NO = FD.HIERARCHY_NO
	...
	LEFT JOIN #PRIOR_M_ACTUAL_DISCOUNT MAD ON MAD.YEAR = FD.YEAR
		...
		AND MAD.LEVEL_NAME = fd.LEVEL_NAME
		AND COALESCE(MAD.PARENT_HIERARCHY_NO, MAD.HIERARCHY_NO) = COALESCE(FD.PARENT_HIERARCHY_NO, FD.HIERARCHY_NO)
	LEFT JOIN #TEMP_PRIOR_HIER_DISC TD ON TD.HIERARCHY_NO = FD.HIERARCHY_NO
		AND FD.CFF_MASTER_SID = TD.CFF_MASTER_SID;
	...
	IF Object_id('TEMPDB..#PRIOR_PRODUCT_FILE_DATA') IS NOT NULL
		...
		FROM CFF_PRODUCT_LEVEL_FILES_DATA CF
			 JOIN (SELECT DISTINCT CFF_MASTER_SID,ITEM_MASTER_SID,HIERARCHY_NO
			,LEVEL_NAME
			,PARENT_HIERARCHY_NO FROM #PRIOR_TEMP_CCP) B
			 ON CF.CFF_MASTER_SID=B.CFF_MASTER_SID
			 AND CF.ITEM_MASTER_SID=B.ITEM_MASTER_SID
	INNER JOIN #PERIOD P ON P.PERIOD_SID = CF.PERIOD_SID
		GROUP BY P.YEAR
	...
	BEGIN
		...
		BEGIN
			...
			JOIN RS_CONTRACT RC ON RS_CONTRACT_SID = ND.RS_NAME
			LEFT JOIN #TEMP_PRIOR_HIER_SALES TPS ON TPS.HIERARCHY_NO = RD.HIERARCHY_NO
				AND TPS.CFF_MASTER_SID = RD.CFF_MASTER_SID
			LEFT JOIN #TEMP_PRIOR_HIER_DISC TPD ON TPD.HIERARCHY_NO = ND.HIERARCHY_NO
			...
			LEFT JOIN #TEMP_PRIOR_HIER_SALES TPS ON TPS.HIERARCHY_NO = RD.HIERARCHY_NO
				AND TPS.CFF_MASTER_SID = RD.CFF_MASTER_SID
			LEFT JOIN #TEMP_PRIOR_HIER_DISC TPD ON TPD.HIERARCHY_NO = PD.HIERARCHY_NO
	...
	BEGIN
		...
		LEFT JOIN #TEMP_PRIOR_HIER_SALES TPS ON TPS.HIERARCHY_NO = RD.HIERARCHY_NO
			AND TPS.CFF_MASTER_SID = RD.CFF_MASTER_SID
		LEFT JOIN #TEMP_PRIOR_HIER_DISC TPD ON TPD.HIERARCHY_NO = MD.HIERARCHY_NO
		...
		LEFT JOIN #TEMP_PRIOR_HIER_SALES TPS ON TPS.HIERARCHY_NO = RD.HIERARCHY_NO
			AND TPS.CFF_MASTER_SID = RD.CFF_MASTER_SID
		LEFT JOIN #TEMP_PRIOR_HIER_DISC TPD ON TPD.HIERARCHY_NO = SD.HIERARCHY_NO
...
DISCOUNT_OF_EX_FACTORY_ACTUALS_PERCENT_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR 
			AS VARCHAR(10)) + ' THEN DISCOUNT_OF_EX_FACTORY_ACTUALS_PERCENT END), 
