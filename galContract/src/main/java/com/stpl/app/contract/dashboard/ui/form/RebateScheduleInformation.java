...
public class RebateScheduleInformation extends CustomComponent {
     ...
     */
    private String rebatePlanLevelId;
    ...
    private String[] HISTORY_HEADER = {"Rebate Schedule ID", "Rebate Schedule No", "Rebate Schedule Name", "Rebate Schedule Status", "Rebate Schedule Type", "Rebate Program Type", "Rebate Schedule Category",
        "RS Trade Class", "RS Designation", "Rebate Schedule Alias ID", "UDC 1", "UDC 4", "Parent Rebate Schedule ID", "RS Transaction Ref ID", "UDC 2", "UDC 5", "Parent RS Name",
        "RS Transaction Ref Name", "UDC 3", "UDC 6", "Deduction Inclusion", "Calendar", "Rebate Frequency", "Payment Level", "Payment Frequency", "Payment Grace Period",
        "Payment Terms", "Payment Method", "Interest Bearing Basis", "Evaluation Rule Level", "Evaluation Rule Type", "Evaluation Rule/Association", "Interest Bearing Indicator",
        "Calculation Rule Level", "Calculation Type", "Calculation Rule", "Calculation Level", "Modified Date"};
    ...
    private String[] PROCESSING_HEADER = {"Rebate Processing Type", "Validation Profile", "Interest Bearing Indicator", "Interest Bearing Basis", "Selected Rule No", "Selected Rule Name", "Selected Rule Version"};
    ...
    public RebateScheduleInformation(final RsItemDetailsDTO rsItemDetailsDTO, final CustomFieldGroup rebateBinder, final BeanItemContainer<TempRebateDTO> rsDetailsResultsBean,
            final CustomFieldGroup contractBinder, final boolean isEditable, final SessionDTO sessionDTO) throws SystemException, PortalException, Exception {
        super();
        ...
        init();
        if (rebateSetup == null) {
            LOGGER.info("Initialising RebateSetup");
            try {
                rebateSetup = new RebateSetup(rsItemDetailsDTO, rebateBinder, rsDetailsResultsBean, contractBinder, isEditable, this.sessionDTO);
            } catch (Exception ex) {
    ...
    private void addResponsiveTab() throws PortalException, SystemException, Exception {
     ...
     */
    private void configureProcessingOption() throws Exception {
        final StplSecurity stplSecurity = new StplSecurity();
        IfpLogic ifpLogic = new IfpLogic(sessionDTO);
        final Map<String, AppPermission> contractDashboard = stplSecurity.getFieldOrColumnPermission(userId, UISecurityUtil.CONTRACT_DASHBOARD + Constants.COMMA + Constants.RS_INFORMATION, false);
        List<Object> resultList = contractLogic.getFieldsForSecurity(UISecurityUtil.CONTRACT_DASHBOARD, Constants.RS_INFORMATION);
    ...
    public void configureFields() throws SystemException, Exception {
        ...
        try {
            ...
            rebateScheduleId.addValidator(new RegexpValidator("([0-9|a-z|A-Z|\\.|\\,|\\_|\\-|\\@|\\#|\\$|\\&|\\%|\\s|\\/|\\(|\\!|\\)])*", "Allowed Special characters are @,#,.,%,$,&,_,-,(,),/,!"));
            ...
            rebateScheduleId.addValidator(new StringLengthValidator(" Rebate Schedule ID should be less than 38 Characters", 0, 38, true));
            ...
            rebateScheduleNo.addValidator(new RegexpValidator("([0-9|a-z|A-Z|\\.|\\,|\\_|\\-|\\@|\\#|\\$|\\&|\\%|\\s|\\/|\\(|\\!|\\)])*", "Allowed Special characters are @,#,.,%,$,&,_,-,(,),/,!"));
            ...
            rebateScheduleNo.addValidator(new StringLengthValidator(" Rebate Schedule No should be less than 50 Characters", 0, 50, true));
            ...
            rebateScheduleName.addValidator(new RegexpValidator("([0-9|a-z|A-Z|\\.|\\,|\\_|\\-|\\@|\\#|\\$|\\&|\\%|\\s|\\/|\\(|\\!|\\)])*", "Allowed Special characters are @,#,.,%,$,&,_,-,(,),/,!"));
            ...
            rebateScheduleName.addValidator(new StringLengthValidator("Rebate Schedule Name should be less than 100 Characters", 0, 100, true));
            ...
            paymentGracePeriod.addValidator(new StringLengthValidator("Rebate Schedule Reference Name should be less than 10 Characters", 0, 10, true));
            ...
            rebateScheduleAlias.addValidator(new StringLengthValidator("Rebate Schedule Alias length should be less than 50 characters", 0, 50, true));
            ...
            rebateScheduleAlias.addValueChangeListener(new Property.ValueChangeListener() {
                ...
                @SuppressWarnings("PMD")
                public void valueChange(final ValueChangeEvent event) {
                    LOGGER.info("valueChange(ValueChangeEvent event) rebateScheduleAlias=" + rebateScheduleAlias.getValue());
    ...
    public class DateValidator extends AbstractValidator {
        ...
        public void validate(final Object value) throws Validator.InvalidValueException {
            ...
            if (dates.length > 0) {
                ...
                if ("EndDate".equals(field) && itemRebateEndDate.getValue() != null) {
                    if (dates[1] != null && itemRebateEndDate.getValue().after((Date) dates[1])) {
            ...
            public void valueChange(ValueChangeEvent event) {
                try {
                    if (valueChange) {
                        ...
                        } else if (component.equals(Constants.END_DATE_CAPS)) {
                            if (event != null) {
                                Date contractSD = (Date) dates[0];
