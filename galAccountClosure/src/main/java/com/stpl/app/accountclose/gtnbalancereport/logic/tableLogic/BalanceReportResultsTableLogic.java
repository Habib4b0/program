 ...
 * @author santanukumar
 */
public class BalanceReportResultsTableLogic extends PageTreeTableLogic {

    boolean firstGenerated = false;
    SessionDTO session;
    BalanceReportDTO balanceReportDTO;
    List<String> selectedVariables;
    List<String> doubleHeaderList;
    private BalanceReportLogic balanceRepLogic=new BalanceReportLogic();
    public static final Logger LOGGER = Logger.getLogger(BalanceReportResultsTableLogic.class);
    ...
    public Map<Integer,Object> loadData(int start, int offset) {
       ...
       List<BalanceReportDTO> list = new ArrayList<BalanceReportDTO>();
        if (firstGenerated) {
            list=balanceRepLogic.getBalanceReportResults(getLastParent(),start, offset, session,balanceReportDTO,selectedVariables ,true,doubleHeaderList, false);
        }
          int i=start;
    ...
    public int getCount(){
        int count = 0;
        if (firstGenerated) {
            count=balanceRepLogic.getBalanceReportCount(getLastParent(), session,balanceReportDTO,selectedVariables, true);
        }
        return count;
    }
   ...
   @Override
    ...
    public void setData(SessionDTO session, BalanceReportDTO balanceReportDTO,List<String> selectedVariables,boolean flag,List<String> doubleHeaderList) {
    ...
    protected void recursivelyLoadExpandData(Object parentId, String treeLevel, BalanceReportDTO queryDto, int expandLevelNo) {
        ...
        int count = balanceRepLogic.getExpandLevelCount(parentId, session, queryDto, selectedVariables, false);
        ...
        if ((expandLevelNo) > (levelNo+1)) {
            List<BalanceReportDTO> list = balanceRepLogic.getExpandLevelResult(parentId, 0, count, session, queryDto, selectedVariables, false,doubleHeaderList);
