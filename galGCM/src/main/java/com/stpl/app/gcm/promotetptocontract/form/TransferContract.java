...
//empty
    ...
    protected void initializeTabs() {
        ...
        try {
            ...
            tabSheet.addTab(existingComponents, Constants.IndicatorConstants.TAB_PROMOTE_TP_EXISTING_COMPONENTS.getConstant(), null, 2);
    ...
    public void populateBtn1Logic(Button.ClickEvent event) throws ParseException, SystemException {
        ...
        if (StringUtils.EMPTY.equals(cName) || StringUtils.EMPTY.equals(cNo) || Constants.NULL.equals(sDate) || Constants.NULL.equals(eDate) || mType.equals(Constants.NULL) || StringUtils.EMPTY.equals(cHolder)
          ...
          if(cfpNoFlag){
            ...
            if (tabPosition == 1) {
                if (contractDashBoardTable1.getItemIds().isEmpty()) {
            ...
            if (tabPosition == 0) {
                if (contractDashBoardTable2.getItemIds().isEmpty()) {
    ...
    public void saveContract() {
         ...
         if (tabPosition == 0) {
             try {
                 ...
                 if(list!=null && list.size()>0){
                     contractId=String.valueOf(list.get(0));
                     companyId=String.valueOf(list.get(1));
                 }
                 Boolean flag=logic.callCcpInsertProcedure();
                 SummaryTPDetails tpLogic=new SummaryTPDetails();
             ...
             try {
                 List<Integer> list=existingComponents.saveExistingContract();
             } catch (SystemException ex) {
                 java.util.logging.Logger.getLogger(TransferContract.class.getName()).log(Level.SEVERE, null, ex);
             } catch (PortalException ex) {
                 java.util.logging.Logger.getLogger(TransferContract.class.getName()).log(Level.SEVERE, null, ex);
      ...
      if (tabPosition == 1) {
             try {
                   List<Integer> list=newComponents.saveNewContract();
             } catch (SystemException ex) {
                 java.util.logging.Logger.getLogger(TransferContract.class.getName()).log(Level.SEVERE, null, ex);
             } catch (PortalException ex) {
                 java.util.logging.Logger.getLogger(TransferContract.class.getName()).log(Level.SEVERE, null, ex);
