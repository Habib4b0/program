 ...
 */
package com.stpl.app.galworkflow.logic;
...
import com.stpl.app.galworkflow.util.AbstractFilterLogic;
...
import com.stpl.app.galworkflow.util.QueryUtils;
...
public class WorkflowLogic {
    ...
    private WorkFlowDAO dao = WorkFlowImpl.getInstance();
    ...
    public String getWorkflowSearchQuery(InboxDashboardDTO inboxdto, final int start, final int offset, final List<SortByColumn> columns, final Set<Container.Filter> filterSet, boolean isCount) {
        ...
        String deductionvalueBinder = inboxdto.getDeductionValue() != null ? String.valueOf(inboxdto.getDeductionValue().getId()) : StringUtils.EMPTY;
        ...
        query.append(CustomSQLUtil.get("wi.inbox_search"));
        ...
        if (!itemIdBinder.isEmpty() || !itemNoBinder.isEmpty() || !itemNameBinder.isEmpty()) {
            additionalJoins.append(" JOIN ITEM_MASTER IM ON IM.ITEM_MASTER_SID = CCP.ITEM_MASTER_SID");
            isItemSearchCriteriaPresent = true;
        }
        if (!deductionLevelBinder.isEmpty() && !deductionvalueBinder.isEmpty() && !"Accrual Rate Projection".equals(businessProcessBinder)) {
        ...
        if("Accrual Rate Projection".equals(businessProcessBinder)) {
            projDetailsTable = "ACCRUAL_PROJ_DETAILS ";
        } else if("Forecasting".equals(businessProcessBinder)) {
            projDetailsTable = "PROJECTION_DETAILS ";
        } else if("Returns".equals(businessProcessBinder)) {
            projDetailsTable = "RETURNS_PROJ_DETAILS ";
        }
        if (!"Contract Management".equals(businessProcessBinder) && (isContractSearchCriteriaPresent || isCompanySearchCriteriaPresent || isItemSearchCriteriaPresent || isDeductionSearchCriteriaPresent)) {
            query.append(" JOIN ").append(projDetailsTable).append("PD ON PD.PROJECTION_MASTER_SID = PM.PROJECTION_MASTER_SID ");
            if(!"Returns".equals(businessProcessBinder)){
        ...
        if (businessProcessBinder != null && !businessProcessBinder.isEmpty()) {
            ...
            switch (businessProcessBinder) {
                case "Forecasting":
                    ...
                    break;
                case "Accrual Rate Projection":
                    businessProcessKey = "AR%";
                    break;
        ...
        if (isDeductionSearchCriteriaPresent && !"Accrual Rate Projection".equals(businessProcessBinder)) {
        ...
        if (approvedFrom != null) {
            ...
            query.append(" AND CONVERT(DATE, WM.MODIFIED_DATE) >= '").append(formattedDate).append("'");
        }
        if (approvedTo != null) {
            ...
            for (Container.Filter filter : filterSet) {
                if (filter instanceof SimpleStringFilter) {
                    ...
                    if (!"null".equals(String.valueOf(stringFilter.getPropertyId()))) {
                        query.append(" AND ").append(getDbColumnName(String.valueOf(stringFilter.getPropertyId()),"Contract Management".equals(businessProcessBinder))).append(" like '").append(filterString).append("'");
                    }
                    ...
                    if ("creationDate".equals(betweenFilter.getPropertyId())) {
                        if (startValue != null) {
                    ...
                    } else if ("approvedDate".equals(betweenFilter.getPropertyId())) {
                        if (startValue != null) {
        ...
        if (!isCount) {
            ...
            if (columns != null && columns.size() > 0) {
                ...
                for (final Iterator<SortByColumn> iterator = columns.iterator(); iterator.hasNext();) {
                    ...
                    if(columnName.equalsIgnoreCase("approvedBy"))
    ...
    public static List getWorkflowIdForBusinessUnit(int businessUnit) {
        LOGGER.info(" Entering getWorkflowIdForBusinessUnit");
        ...
        LOGGER.info("Exiting getWorkflowIdForBusinessUnit");
        return resultList;
    }
    ...
    public List<WorkFlowHistoryLookupDTO> workFlowHistorySearch(int workFlowMasterSystemId, Set<Container.Filter> filters, List<SortByColumn> sortByColumns) {
        ...
        if (parameters.get("status") != null && !StringUtils.EMPTY.equals(String.valueOf(parameters.get("status")))) {
            query += " AND HWM.WORKFLOW_STATUS_ID = " + Integer.valueOf(String.valueOf(parameters.get("status")));
        }
        if (parameters.get("modifiedBy") != null && !StringUtils.EMPTY.equals(String.valueOf(parameters.get("modifiedBy")))) {
            query += " AND HWM.MODIFIED_BY = " + Integer.valueOf(String.valueOf(parameters.get("modifiedBy")));
        }
        if (parameters.get("notes") != null && !StringUtils.EMPTY.equals(String.valueOf(parameters.get("notes")))) {
            query += " AND HWM.NOTES LIKE " + String.valueOf(parameters.get("notes"));
        }
        if ((parameters.get("modifiedDatefrom") != null && !StringUtils.EMPTY.equals(String.valueOf(parameters.get("modifiedDatefrom"))))) {
            query += (" AND HWM.MODIFIED_DATE >='") + (String.valueOf(parameters.get("modifiedDatefrom"))) + ("'");
        }
        if ((parameters.get("modifiedDateto") != null && !StringUtils.EMPTY.equals(String.valueOf(parameters.get("modifiedDateto"))))) {
            query += (" AND HWM.MODIFIED_DATE <='") + (String.valueOf(parameters.get("modifiedDateto"))) + ("'");
        }
        else if (!columnName.isEmpty() && "status".equalsIgnoreCase(columnName)) {
            query += " ORDER BY HWM.WORKFLOW_STATUS_ID " + orderBy;
        }
        else if (!columnName.isEmpty() && "modifiedBy".equalsIgnoreCase(columnName)) {
            query += " ORDER BY HWM.MODIFIED_BY  " + orderBy;
        }
        else if (!columnName.isEmpty() && "notes".equalsIgnoreCase(columnName)) {
        ...
        LOGGER.info("workFlowHistorySearch" +query);
        try {
    ...
    private List<WorkFlowHistoryLookupDTO> getWorkFlowHistory(List resultList) {
        ...
        try {
            for (int loop = 0, limit = size; loop < limit; loop++) {
                ...
                workFlowHistoryLookupDTO.setStatus(helperListutil.getIdDescMap().get(Integer.parseInt(String.valueOf(objects[2]))));
                workFlowHistoryLookupDTO.setNotes(objects[3] != null ? String.valueOf(objects[3]) : StringUtils.EMPTY);
                workFlowHistoryLookupDTO.setCheckbox((String.valueOf(objects[4]).isEmpty()) || (String.valueOf(objects[4]).equals("null")) ? false : true);
                workFlowHistoryLookupDTO.setFileName(objects[4] != null ? String.valueOf(objects[4]) : StringUtils.EMPTY);
    ...
    public List<UserViewDTO> UserSearch(UserViewDTO userdto) {
        ...
        if (firstNameBinder != null && StringUtils.isNotEmpty(firstNameBinder)) {
            ...
            userSearchDynamicQuery.add(RestrictionsFactoryUtil.ilike("firstName", firstName));
        }
        if (lastNameBinder != null && StringUtils.isNotEmpty(lastNameBinder)) {
            lastName = lastNameBinder.replace("*", "%");
            userSearchDynamicQuery.add(RestrictionsFactoryUtil.ilike("lastName", lastName));
        }
        List<User> resultList;
        try {
            resultList = UserLocalServiceUtil.dynamicQuery(userSearchDynamicQuery);
    ...
    public int ViewResultsCount(final InboxDashboardDTO inboxDashboardDTO, int start, int offset, final List<SortByColumn> columns, final Set<Container.Filter> filterSet, boolean isCount) {
        LOGGER.info("Entering workflowtablelogic getWorkflowinboxCount");
    ...
    public List loadviewResults(final InboxDashboardDTO inboxDashboardDTO, int start, int offset, final List<SortByColumn> columns, final Set<Container.Filter> filterSet, boolean isCount) {
        LOGGER.info("Entering workflowtablelogic loadviewResults");
     ...
     * @param inboxDashboardDTO
     */
    public void viewInsertOrUpdate(boolean isInsert, InboxDashboardDTO inboxDashboardDTO) {
        if (isInsert) {
    ...
    private List getViewInput(InboxDashboardDTO inboxDashboardDTO, int start, int offset, final List<SortByColumn> columns, final Set<Container.Filter> filterSet, boolean isCount) {
        ...
        LOGGER.info("filter query" + filterQString);
    ...
    private List getCustomizedViewData(List<Object[]> searchResultsList) {
        ...
        for (int i = 0; i < searchResultsList.size(); i++) {
            ...
            searchDTO.setBusinessProcess(checkAndReturnString(object[2]));
            searchDTO.setWorkflowId(checkAndReturnString(object[3]));
            searchDTO.setWorkflowName(checkAndReturnString(object[4]));
            searchDTO.setWorkflowDescription(checkAndReturnString(object[5]));
            searchDTO.setCreationDateRangeFrom(checkAndReturnDate(object[6]));
            searchDTO.setCreationDateRangeTo(checkAndReturnDate(object[7]));
            searchDTO.setApprovedDateRangeFrom(checkAndReturnDate(object[8]));
            searchDTO.setApprovedDateRangeTo(checkAndReturnDate(object[9]));
            searchDTO.setCreatedBy(CommonUtils.getUserInfo(String.valueOf(object[10])));
            searchDTO.setApprovedBy(CommonUtils.getUserInfo(String.valueOf(object[11])));
            searchDTO.setContractId(checkAndReturnString(object[12]));
            searchDTO.setContractNo(checkAndReturnString(object[13]));
            searchDTO.setContractName(checkAndReturnString(object[14]));
            searchDTO.setContractType(checkAndReturnHelper(object[15]));
            searchDTO.setCompanyId(checkAndReturnString(object[16]));
            searchDTO.setCompanyNo(checkAndReturnString(object[17]));
            searchDTO.setCompanyName(checkAndReturnString(object[18]));
            searchDTO.setItemId(checkAndReturnString(object[19]));
            searchDTO.setItemNo(checkAndReturnString(object[20]));
            searchDTO.setItemName(checkAndReturnString(object[21]));
            searchDTO.setDeductionLevel(checkAndReturnString(object[22]));
            searchDTO.setDeductionValue(checkAndReturnHelper(object[23]));

            searchDTO.setWorkflowInboxSystemId(object[24] != null ? Integer.valueOf(String.valueOf(object[24])) : 0);
    ...
    private Map<String, String> getQueryMap() {
        ...
        viewsearchColumn.put("status", "WORKFLOW_STATUS");
        viewsearchColumn.put("businessProcess", "BUSINESS_PROCESS");
        viewsearchColumn.put("businessUnit", "BUSINESS_UNITS");
        viewsearchColumn.put("createdBy", "CREATED_BY");
        viewsearchColumn.put("approvedBy", "APPROVED_BY");
    ...
    private String getDbColumnName(String columnName,boolean isContract) {
        switch (columnName) {
            ...
            case "workflowName":
              return isContract ? "CM.CONTRACT_NAME" : "PM.PROJECTION_NAME";
