...
public class FileManagementLookup extends Window {
			...
			public void buttonClick(final ClickEvent event) {
				...
				if (detailsFlag == 'N') {
					AbstractNotificationUtils.getErrorNotification(CommonUtils.DETAILS_ERROR,
							CommonUtils.PLEASE_CLICK_RECORD);
					return;
				}
						...
						for (FileMananagementResultDTO obj : idList) {
							if (detailsFilterTable.removeItem(obj)) {
								if (addlineList.contains(obj)) {
									...
									obj.setCheck(BooleanConstant.getFalseFlag());
									addlineList.add(obj);
								}
							}
						}
		...
		resetBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(final Button.ClickEvent event) {
				...
				MessageBox.showPlain(Icon.QUESTION, ConstantsUtils.CONFORMATION,
						StringConstantUtils.ARE_YOU_SURE_YOU_WANT_TO_RESET + ConstantsUtils.QUESTION_MARK,
	...
	private void populateButtonLogic() {
		...
		populate.addClickListener(new ClickListener() {
			...
			public void buttonClick(final ClickEvent event) {
				...
				if (fieldName.getValue() == null) {
					...
					if (CommonUtil.getSelectedFileType(fmFileType).getDescription()
						...
						if (fieldName.getValue().equals(ConstantsUtils.CAPS_YEAR)
								|| fieldName.getValue().equals(ConstantsUtils.CAPS_MONTH)) {
							if (value.getValue() == null || value.getValue().equals(ConstantsUtils.EMPTY)) {
								AbstractNotificationUtils.getErrorNotification(ConstantsUtils.FIELD_ERROR,
										StringConstantUtils.PLEASE_ENTER_VALUE);
								return;
							}
						...
						if (fieldName.getValue().equals(ConstantsUtils.CAPS_PRICE)
								|| fieldName.getValue().equals(ConstantsUtils.CAPS_UNITS)) {
							if (value.getValue() == null || value.getValue().equals(ConstantsUtils.EMPTY)) {
								AbstractNotificationUtils.getErrorNotification(ConstantsUtils.FIELD_ERROR,
										StringConstantUtils.PLEASE_ENTER_VALUE);
							}
						...
						if (fieldName.getValue().equals(ConstantsUtils.CAPS_ITEMNO) && (itemNoSearch.getValue() == null
								|| itemNoSearch.getValue().equals(ConstantsUtils.EMPTY))) {
							AbstractNotificationUtils.getErrorNotification(ConstantsUtils.FIELD_ERROR,
						...
						if (fieldName.getValue().equals(StringConstantUtils.MARKET_SHARE_RATIO_LABEL)
								|| fieldName.getValue().equals(StringConstantUtils.MARKET_SIZE_UNITS_LABEL)
								|| fieldName.getValue().equals(StringConstantUtils.MARKET_SHARE_UNITS_LABEL)
								|| fieldName.getValue().equals(StringConstantUtils.UNCAPTURED_UNITS_LABEL)
								|| fieldName.getValue().equals(StringConstantUtils.TOTAL_DEMAND_UNITS_LABEL)
								|| fieldName.getValue().equals(StringConstantUtils.TOTAL_DEMAND_AMOUNT_LABEL)) {
							if (value.getValue() == null || value.getValue().equals(ConstantsUtils.EMPTY)) {
								AbstractNotificationUtils.getErrorNotification(ConstantsUtils.FIELD_ERROR,
										StringConstantUtils.PLEASE_ENTER_VALUE);
							}
							...
							.equals(ConstantsUtils.CUSTOMERGTS)
							&& (fieldName.getValue().equals(StringConstantUtils.DEDUCTION_RATE)
									|| fieldName.getValue().equals(StringConstantUtils.DEDUCTION_AMOUNT_LABEL)
									|| fieldName.getValue().equals(StringConstantUtils.PRICE_LABEL)
									|| fieldName.getValue().equals(StringConstantUtils.SALES_AMOUNT_LABEL)
						...
						for (int i = 0; i < itemIds.size(); i++) {
							...
							if (beanItem.getCheck()) {
								...
								if (fieldName.getValue().equals(ConstantsUtils.CAPS_YEAR)) {
									...
									if (detailsFilterTable.getContainerProperty(beanItem, ConstantsUtils.INTERFACE_FLAG)
											.getValue().equals("Y")) {
										detailsFilterTable.getContainerProperty(beanItem, ConstantsUtils.MONTH)
												.setValue(value.getValue());
									}
									...
									if (detailsFilterTable.getContainerProperty(beanItem, ConstantsUtils.INTERFACE_FLAG)
										...
										&& detailsFilterTable
												.getContainerProperty(beanItem, ConstantsUtils.INTERFACE_FLAG)
												.getValue().equals("Y")) {
									detailsFilterTable.getContainerProperty(beanItem, ConstantsUtils.START_DATE)
											.setValue(startDate.getValue());
								}
								if (fieldName.getValue().equals(ConstantsUtils.CAPS_PRICE)) {
				...
				if (flag) {
					AbstractNotificationUtils.getErrorNotification(ConstantsUtils.FIELD_ERROR,
							"Please Select Atleast one Record in details section");
				}
			}
		});
	}
	...
	protected void searchButtonClickLogic() throws ParseException {
		...
		if (StringUtils.isEmpty(fileName.getValue().trim()) && StringUtils.isEmpty(type.getValue().trim())
			...
			MessageBox.showPlain(Icon.ERROR, CommonUtils.SEARCH_ERROR,
			...
			CommonUIUtils.getMessageNotification(ConstantsUtils.NO_RESULSTS);
			 ...
			 if ((CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.DEMAND)
					...
					|| CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.CUSTOMERGTS))||
                                (CommonUtil.getSelectedFileType(fmFileType).getDescription()
					.equals(ConstantsUtils.ADJUSTED_DEMAND))|| (CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.EX_FACTORY_SALES))) {
				final FileManagementDTO fileManageDTO = fmLogic.getDetailsSumm(String.valueOf(fileNameList.getValue()),
						String.valueOf(versionList.getValue()), CommonUtil.getSelectedFileType(fmFileType),
	...
	private class DateValidator extends AbstractValidator {
		...
		public void validate(final Object value) {
			LOGGER.debug("validate Method started");
				if ((fromDate.getValue() != null && toDate.getValue() != null) && (fromDate.getValue().after(toDate.getValue()) || fromDate.getValue().equals(toDate.getValue()))) {
					toDate.setValue(null);
		...
		try {
			for (int i = 0; i < itemIds.size(); i++) {
				...
				for (int j = i + 1; j < itemIds.size(); j++) {
					...
					if (currentYear.equals(forecastType)
							&& CommonUtil.getSelectedFileType(fmFileType).getDescription()
									.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_SUMMARY)
					...
					if (beanItem.getForecastSystemId() == 0
							&& (CommonUtil.getSelectedFileType(fmFileType).toString().equals(ConstantsUtils.DEMAND)
									|| CommonUtil.getSelectedFileType(fmFileType).toString()
											.equals(ConstantsUtils.ADJUSTED_DEMAND)
									|| CommonUtil
											.getSelectedFileType(fmFileType).toString()
											.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_SUMMARY)
									|| CommonUtil.getSelectedFileType(fmFileType).toString()
											.equals(ConstantsUtils.CUSTOMERGTS)
									|| CommonUtil.getSelectedFileType(fmFileType).toString()
				...
				MessageBox.showPlain(Icon.QUESTION, ConstantsUtils.CONFORMATION,
						"Save record " + selectFile.getValue() + "?", new MessageBoxListener() {
							...
							public void buttonClicked(ButtonId buttonId) {
								...
								if (buttonId.name().equals(ConstantsUtils.YES)) {
									String finalVersion;
									final String maxVersion = logic.getMaxVersion(forecastName.getValue(),
											CommonUtil.getSelectedFileType(fmFileType).getDescription());
	...
	public void setTableDefaultConfig(ExtPagedTable resultsTable) {
		...
		resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupResultColumns());
		resultsTable.setColumnHeaders(CommonUIUtil.getfileMgmtLookupResultHeader());
	...
	private void configureDetailsTable() {
		...
		if (CommonUtil.getSelectedFileType(fmFileType).toString().equals(ConstantsUtils.EX_FACTORY_SALES)) {
			...
			detailsFilterTable.setColumnAlignment(StringConstantUtils.FORECAST_TYPE_PROPERTY,
					ExtCustomTable.Align.LEFT);
			detailsFilterTable.setColumnAlignment("forecastYear", ExtCustomTable.Align.LEFT);
			detailsFilterTable.setColumnAlignment(StringConstantUtils.FORECAST_MONTH_PROPERTY,
					ExtCustomTable.Align.LEFT);
			detailsFilterTable.setColumnAlignment(StringConstantUtils.ITEM_ID, ExtCustomTable.Align.LEFT);
			detailsFilterTable.setColumnAlignment(StringConstantUtils.ITEM_IDENTIFIER_CODE_QUALIFIER,
					ExtCustomTable.Align.LEFT);
			...
			detailsFilterTable.setColumnAlignment(StringConstantUtils.UNCAPTURED_UNITS_RATIO,
					ExtCustomTable.Align.LEFT);
			...
			detailsFilterTable.setColumnAlignment(StringConstantUtils.FORECAST_NAME_PROPERTY,
					ExtCustomTable.Align.LEFT);
			detailsFilterTable.setColumnAlignment(StringConstantUtils.FORECAST_VERSION_PROPERTY,
					ExtCustomTable.Align.LEFT);
		...
		try {
		    ...
		    resultsTable.removeAllItems();
                    if (fileType.equals(ConstantsUtils.EX_FACTORY_SALES)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupDetailsColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgmtDetailsLookupHeader());
                    } else if (fileType.equals(ConstantsUtils.DEMAND)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupDemandDetailsColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgtLookupDemandDetailsHeader());
                    } else if (fileType.equals(ConstantsUtils.ADJUSTED_DEMAND)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgtLookupAdjustedDemandDetailsColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgtLookupAdjustedDemandDetailsHeader());
                    } else if (fileType.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_SUMMARY)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupInventoryDetailsSummaryColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgmtLookupInventoryDetailsSummaryHeaders());
                    } else if (fileType.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_DETAIL)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupInventoryDetailsColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgmtLookupInventoryDetailsHeader());
                    } else if (fileType.equals(ConstantsUtils.CUSTOMERGTS)) {
                        resultsTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupCustomerColumns());
                        resultsTable.setColumnHeaders(CommonUIUtil.getfileMgmtLookupCustomerHeader());
	...
	private void configureExcelResultTable() {
		...
		excelTable.setVisibleColumns(CommonUIUtil.getfileMgmtLookupResultColumns());
		excelTable.setColumnHeaders(CommonUIUtil.getfileMgmtLookupResultHeader());
	...
	private String[] configureExcelDetailsTable() {
		...
		if (CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.EX_FACTORY_SALES)) {
			...
			bcpHeader = CommonUIUtil.getexcelFileMgtLookupDemandDetailsHeader();
		} else if (CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.ADJUSTED_DEMAND)) {
			bcpHeader = CommonUIUtil.getexcelFileMgtLookupAdjustedDemandDetailsHeader();
		} else if (CommonUtil.getSelectedFileType(fmFileType).getDescription()
				.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_SUMMARY)) {
			bcpHeader = CommonUIUtil.getexcelFilemgtLookupInventorySummaryHeader();
		} else if (CommonUtil.getSelectedFileType(fmFileType).getDescription()
				.equals(ConstantsUtils.INVENTORY_WITHDRAWAL_DETAIL)) {
			bcpHeader = CommonUIUtil.getexcelFileMgtLookupInventoryDetailsHeader();
		} else if (CommonUtil.getSelectedFileType(fmFileType).getDescription().equals(ConstantsUtils.CUSTOMERGTS)) {
				...
				if (interfaceFlag.equals(ConstantsUtils.CHAR_N)) {
					...
					if (propertyId.equals(ConstantsUtils.ITEM_NAME)) {
						...
						itemName.addClickListener(new CustomTextField.ClickListener() {
							...
							@SuppressWarnings("PMD")
							public void click(CustomTextField.ClickEvent event) {
								final ItemSearchLookup lookUp = new ItemSearchLookup(itemNoSearch, itemName,
										sessionDTO);
								try {
									lookUp.init();
								} catch (Exception ex) {
