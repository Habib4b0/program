...
@Connect(CustomMenuBar.class)
public class CustomMenuBarConnector extends AbstractComponentConnector implements
    ...
    public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
        ...
        getWidget().setScrollable(uidl.getBooleanAttribute("scrollable"));
        getWidget().setPageLength(uidl.getIntAttribute("pageLength"));
        ...
        if (null != getState()
            ...
            getWidget().moreItem = GWT.create(VCustomMenuBar.CustomMenuItem.class);
            getWidget().moreItem.setHTML(itemHTML.toString());
            getWidget().moreItem.setCommand(VCustomMenuBar.emptyCommand);

            getWidget().collapsedRootItems = new VCustomMenuBar(true, getWidget());
            getWidget().collapsedRootItems.setScrollable(getWidget().isScrollable());
            getWidget().collapsedRootItems.setPageLength(getWidget().getPageLength());
        ...
        Stack<VCustomMenuBar> menuStack = new Stack<VCustomMenuBar>();
        VCustomMenuBar currentMenu = getWidget();     
        currentMenu.setCheckAllItem(0);
        while (itr.hasNext()) {
            UIDL item = (UIDL) itr.next();
            VCustomMenuBar.CustomMenuItem currentItem = null;
            ...
            if (item.getChildCount() > 0) {
                ...
                currentMenu = new VCustomMenuBar(true, currentMenu);
                currentMenu.setCheckAllItem(0);
            ...
            while (!itr.hasNext() && !iteratorStack.empty()) {
                ...
                for (VCustomMenuBar.CustomMenuItem menuItem : currentMenu.getItems()) {
    ...
    @Override
    public VCustomMenuBar getWidget() {
        return (VCustomMenuBar) super.getWidget();
    }
    ...
    public TooltipInfo getTooltipInfo(Element element) {
        ...
        if (element != getWidget().getElement()) {
            ...
            VCustomMenuBar.CustomMenuItem item = getWidget().getMenuItemWithElement(
                    element);
            if (item != null) {
