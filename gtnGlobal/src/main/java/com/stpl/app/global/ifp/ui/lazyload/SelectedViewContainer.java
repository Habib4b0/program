 ...
 * @author Harlin
 */
public class SelectedViewContainer implements BeanDAO<TempItemDTO> {
    
    SessionDTO sessionDTO;
    private final IFPLogic ifpLogic;
    
    public SelectedViewContainer(final SessionDTO sessionDTO){
    this.sessionDTO=sessionDTO;
    ifpLogic = new IFPLogic(this.sessionDTO);
    }
    public int count(BeanSearchCriteria sc) {
        try {
            return ifpLogic.ifpViewCount(StringUtils.EMPTY,true);
    ...
    public List<TempItemDTO> find(BeanSearchCriteria sc, int i, int i1, List<OrderByColumn> list) {
        try {
            return ifpLogic.getSelectedTableResult(i,i+i1,list);
        } catch (SystemException ex) {
            final String errorMsg = ErrorCodeUtil.getErrorMessage(ex);
            final MessageBox msg = MessageBox.showPlain(Icon.ERROR, ErrorCodeUtil.getEC(ErrorCodes.ERROR_CODE_1001), errorMsg, new MessageBoxListener() {  
                /**            
