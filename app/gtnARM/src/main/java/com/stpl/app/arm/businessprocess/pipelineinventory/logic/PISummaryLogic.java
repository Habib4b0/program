...
public class PISummaryLogic<T extends AdjustmentDTO> extends AbstractPipelineSummaryLogic<T> {
    ...
    protected int getSummaryCount(List<Object> inputs, Criteria criteria) {
        ...
        try {
            ...
            if (isView) {
                resultList = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_generate_Inventory_count", null, null, CommonConstant.ARM_INVENTORY_RATE, null, null);
            } else {
                resultList = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_generate_Inventory_count", null, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INVENTORY_RATE), null, null);
            }
    ...
    protected DataResult<T> getSummaryData(List<Object> inputs, Criteria criteria, TreeMap<String, Integer> masterSids) {
        ...
        List<Object[]> data;
        if (isView) {
            data = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, CommonConstant.SUMMARY_GENERATE_INVENTORY_SELECT, null, "Summary_generate_Inventory_select_orderby", CommonConstant.ARM_INVENTORY_RATE, null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        } else {
            data = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, CommonConstant.SUMMARY_GENERATE_INVENTORY_SELECT, null, "Summary_generate_Inventory_select_orderby", criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INVENTORY_RATE), null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        }
        ...
        if (totalFlag) {
            ...
            totalInputs.remove(totalInputs.size() - 1);
            if (isView) {
                totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_PI_total_data", CommonConstant.SUMMARY_GENERATE_INVENTORY_SELECT, null, CommonConstant.ARM_INVENTORY_RATE, null, "");
            } else {
                totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_PI_total_data", CommonConstant.SUMMARY_GENERATE_INVENTORY_SELECT, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INVENTORY_RATE), null, "");
            }
        ...
        for (int j = 0; j < data.size(); j++) {
            ...
            if (dto != null) {
                if (index < totalColumnIndex) {
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.TWO] == null ? org.apache.commons.lang.StringUtils.EMPTY : zeroDecimals.format(Double.valueOf(String.valueOf(get[NumericConstants.TWO]))));
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.THREE] == null ? org.apache.commons.lang.StringUtils.EMPTY : zeroDecimals.format(Double.valueOf(String.valueOf(get[NumericConstants.THREE]))));
                    ...
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.FIVE] == null ? org.apache.commons.lang.StringUtils.EMPTY : zeroDecimals.format(Double.valueOf(String.valueOf(get[NumericConstants.FIVE]))));
