...
public class AbstractSearchForm extends CustomComponent {
    ...
    public void btnSearchLogic(Button.ClickEvent event) {
        ...
        if (!searchLogic.checkSearchCriteria(binder)) {
            ...
            try {
                ...
                tableLogic.configureSearchData(binder, this.moduleName, this, false);
                resultTable.setFilterDecorator(new ExtDemoFilterDecorator());
 ...
 viewBtn.addClickListener(new Button.ClickListener() {
    ...
    public void itemselectLogic(final ItemClickEvent event) {
        try {
            LOGGER.debug("Entering itemSelectLogic() with event parameter");
            SearchResultsDTO searchResultsDTO = (SearchResultsDTO) getBeanFromId(event.getItemId());
            LOGGER.debug("systemId=" + searchResultsDTO.getContractSystemId());
            ...
            LOGGER.debug("End of itemSelectLogic() ");
        } catch (Exception ex) {
           LOGGER.error(ex);
        }
    }
    ...
    public SearchResultsDTO getBeanFromId(Object obj) {
     ...
     public void setTableReset() {
        ...
        resultTable.setContainerDataSource(resultBean);
        tableLogic.configureSearchData(binder, this.moduleName, this, true);
    }
}
