...
public class CalendarFieldConnector extends AbstractFieldConnector implements Paintable {
	...
	public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
		...
		if (uidl.hasAttribute("locale")) {
			...
			try {
				...
				VConsole.error("Tried to use an unloaded locale \"" + locale + "\". Using default locale ("
		...
		getWidget().setShowISOWeekNumbers(
				uidl.getBooleanAttribute("wn") && getWidget().dateTimeService.getFirstDayOfWeek() == 1);

		getWidget().setCurrentResolution(DateTimeResolution.DAY);

		// Add stylename that indicates current resolution
		setWidgetStyleName(getWidget().getStylePrimaryName() + "-"
				+ VCalendarField.resolutionToString(getWidget().getCurrentResolution()), true);
		...
		getWidget().headerCalendarPanel
				.manageFocusRange(new CalenderFieldUtil.CalendarDate(focusYear, focusMonth, focusDate));
		if (uidl.hasAttribute("disabledWeekDays")) {
			getWidget().headerCalendarPanel
					.setDisabledWeekDays(Arrays.asList(uidl.getStringArrayAttribute("disabledWeekDays")));
		}
		if (uidl.hasAttribute("disabledMonthlyDays")) {
			getWidget().headerCalendarPanel
					.setDisabledMonthlyDays(Arrays.asList(uidl.getStringArrayAttribute("disabledMonthlyDays")));
		}
		...
		if (disUidl != null) {
			...
			while (it.hasNext()) {
				...
				getWidget().headerCalendarPanel
						.addDisabledDate(new CalenderFieldUtil.CalendarDate(year, month, date).toString());
			}
		}
		...
		if (vc != null) {
			...
			while (it.hasNext()) {
				...
				int month = col.getIntAttribute("month");
				int date = col.getIntAttribute("date");
				getWidget().headerCalendarPanel.addDate(new CalenderFieldUtil.CalendarDate(year, month, date));
			}
		}

		getWidget().headerCalendarPanel.setShowISOWeekNumbers(getWidget().isShowISOWeekNumbers());
		getWidget().headerCalendarPanel.setDateTimeService(getWidget().getDateTimeService());
		getWidget().headerCalendarPanel.setResolution(getWidget().getCurrentResolution());
	...
	public void onStateChanged(StateChangeEvent stateChangeEvent) {
		...
		try {
			...
			getWidget().headerCalendarPanel.setRangeStart(rangeStartDate);
			getWidget().headerCalendarPanel.setRangeEnd(rangeEndDate);
