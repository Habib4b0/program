...
public class RSRebateSetupTabForm extends CustomComponent implements View {
    ...
    public RSRebateSetupTabForm(final ErrorfulFieldGroup binder, final RebateScheduleLogic rebateLogic, final BeanItemContainer<ItemDetailsDTO> itemResultBean,
            ...
            String mode, final SessionDTO sessionDTO) throws PortalException, SystemException {
        this.binder = binder;
    ...
    public void addItemDetailsTable() throws PortalException, SystemException {
        ...
        rebateSetupTableLogic.setPageLength(NumericConstants.SEVEN);
    ...
    private void configureFields() {
        ...
        massDDLB.setPageLength(NumericConstants.SEVEN);
    ...
    public void addBtnPopulate() {
        ...
        btnPopulate.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                try {
                    LOGGER.debug("Entering Rebate Schedule Populate operation in Add");
                    binder.getErrorDisplay().clearError();
                    boolean massUpdate;
                    boolean tempflag = false;
                    
                    if (massField.getValue() == null || massField.getValue().toString().equals(ConstantsUtils.SELECT_ONE)) {
                        final MessageBox msg = MessageBox.showPlain(Icon.ERROR, ConstantsUtils.ERROR, "Select a Field.", new MessageBoxListener() {  
                            /**            
                             * The method is triggered when a button of the message box is 
                             * pressed .          
                             *               
                    ...
                    if (massUpdate) {
                        ...
                        if (RsUtils.REBATE_AMOUNT.equals(String.valueOf(massField.getValue())) && massValue.getValue() != null) {
                            ...
                            massValue.addValidator(new StringLengthValidator(" Rebate Amount should be less than 38 Characters", 0, NumericConstants.THIRTY_EIGHT, true));
                    ...
                    LOGGER.debug("Ending Rebate Schedule Populate operation in Add");
                } catch (Exception exception) {
    ...
    public void addAllBtnPopulate() {
        ...
        btnAllPopulate.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                try {
                    LOGGER.debug("Entering Rebate Schedule Populate All operation in Add");
                    binder.getErrorDisplay().clearError();
                    boolean massUpdate;
                    ...
                    if (massUpdate) {
                        LOGGER.debug("Entering  Populate all Mass update operation");
                        rebateLogic.saveToTemp(itemResultBean.getItemIds(), false);
                        populate = true;
                        final String populateField;
                        final String populateValue;
                        ...
                        if (populate) {
                         ...
                         LOGGER.debug("Ending Populate all Mass update operation");
                    }
                    LOGGER.debug("Ending Rebate Schedule Populate All operation in Add");
                } catch (Exception exception) {
...
public void setMassCheckValue(String values) {
    ...
    public void loadBasedOnCalculationType(String calculationType) {
        ...
        switch (calculationType) {
            ...
            default:
                ...
                itemDetailsTable.setVisibleColumns(tableResultCustom.getObjResult());
                itemDetailsTable.setColumnHeaders(tableResultCustom.getObjResultHeader());
                LOGGER.debug("Calculation type is not selected in Information Tab");
                break;
        }
        if (isViewMode) {
    ...
    private void createWorkSheet() throws PortalException, SystemException,  NoSuchMethodException, IllegalAccessException, InvocationTargetException {
      ...
      String[] header= getExcelHeader(itemDetailsTable);
        ...
        ExcelExportforBB.createWorkSheet(header, recordCount, this, getUI(), "Rebate_Setup");
    ...
    private String[] getExcelHeader(ExtPagedTable itemDetailsTable) {
       int j=0;
        int headerLength=itemDetailsTable.getColumnHeaders().length;
        int arrayLength=0;
        if (!isViewMode) {
            arrayLength = headerLength - 1;
        } else {
            arrayLength = headerLength;
        }
        String[] header=new String[arrayLength];
        for (int i = 0; i < headerLength; i++) {
            if(!itemDetailsTable.getColumnHeaders()[i].isEmpty()){
           header[j]=itemDetailsTable.getColumnHeaders()[i];
           j++;
            }
        }
        return header;
