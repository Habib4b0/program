...
public class NsfLogic {
        ...
        for (String fields : keys) {
            ...
            if (searchFields.getField(fields).getValue() != null && !ConstantUtil.SELECT_ONE.equals(searchFields.getField(fields).getValue().toString()) && !searchFields.getField(fields).getValue().toString().trim().isEmpty()) {
                if ("combo2".equalsIgnoreCase(fields)) {
                    queryBuilder.append(" AND ").append(criteria.get(fields)).append(" = ").append(((com.stpl.app.util.HelperDTO) searchFields.getField("combo2").getValue()).getSystemId());
                } else {
        ...
        try {
            if (list != null) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    searchDTO.setSystemID(!ConstantsUtils.NULL.equals(String.valueOf(object[8])) && StringUtils.isNotBlank(String.valueOf(object[8])) ? String.valueOf(object[8]) : "0");
        ...
        if (filterSet != null) {
            for (Container.Filter filter : filterSet) {
                if (filter instanceof SimpleStringFilter) {
                    ...
                    if (startValue != null) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" >= '").append(startValue).append("' ");
                    } 
                    if (endValue != null) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" <= '").append(endValue).append("' ");
                    }
                    ...
                    if (stringFilter.getValue() instanceof Date) {
                        ...
                        if (Compare.Operation.GREATER_OR_EQUAL.toString().equals(operation.name())) {
                            stringBuilder.append(" AND ").append(constantProperties.getString(String.valueOf(stringFilter.getPropertyId()))).append(" >= '").append(filterString).append("' ");
                        } else {
                            stringBuilder.append(" AND ").append(constantProperties.getString(String.valueOf(stringFilter.getPropertyId()))).append(" <= '").append(filterString).append("' ");
                        }
                    }
    ...
    public int getNsRuleCount(final ErrorfulFieldGroup searchFields, final Set<Container.Filter> filterSet,String tabName) throws Exception {
        int count = 0;
        StringBuilder queryBuilder = new StringBuilder();
    ...
    public List<NetSalesRuleLookupDto> loadNsRuleResults(
            final ErrorfulFieldGroup searchFields, final int start, final int end, final List<SortByColumn> columns, final Set<Container.Filter> filterSet,String tabName) throws SystemException, Exception {
        ...
        for (String fields : keys) {
            ...
            if (searchFields.getField(fields).getValue() != null && !ConstantUtil.SELECT_ONE.equals(searchFields.getField(fields).getValue().toString()) && !searchFields.getField(fields).getValue().toString().trim().isEmpty()) {
                if ("ruleType".equalsIgnoreCase(fields) || "ruleCategory".equalsIgnoreCase(fields)) {
                    ...
                    queryBuilder.append(" AND ").append(nsRuleCriteria.get(fields)).append(" LIKE '").append(String.valueOf(searchFields.getField(fields).getValue()).trim().replace("*", "%")).append("'");
                }
            }
        }
        ...
        try {
            if (list != null) {
                ...
                for (Object nsrList : list) {
                    ...
                    searchDTO.setValue(!ConstantsUtils.NULL.equals(String.valueOf(object[5])) && StringUtils.isNotBlank(String.valueOf(object[5])) ? String.valueOf(object[5]) : StringUtils.EMPTY);
    ...
    private StringBuilder getNsRuleDetailsOrderQuery(final StringBuilder stringBuilder, final List<SortByColumn> sortByColumns, final int startIndex, final int endIndex) {
     ...
     public Object nsfInsert(SessionDTO sessionDto,final String queryName,boolean isSelected,String sbName) throws SystemException, Exception {                     
   ...
   public void saveNsf(SessionDTO sessionDto, ErrorfulFieldGroup binder,HelperDTO formulaType, String contractSelected, int nsrId) throws Exception {
        ...
        if(contractSelected.startsWith("Existing")){
           if (isPresent(sessionDto.getSystemId())) {
               ...
               query.append(" WHERE NET_SALES_FORMULA_MASTER_SID = '").append(sessionDto.getSystemId()).append(ConstantsUtils.SINGLE_QUOTE);
    ...
    public NsfDto getNsfMasterById(final int idValue) throws SystemException, PortalException {
        ...
        LOGGER.info("In getNsfMasterById P1:id=" + idValue);
        ...
        LOGGER.info("getNsfMasterById Returns NetSalesFormulaMaster NsfDto");
        return nsfDTO;
    ...
    public void deleteNsfMasterById(final int idValue) throws SystemException, PortalException {
        LOGGER.info("In deleteNsfMasterById P1:id=" + idValue);
        StringBuilder deleteQuery = new StringBuilder();
      ...
      public boolean checkNsf(SessionDTO sessionDto,String queryCheck,String dbTableName){
        ...
        LOGGER.info("End of checkNsf checkFlag="+checkFlag);
        return checkFlag;
    }
    ...
    public void saveSelection(Map map, int projectionID, String screenName, String saveOrUpdate) throws PortalException, SystemException {
        ...
        if ("save".equalsIgnoreCase(saveOrUpdate)) {
            ...
            for (int i = 0; i < map.size(); i++) {
                ...
                queryBuilder.append(" WHERE NET_SALES_FORMULA_MASTER_SID = '").append(projectionID).append("' AND SCREEN_NAME = '").append(screenName).append("' AND FIELD_NAME ='").append(obj[i]).append("'\n");
            }
        }
...
// To load Available Customers Table
    ...
    public boolean isPresent(int nsfId) {
        ...
        query.append(" WHERE NET_SALES_FORMULA_MASTER_SID = '").append(nsfId).append(ConstantsUtils.SINGLE_QUOTE);;
