...
public class SelectedCustomerTableLogic extends PageTableLogic{
    ...
    public static final Logger LOGGER = Logger.getLogger(SelectedCustomerTableLogic.class);

    public SelectedCustomerTableLogic() {
    }
    ...
    public int getCount() {
        ...
        if (generate) {
            try {
                count=dataSelectionLogic.getCommonCountForSelectedCP(binderDto,getFilters(),sessionDTO);
            } catch (Exception ex) {
               LOGGER.error(ex);
            }
        }
