...
public class PropertiesReader {
     ...
     */
    private static final String JAR_FILE_NAME = "galGtnProperties";
    /**
    ...
    private PropertiesReader() {
        ...
        LOGGER.info("Getting jboss.server.config.dir :" + path);
        path = path.replace("standalone", FOLDER_NAME);
        path = path.replace("configuration", JAR_FILE_NAME + JAR_FILE_EXTENSION);
        try {
            file = new File(path);
            LOGGER.info("File Name :{0}", new Object[]{file.getAbsoluteFile()});
            spilitAndLoadJarFileEntries(file);
        } catch (Exception e) {
            LOGGER.error("Error While reading the Properties from Jboss:{0}", new Object[]{e.getMessage()});
        }

    }

//    public static void main(String[] arg) {
////        file = new File("D:\\Karthik\\galderma\\bundle_Jboss\\bpigtn-dev\\jboss-7.1.1\\" + FOLDER_NAME + "\\" + JAR_FILE_NAME + JAR_FILE_EXTENSION);
////        LOGGER.log(Level.INFO, "File Name :{0}", file);
...
//    }
    private void loadProperties(InputStream is, String fileName) {
        try {
            ...
            LOGGER.error("Unable to load file :{0} from {1} with error{2}", new Object[]{fileName, JAR_FILE_NAME, ex.getMessage()});
        }
    }
    ...
    private void spilitAndLoadJarFileEntries(File jarFile) {
        ...
        try {
            ...
            LOGGER.error("Unable to load_Properties jar file from repository :with Error :{0}", new Object[]{e.getMessage()});
        } finally {
            try {
                LOGGER.info("Size after laoding Properties :{0}", new Object[]{messages.size()});
                if (jar != null) {
                    jar.close();
                    LOGGER.info("Closing the jar:");
                }
            } catch (IOException e) {
                LOGGER.error("Unable to load_Properties jar file from repository :with Error :{0}", new Object[]{e.getMessage()});
            }
...
//                    Path myDir = Paths.get("D:\\Karthik\\galderma\\bundle_Jboss\\bpigtn-dev\\jboss-7.1.1\\" + FOLDER_NAME);
                    File file = myDir.toFile();
                    LOGGER.info("file exist:{0}", new Object[]{file.exists()});
                    LOGGER.info("Path Name  : {0}", new Object[]{myDir.toString()});
                    ...
                    LOGGER.info("Creating watch key:");
                    boolean callPropertiesLoader = Boolean.FALSE;
                    WatchKey watckKey = watcher.take();
                    while (watckKey != null) {
                        List<WatchEvent<?>> events = watckKey.pollEvents();
                        for (WatchEvent event : events) {
                            LOGGER.info("Event Fired : {0}", new Object[]{event.kind()});
                    ...
                    LOGGER.error("Error while Starting the Watch Service API: {0}", new Object[]{e.toString()});
                }
            }
