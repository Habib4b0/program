...
//
   ...
   VerticalLayout rpCalculationLayout;
     ...
     * @throws Exception
     */
    public RebatePlanCalculation(ErrorfulFieldGroup binder, BeanItemContainer<RebatePlanTierResults> searchResultbeans, final SessionDTO sessionDTO,final RebatePlanMasterDTO rebatePlanMasterDTO) throws PortalException, SystemException {
   ...
   configureOnViewMode();
     ...
     * @throws Exception 
     */
    public void configureResultTable() throws PortalException, SystemException {
            ...
            resultsTable.setPageLength(NumericConstants.NINE);
    ...
    private void configureFields() throws SystemException, PortalException {                
        ...
        tierFrom.addValueChangeListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(final ValueChangeEvent event) {
                tierFrom.setDescription((String) tierFrom.getValue());
                String tfrom = tierFrom.getValue().replaceAll(",", "");
                if (!tierFrom.getValue().isEmpty() && !tfrom.matches("([\\-|0-9]{1,22}+(\\.[0-9]{1,6}+)?$)")) {
                    binder.getErrorDisplay().setError("Please enter only numbers with two decimals places in From");
                } 
                else if(binder.getErrorDisplay() != null)
                    {
        ...
        tierTo.addValueChangeListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(final ValueChangeEvent event) {
                tierTo.setDescription((String) tierTo.getValue());
                String tTo = tierTo.getValue().replaceAll(",", "");
                if (!tierTo.getValue().isEmpty() && !tTo.matches("([\\-|0-9]{1,22}+(\\.[0-9]{1,6}+)?$)")) {
                    ...
                    try {
                        ...
                        netSalesLookup.addCloseListener(new Window.CloseListener() {
                            ...
                            public void windowClose(Window.CloseEvent e) {
                                ...
                                LOGGER.debug("netSaleFormula.getSystemID()"+netSaleFormula.getSystemID());
    ...
    public void detachListeners(final AbstractField field) {
        ...
        LOGGER.debug("Entering detachListeners method");
        ...
        LOGGER.debug("End of detachListeners method");
    ...
    private void configureRemoveButton() {
        ...
        btnRemove.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                if (event != null) {
                    LOGGER.debug("Entering Rebate Plan remove operation");
                            ...
                            public void buttonClicked(final ButtonId buttonId) {
                                try {
                                    if (buttonId.name().equals("YES")) {
                                        ...
                                        if (searchResultbeans.size() != 0) {
                                            tierFrom.setValue(tierFromValue.setScale(NumericConstants.TWO, RoundingMode.HALF_UP).toPlainString());
                ...
                LOGGER.debug("Ending Rebate Plan remove operation");
            }
        });
    }
     ...
     * @throws Exception the exception
     */
    private void configureCalculationAddButton() {
            ...
            public void buttonClick(final ClickEvent event) {
                ...
                try {  
                    ...
                    if (StringUtils.isNotBlank(tierTo.getValue()) && !ConstantsUtils.NULL.equals(tierTo.getValue()) && !ConstantsUtils.ZERO.equals(tierTo.getValue())) {
                        tierToString = tierTo.getValue().replaceAll(",", StringUtils.EMPTY);
                        tierTo.setValue(StringUtils.EMPTY);
                    } else {
                        tierFrom.setReadOnly(true);
                    ...
                    if (!StringUtils.EMPTY.equals(tierToString)) {
                        tierFrom.setReadOnly(false);
                        String toval = (new BigDecimal(tierToString).add(BigDecimal.valueOf(NumericConstants.DOUBLE_ZERO_ZERO_ONE))).setScale(NumericConstants.TWO, RoundingMode.HALF_UP).toPlainString();
                        tierFrom.setValue(commaFormatter.format(Double.valueOf(toval)));
                        tierFrom.setReadOnly(true);
                    }
                    LOGGER.debug("Ending Rebate Plan Calculate operation");
    ...
    public void configureOnView() {
        ...
        percentvalueDDLB.setEnabled(false);
            ...
            public void buttonClick(final ClickEvent event) {
                MessageBox.showPlain(Icon.QUESTION, "Confirmation", commonMsg.getResetMessage(), new MessageBoxListener() {
                    ...
                    public void buttonClicked(final ButtonId buttonId) {
                        if (buttonId.name().equals("YES")) {
                            try {
                                LOGGER.debug("Entering inside  reset  method for calculation ");
                                binder.getErrorDisplay().clearError();
                                if (sessionDTO.getMode().equalsIgnoreCase(ConstantsUtils.EDIT)||sessionDTO.getMode().equalsIgnoreCase(ConstantsUtils.COPY)) {
                                    ...
                                    if (toToValue != null && !toToValue.equals(BigDecimal.ZERO)) {
                                        ...
                                        tierFrom.setValue(toToValue.setScale(NumericConstants.TWO, RoundingMode.HALF_UP).toPlainString());
                                        tierFrom.setReadOnly(true);
                                    } else {
                                        tierFrom.setReadOnly(false);
                                        tierFrom.setValue("");
    ...
    private void addScrollBar() {
        ...
        if (Page.getCurrent().getBrowserWindowWidth() >= NumericConstants.ONE_NINE_ZERO_ZERO) {
        ...
        Page.getCurrent().addBrowserWindowResizeListener(new Page.BrowserWindowResizeListener() {
            ...
            public void browserWindowResized(Page.BrowserWindowResizeEvent event) {
                if (event.getWidth() >= NumericConstants.ONE_NINE_ZERO_ZERO && !extended) {
                    HorizontalLayout layout = (HorizontalLayout) tierPanel.getContent();
                    rpCalculationLayout.removeComponent(tierPanel);
                    rpCalculationLayout.addComponent(layout);
                    extended = true;
