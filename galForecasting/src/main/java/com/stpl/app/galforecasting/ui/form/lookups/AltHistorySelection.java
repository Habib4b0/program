...
import com.stpl.app.galforecasting.salesprojection.logic.tablelogic.AlternateHistoryAllocationTableLogic;
...
public class AltHistorySelection extends CustomComponent implements View {
   ...
   @UiField("resetBtn")
    ...
    public void generateBtnClick(Button.ClickEvent event) {
        LOGGER.info("Entered inside generateBtnClick method");
        try {
        ...
        LOGGER.info("Ending generateBtnClick method");
    }
     ...
     * @param isGenarate
     */
    public void configureTable(boolean isGenarate) throws Exception {
...
//            rightTable.setColumnAlignment(rightDTO.getDoubleColumns().get(i), ExtCustomTable.Align.CENTER);
        ...
        leftTable.setColumnWidth("check", 120);
        ...
        for (Object object : singleCol) {
            ...
            } else if ("Sales Projection".equals(session.getForecastName())) {

                if (String.valueOf(object).contains("Units")) {
                    rightTable.setColumnCheckBox(object, true, false);
                }
            }
        }
    ...
    private ForecastDTO getHistoricalPeriods(final ForecastDTO forecastDTO, boolean isGenerate) {
        LOGGER.info("getHistoricalPeriods method Starts");
        ...
        String fromYear = startPeriod.substring(startPeriod.length() - 4, startPeriod.length());
        String toYear = endPeriod.substring(endPeriod.length() - 4, endPeriod.length());
        int startMonth = 0;
        int endMonth = 0;
        int startYear = 0;
        int endYear = 0;
        ...
        if (isGenerate) {
            if (freq.equals(Constant.QUARTERLY)) {
                ...
                int startMon = monthMap.get(startPeriod.substring(startPeriod.length() - 7, startPeriod.length() - 4));
                int endMon = monthMap.get(endPeriod.substring(endPeriod.length() - 7, endPeriod.length() - 4));
    ...
    private String formDate(String value, String freq, boolean isFromDate) {
        LOGGER.info("formDate method Starts");
    ...
    private void getnerateLogic() {
        LOGGER.info("getnerateLogic method Starts");
        String fromDate = String.valueOf(from.getValue());
        String toDate = String.valueOf(to.getValue());
