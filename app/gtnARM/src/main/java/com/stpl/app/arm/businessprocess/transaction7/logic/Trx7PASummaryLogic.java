...
public class Trx7PASummaryLogic<T extends AdjustmentDTO> extends AbstractPipelineSummaryLogic<T> {
    ...
    public static final Logger LOGGERFORSUMMAEYTAB = Logger.getLogger(Trx7PADetailsLogic.class);
    ...
    public List<Object> generateHeader(AbstractSelectionDTO selection, String[] columns) {
        ...
        for (String[] detection : doubleHeaderVariables) {
            ...
            doubleVisibleColumn.add(detection[2] + "~" + detection[1].replace(" ", org.drools.core.util.StringUtils.EMPTY));
            doubleVisibleHeader.add(detection[1]);
            doubleSingleVisibleColumn.put(detection[2] + "~" + detection[1].replace(" ", org.drools.core.util.StringUtils.EMPTY), doubleSingleColumn.toArray());
    ...
    protected DataResult<T> getSummaryData(List<Object> inputs, Criteria criteria, TreeMap<String, Integer> masterSids) {
        ...
        List<Object[]> data;
        if (isView) {
            data = QueryUtils.getItemData(inputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, CommonConstant.TRX7_SUMMARY_PA_GENERATE_DATA, null, "trx7_Summary_PA_generate_data_orderby", CommonConstant.ARM_DISTRIBUTION_FEES_RATE, null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        } else {
            data = QueryUtils.getItemData(inputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, CommonConstant.TRX7_SUMMARY_PA_GENERATE_DATA, null, "trx7_Summary_PA_generate_data_orderby", criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_DISTRIBUTION_FEES_RATE), null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        }
        ...
        if (totalFlag) {
            ...
            totalInputs.remove(totalInputs.size() - 1);
            if (isView) {
                totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, "trx7_Summary_PA_total_data", CommonConstant.TRX7_SUMMARY_PA_GENERATE_DATA, null, CommonConstant.ARM_DISTRIBUTION_FEES_RATE, null, "");
            } else {
                totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, "trx7_Summary_PA_total_data", CommonConstant.TRX7_SUMMARY_PA_GENERATE_DATA, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_DISTRIBUTION_FEES_RATE), null, "");
            }
    ...
    protected int getSummaryCount(List<Object> inputs, Criteria criteria) {
        ...
        boolean isView = criteria.getSelectionDto().getSessionDTO().getAction().equals(ARMUtils.VIEW_SMALL);
        if (isView) {
            resultList = QueryUtils.getItemData(inputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, "Summary_PA_generate_count", null, null, CommonConstant.ARM_DISTRIBUTION_FEES_RATE, null, null);
        } else {
            resultList = QueryUtils.getItemData(inputs, CommonConstant.TRX7_SUMMARY_PA_GENERATE_EDIT, "Summary_PA_generate_count", null, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_DISTRIBUTION_FEES_RATE), null, null);
        }
