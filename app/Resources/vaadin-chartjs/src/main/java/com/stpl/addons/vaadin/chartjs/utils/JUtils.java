...
public abstract class JUtils {
	...
	public static void putNotNullAsStringArrayList(JsonObject obj, String key, List<String[]> list) {
		if (list != null) {
			JsonArray arr = Json.createArray();
			for (String[] entry : list) {
				JsonArray arrTemp = Json.createArray();
				for (String temp : entry) {
					arrTemp.set(arrTemp.length(), temp);
				}
				arr.set(arr.length(), arrTemp);
			}
			obj.put(key, arr);
		}
	}
