...
public class CompanyDetailsContainer implements BeanDAO<CFPCompanyDTO> {
    ...
    public List<CFPCompanyDTO> find(final BeanSearchCriteria searchCriteria, final int startIndex, final int offset, final List<OrderByColumn> list) {
        try {
            LOGGER.info("Entering AvailableCompanyAddition find method for third tab:");
            ...
            if (viewModeFlag) {
                ...
                if (companyDetailsResultSaveBean.size() > Constants.ZERO) {
                    if (CFPSearchLogic.saveToTempTable(companyDetailsResultSaveBean.getItemIds())) {
