...
public class ContractDashboardTableLogic extends PageTreeTableLogic {
    ...
    public Object configureContainer(Object object, Container datasource) {
        ...
        ((CustomTreeContainer<ContractDashboardDTO>) datasource).addBean(dto);
        if (dto.getParent() == 1) {
            ((CustomTreeContainer<ContractDashboardDTO>) datasource).setChildrenAllowed(dto, true);
        } else {
            ((CustomTreeContainer<ContractDashboardDTO>) datasource).setChildrenAllowed(dto, false);
        }
        return dto;
    }
