...
public class RelationshipBuilderTree extends CustomComponent implements View {
     ...
     */
    public RelationshipBuilderTree(final CustomFieldGroup relationshipBuilderBinder, final RelationshipBuilderDTO relationshipBuilderDTO, final SessionDTO sessionDTO) throws SystemException, Exception {
        super();
        LOGGER.info("RelationshipBuilderTree Constructor started");
        ...
        init();
        LOGGER.info("RelationshipBuilderTree Constructor Ended");
    }
     ...
     * @throws Exception the exception
     */
    public final void init() throws SystemException, Exception {
     ...
     * @throws Exception the exception
     */
    private void addToContent() throws SystemException, Exception {
    ...
    private CustomFieldGroup getRelationBuilderBinder() {
        LOGGER.info("getRelationBuilderBinder Method started");
        ...
        LOGGER.info("getRelationBuilderBinder Method returns relationshipBuilderBinder");
        return relationshipBuilderBinder;
    }
     ...
     * @return the panel
     */
    private HorizontalLayout addRelationshipInfo() {
        LOGGER.info("addRelationshipInfo Method started");
        ...
        relDesc.setWidth("187px");
        cssLayout.addComponent(relDesc);
        cssLayout.addComponent(relationshipDesc);

        cssLayout.addComponent(ResponsiveUtils.makeLabel("Hierarchy Name :", true));
        cssLayout.addComponent(hierarchyNameDdlb);
        cssLayout.addComponent(new Label("Relationship Type :"));
        relationshipType.setWidth("198px");
        cssLayout.addComponent(relationshipType);

        Label version = ResponsiveUtils.makeLabel("Version No :", true);
        version.addStyleName("lableright");
        version.setWidth("194px");
        cssLayout.addComponent(version);

        cssLayout.addComponent(versionNo);

        Label sDate = ResponsiveUtils.makeLabel("Start Date :", true);
        sDate.addStyleName("lableright");
        sDate.setWidth("183px");

        cssLayout.addComponent(sDate);

        cssLayout.addComponent(startDate);

        cssLayout.addComponent(new Label("Build Type :"));
        buildType.setWidth("198px");
        cssLayout.addComponent(buildType);
        hlayout1.addComponent(cssLayout);

        LOGGER.info("addRelationshipInfo Method returns panel");
        return hlayout1;
    }
    ...
    private Component addActionButtons() {
        LOGGER.info("addActionButtons Method started");
        ...
        layout.addComponent(resetAll);
        resetAll.setEnabled(true);
        LOGGER.info("addActionButtons Method returns layout");
        return layout;
    }
    ...
    private void addBuilderWindow() {
        LOGGER.info("addBuilderWindow Method started");
        addLevelBuilder();
        addHierarchyTree();
        LOGGER.info("addBuilderWindow Method returns panel");
    }
    ...
    private void configureFields() throws SystemException, Exception {
        ...
        startDate.setWidth(13.6f, DateField.UNITS_EM);
    ...
    void resetButton() {
        resetAll.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("In configureFields resetAll.addClickListener started");
                ...
                LOGGER.info("In configureFields resetAll.addClickListener Ended");
            }
        });
    }
    ...
    void backButton() {
        backButton.addClickListener(new Button.ClickListener() {
             ...
             */
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("In configureFields backButton.addClickListener started");
    ...
    private void addSaveListener() {
        save.addClickListener(new Button.ClickListener() {
             ...
             */
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("In configureFields save.addClickListener started");
                saveButtonClickLogic(true);
                LOGGER.info("In configureFields save.addClickListener Ended");
            }
        });
    }
    ...
    public void saveButtonClickLogic(boolean flag) {
        try {
            ...
            if (flag) {
                        ...
                        if (buttonId.name().equals("YES")) {
                            try {
                                ...
                                if (page.equalsIgnoreCase(ConstantsUtils.ADD)) {
                                    ...
                                    sessionDTO.setHierarchyVersion(idList.get(2));
                ...
                logic.saveRelationshipBuilder(relationshipBuilderBinder, selectedItems, sessionDTO);
                backFlag = false;
                getUI().getNavigator().navigateTo(RelationshipBuilderIndex.NAME);
    ...
    private void addListeners() {
        ...
        startDate.addValueChangeListener(new Property.ValueChangeListener() {
            public void valueChange(final Property.ValueChangeEvent event) {
                try {
                    LOGGER.info("In configureFields startDate.addValueChangeListener started");
                    ...
                    LOGGER.info("In configureFields startDate.addValueChangeListener Ended");
                } catch (SystemException ex) {
                    final String errorMsg = ErrorCodeUtil.getErrorMessage(ex);
                    AbstractNotificationUtils.getErrorNotification(ErrorCodeUtil.getEC(ErrorCodes.ERROR_CODE_1001), errorMsg);
                    LOGGER.error(ex);
     ...
     * @throws Exception the exception
     */
    protected void hierarchyOnChangeEvent() throws SystemException, PortalException, Exception {
     ...
     * @throws Exception the exception
     */
    protected void versionOnChangeEvent() throws SystemException, PortalException, Exception {
    ...
    private void addHierarchyTree() {
        LOGGER.info("addHierarchyTree Method started");
        ...
        hierarchyTree.addItemClickListener(new ItemClickEvent.ItemClickListener() {
            ...
            public void itemClick(final ItemClickEvent event) {
                ...
                if (levelNo < availableLevelsBeanList.size()) {
                    ...
                    List<HierarchyLevelsDTO> filteredValues = BpmLogic.getFilteredValues(selectedHierarchySysId, version, levelNo, levelValues);
                    if (filteredValues != null) {
                        ...
                        for (; levelNo < availableLevelsBeanList.size(); levelNo++) {
                            LOGGER.info("Going for next level");
                            filteredValues = BpmLogic.getFilteredValues(selectedHierarchySysId, version, levelNo, levelValues);
                            if (filteredValues != null) {
                                availableLevelsBeanList.get(levelNo).removeAllItems();
                                availableLevelsBeanList.get(levelNo).addAll(filteredValues);
                                break outer;
                    ...
                    final Collection<Object> parentChilds = (Collection<Object>) hierarchyTree.getChildren(hierarchyTree.getParent(treeBeanId));
                    if (parentChilds != null && !parentChilds.isEmpty()) {
                        for (final Iterator<Object> iterator1 = parentChilds.iterator(); iterator1.hasNext();) {
                            ...
                            Collection<Object> finalChilds = (Collection<Object>) hierarchyTree.getChildren(item1);
                            if (finalChilds != null && !finalChilds.isEmpty()) {
                                for (final Iterator<Object> iterator2 = finalChilds.iterator(); iterator2.hasNext();) {
                                    final Object removeObj = iterator2.next();
                                    availableLevelsBeanList.get(levelNo).removeItem(removeObj);
                                }
                            }
                        }
                    }
        ...
        remove.addClickListener(new Button.ClickListener() {
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("In addHierarchyTree remove.addClickListener started");
                            ...
                            if (hierarchyTree.getValue() instanceof Collection) {
                                ...
                                for (final Iterator<HierarchyLevelsDTO> iterator = hierarchyList.iterator(); iterator.hasNext();) {
                                    ...
                                    if (collection == null) {
                                        ...
                                        if (filteredValues != null) {
                                            LOGGER.info("Refreshing Level " + (levelNo + 1) + " first");
                                            availableLevelsBeanList.get(levelNo).removeAllItems();
                                            availableLevelsBeanList.get(levelNo).addAll(filteredValues);
                                            if (parentChilds != null && !parentChilds.isEmpty()) {
                                                LOGGER.info("Already " + parentChilds.size() + " are attached");
                                                for (final Iterator<Object> iterator1 = parentChilds.iterator(); iterator1.hasNext();) {
                                                    final Object item1 = iterator1.next();
                                            ...
                                            if (levelNo < availableLevelsBeanList.size()) {
                                                ...
                                                if (filteredValues != null) {
                                                    LOGGER.info("Refreshing Level " + (levelNo + 1) + " second");
                                                    availableLevelsBeanList.get(levelNo).removeAllItems();
                                                    availableLevelsBeanList.get(levelNo).addAll(filteredValues);
                                                }
                                            }
                ...
                treeBean = null;
                LOGGER.info("In addHierarchyTree remove.addClickListener Ended");
            }
        });

        LOGGER.info("addHierarchyTree Method Ended");
     ...
     * @throws Exception the exception
     */
    public void loadHierarchyLevels(final SessionDTO sessionDTO) throws SystemException, PortalException, Exception {
        LOGGER.info("loadHierarchyLevels Method started");
        final int selectedHierarchyId = sessionDTO.getSelectedHierarchySessionId();
        ...
        if (selectedHierarchyId != ConstantsUtils.ZERO_NUM) {
            if (fromViewPage.equalsIgnoreCase(ConstantsUtils.SMALL_YES) || fromViewPage.equalsIgnoreCase(ConstantsUtils.ADD) || fromViewPage.equalsIgnoreCase(ConstantsUtils.EDIT)) {
                ...
                if (rbSystemId == ConstantsUtils.ZERO_NUM) {
                    ...
                    final Map<String, List<HierarchyLevelsDTO>> finalSavedLevelsList = availSavedLevelValuesList.get(2);
                ...
                for (int i = 0; i < levelCount; i++) {
                    ...
                    addSelectedLevelValues(levelObj.getLevelName(), savedLevelValues, levelNo);
     ...
     * @param obj the i
     */
    private void addSelectedLevelValues(final String levelName, final List<HierarchyLevelsDTO> savedLevelValues, final int obj) {
    ...
    protected void addItemsButtonClick(final Button.ClickEvent event, final int obj, final int levelNo) {
        LOGGER.info("addItemsButtonClick Method started with i value:" + obj);
        ...
        if (levelNo != 1 && treeBean == null && hierarchyTree.getItemIds().isEmpty()) {
            ...
            for (final Iterator<HierarchyLevelsDTO> iterator = itemMasterDetailsList.iterator(); iterator.hasNext();) {
                if (hierarchyTree.getItemIds().isEmpty()) {
                    ...
                    final HierarchyLevelsDTO itemId = iterator.next();
                    if (Integer.valueOf(treeBean.getLevelNo()) == Integer.valueOf(itemId.getLevelNo()) - 1) {
                        if (hierarchyTree.getItemIds().contains(itemId)) {
                            ...
                            if (levelNo == ConstantsUtils.ONE) {
                                ...
                                if (hierarchyTree.getChildren(treeBeanId) == null || hierarchyTree.getChildren(treeBeanId).size() == 0) {
                                    ...
                                    itemId.setHierarchyNo(hierrachyNo);
                                    LOGGER.info("Hierarchy No Final --- >> " + hierrachyNo);
     ...
     * @throws Exception the exception
     */
    private void loadVersion() throws SystemException, PortalException, Exception {
