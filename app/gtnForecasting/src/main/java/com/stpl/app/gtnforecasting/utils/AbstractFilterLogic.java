...
public class AbstractFilterLogic {
    ...
    public StringBuilder orderByQueryDefaultOrder(List<SortByColumn> sortByColumns, Map<String, String> queryMap,String defaultOrder) {
        boolean asc = false;
        StringBuilder tempStart = new StringBuilder("ORDER BY * ?");
        if (sortByColumns != null && !sortByColumns.isEmpty()) {
            for (final Iterator<SortByColumn> iterator = sortByColumns.iterator(); iterator.hasNext();) {
    ...
    public StringBuilder filterQueryDefaultOrder(java.util.Set<Container.Filter> filterSet, Map<String, String> queryMap) {
        StringBuilder str = new StringBuilder("AND ( * LIKE '?' OR * IS NULL )");
        StringBuilder sql = new StringBuilder();
        if (filterSet!=null && !filterSet.isEmpty()) {
            for (Container.Filter filter : filterSet) {
                ...
                if (filter instanceof Between) {
                    Between betweenFilter = (Between) filter;
                    StringBuilder dateStartstr = new StringBuilder("AND ( CONVERT(CHAR(10),*,120)  >='?')");
                    StringBuilder dateEndstr = new StringBuilder("AND (  CONVERT(CHAR(10),*,120) <='?')");
                    if (!queryMap.get(betweenFilter.getPropertyId().toString()).isEmpty()) {
                        Date startValue = (Date) betweenFilter.getStartValue();
                        Date endValue = (Date) betweenFilter.getEndValue();
                        StringBuilder initialStart = new StringBuilder("where ( (  CONVERT(CHAR(10),*,120) >= '?' )");
                        StringBuilder initialEnd = new StringBuilder("where ( (  CONVERT(CHAR(10),*,120) <= '?' )");
                        if (!betweenFilter.getStartValue().toString().isEmpty()) {
                            StringBuilder tempStart;
                            if (sql.length() == 0) {
                                tempStart = new StringBuilder(initialStart);
                    ...
                    if (!queryMap.get(stringFilter.getPropertyId().toString()).isEmpty()) {
                        Compare.Operation operation = stringFilter.getOperation();
                        if (Compare.Operation.EQUAL.toString().equals(operation.name())) {
                            StringBuilder startsb = new StringBuilder("AND ( * ='?')");
                            StringBuilder intStartstr = new StringBuilder("where ( ( * = '?' )");
                            StringBuilder tempStart;
                            String value;
                            if (((Integer) stringFilter.getValue()) == 0) {
                            ...
                            if (!value.isEmpty()) {
                                if (sql.length() == 0) {
                                    ...
                                    tempStart = new StringBuilder(startsb);
                                }
                                tempStart.replace(tempStart.indexOf("*"), tempStart.indexOf("*") + 1, queryMap.get(stringFilter.getPropertyId().toString()));
                                tempStart.replace(tempStart.indexOf("?"), tempStart.indexOf("?") + 1, value);
                                sql.append(tempStart);
                        ...
                        if (stringFilter.getValue() instanceof Date) {
                            ...
                            if (Compare.Operation.GREATER_OR_EQUAL.toString().equals(operation.name())) {
                                if (sql.length() == 0) {
                                    ...
                                    tempStart = new StringBuilder("AND (  CONVERT(CHAR(10),*,120) >='?')");
                                }
                            } else {
                                if (sql.length() == 0) {
                                    tempStart = new StringBuilder("where ( (  CONVERT(CHAR(10),*,120) <='?')");
