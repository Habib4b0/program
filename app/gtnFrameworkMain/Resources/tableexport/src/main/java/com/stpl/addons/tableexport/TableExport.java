...
public abstract class TableExport implements Serializable {
    ...
    public List<Object> getPropIds(int headerNo) {
        return tableHolder.getPropIds(headerNo);
    }

    public List<Object> getPropIds() {
        return tableHolder.getPropIds(0);
    }
    ...
    /**
     * When creating multiple tabs in single excel , 
     * isdownloadable -true , when all the tabs are get ready to download
     * otherwise , false
     * @param isdownloadable 
     */
    public void exportMultipleTabs(boolean isdownloadable) {
        convertTable();
        if (isdownloadable) {
            sendConverted();
        }
