...
//                    saveCheckRecord(checkbox.getValue() ? 1 : 0);
                    temp.getTempItemPriceRebateSystemId();
                    IfpLogic.updateTempCheck(sessionDTO, true,temp.getTempItemPriceRebateSystemId(),checkbox.getValue());
//                    saveContainer.addItem(itemId);
                }
                 private void saveCheckRecord(int value) {
                    List input = new ArrayList<Object>();
                    input.add(value);
                    input.add(temp.getTempItemPriceRebateSystemId());
                    QueryUtil.itemUpdate(input, "Rebate Plan checkrecord UPdate");
                }
                }
            );
            return checkbox;
...
//            bundleNo.setValidationVisible(true);
                            ...
                            formulaLookup.addCloseListener(new Window.CloseListener() {
                                ...
                                public void windowClose(Window.CloseEvent e) {
                                    if (formulaLookup.isSelected()) {
                                        ...
                                        saveContainer.addItem(updateChanged(itemId, rSFormulaDTO.getFormulaNo(), rSFormulaDTO.getFormulaName(), rSFormulaDTO.getFormulaType().getDescription()));
                                        saveContainer.addItem(itemId);
                                    }
                                }
                        });
    ...
    public void attachListeners(final AbstractField field, final String component, final Object itemId, final TempRebateDTO temp) throws Exception {
