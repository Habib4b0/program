...
public class NSFMainTab extends StplCustomComponent implements AddBaseForm {
    ...
    public NSFMainTab(SessionDTO sessionDto,ErrorfulFieldGroup binder,NsfDto nsfDto,SalesBasisDto salesBasisDto) throws SystemException, PortalException, Exception {     
         super();
         this.binder=binder;
         this.nsfDto=nsfDto;
    ...
    boolean validateSave() {
        ...
        try {
            ...
            if (deductFlag) {
                if (logic.checkNsf(sessionDto, "INDICATOR_CHECK", "IMTD_DEDUCTION_DETAILS")) {
                    if (sbFlag) {
                        errorMessage.append(ConstantsUtils.BREAK);
                    }
