...
public abstract class AbstractHistoryLookup extends AbstractLookup {
    ...
    public AbstractHistoryLookup(final String windowName, final String moduleIndicator) {
        super(windowName);
        setWidth(859, Sizeable.Unit.PIXELS);
        setHeight(881, Sizeable.Unit.PIXELS);
     ...
     *
     * @param contractResults The results table of contract section
     * @param brandResults The results table of brandSearch section
     * @param customerNativeSelect Customer DDLB
     * @return
     */
    public Panel buildHistoryLookup(final ExtFilterTable contractResults, final ExtFilterTable brandResults, final NativeSelect customerNativeSelect) {
        ...
        HorizontalLayout contractBrandLayout = (HorizontalLayout) UiUtils.getLayout(HorizontalLayout.class);
     ...
     *
     * @param brandResults The brandSearch search result table
    ...
    private Panel addContractSection(final ExtFilterTable contractResults, final NativeSelect customerNativeSelect) {
        ...
        layout.addComponent(addCustomerSearchButtons(contractName, contractNo, customerNativeSelect, customerId, contractHolder));
    ...
    private VerticalLayout addContractSearchCriteria(final NativeSelect customerNativeSelect) {
        ...
        contractName.setValue(StringUtils.EMPTY);
        VerticalLayout criteriaLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
        ...
        gridLayout.addComponent((moduleIndicator.equals(INDICATOR_MODULE_NON_MANDATED.getConstant()) ? customerId : contractNo));
        Label tempLabel1 = UiUtils.makeLabel(textFieldLable2);
        tempLabel1.setWidth("157px");
        gridLayout.addComponent(tempLabel1);
        gridLayout.addComponent((moduleIndicator.equals(INDICATOR_MODULE_NON_MANDATED.getConstant()) ? contractHolder : contractName));
    ...
    private HorizontalLayout addBrandSearchCriteria() {
        HorizontalLayout searchCriteriaLayout = (HorizontalLayout) UiUtils.getLayout(HorizontalLayout.class);
     ...
     *
     * @param results The table to be added
     * @param indicator Indicates whether its contract section or brandSearch
     * section
     ...
     * @param indicator To indicate contract results or brandSearch results
     */
    @Override
