...
'
	...
	LEFT JOIN '
				...
				@PRODUCT_TABLE,
				' PF 
		ON PF.ITEM_MASTER_SID = B.ITEM_MASTER_SID AND PF.PERIOD_SID = A.PERIOD_SID
	GROUP BY A.PERIOD
		,A.YEAR
		,A.HIERARCHY_NO
		,A.LEVEL_NAME
		,A.PARENT_HIERARCHY_NO) GTS
                   ...
                   LEFT JOIN (SELECT CONTRACT_SALES_ACTUALS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_SALES, NULL))
		,CONTRACT_UNITS_ACTUALS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_UNITS, NULL))
		,COGS_ACTUALS = SUM(ISNULL(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_UNITS, NULL), 0) * ISNULL(U.ITEM_PRICE, 0) * ISNULL(UOM_VALUE, 0))
		,CONTRACT_SALES_PROJECTED = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_SALES, NULL))
		,CONTRACT_UNITS_PROJECTED = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_UNITS, NULL))
		,COGS_PROJECTED = SUM(ISNULL(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_UNITS, NULL), 0) * ISNULL(U.ITEM_PRICE, 0) * ISNULL(UOM_VALUE, 0))
		,A.PERIOD
		,A.[YEAR]
		,A.HIERARCHY_NO
		,A.LEVEL_NAME
 ...
 LEFT JOIN (	SELECT CONTRACT_DISCOUNT_ACTUAL = SUM(IIF((DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL), ACTUAL_SALES, NULL))
		,A.PERIOD
		,A.[YEAR]
		,A.HIERARCHY_NO
		,A.LEVEL_NAME
		...
		,A.PARENT_HIERARCHY_NO
		,CONTRACT_DISCOUNT_PROJECTED = SUM(IIF((DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL), PROJECTION_SALES, NULL))
	FROM #DATA_TABLE A
	INNER JOIN CCP_DETAILS B
		ON A.CCP_DETAILS_SID = B.CCP_DETAILS_SID
	INNER JOIN '
	...
	LEFT JOIN ',
				@D_PROJECTION_TABLE,
				' NDP
		ON NAD.CCP_DETAILS_SID = NDP.CCP_DETAILS_SID AND NDPM.RS_CONTRACT_SID = NDP.RS_CONTRACT_SID
	...
	FROM (SELECT PPA_RPU = SUM(ACTUAL_DISCOUNT_DOLLAR)
			...
			,A.LEVEL_NAME
			,A.PARENT_HIERARCHY_NO
			,ACTUAL_SALES = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), NS.ACTUAL_SALES, NULL))
			,ACTUAL_UNITS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), NS.ACTUAL_UNITS, NULL) * ISNULL(UOM_VALUE, 0))
			,PPA_RPU_PROJ = SUM(PROJECTION_DISCOUNT_DOLLAR)
			,PROJECTION_PPA_SALES = SUM(PROJECTION_DISCOUNT_DOLLAR)
			,PROJECTION_SALES = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), NSP.PROJECTION_SALES, NULL))
			,PROJECTION_UNITS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), NSP.PROJECTION_UNITS, NULL) * ISNULL(UOM_VALUE, 0))
		FROM #DATA_TABLE A
		INNER JOIN CCP_DETAILS B
		ON A.CCP_DETAILS_SID = B.CCP_DETAILS_SID
		INNER JOIN '
		...
		BEGIN
			...
			LEFT JOIN (
				SELECT CONTRACT_SALES_ACTUALS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_SALES, NULL)),
					CONTRACT_UNITS_ACTUALS = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_UNITS, NULL)),
					COGS_ACTUALS = SUM(ISNULL(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_UNITS, NULL), 0) * ISNULL(U.ITEM_PRICE, 0) * ISNULL(UOM_VALUE, 0)),
					CONTRACT_SALES_PROJECTED = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_SALES, NULL)),
					CONTRACT_UNITS_PROJECTED = SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_UNITS, NULL)),
			...
			LEFT JOIN (
				SELECT CONTRACT_DISCOUNT_ACTUALS = SUM(IIF((DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL), ACTUAL_SALES, NULL)),
					A.PERIOD,
					A.[YEAR],
					A.HIERARCHY_NO,
					A.LEVEL_NAME,
				...
				LEFT JOIN NM_DISCOUNT_PROJECTION NDP
					ON NAD.PROJECTION_DETAILS_SID = NDP.PROJECTION_DETAILS_SID AND NDPM.RS_CONTRACT_SID = NDP.RS_CONTRACT_SID AND NDP.PERIOD_SID = A.PERIOD_SID
				...
				WHERE EXISTS (
						SELECT 1
						FROM #DISCOUNT_INFO A
						WHERE CASE WHEN 'PV' = 'PROGRAM' THEN CAST(NDPM.RS_CONTRACT_SID AS VARCHAR(20)) ELSE NDPM.PRICE_GROUP_TYPE END = A.TOKEN
						)
				GROUP BY PERIOD,
					[YEAR],
					A.HIERARCHY_NO,
...
JOIN #CCP_DETAILS_TEMP CD ON C.CCP_DETAILS_SID = CD.CCP_DETAILS_SID'
	...
	,PARENT_HIERARCHY_NO'
		...
		SET @D_SQL = 'INSERT INTO #SALES_ACTUAL_DATA(
            ...
            SELECT CONTRACT_SALES_ACTUALS = SUM(IIF((SALES_INCLUSION=CONVERT (CHAR(1),@SALES_INCLUSION) OR @SALES_INCLUSION  IS NULL),ACTUAL_SALES,NULL) ),
                   CONTRACT_UNITS_ACTUALS = SUM(IIF((SALES_INCLUSION=CONVERT (CHAR(1),@SALES_INCLUSION) OR @SALES_INCLUSION  IS NULL),ACTUAL_UNITS,NULL) * ISNULL(UOM_VALUE,0)),
                   PERIOD,
                   [YEAR],
                   HIERARCHY_NO,
                   LEVEL_NAME,
		...
		SET @D_SQL = 'INSERT INTO #SALES_DATA(
            ...
            SELECT CONTRACT_SALES_PROJECTED = SUM( IIF((SALES_INCLUSION=CONVERT (CHAR(1),@SALES_INCLUSION) OR @SALES_INCLUSION   IS NULL),PROJECTION_SALES,NULL)),
                   CONTRACT_UNITS_PROJECTED = SUM( IIF((SALES_INCLUSION=CONVERT (CHAR(1),@SALES_INCLUSION) OR @SALES_INCLUSION  IS NULL),PROJECTION_UNITS,NULL)  * ISNULL(UOM_VALUE,0)),
                   PERIOD,
                   [YEAR],
                   HIERARCHY_NO,
                   LEVEL_NAME
...
SELECT CONTRACT_SALES_ACTUALS = SUM(IIF(( DEDUCTION_INCLUSION = CONVERT(CHAR(1), @DEDUCTION_INCLUSION) OR @DEDUCTION_INCLUSION IS NULL ), ACTUAL_SALES, NULL))
...
'
		...
		BEGIN
			...
			SELECT PROJECTION_MASTER_SID,
				...
				PERIOD,
				[YEAR],
				SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_SALES, NULL)) AS CONTRACT_SALES_ACTUALS,
				SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), ACTUAL_UNITS, NULL) * ISNULL(UOM_VALUE, 0)) AS CONTRACT_UNITS_ACTUALS,
				PARENT_HIERARCHY_NO
			...
			SELECT PROJECTION_MASTER_SID,
				...
				PERIOD,
				[YEAR],
				SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_SALES, NULL)) AS CONTRACT_SALES_PROJECTED,
				SUM(IIF((SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL), PROJECTION_UNITS, NULL) * ISNULL(UOM_VALUE, 0)) AS CONTRACT_UNITS_PROJECTED,
				PARENT_HIERARCHY_NO
			...
			SELECT s.PROJECTION_MASTER_SID,
				...
				PERIOD,
				[YEAR],
				ISNULL(SUM(IIF((DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL), ACTUAL_SALES, NULL)), 0) AS CONTRACT_SALES_ACTUALS,
				SUM(ISNULL(DISCOUNT_AMOUNT, 0)) ACCRUAL_DISCOUNT_ACTUAL,
			...
			SELECT s.PROJECTION_MASTER_SID,
				...
				PERIOD,
				[YEAR],
				ISNULL(SUM(IIF((DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL), PROJECTION_SALES, NULL)), 0) AS CONTRACT_SALES_PROJECTED,
				SUM(ISNULL(DISCOUNT_AMOUNT, 0)) ACCRUAL_DISCOUNT_PROJ,
				S.PARENT_HIERARCHY_NO
