...
public class GtnWsAllListConfig {
	...
	private void loadcomboBoxTypeMap() {
		...
		comboBoxQueryMap.put("adjustmentTypeDdlb",
				"SELECT ADJ.ARM_ADJUSTMENT_CONFIG_SID,ADJ.TRANSACTION_NAME,HT.DESCRIPTION FROM ARM_ADJUSTMENT_CONFIG ADJ JOIN HELPER_TABLE HT ON HT.HELPER_TABLE_SID = ADJ.METHODOLGY");
		...
		comboBoxQueryMap.put("REPORT_CUSTOM_VIEW",
				"SELECT DISTINCT   CUST_VIEW_MASTER_SID,CUST_VIEW_NAME from CUST_VIEW_MASTER WHERE CUST_VIEW_TYPE LIKE 'repor%'");
