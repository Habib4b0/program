...
public class CommonLogic {
    ...
    public ExtTreeContainer<ContractsDetailsDto> getLevel3Hierarchy(final ContractsDetailsDto parent2, final ExtTreeContainer<ContractsDetailsDto> container, final List<ContractsDetailsDto> ifpList, final List<ContractsDetailsDto> psList, final List<ContractsDetailsDto> rsList) throws SystemException, PortalException {
        ...
        for (final Iterator<ContractsDetailsDto> iterator = contractML.iterator(); iterator.hasNext();) {
            ...
            if ((!IndicatorConstants.RS_VALUE.getConstant().equals(contractMember.getCategory()) && isLevel4Or5ListAvlbl(contractMember.getSystemId(), contractMember.getCategory()))|| !psList.isEmpty()) {
                container.setChildrenAllowed(contractMember, true);
            } else {
                container.setChildrenAllowed(contractMember, false);
            }
    ...
    public ExtTreeContainer<ContractsDetailsDto> getLevel4Hierarchy(final ContractsDetailsDto parent3, final ExtTreeContainer<ContractsDetailsDto> container, final List<ContractsDetailsDto> psList, final List<ContractsDetailsDto> rsList) throws SystemException, PortalException {
        ...
        for (final Iterator<ContractsDetailsDto> iterator = contractList.iterator(); iterator.hasNext();) {
            ...
            if ((!IndicatorConstants.RS_VALUE.getConstant().equals(contractMember.getCategory()) && isLevel4Or5ListAvlbl(contractMember.getSystemId(), contractMember.getCategory())) || !rsList.isEmpty()) {
                container.setChildrenAllowed(contractMember, true);
            } else {
                container.setChildrenAllowed(contractMember, false);
            }
    ...
    public ExtTreeContainer<ContractsDetailsDto> getLevel5Hierarchy(final ContractsDetailsDto parent4, final ExtTreeContainer<ContractsDetailsDto> container, final List<ContractsDetailsDto> rsList) throws SystemException, PortalException {
        ...
        container.setParent(parent4, parent4.getParent3());
            ...
            for (final Iterator<ContractsDetailsDto> iterator = contractList.iterator(); iterator.hasNext();) {
                contractMember = iterator.next();
                container.addBean(contractMember);
            if ((!IndicatorConstants.RS_VALUE.getConstant().equals(parent4.getCategory()) && isLevel5ListAvlbl(parent4.getSystemId())) || !rsList.isEmpty()) {
                container.setChildrenAllowed(contractMember, true);
            } else {
                container.setChildrenAllowed(contractMember, false);
            }
    ...
    public boolean isLevel4Or5ListAvlbl(final int contractSystemId, final String category) throws SystemException {
        ...
        boolean available;
        available = (!IndicatorConstants.PS_VALUE.getConstant().equals(category) && getPSQueriedCount(contractSystemId) > Constants.ZERO)
                || (!IndicatorConstants.RS_VALUE.getConstant().equals(category) && getRSQueriedCount(contractSystemId) > Constants.ZERO);
                          ...
                          if (isProjectionCustUpdated && isProjectionProdUpdated) {
                            if (insertIntoProjectionDetails(oldProjectionId, newProjectionId,session)) {
                                ...
                                if (Constants.IndicatorConstants.MANDATED.equals(moduleName)) {
                                    marketType = getMarketType(String.valueOf(projectionMasterRow[NumericConstants.THREE]), String.valueOf(projectionMasterRow[0]));
                                }

                                if (isAddModule) {
                                    LOGGER.debug(" Old Projection - " + oldProjectionId + " New projection - " + newProjectionId + " Market Type" + marketType);
...
/**                                       Commented for CEL-869
    ...
    public boolean insertIntoProjectionDetails(int oldProjectionId, int newProjectionId,SessionDTO session) {
        ...
        LOGGER.debug("Entering insertIntoProjectionDetails method oldProjectionId " + oldProjectionId + " newProjectionId " + newProjectionId);
    ...
    public static void callPrcFeSalesTransfer(final String sessionId) {
        ...
        try {
            ...
            LOGGER.debug("Ending callPrcFeSalesTransfer Procedure");
    ...
    public static void callPrcFeProjectionDetailsTransfer(final String sessionId) {
        ...
        try {
            ...
            LOGGER.debug("Ending callPrcFeProjectionDetailsTransfer Procedure");
    ...
    public void updateMainTable(List<String> companyMasterSids, int newProjectionId, String contractMasterSid, String forecastingType, String moduleName) {
        ...
        salesQuery.replace(salesQuery.indexOf("?CON"), salesQuery.indexOf("?CON") + NumericConstants.FOUR, contractMasterSid);
        salesQuery.replace(salesQuery.indexOf("?COM"), salesQuery.indexOf("?COM") + NumericConstants.FOUR, CommonUtils.CollectionToString(companyMasterSids, false));
        salesQuery.replace(salesQuery.indexOf("?PM"), salesQuery.indexOf("?PM") + NumericConstants.THREE, String.valueOf(newProjectionId));

        discountQuery.replace(discountQuery.indexOf("?CON"), discountQuery.indexOf("?CON") + NumericConstants.FOUR, contractMasterSid);
        discountQuery.replace(discountQuery.indexOf("?COM"), discountQuery.indexOf("?COM") + NumericConstants.FOUR, CommonUtils.CollectionToString(companyMasterSids, false));
        discountQuery.replace(discountQuery.indexOf("?PM"), discountQuery.indexOf("?PM") + NumericConstants.THREE, String.valueOf(newProjectionId));

        rebateQuery.replace(rebateQuery.indexOf("?CON"), rebateQuery.indexOf("?CON") + NumericConstants.FOUR, contractMasterSid);
        rebateQuery.replace(rebateQuery.indexOf("?COM"), rebateQuery.indexOf("?COM") + NumericConstants.FOUR, CommonUtils.CollectionToString(companyMasterSids, false));
                    ...
                    if (flag) {
                        ...
                        if (TRADING_PARTNER_UPDATE.getConstant().equals(session.getModuleName())) {
                            ...
                            if (!session.getModuleName().contains("Item")) {
                                updateMainTable(masterSids, newProjectionId, session.getContractMasterSid(), String.valueOf(projectionMasterRow[NumericConstants.TWO]), session.getModuleName());
                            }
                        }
                        loadTabLogic.setForecastingType(newProjectionId);
                        tempList.add(swapForecastingType(loadTabLogic.forecatingType));
    ...
    public List<String> copyProjection(int oldProjectionId, boolean isDiscountModule, List<String> contractList, List<String> companyList, List<String> rsList,SessionDTO sessionDTO) {
        ...
        if (newProjectionId != 0) {            
            ...
            if (insertIntoProjectionDetails(oldProjectionId,newProjectionId,sessionDTO)) {
                ...
                if (flag) {
                    if (isDiscountModule) {
                        updateMainTableRemove(companyList, newProjectionId, contractList, String.valueOf(projectionMasterRow[NumericConstants.TWO]), rsList);
                    }
                    LoadTabLogic loadTabLogic = new LoadTabLogic();
                    loadTabLogic.setForecastingType(newProjectionId);
                    tempList.add(swapForecastingType(loadTabLogic.forecatingType));
    ...
    public void updateMainTableRemove(List<String> companyMasterSids, int newProjectionId, List<String> contractList, String forecastingType, List<String> rsList) {
        ...
        try {
            ...
            salesQuery.replace(salesQuery.indexOf("?CON"), salesQuery.indexOf("?CON") + NumericConstants.FOUR, String.valueOf(contractList.get(0)));
            salesQuery.replace(salesQuery.indexOf("?COM"), salesQuery.indexOf("?COM") + NumericConstants.FOUR, CommonUtils.CollectionToString(companyMasterSids, true));
            salesQuery.replace(salesQuery.indexOf("?PM"), salesQuery.indexOf("?PM") + NumericConstants.THREE, String.valueOf(newProjectionId));

            discountQuery.replace(discountQuery.indexOf("?CON"), discountQuery.indexOf("?CON") + NumericConstants.FOUR, String.valueOf(contractList.get(0)));
            discountQuery.replace(discountQuery.indexOf("?COM"), discountQuery.indexOf("?COM") + NumericConstants.FOUR, CommonUtils.CollectionToString(companyMasterSids, true));
            discountQuery.replace(discountQuery.indexOf("?PM"), discountQuery.indexOf("?PM") + NumericConstants.THREE, String.valueOf(newProjectionId));
            discountQuery.replace(discountQuery.indexOf("?RS"), discountQuery.indexOf("?RS") + NumericConstants.THREE, CommonUtils.CollectionToString(rsList, false));

            rebateQuery.replace(rebateQuery.indexOf("?CON"), rebateQuery.indexOf("?CON") + NumericConstants.FOUR, String.valueOf(contractList.get(0)));
    ...
    public static void insertInputsBeforeTranfer(int fromProjectionId, int copyFromProjectionId, int toProjectionid, int copyToProjectionId,
            int fromContractSid, int toContractSid, String companies, String fromEndDate, String toStartDate, boolean transferFlag, String sessionId, boolean transferSalesFlag) {
        ...
        String query = "INSERT INTO FUTURE_EVENTS_INPUT (FROM_PROJECTION, COPY_FROM_PROJECTION, TO_PROJECTION, COPY_TO_PROJECTION, FROM_CONTRACT_MASTER_SID, TO_CONTRACT_MASTER_SID, \n"
                + "COMPANY_MASTER_SID, FROM_END_DATE, TO_START_DATE, TRANSFER_FLAG, SESSION_ID,TRANSFER_SALES_FLAG) VALUES('" + fromProjectionId + "','" + copyFromProjectionId + "','" + toProjectionid + "'"
                + ",'" + copyToProjectionId + "','" + fromContractSid + "','" + toContractSid + "','" + companies + "','" + fromEndDate + "','" + toStartDate + "','" + transferFlag + "', '" + sessionId + "', '" + transferSalesFlag + "')";
