...
public class UpdatedContractSelection extends VerticalLayout {
    ...
    private CustomTreeContainer<ContractResultDTO> selectedContractContainer = new CustomTreeContainer<ContractResultDTO>(ContractResultDTO.class);
    ...
    CustomTreeContainer<ContractResultDTO> tradingPartnerDetailsContainer = new CustomTreeContainer<ContractResultDTO>(ContractResultDTO.class);
    ...
    private CustomTreeContainer<ContractResultDTO> contractExcelResultBean = new CustomTreeContainer<ContractResultDTO>(ContractResultDTO.class);
   ...
   CommonUtil commonMsg = CommonUtil.getInstance();
    ...
    int timeGap;
    String[] excelComponentId = new String[5];
    ...
    public void configureCurrentTradingPartnerTable() {
        ...
        pagedTable.setColumnWidth("checkRecord", 100);
                    ...
                    if (!dto.getWorkflowStatus().trim().isEmpty()) {
                        ...
                        check.addClickListener(new ExtCustomCheckBox.ClickListener() {
                            public void click(ExtCustomCheckBox.ClickEvent event) {
                                int count = logic.callCheckRecUpdate(check.getValue(), (ContractResultDTO) itemId, screenName, session);
                                if (!check.getValue()) {
                        ...
                        compStartDate.addFocusListener(new FieldEvents.FocusListener() {
                            ...
                            public void focus(FieldEvents.FocusEvent event) {
                                Property.ValueChangeListener valueChangeListner = new Property.ValueChangeListener() {
                                    ...
                                    public void valueChange(Property.ValueChangeEvent event) {
                                        if (!isTableUpdate) {
                                            if (compStartDate!= null && compStartDate.getValue() != null) {
                                                Date enteredDate =  compStartDate.getValue();
                                                Date maxEndDate = new Date(1, 1, 1900);
    ...
    public void configureComponentInformationTable() {
        ...
        componentInformationTable.setWidth(100, Sizeable.Unit.PERCENTAGE);
        componentInformationTable.setHeight(400, Sizeable.Unit.PIXELS);
        componentInformationTable.setPageLength(5);
    ...
    private void loadComponentInformation(String componentSelectionValue, Object selectedItem) {
        ...
        if (selectedItem != null) {
            ...
            id[2] = String.valueOf(crDTO.getIfpContSid());
            id[3] = String.valueOf(crDTO.getPsContSid());
            id[4] = String.valueOf(crDTO.getRsContSid());
            excelComponentId = id;
    ...
    public void nextButtonLogic(Button.ClickEvent event) {
        LOGGER.info("Contract selection submitAndNextLogic initiated");
    ...
    public void refreshContractSelectionTable() {
        LOGGER.info("Contract Selection Refreshed");
    ...
    public void createWorkSheet(String fileName, ExtFilterTable resultTable) throws SystemException, PortalException, Exception {
    ...
    public void createWorkSheetInfo(String fileName, ExtFilterTable resultTable) throws SystemException, PortalException, Exception {
    ...
    public void allCustomers(Button.ClickEvent event) {
        LOGGER.info("Entered allCustomers method");
        AllCustomers psLookup = new AllCustomers(session.getCompanyMasterSids());
        getUI().addWindow(psLookup);
        LOGGER.info("Ending allCustomers method");
    }
    ...
    public boolean submitAndNextLogic(final boolean isNextButtonClicked) {
        ...
        if (session.getModuleName().equals(TRANSFER_TRADING_PARTNER.getConstant()) || session.getModuleName().equals(PROJECTION_DETAILS_TRANSFER.getConstant())) {
            if (TAB_CURRENT_CONTRACT.getConstant().equals(screenName)) {
                ...
                LOGGER.info("Entering Transfer submit next checks");
        ...
        LOGGER.info("Contract selection submitAndNextLogic ends ");
        return true;
    }
    ...
    private void checkNonAssociatedProducts(final boolean isNextButtonClicked, final boolean isProjectionDetailsTransferModule) {
        LOGGER.info("checkNonAssociatedProducts starts ");
        ...
        if (isProjectionDetailsTransferModule) {
            ...
            if (logic.isHavingAnyCommonProducts(session.getCompanyMasterSids(), session.getPhCompanyMasterSids(), sourceProjectionId, destProjectionId, sourceContractId, destContractId)) {
                ...
                logic.updateSubmitFlagWithoutCheckRecord(session.getModuleName(), screenName, session.getUserId(), session.getSessionId(), false);
                ContractTableLogic.handleFilterChange();
                LOGGER.info("No common products");
            }
        } else {
            if (isHavingDifferentProducts()) {
        ...
        LOGGER.info("checkNonAssociatedProducts ends ");
    }

    private void checkTimeGap(final boolean isNextButtonClicked) {
        LOGGER.info("Entering checkTimeGap");
        if (isTimeGapPresent()) {
            new AbstractNotificationUtils() {
                public void noMethod() {
                    logic.updateSubmitFlagWithoutCheckRecord(session.getModuleName(), screenName, session.getUserId(), session.getSessionId(), false);
        ...
        LOGGER.info("Exiting checkTimeGap");
    }

    private void checkProjectionSales(final boolean isNextButtonClicked) {
        LOGGER.info("Entering checkProjectionSales");
        ...
        LOGGER.info("Exiting checkProjectionSales");
    }

    private boolean isTimeGapPresent() {
        LOGGER.info("Inside isTimeGapPresent");
        String endDate = CommonLogic.getDateForSubmittedContract(session.getSessionId(), false, false, true);
        ...
        timeGap = diffMonth - 2;
        LOGGER.info("Exiting isTimeGapPresent");
        if (timeGap > 0) {
            return true;
        } else {
            return false;
    ...
    private void submition(boolean isNextButtonClicked) {
        ...
        changeTab();
        LOGGER.info("Exiting submition");
    }

    private boolean isHavingDifferentProducts() {
        LOGGER.info("Inside isHavingDifferentProducts");
        ...
        if (nonAssociatedProductsList != null && !nonAssociatedProductsList.isEmpty()) {
            ...
            LOGGER.info("Exiting isHavingDifferentProducts");
            return true;
        } else {
            LOGGER.info("Exiting isHavingDifferentProducts");
            return false;
