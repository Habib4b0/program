...
public class NMSalesProjectionTableLogic extends PageTreeTableLogic {
	...
	public static final org.jboss.logging.Logger LOGGER = org.jboss.logging.Logger
			.getLogger(NMSalesProjectionTableLogic.class);
	...
	public GtnSmallHashMap loadData(int start, int offset) {
		LOGGER.info("loadData initiated with firstGenerated=" + firstGenerated + " and start=" + start + " and offset="
				+ offset);
	...
	public int getCount() {
		...
		if (firstGenerated) {
			...
			int forecastlevel = Integer.valueOf(
					projSelDTO.getHierarchyIndicator().equals("C") ? projSelDTO.getSessionDTO().getCustomerLevelNumber()
							: projSelDTO.getSessionDTO().getProductLevelNumber());
	...
	private void findRequiredHierarchies(SalesBaseNode apex, int expandLevelNo) {
		...
		int i = 0;
		int initial = hierarchySet.size() % getItemsPerPage() == 0 ? (hierarchySet.size() / getItemsPerPage()) - 1
				: hierarchySet.size() / getItemsPerPage();
	...
	private void loadDataForBulkNode(Set<TreeNode> nodeSet) {
		...
		try {
			...
			Logger.getLogger(NMSalesProjectionTableLogic.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
