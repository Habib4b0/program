...
@RestController
public class GtnWsReportDSController {

	public GtnWsReportDSController() {
		/**
		 * empty constructor
		 */
	}

	private static final GtnWSLogger LOGGER = GtnWSLogger.getGTNLogger(GtnWsReportDSController.class);
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/gtnReport/lookUp/getCustomerHierarchyLookUp", method = RequestMethod.POST)
	public GtnUIFrameworkWebserviceResponse getCustomerHierarchyLookUp(
		...
		try {
			...
			String queryName = customerHierarchyWsRequest.getGtnWsSearchRequest().isCount()
					? "getCountReportCustomerHierarchy"
					: "getDataReportCustomerHierarchy";
			List<Object[]> result = gtnGeneralServiceController
					.executeQuery(gtnWsSqlService.getQuery(getCustomerHierarchyLookUpInput(customerHierarchyWsRequest), queryName));
	...
	private List<Object> getCustomerHierarchyLookUpInput(GtnUIFrameworkWebserviceRequest gtnWsRequest)
		...
		for (GtnWebServiceSearchCriteria searchCriteria : gtnWsRequest.getGtnWsSearchRequest()
			...
			if (searchCriteria.getFilterValue1() != null && !searchCriteria.getFilterValue1().isEmpty()) {
				switch (searchCriteria.getFieldId()) {
				case "reportCustomerHierarchyLookup_reportCustomerHierarchyTypeOptionGroup":
					hierarchyType = searchCriteria.getFilterValue1().replace("*", "%");
					break;
				case "reportCustomerHierarchyLookup_reportCustomerHierarchyNameTextBox":
					hierarchyName = searchCriteria.getFilterValue1().replace("*", "%");
					break;
