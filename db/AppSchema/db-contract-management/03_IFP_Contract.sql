...
IF NOT EXISTS (SELECT 1
               FROM   INFORMATION_SCHEMA.COLUMNS
               WHERE  TABLE_NAME = 'IMTD_IFP_DETAILS_PENDING'
                  AND COLUMN_NAME = 'IFP_CONTRACT_SID')
  BEGIN
      ALTER TABLE IMTD_IFP_DETAILS_PENDING
        ADD IFP_CONTRACT_SID INT NOT NULL
  END

GO
-----------Addition of INBOUND_STATUS Column-----------
...
IF NOT EXISTS (SELECT 1
  ...
  BEGIN
      ALTER TABLE IMTD_IFP_DETAILS_PENDING
        ADD INBOUND_STATUS CHAR(1) NULL
  END

GO 
IF EXISTS (
    SELECT 1 FROM IMTD_IFP_DETAILS_PENDING
      WHERE INBOUND_STATUS IS NULL)
BEGIN

UPDATE IMTD_IFP_DETAILS_PENDING
