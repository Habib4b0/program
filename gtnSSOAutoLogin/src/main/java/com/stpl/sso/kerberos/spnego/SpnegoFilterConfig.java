 ...
 */
package com.stpl.sso.kerberos.spnego;
 ...
 *
 * <p>
 * This class also validates if a keyTab should be used and if all of the
 * LoginModule options have been set.</p>
 *
 * <p>
 * To see a working example and instructions on how to use a keytab, take a look
 * at the <a href="http://spnego.sourceforge.net/server_keytab.html"
 * target="_blank">creating a server keytab</a> example.
 * </p>
 *
 * <p>
 * The class should be used as a Singleton:<br />
 * <code>
 ...
 *
 * <p>
 * See an example web.xml configuration in the
...
final class SpnegoFilterConfig { // NOPMD
     ...
     */
    private SpnegoFilterConfig(final FilterConfig config) throws FileNotFoundException, URISyntaxException {

        // specify logging level
        setLogLevel(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getLogLevel());

        // check if exists
        assert loginConfExists(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getLoginConf());

        // specify krb5 conf as a System property
        if (null == StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getKrbConf()) {
            throw new IllegalArgumentException(
                    SpnegoFilterConfig.MISSING_PROPERTY + Constants.KRB5_CONF);
        } else {
            System.setProperty("java.security.krb5.conf", StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getKrbConf());
        }

        // specify login conf as a System property
        if (null == StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getLoginConf()) {
            throw new IllegalArgumentException(
                    SpnegoFilterConfig.MISSING_PROPERTY + Constants.LOGIN_CONF);
        } else {
            System.setProperty("java.security.auth.login.config", StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getLoginConf());
        }

        // check if exists and no options specified
        doClientModule(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getCliendModule());

        // determine if all req. met to use keyTab
        doServerModule(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getServerModule());

        // if username/password provided, don't use key tab     
        this.setUsernamePassword(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getUserName(), StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().getPassWord());
        
        // determine if we should support Basic Authentication
        setBasicSupport(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().isAllowBasic(), StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().isAllowUnsecureBasic());

        // determine if we should Basic Auth prompt if rec. NTLM token
        setNtlmSupport(StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().isPromtNtlm());

        // requests from localhost will not be authenticated against the KDC 
        this.allowLocalhost
                = StplConfigReader.getInstance().getPropertyBean().getSpnegoPropertyBean().isAllowLocalHost();

        // determine if the server supports credential delegation 
        if (null != System.getProperty(Constants.ALLOW_DELEGATION)) {
            this.allowDelegation
                    = Boolean.parseBoolean(System.getProperty(Constants.ALLOW_DELEGATION));
        }
    }
    ...
    /**
     * Set the canUseKeyTab flag by determining if all LoginModule options have
     * been set.
     *
     * <pre>
    ...
    /**
     * Returns true if a client sends an NTLM token and the filter should ask
     * client for a Basic Auth token instead.
     *
    ...
    /**
     * Return the value defined in the servlet's init params in the web.xml
     * file.
     *
    ...
    /**
     * Specify if Basic authentication is allowed and if un-secure/non-ssl Basic
     * should be allowed.
     *
     * @param basic true if basic is allowed
     * @param unsecure true if un-secure basic is allowed
     */
    private void setBasicSupport(final boolean basic, final boolean unsecure) {

        this.allowBasic = basic;
    ...
    /**
     * If request contains NTLM token, specify if a 401 should be sent back to
     * client with Basic Auth as the only available authentication scheme.
     *
     * @param ntlm true/false
     */
    private void setNtlmSupport(final boolean ntlm) {

        final boolean downgradeNtlm = ntlm;
