 ...
 */
public class GtnUIFrameworkReturnsDualListBoxConfigAction
		...
		try {
			...
			if (actionParametersList.get(1).equals("ResetLeftTableHeaders")) {
				reloadDualListBoxLeftTableHeaders(actionParametersList,
						GtnFrameworkCommonStringConstants.RETURNS_FORECAST_DUALLIST_CONFIG_ACTION, componentId);
			}
		} catch (Exception ex) {
	...
	private List<Object> configureVisibleColumnsBasedOnLevel(Object componentId, String action,
		...
		if (gtnUIFrameworkBaseComponent.getObjectFromField() != null) {
			...
			if ("company".equalsIgnoreCase(levelName)) {
				leftVisibleColumns.add(GtnFrameworkCommonConstants.LEVEL_VALUE);
				leftHeaders.add("Company");
			} else if ("brand".equalsIgnoreCase(levelName)) {
				leftVisibleColumns.add(GtnFrameworkCommonConstants.LEVEL_VALUE);
				leftHeaders.add("Brand");
			} else if ("ndc".equalsIgnoreCase(levelName) || "product".equalsIgnoreCase(levelName)
					|| "item".equalsIgnoreCase(levelName)) {
				leftVisibleColumns.addAll(
						Arrays.asList(new Object[] { GtnFrameworkCommonConstants.LEVEL_VALUE, "form", "strength" }));
				leftHeaders.addAll(Arrays.asList(new String[] { "NDC", "Form", "Strength" }));
			}
