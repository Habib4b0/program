...
public class HistRelationshipBuilderFinderImpl extends BasePersistenceImpl<HistRelationshipBuilder> implements HistRelationshipBuilderFinder{
	...
	public List findByTableName(String tableName, String columnName) {
		LOGGER.debug("Entering findByTableName with P1:String tableName=" + tableName + " P2:String columnName=" + columnName);
		...
		LOGGER.debug("findByTableName started with P1:String tableName=" + tableName + " P2:String columnName=" + columnName + " P3:List hierListValues");
		List list = new ArrayList();
		Session session = null;
		try {
                    session = openSession();

			LOGGER.debug("Query" + tableName);
	...
	public List findFilteredLevelValues(String query) {
		...
		LOGGER.debug("Filtered Values list size " + list.size());
		return list;
	}
