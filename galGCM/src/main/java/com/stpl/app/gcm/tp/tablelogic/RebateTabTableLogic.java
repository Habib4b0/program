...
public class RebateTabTableLogic extends PageTreeTableLogic {
    ...
    public Map<Integer, Object> loadData(int start, int offset) {
        ...
        List<RebateTabDTO> list = logic.getConfiguredRebateTabResults(getLastParent(), start, offset, tabSelectionDTO, false);
        int i = start;
    ...
    public int getCount() {
        int count = 0;
        if (firstGenerated) {
            count = logic.getRebateLevelCount(getLastParent(), tabSelectionDTO, projectionId, true, session);
        }
        return count;
    }
    ...
    public Object configureContainer(Object object, Container datasource) {
        ...
        ((CustomTreeContainer<RebateTabDTO>) datasource).addBean(itemId);
        if (itemId.getParent() == 1) {
            ((CustomTreeContainer<RebateTabDTO>) datasource).setChildrenAllowed(itemId, true);
        } else {
            ((CustomTreeContainer<RebateTabDTO>) datasource).setChildrenAllowed(itemId, false);
        }
        return itemId;
    }
