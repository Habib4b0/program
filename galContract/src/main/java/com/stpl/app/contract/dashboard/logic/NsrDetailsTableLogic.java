 ...
 * @author vinodhini
 */
public class NsrDetailsTableLogic extends PageTableLogic {
    ...
    public int getCount() {
        ...
        try {
            if (isFirstLoad) {
                count = (Integer) searchLogic.getNsRuleDetailsCount(ruleSystemId, this.getFilters());
            }
    ...
    public List loadData(int start, int offset) {
        ...
        if (isFirstLoad) {
            try {
                list = (List) searchLogic.loadNsRuleDetailsResults(ruleSystemId, start, offset, this.getSortByColumns(), this.getFilters());
            } catch (Exception ex) {
                LOGGER.error(ex);
            }
        }
    ...
    public void configureSearchData(final String ruleSystemId) {
        isFirstLoad = true;
        isReset = false;
        this.clearAll();
