...
//    }
    public List<GtnWsRecordBean> getDashboardLeftData(  GtnWsSearchRequest gtnWsSearchRequest) {
        ...
        FindIterable<Document> documents = gtnMongoService.fetchDataFromMongo("computedResults", inputs, values);
        List<GtnWsRecordBean> data=new ArrayList<>();
        displayNodeValues(documents,data,gtnWsSearchRequest.getRecordHeader());
        return data;
    }

    public void displayNodeValues(FindIterable<Document> documents,List<GtnWsRecordBean> data,List<Object> recordHeader) {
        if(documents!=null){
