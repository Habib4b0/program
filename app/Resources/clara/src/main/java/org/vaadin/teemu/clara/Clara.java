...
public class Clara {
	...
	Class[] customComponents = { ExtCustomTable.class, CustomTextField.class, CustomComboBox.class,
			CustomDateField.class, CustomWindow.class, CustomMenuBar.class, ExtCustomCheckBox.class,
			CalendarField.class, ErrorLabel.class };

	/**
	 * Creates an instance of {@link ClaraBuilder} that allows for more control over
	 * instantion of components by Clara.
	 *
	...
	/**
	 * Returns a {@link Component} that is read from the XML representation given as
	 * {@link InputStream}. If you would like to bind the resulting
	...
	/**
	 * Returns a {@link Component} that is read from the XML representation given as
	 * {@link InputStream} and binds the resulting {@link Component} to the given
	 * {@code controller} object.
	 * <p>
	 * Optionally you may also provide {@link AttributeFilter}s to do some
	 * modifications (or example localized translations) to any attributes present
	 ...
	 *            optional {@link AttributeFilter}s to do attribute modifications.
	 ...
	 */
	public static Component create(InputStream xml, Object controller, AttributeFilter... attributeFilters) {
		return create(xml, controller, Collections.<ComponentProvider>emptyList(), attributeFilters);
	}
	 ...
	 *             if an error is encountered during the binding.
	 */
	public static Component create(InputStream xml, Object controller, List<ComponentProvider> componentProviders,
			AttributeFilter... attributeFilters) {
		return new ClaraBuilder().withController(controller).withComponentProviders(componentProviders)
				.withAttributeFilters(attributeFilters).createFrom(xml);
	}

	/**
	 * Returns a {@link Component} that is read from an XML file in the classpath
	 * and binds the resulting {@link Component} to the given {@code controller}
	 * object.
	 * <p>
	 * The filename is given either as a path relative to the class of the
	 * {@code controller} object or as an absolute path. For example if you have a
	 * {@code MyController.java} and {@code MyController.xml} files in the same
	 * package, you can call this method like
	 ...
	 *            filename of the XML representation (within classpath, relative to
	 *            {@code controller}'s class or absolute path).
	 * @param controller
	 *            controller object to bind the resulting {@code Component}
	 *            (non-{@code null}).
	 * @param attributeFilters
	...
	public static Component create(String xmlClassResourceFileName, Object controller,
			AttributeFilter... attributeFilters) {
		return create(xmlClassResourceFileName, controller, Collections.<ComponentProvider>emptyList(),
				attributeFilters);
	}
	...
	/**
	 * Searches the given component hierarchy {@code root} for a {@link Component}
	 * with the given {@code componentId} as its {@code id} property (see
	 * {@link Component#setId(String)}).
	 * <p>
	 * If the given {@code root} is a {@link ComponentContainer}, this method will
	 * recursively iterate the component hierarchy in search for the correct
	 * {@link Component}. Otherwise if the given {@code root} is a single
	 * {@link Component}, only it is checked for its {@code id} value.
	 * </p>
	 * <p>
	 * <b>Warning</b>: if you use this method to search for a component created with
	 * an id prefix (see {@link ClaraBuilder#withIdPrefix(String)}, then this method
	 * will only find the component if the prefix is included in
	 ...
	 * @return {@link Component} with a given {@code componentId} as its {@code id}
	...
	/**
	 * Searches the given component hierarchy {@code root} for a {@link Component}
	 * with the given {@code idPrefix} and {@code componentId} as its {@code id}
