...
-----------------------------CALCUATING THE RPU FOR THE CCP+D PERIOD WISE  BASED ON THE NEETING LOGIC ATTACHED TO THE VAIBALES------------------
      ...
      CREATE TABLE #TEMP_REBATE_RPU
        (
		   ...
		   MONTHLY           NUMERIC(22, 6),
		   QUARTERLY NUMERIC(22, 6),
		   SEMI_ANNUAL NUMERIC(22, 6),
		   ANNUALLY NUMERIC(22, 6)
        )
 ...
 ANNUALLY)
 SELECT SP.CCP_DETAILS_SID --PROJECTION_DETAILS_SID
	,RM.REBATE_PROGRAM_TYPE AS REBATE_TYPE
	,DP.PERIOD_SID
	,COALESCE(SUM(DP.PROJECTION_SALES) OVER (
			PARTITION BY SP.CCP_DETAILS_SID
			,RM.REBATE_PROGRAM_TYPE
			,DP.PERIOD_SID 
			) / NULLIF(SP.PROJECTION_UNITS, 0), 0) DEDUCTION_PER_UNIT
    	,COALESCE(SUM(DP.PROJECTION_SALES) OVER (
			PARTITION BY SP.CCP_DETAILS_SID
			,RM.REBATE_PROGRAM_TYPE
			,DP.PERIOD_SID 
			) / NULLIF(SP.PROJECTION_UNITS, 0), 0) AS  MONTHLY
	,COALESCE(SUM(DP.PROJECTION_SALES) OVER (
			PARTITION BY SP.CCP_DETAILS_SID
			,RM.REBATE_PROGRAM_TYPE
			,PE.QUARTER
			) / NULLIF(SP.PROJECTION_UNITS, 0), 0) AS  QUARTERLY
	,COALESCE(SUM(DP.PROJECTION_SALES) OVER (
			PARTITION BY SP.CCP_DETAILS_SID
			,RM.REBATE_PROGRAM_TYPE
			,PE.SEMI_ANNUAL
			) / NULLIF(SP.PROJECTION_UNITS, 0), 0) AS  SEMI_ANNUAL
	,COALESCE(SUM(DP.PROJECTION_SALES) OVER (
			PARTITION BY SP.CCP_DETAILS_SID
			,RM.REBATE_PROGRAM_TYPE
			,PE.YEAR
			) / NULLIF(SP.PROJECTION_UNITS, 0), 0) AS  ANNUALLY
...
JOIN  ', @S_PROJECTION_TABLE, '  SP ON SP.CCP_DETAILS_SID = DP.CCP_DETAILS_SID
	AND DP.PERIOD_SID = SP.PERIOD_SID
JOIN #PROJECTION_DATES P ON P.CCP_DETAILS_SID = SP.CCP_DETAILS_SID
JOIN #PERIOD PE ON PE.PERIOD_SID = DP.PERIOD_SID ');
 EXEC (@SQL);
end
            ...
            WITH CTE
              ...
              AS (SELECT A.CCP_DETAILS_SID,
                            ...
                            CASE
                              ...
                              ELSE 0
                            END AS RESULT,RES
							---,CS.FIRST_NET_BASE_PRICE_VALUE
                            RESET_GRP,A.PPA_INDEX_NO
                     FROM   CTE A
                            CROSS APPLY (SELECT SUM(CASE WHEN MX_NEW_RN = NEW_RN AND B.NET_BASE_PRICE='YES' THEN ( B.NET_BASE_PRICE_VALUE * POWER(( 1 + B.PRICE_TOLERANCE / 100.00 ), NEW_RN-NETTING_EXCLUSION) ) ELSE 0 END + CASE
                                                                                                                                       WHEN MX_NEW_RN = NEW_RN AND RESET_ELIGIBLE='YES' AND B.NET_RESET_PRICE_TYPE='YES' THEN ( B.FIRST_RESET_VAL * POWER(( 1 + B.PRICE_TOLERANCE / 100.00 ), NEW_RN-NETTING_EXCLUSION-CASE WHEN NEW_RN=1 THEN 1 ELSE 0 END ) )
                                                                                                                                       ELSE 0
                                       ...
                                       --MAX(( B.FIRST_NET_BASE_PRICE_VALUE * POWER(( 1 + B.PRICE_TOLERANCE / 100.00 ), NEW_RN-NETTING_EXCLUSION) ) + CASE
                                       --                                                                                               WHEN MX_NEW_RN = NEW_RN THEN ( B.FIRST_RESET_VAL * POWER(( 1 + B.PRICE_TOLERANCE / 100.00 ), NEW_RN-NETTING_EXCLUSION) )
                                       --                                                                                               ELSE 0
                                       --                                                                                             END + ( IIF(B.NET_SUBSEQUENT_PERIOD_PRICE = 'YES', B.NET_SUBSEQUENT_PERIOD_PRICE_VALUE, 0) * POWER(( 1 + B.PRICE_TOLERANCE / 100.00 ), NEW_RN-NETTING_EXCLUSION) )) AS RES
                                        --,B.FIRST_NET_BASE_PRICE_VALUE -- ,NEW_RN,B.NET_BASE,B.FIRST_RESET_VAL,B.NET_SUBSEQUENT,B.RNO,MX_NEW_RN
                                                 ...
                                                 FROM   (SELECT ROW_NUMBER()
                                                                ...
                                                                FIRST_VALUE(NET_RESET_PRICE)
                                                                  OVER(
																	...
																	FIRST_VALUE(NET_BASE_PRICE_VALUE)
                                                                  OVER(
                                                                    PARTITION BY CCP_DETAILS_SID, RS_CONTRACT_SID,PPA_INDEX_NO, RESET_GRP
                                                                    ORDER BY PERIOD_SID )     AS FIRST_NET_BASE_PRICE_VALUE,
                                                                *
                                                         FROM   CTE B
        ...
        BEGIN ;
            WITH CTE
                 ...
                 AS (SELECT A.CCP_DETAILS_SID,
                            ...
                            RESET_GRP,A.PPA_INDEX_NO
                     FROM   CTE A
                        CROSS APPLY (SELECT SUM(CASE WHEN MX_NEW_RN = NEW_RN AND B.NET_BASE_PRICE='YES' THEN ( B.NET_BASE_PRICE_VALUE ) ELSE 0 END  + CASE
                                                                                   WHEN MX_NEW_RN = NEW_RN AND RESET_ELIGIBLE='YES' AND B.NET_RESET_PRICE_TYPE='YES' THEN ( B.FIRST_RESET_VAL )
                                                                                   ELSE 0
                                                                                 END + ( IIF(B.NET_SUBSEQUENT_PERIOD_PRICE = 'YES', B.NET_SUBSEQUENT_PERIOD_PRICE_VALUE, 0) )) AS RES
						
						--SELECT MAX(( B.FIRST_NET_BASE_PRICE_VALUE ) + CASE
