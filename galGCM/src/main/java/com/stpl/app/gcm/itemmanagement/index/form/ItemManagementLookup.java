...
public class ItemManagementLookup extends CustomWindow {
    ...
    public ItemManagementLookup(List<ItemIndexDto> itemList, SelectionDTO selection) throws Exception {
    ...
    private void configureFields() throws Exception {
        ...
        if (selection.isIsIFP()) {
            ...
            removeSummary.getContent(itemList, selection);
            mainTab.addTab(removeSummary, "Summary", null, 2);
            ...
            mainTab.addTab(transferContract.getContent(), "Transfer Contract", null, 2);
            removeSummary.getContent(itemList, selection);
            mainTab.addTab(removeSummary, "Summary", null, 3);
...
//        });
    ...
    private void tabLazyLoad(int tabPosition) throws Exception {
        LOGGER.info("tabLazyLoad inside");
        ...
        LOGGER.info("tabLazyLoad outside");
        ...
        new AbstractNotificationUtils() {
            ...
            public void yesMethod() {
                ...
                if (!builderType.equals(StringUtils.EMPTY) && !builderType.equalsIgnoreCase("Automatic")) {
                    ...
                    List<String> tempTransferList = logic.generateNewProjection(String.valueOf(selection.getUserId()), String.valueOf(selection.getSessionId()), (Integer) list.get(0), (List<String>) list.get(1), Boolean.FALSE, Boolean.TRUE);
        ...
        new AbstractNotificationUtils() {
            ...
            public void yesMethod() {
                ...
                if (!builderType.equals(StringUtils.EMPTY) && !builderType.equalsIgnoreCase("Automatic")) {
                    ...
                    List<String> tempTransferList = logic.createNewProjection((Integer) list.get(0), (List<String>) list.get(1), Boolean.FALSE, selection.getSessionDTO());
        ...
        if (isContainerEmpty) {
            new AbstractNotificationUtils() {
                ...
                public void yesMethod() {
                    ...
                    if (selection.getButtonMode().equals(ConstantsUtil.TRANSFER)) {
                        tempTransferList = logic.generateNewProjection(String.valueOf(selection.getUserId()), String.valueOf(selection.getSessionId()), (Integer) correntTransfer.get(0), (List<String>) correntTransfer.get(1), Boolean.FALSE, Boolean.FALSE);
                    }
                    if (selection.getButtonMode().equals(ConstantsUtil.PROJECTIONTRANSFER)) {
                        tempTransferList = logic.copyProjection(fromProjection, true, null, null, null);
                    }

                    int transferFromProjection = Integer.parseInt(tempTransferList.get(2).toString());
                    int toProjection = Integer.valueOf(transferContract.get(0).toString());

                    if (selection.getButtonMode().equals(ConstantsUtil.TRANSFER)) {
                        tempTransferList1 = logic.generateNewProjection(String.valueOf(selection.getUserId()), String.valueOf(selection.getSessionId()), toProjection, (List<String>) transferContract.get(1), Boolean.FALSE, Boolean.FALSE);
                    }
                    if (selection.getButtonMode().equals(ConstantsUtil.PROJECTIONTRANSFER)) {
                        tempTransferList1 = logic.copyProjection(toProjection, false, null, null, null);
                    }
                    int transferToProjection = Integer.parseInt(tempTransferList1.get(2).toString());
    ...
    public void changeToCurrentContract() {
        if (tabPosition == 0 || tabPosition == 1 || tabPosition == 2) {
            mainTab.setSelectedTab(0);
        }
    }
    ...
    public void insertToTempTable() {
     ...
     public static void waitForSave(SelectionDTO selection) {
        ...
        for (Thread saveThread : selection.getThreadList()) {
            System.out.println("saveThread-->>" + saveThread.getName());
    ...
    private void setSecurityMap() throws Exception {
        if (selection.getButtonMode().equals(ConstantsUtil.ADD)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_ADD, SecurityConstants.SUMMARY);
            SecurityLogic.isPermitted(functionHM, "PROCESS", addBtn);
        } else if (selection.getButtonMode().equals(ConstantsUtil.EDIT)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_UPDATE, SecurityConstants.SUMMARY);
            SecurityLogic.isPermitted(functionHM, "UPDATE", editBtn);
        } else if (selection.getButtonMode().equals(ConstantsUtil.DELETE)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_REMOVE, SecurityConstants.SUMMARY);
            SecurityLogic.isPermitted(functionHM, "REMOVE", removeBtn);
        } else if (selection.getButtonMode().equals(ConstantsUtil.TRANSFER)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_TRANSFER, SecurityConstants.SUMMARY);
            SecurityLogic.isPermitted(functionHM, "TRANSFER", transferBtn);
        }
        SecurityLogic.isPermitted(functionHM, "closeBtn", closeBtn);
        SecurityLogic.isPermitted(functionHM, "nextBtn", nextBtn);
        SecurityLogic.isPermitted(functionHM, "previousBtn", previousBtn);
    }
    ...
    private void tabChangeSubmitCheck() {
        if (selection.getButtonMode().equals(ConstantsUtil.TRANSFER)) {
            ...
            if (lasttabPosition == 0) {
                ...
                } else if (!addedTabList.contains(1) && (tabPosition == 2 || lasttabPosition == 3)) {
                    forceTabRefresh(3);
                    MessageBox.showPlain(Icon.ERROR, "No Item Information Submitted", "Please submit a contract to transfer the selected Item from. ", ButtonId.OK);
                } else {
                    addedTabList.add(lasttabPosition);
                ...
                if (AbstractLogic.getCount(list) == 0 && lasttabPosition != 2) {
                    if (!addedTabList.contains(tabPosition)) {
                        forceTabRefresh(3);
                        if (lasttabPosition == 1) {
                            MessageBox.showPlain(Icon.ERROR, "No Item Information Submitted", "Please submit a contract to transfer the selected Item from. ", ButtonId.OK);
