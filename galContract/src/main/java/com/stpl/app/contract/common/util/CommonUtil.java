...
public class CommonUtil {
    ...
    public ComboBox loadComboBox(final ComboBox select,
           final String listName, boolean isFilter) throws Exception {
        final HelperDTO defaultValue = new HelperDTO(0, isFilter ? Constants.SHOW_ALL : Constants.SELECT_ONE);
        ...
        select.setDescription((String) (select.getValue() == null ? Constants.SELECT_ONE : select.getValue()));
         if(listName.equals(CommonUtils.CALENDAR)){
                select.setValue(helperListUtil.getListNameMap().get(listName).get(0));
                select.select(helperListUtil.getListNameMap().get(listName).get(0));
