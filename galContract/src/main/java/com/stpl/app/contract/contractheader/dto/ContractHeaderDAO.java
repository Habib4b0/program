...
public class ContractHeaderDAO implements BeanDAO<SearchResultsDTO> {
    ...
    public ContractHeaderDAO(final CustomFieldGroup binder) {
        ...
        LOGGER.info("Enters ContractHeaderDAO ");
        this.binder = binder;
        binderFlag = true;
        LOGGER.info("ContractHeaderDAO ends");
    ...
    public int count(final BeanSearchCriteria criteria) {
        ...
        LOGGER.info("Enters count() with parameter search");
        if (binderFlag) {
            try {
        ...
        LOGGER.info("Returns count= " + countVal + " and the method count ends");
        return countVal;
    }
    ...
    public List<SearchResultsDTO> find(final BeanSearchCriteria criteria, final int startIndex, final int offset, final List<OrderByColumn> columns) {
        ...
        try {
            LOGGER.info("Enters find method with parameters startIndex=" + startIndex + " offset=" + offset + " columns list size =" + columns.size());
            if (binderFlag) {
                return chLogic.searchContractMaster(binder, startIndex, offset, columns,criteria);
            }
            LOGGER.info("End of  find method after returning contractList ");
