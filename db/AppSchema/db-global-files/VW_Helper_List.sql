...
SELECT Row_number() OVER (
	...
	,MAPPING_COLUMN_NAME
	,DESC_COLUMN_NAME,PRIMARY_KEY_COLUMN_NAME,LIST_NAME
FROM (
	SELECT DISTINCT a.ACTUAL_TABLE_NAME
		...
		,DESC_COLUMN_NAME
		,CASE 
			WHEN ACTUAL_COLUMN_NAME = 'RECORD_LOCK_STATUS'
				THEN NULL
			WHEN ACTUAL_COLUMN_NAME = 'INBOUND_STATUS'
				THEN NULL
			WHEN ACTUAL_COLUMN_NAME = 'AUTHORITIES'
				THEN NULL
			WHEN ACTUAL_COLUMN_NAME LIKE '%STATUS%'
				THEN 'STATUS'
			WHEN ACTUAL_TABLE_NAME = 'CFP_MODEL'
				AND ACTUAL_COLUMN_NAME = 'CFP_DESIGNATION'
				THEN 'CFP_DESIGNATION'
			WHEN ACTUAL_TABLE_NAME = 'CFP_CONTRACT_DETAILS'
				AND ACTUAL_COLUMN_NAME = 'TRADE_CLASS'
				THEN 'CFP_TRADE_CLASS'
			WHEN ACTUAL_TABLE_NAME = 'COMPANY_TRADE_CLASS'
				AND ACTUAL_COLUMN_NAME = 'PRIOR_TRADE_CLASS'
				THEN 'COMPANY_TRADE_CLASS'
			WHEN ACTUAL_TABLE_NAME = 'ITEM_MASTER'
				AND ACTUAL_COLUMN_NAME = 'PRIMARY_UOM'
				THEN 'UOM'
			WHEN ACTUAL_TABLE_NAME = 'ITEM_MASTER'
				AND ACTUAL_COLUMN_NAME = 'SECONDARY_UOM'
				THEN 'UOM'
			WHEN ACTUAL_TABLE_NAME = 'ITEM_PRICING'
				AND ACTUAL_COLUMN_NAME = 'ITEM_UOM'
				THEN 'UOM'
			WHEN ACTUAL_TABLE_NAME = 'PS_DETAILS'
				AND ACTUAL_COLUMN_NAME = 'RESET_FREQUENCY'
				THEN 'PRICE_TOLERANCE_FREQUENCY'
			WHEN ACTUAL_TABLE_NAME = 'PS_DETAILS'
				AND ACTUAL_COLUMN_NAME = 'RESET_INTERVAL'
				THEN 'PRICE_TOLERANCE_INTERVAL'
			WHEN ACTUAL_TABLE_NAME = 'WORKFLOW_MASTER'
				AND ACTUAL_COLUMN_NAME = 'WORKFLOW_STATUS_ID'
				THEN 'WorkFlowStatus'
			ELSE ht.list_name
			END LIST_NAME
		,cu.COLUMN_NAME PRIMARY_KEY_COLUMN_NAME
	FROM (
		SELECT C.TABLE_NAME ACTUAL_TABLE_NAME
			,C.COLUMN_NAME ACTUAL_COLUMN_NAME
			,CASE 
				WHEN CU.REFERENCED_TABLE IS NULL
					THEN C.TABLE_NAME
				ELSE CU.REFERENCED_TABLE
				END REFERENCE_TABLE_NAME
			,CASE 
				WHEN CU.REFERENCED_COLUMN = 'HELPER_TABLE_SID'
					THEN 'DESCRIPTION'
				WHEN CU.REFERENCED_COLUMN IS NULL
					THEN C.COLUMN_NAME
				ELSE CU.REFERENCED_COLUMN
				END REFERENCE_COLUMN_NAME
			,CASE 
				WHEN CU.REFERENCED_COLUMN IS NOT NULL
					THEN CU.REFERENCED_COLUMN
				ELSE CU.REFERENCED_COLUMN
				END MAPPING_COLUMN_NAME
			,CASE 
				WHEN CU.REFERENCED_COLUMN LIKE '%_SID'
					THEN Isnull((
								SELECT column_name
		...
		LEFT OUTER JOIN (
			SELECT TAB1.NAME AS [MAIN_TABLE]
			...
			) cu ON C.TABLE_NAME = CU.MAIN_TABLE
			AND C.COLUMN_NAME = CU.MAIN_COLUMN
		--and cu.constraint_name like '%FK%' 
		--and cu.column_name not like '%_SID%'
		WHERE C.TABLE_NAME NOT LIKE 'SNAP_%'
			...
			--and C.TABLE_NAME not LIKE 'UDCS%'
	...
	LEFT OUTER JOIN helper_table ht ON a.ACTUAL_COLUMN_NAME = ht.list_name
	JOIN information_schema.CONSTRAINT_COLUMN_USAGE cu ON a.ACTUAL_TABLE_NAME = cu.TABLE_NAME
	WHERE cu.constraint_name LIKE '%PK%'
	) b
GO
