...
import com.stpl.app.galforecasting.salesprojection.logic.tablelogic.AlternateHistoryTableLogic;
...
public class ItemSelection extends CustomComponent implements View {
    ...
    public void resetBtnClick(Button.ClickEvent event) {
        LOGGER.info("Entered inside reset method");
    ...
    public void configureTable() {
        ...
        availableItemsTable.setWidth(100, Unit.PERCENTAGE);
        availableItemsTable.setHeight(400, Unit.PIXELS);
        availableItemsTable.setPageLength(5);
        ...
        availableItemsTable.setColumnWidth(Constant.CHECK, 50);
        ...
        selectedItemsTable.setWidth(100, Unit.PERCENTAGE);
        selectedItemsTable.setHeight(400, Unit.PIXELS);
        selectedItemsTable.setPageLength(5);
        ...
        selectedItemsTable.setColumnWidth(Constant.CHECK, 50);
    ...
    public void addBtnClick(Button.ClickEvent event) {
        try {
            LOGGER.info("Entered inside addBtn method");
    ...
    public void addAllBtnClick(Button.ClickEvent event) {
        LOGGER.info("Entering inside Addall Button");
        selectedItemsContainer.removeAllItems();
        try {
            itemSearchBinder.commit();
        } catch (FieldGroup.CommitException ex) {
        ...
        LOGGER.info("Ending inside Addall Button");
    }

    @UiHandler("remove")
    public void removeBtnClick(Button.ClickEvent event) {
        try {
            LOGGER.info("Entered inside Remove button method");
            if (logic.checkForAtleastOneCheckedItem(session,false)) {
                ...
                altHistoryDTO.getSelectedProductSet().clear();
                logic.getCheckedItemsFromTemptable(itemSearchBinder, altHistoryDTO, null, 0, 0, session.getSessionId());
            ...
            LOGGER.info("Ending Remove button method");
        } catch (Exception ex) {
            LOGGER.error(ex);
        }
    }
    ...
    public void removeAllBtnBtnClick(Button.ClickEvent event) {
        LOGGER.info("Entered inside removeAllBtn btn method");
        if (logic.checkForAtleastOneCheckedItem(session, false)) {
            logic.removeItems(session);
            altHistoryDTO.getSelectedProductSet().clear();
            selectedItemsTableLoic.setFireData(altHistoryDTO, session);
        ...
        LOGGER.info("Ending removeAllBtn btn method");
    }
    ...
    public void selectedCustomerExport(Button.ClickEvent event) {
        try {
            infoExcelFlag = true;
            final int recordCount = logic.getCheckedItemsCount(itemSearchBinder, altHistoryDTO, null, session.getSessionId());
            if (recordCount > 0) {
        ...
        try {
            if (end != 0) {
                if (contractExcelFlag) {
                    ...
                    List<AlternateHistoryDTO> searchList = logic.getCheckedItemsFromTemptable(itemSearchBinder, altHistoryDTO, null, start, end, session.getSessionId());
    ...
    private void unCheckRecords() {
        for (AlternateHistoryDTO temp : availableItemsContainer.getItemIds()) {
            availableItemsContainer.getItem(temp).getItemProperty("check").setValue(false);
        }
    }
