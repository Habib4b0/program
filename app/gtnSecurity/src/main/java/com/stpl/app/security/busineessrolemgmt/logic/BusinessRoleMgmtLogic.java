...
public class BusinessRoleMgmtLogic extends BeanItemContainer<BusinessroleMasterDTO> {
	private static final Logger LOGGER = LoggerFactory.getLogger(BusinessRoleMgmtLogic.class.getName());
	...
	public List<BusinessroleMasterDTO> getCustomizedListFromModel(List<BusinessroleMaster> list) {
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				businessroleMasterDTO
						.setCreatedDate(CommonUtils.convertDateToString(businessroleMaster.getCreatedDate()));
				...
				businessroleMasterDTO
						.setModifiedDate(CommonUtils.convertDateToString(businessroleMaster.getModifiedDate()));
	...
	public String saveBusinessRoleMgmt(ErrorfulFieldGroup searchBusinessRoleForm) {
		...
		if (searchBusinessRoleForm.getField(CommonUtils.BUSINESS_ROLE_MASTERSID).getValue() != null && !"0"
				.equals(searchBusinessRoleForm.getField(CommonUtils.BUSINESS_ROLE_MASTERSID).getValue().toString())) {
	...
	private void updateBusinessRoleMgmt(ErrorfulFieldGroup searchBusinessRoleForm) {
		...
		Integer businessroleMasterSid = Integer
				.valueOf(searchBusinessRoleForm.getField(CommonUtils.BUSINESS_ROLE_MASTERSID).getValue().toString());
		try {
			BusinessroleMaster businessroleMaster = BusinessroleMasterLocalServiceUtil
					.fetchBusinessroleMaster(businessroleMasterSid);
			String beforeUpdate = businessroleMaster.toString();
			...
			String afterUpdate = businessroleMaster.toString();
			dao.updateBusinessRoleMgmt(businessroleMaster);
			String auditMessage = beforeUpdate + " changed to " + afterUpdate;
			auditMessageProducer.logAuditMessage(PortalUtil.getDefaultCompanyId(), auditMessage,
	...
	private String addBusinessRoleMgmt(ErrorfulFieldGroup searchBusinessRoleForm) {
		...
		try {
			DynamicQuery businessroleMasterDynamicQuery = BusinessroleMasterLocalServiceUtil.dynamicQuery();
			businessroleMasterDynamicQuery
	...
	public void deleteBusinessRole(int businessroleMasterSid) {
		try {
			...
			dao.updateBusinessRoleMgmt(businessroleMaster);
			String auditMessage = businessroleMaster.toString() + " deleted " ;
			auditMessageProducer.logAuditMessage(PortalUtil.getDefaultCompanyId(), auditMessage,
					this.getClass().getName());
		} catch (SystemException e) {
			LOGGER.error(e.getMessage());
