...
public class CompanyTypeDAO implements DAO<HelperDTO> {
    ...
    public CompanyTypeDAO(final HelperDTO manufactureId) {
    ...
    public List<HelperDTO> find(SearchCriteria searchCriteria, int startIndex, int offset, List<OrderByColumn> list) {
        try {
            ...
            return logic.getLazyCompanyTypeResults(startIndex, startIndex + offset, searchCriteria.getFilter(),manufactureId);
        } catch (PortalException ex) {
            AbstractNotificationUtils.getErrorNotification(ErrorCodeUtil.getEC(ErrorCodes.ERROR_CODE_1001), ErrorCodeUtil.getEC(ErrorCodes.ERROR_CODE_1004));
        } catch (SystemException ex) {
            LOGGER.error(ex);
