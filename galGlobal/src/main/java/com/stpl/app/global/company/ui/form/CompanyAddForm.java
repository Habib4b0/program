...
public class CompanyAddForm extends CustomComponent  {
     ...
     */
    private List<CompanyMasterDTO> companyTradeClassList = new ArrayList<CompanyMasterDTO>();
    /**
     * The tabsheet.
     */
    private TabSheet tabsheet;
    ...
    public CompanyAddForm(SessionDTO sessionDTO) throws PortalException, SystemException, Exception {
        super();
        setSizeFull();
    ...
    public void enableDisableAndInformationTab(String mode) throws PortalException, SystemException, Exception {
    ...
    private void editButton() throws Exception {
        LOGGER.info("Entering editButton");
            ...
            public void buttonClick(final Button.ClickEvent event) {
                try {
                    ...
                    if (tradeClassResultsBean != null && tradeClassResultsBean.size() == 0) {
                        errorMessage.append("Add at least one Trade Class in Company Trade Class tab");
                        flag = true;
                    }
                    if (flag) {
                    ...
                    for (int j = 0; j < tradeClassResultsBean.size(); j++) {
                        ...
                        if (!StringUtils.EMPTY.equals(dto.getTradeClass() )) {
                            ...
                            binder.getErrorDisplay().setError(
                                    "Trade Class is Mandatory in Company TradeClass Tab");
                            return;
                        }
                    ...
                    if (companyTradeList.isEmpty()) {
                        binder.getErrorDisplay().setError(
                                "Add at least one Trade Class in Company Trade Class tab");
                        return;
                    }
                    /* End Fourth tab */
                    ...
                    for (int j = 0; j < parentCompanyResultsBean.size(); j++) {
                        ...
                        if (dto.getParentStartDate() != null) {
                            ...
                            binder.getErrorDisplay().setError(
                                    "Parent Company Start Date is Mandatory in Parent Company Tab");
                            return;
                        }
                    ...
                    for (int temp : unique) {
                        ...
                        if (count > 1) {
                            ...
                            for (CompanyMasterDTO ob : parentCompanyList) {
                                if (ob.getParentCompanySysId() == temp) {
                                    if ((duplicateCheck.get(temp).getParentStartDate()).equals(ob.getParentStartDate())) {
                                        dupcount++;
                                    }
                                }
                            }
                    ...
                    for (int i = 0; i < identifierResultsBean.size(); i++) {
                        ...
                        if (!companyIdentiiferList.isEmpty()) {
                            binder.getErrorDisplay().setError("Company Identifier already exists for another Company");
                            return;
                        }
                        for (int k = 0; k < identifierList.size(); k++) {
                            if (identifierList.get(k).getCompanyIdentifier().equals(iden.getCompanyIdentifier())) {
                    ...
                    LOGGER.info("before calling save method");
                    ...
                    LOGGER.info("after calling save method");

                    LOGGER.info("Ending  SAVE  method from ADD");
    ...
    private void backButton() {
        ...
        backBtn.addClickListener(new Button.ClickListener() {
            ...
            @SuppressWarnings("PMD")
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("Entering inside  BACK  method from ADD ");
                if (sessionDTO.getMode().equals(ConstantsUtils.VIEW)) {
                    ...
                    LOGGER.info("Ending  BACK  method from ADD");
                }
            }
        });
                        ...
                        public void buttonClicked(final ButtonId buttonId) {
                            try {
                                ...
                                if ((ConstantsUtils.ADD).equals(mode)) {
                                    ...
                                    CompanyCrtIdentifierDTO identifierDTO = new CompanyCrtIdentifierDTO();
                                    if (selectedTabIndex == 0) {
                                        TextField id = (TextField) binder.getField(FiledNameUtils.COMPANY_ID);
    ...
    private void updateButton() throws Exception {
        ...
        saveBtn.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(final Button.ClickEvent event) {
                try {
                    LOGGER.info("Entering inside  UPDATE  method from EDIT ");
                    binder.getErrorDisplay().clearError();
                    binder.getFields();
                    ...
                    for (int i = 0; i < identifierResultsBean.size(); i++) {
                        ...
                        if (iden.getStartDate() == null) {
                            binder.getErrorDisplay().setError(
                                    "Start Date is Mandatory in Identifier Tab");
                            return;
                        }
                        ...
                        if (sysId == StringUtils.EMPTY) {
                            if (!companyIdentiiferList.isEmpty()) {
                                binder.getErrorDisplay().setError(
                                        "Company Identifier already exists for another Company");
                                return;
                            }
                        } else {
                            final int primaryId = Integer.parseInt(sysId);
                            companyIdentifierDynamicQuery.add(RestrictionsFactoryUtil.eq("companyMasterSid",
                                    primaryId));
                     ...
                     MessageBox.showPlain(Icon.QUESTION, commmonUtil.getHeaderMessage(), commmonUtil.getSaveMessage(String.valueOf(binder.getField(ConstantsUtils.COMPANY_NAME).getValue())) , new MessageBoxListener() {
                        ...
                        public void buttonClicked(final ButtonId buttonId) {
                            if (buttonId.name().equals(ConstantsUtils.YES)) {
                                ...
                                companyLogic.saveCompanyMaster(binder, identifierList, companyTradeList, parentCompanyList, notesTabForm.getUploadedData(), notesTabForm.getAddedNotes(), notesTabForm.removeDetailsList());
                ...
                LOGGER.info("Ending  UPDATE  method from EDIT");
            }
        });
    }
    ...
    private void deleteButton() {
        ...
        deleteBtn.addClickListener(new Button.ClickListener() {
            ...
            @SuppressWarnings("PMD")
            public void buttonClick(final Button.ClickEvent event) {
                LOGGER.info("Entering inside  DELETE  method from EDIT");
                binder.getFields();
                ...
                try {
                    ...
                    if (companyParentDetailsList != null && !companyParentDetailsList.isEmpty() && companyParentDetailsList.size() > 0) {
                        ...
                        final MessageBox msg = MessageBox.showPlain(Icon.WARN, "Cannot Delete", "Company cannot be deleted as it is associated as parent to another Company", new MessageBoxListener() {
                            /**
                    ...
                    if (!isParentCompany) {
                        ...
                        cfpDynamicQuery.add(RestrictionsFactoryUtil.eq("companyMasterSid",
                                systemId)); 
                        ...
                        if (cfpDetailsList != null && !cfpDetailsList.isEmpty() && cfpDetailsList.size() > 0) {
                            isInCfp = true;
                            final MessageBox msg = MessageBox.showPlain(Icon.WARN, "Cannot Delete", "Company cannot be deleted as it is associated with CFP", new MessageBoxListener() {
                                /**
                                 * The method is triggered when a button of the
                                 * message box is pressed .
                                 *
                    ...
                    if (!isParentCompany && !isInCfp) {
                        ...
                        contractDynamicQuery.add(RestrictionsFactoryUtil.eq("companyMasterSid", systemId));
                        ...
                        if (compContractList != null && !compContractList.isEmpty() && compContractList.size() > 0) {
                            isInContract = true;
                            final MessageBox msg = MessageBox.showPlain(Icon.WARN, "Cannot Delete", "Company cannot be deleted as it is associated with Contract", new MessageBoxListener() {
                                /**
                                 * The method is triggered when a button of the
                                 * message box is pressed .
                                 *
                    ...
                    if (!isParentCompany && !isInCfp && !isInContract) {
                    if (companyParentDetailsList.isEmpty()) {
                ...
                LOGGER.info("Ending  DELETE  method from EDIT");
                }
        });
    }
    ...
    public void addTabsheet() throws PortalException, SystemException, Exception {
        ...
        tabSheet.addTab(compayInformation.getContent(binder, sessionDTO), "Company Information", null);
        ...
        tabSheet.addTab(compayTradeClass.getContent(binder, tradeClassResultsBean, sessionDTO), "Company Trade Class", null);
        Component companyComponent=compayParent.getContent(binder, parentCompanyResultsBean, sessionDTO);
        tabSheet.addTab(companyComponent, "Parent Company",null);
