...
import static com.stpl.app.util.Constants.FrequencyConstants.*;
...
public class NmDiscountProjMasterFinderImpl extends BasePersistenceImpl<NmDiscountProjMaster> implements NmDiscountProjMasterFinder {
    ...
    public List getDiscountProjection(int projectionId, String userId, String sessionId, String frequency, List<Integer> startAndEndPeriods, String hierarchyNo, boolean isProgram, List<String> discountList,
            ...
            List<String> customViewDetails, boolean isRefresh, String refreshHierarchyNumbers, String relationshipBuilderSid,boolean isAltHistory) {
        Session session = null;
        String customQuery = StringUtils.EMPTY;
        String genQuery = StringUtils.EMPTY;
        if(isCustom){
           genQuery = CustomSQLUtil.get("nm.discCustomGen"); 
        }
        else{
           genQuery = CustomSQLUtil.get("nm.discountGenerate"); 
        }
...
//                        hierarchyNumberSelectionStatement = connector+"HLD.HIERARCHY_NO in(" + refreshHierarchyNumbers + ")";
                        ...
                        List<String> hierarchyNoList = getHierarchyList(projectionId, hierarchy, hierarchyNo, relationshipBuilderSid, levelNo, userGroup, discountList, userId, sessionId, isProgram, startIndex, endIndex,isAltHistory);
...
//                    }
                    ...
                    List<String> hierarchyNoList = getHierarchyListForCustomView(projectionId, hierarchyIndicator, userGroup, customViewDetails, discountList, userId, sessionId, isProgram, startIndex, endIndex,isAltHistory);
                    hierarchyNumbers = CommonUtils.CollectionToString(hierarchyNoList, true);
                }

                if (hierarchyNumbers.isEmpty()) {
                    LOGGER.info("Returning Hierarchy Number Empty:::::::::::::::::::::::::::::::::");
...
//                            }
                    ...
                    ccpDetails = "(SELECT TEMP_CCP.RELATIONSHIP_LEVEL_SID, TEMP_CCP.HIERARCHY_NO,  TEMP_CCP.CCP_DETAILS_SID FROM \n"
                            + "   (SELECT distinct HLD" + hierarchyIndicator + ".RELATIONSHIP_LEVEL_SID,HLD" + hierarchyIndicator + ".HIERARCHY_NO, CCPMAPC.CCP_DETAILS_SID FROM \n"
                ...
                if (!isAltHistory) {
                   ...
                   genQuery = genQuery.replace("?DSC", discType);
                    ...
                    String customSql = "  ST_NM_DISCOUNT_PROJ_MASTER B, PROJECTION_DETAILS E ,\n PROJECTION_MASTER F, "
                            //                        + hierarchy + " G, "
                ...
                String historyQuery = selectClause + "\n COALESCE(Sum(A.ACTUAL_SALES) / (NULLIF(Sum(distinct S.ACTUAL_SALES), 0)), 0) * 100 as ACTUAL_RATE,"
                       ...
                       if(!isAltHistory){
                        ...
                        + "from ST_NM_ACTUAL_DISCOUNT A, ST_NM_ACTUAL_SALES S, \n"
                ...
                String futureQuery = selectClause + "\n NULL as ACTUAL_RATE,"
                        ...
                        + "from ST_NM_DISCOUNT_PROJECTION A,ST_NM_SALES_PROJECTION S, \n"
                        + customSql;
...
//                        + "\n \n  UNION \n \n " + futureQuery + orderBy;
            ...
            LOGGER.info(" Fetching Discount Data");
            Query sqlQuery;
    ...
    private List getHierarchyListForCustomView(int projectionId, String hierarchyIndicator, String userGroup, List<String> customViewDetails, List<String> discountList,
            String userId, String sessionId, boolean isProgram, int startIndex, int endIndex,boolean isAlternate) {
        LOGGER.info(" Inside getHierarchyListForCustomView");
        Session session = null;
        ...
        try {
            ...
            LOGGER.info(" exiting getHierarchyListForCustomView");
            closeSession(session);
        }
    }
    ...
    private List getHierarchyList(int projectionId, String hierarchy, String hierarchyNo, String relationshipBuilderSid, int levelNo, String userGroup,
            List<String> discountList, String userId, String sessionId, boolean isProgram, int startIndex, int endIndex,boolean isAlternate) {
    ...
    public int getCheckedRecordCount(int projectionId, String userId, String sessionId, boolean isProgram, List<String> discountList) {
        ...
        List list = new ArrayList();
        String customSql = StringUtils.EMPTY;
        LOGGER.info(" inside getCheckedRecordCount");
        try {
            session = openSession();
            String discountTypeQuery = "";
            if (discountList != null && !discountList.isEmpty()) {
            ...
            LOGGER.info(" exiting getCheckedRecordCount");
            closeSession(session);
        }
    ...
    public List getDiscountPrograms(int projectionId, String userId, String sessionId, String programType) {
        Session session = null;
String customSql = StringUtils.EMPTY;
        LOGGER.info(" inside getDiscountPrograms");
        try {
            session = openSession();
//                String customSql = CustomSQLUtil.get("findViewByName");
            String selectClause = "";
            ...
            if ("DiscountType".equals(programType)) {
                selectClause = "select Distinct '' as DISCOUNT_ID, DM.PRICE_GROUP_TYPE as DISCOUNT_NAME";
                whereClause = " and DM.PRICE_GROUP_TYPE IS NOT NULL";
            } else if ("DiscountProgram".equals(programType)) {
                selectClause = "select Distinct RS.RS_ID as DISCOUNT_ID, RS.RS_NAME as DISCOUNT_NAME";
                whereClause = " and DM.RS_MODEL_SID = RS.RS_MODEL_SID and RS.RS_NAME IS NOT NULL";
            }

customSql = selectClause + " from ST_NM_DISCOUNT_PROJ_MASTER DM, PROJECTION_DETAILS D , PROJECTION_MASTER M , \n"
                    + " RS_MODEL RS where DM.PROJECTION_DETAILS_SID = D.PROJECTION_DETAILS_SID "
                    + whereClause
                    + " and D.PROJECTION_MASTER_SID = " + projectionId + " and DM.USER_ID = " + userId + "\n"
                    + " and DM.SESSION_ID = " + sessionId + "\n"
            ...
            LOGGER.info(" exiting getDiscountPrograms");
            closeSession(session);
        }
    }
    ...
    public int updateCheckRecord(int projectionId, String userId, String sessionId, boolean checkValue, String hierarchyNo, String userGroup,
        ...
        Session session = null;
        String customSql = "";
        LOGGER.info(" inside updateCheckRecord");
        try {
            session = openSession();
...
//                }
    ...
    public void massUpdate(int projectionId, String userId, String sessionId, String frequency, List<Integer> startAndEndPeriods, String selectedField, String fieldValue,
        ...
        String customSql = "";
        LOGGER.info(" entering massUpdate");
        try {
            String column="";
            String column2="";
...
//                    LOGGER.info(" startAndEndPeriods " + startAndEndPeriods.toString());
            ...
            LOGGER.info(" exiting massUpdate");
            closeSession(session);
        }
    }
    ...
    public List<String> getGroupValues(int projectionId, String userId, String sessionId, String masterTable, List<String> discountList, String relationshipBuilderSid) {
        ...
        Session session = null;
        String customSql = StringUtils.EMPTY;
        LOGGER.info(" entering getGroupValues");
        try {
            session = openSession();
            ...
            LOGGER.info(" exiting getGroupValues");
            closeSession(session);
        }
    }
    ...
    public boolean saveGroupValues(int projectionId, String userId, String sessionId, String hierarchyNo, String groupValue, boolean isProgram,
        ...
        Session session = null;
        String customSql = StringUtils.EMPTY;
        LOGGER.info(" entering saveGroupValues");
        try {
            session = openSession();
            ...
            LOGGER.info(" exiting saveGroupValues");
            closeSession(session);
        }
    }
