...
AS (
	...
	WHERE EXISTS (
			SELECT 1
			FROM #RS_DATA A
			WHERE A.RS_CONTRACT_SID = RS.RS_CONTRACT_SID
			) 
	)
...
AS (
	...
	FROM (SELECT DISTINCT  PROJECTION_MASTER_SID
	    ,YEAR,ITEM_MASTER_SID,PERIOD_SID
...
AS (
	...
	FROM (
		SELECT DT.PROJECTION_MASTER_SID
			,DT.CCP_DETAILS_SID
			,PERIOD
	        ,YEAR
			,DT.PERIOD_SID
...
AS (
	...
	FROM (
		...
		,ACTUAL_UNITS = IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), NS.ACTUAL_UNITS, NULL) * ISNULL(UOM_VALUE, 0)
		,PROJECTION_SALES = IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), NP.PROJECTION_SALES, NULL)
		,PROJECTION_UNITS = IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), NP.PROJECTION_UNITS, NULL) * ISNULL(UOM_VALUE, 0)
		FROM #DATA_TABLE DT
...
LEFT JOIN GTS GTS ON DT.PROJECTION_MASTER_SID = GTS.PROJECTION_MASTER_SID
	AND DT.YEAR = GTS.YEAR
	AND DT.PERIOD = GTS.PERIOD
LEFT JOIN SALES SALES ON DT.PROJECTION_MASTER_SID = SALES.PROJECTION_MASTER_SID
	AND DT.YEAR = SALES.YEAR
	AND DT.PERIOD = SALES.PERIOD
LEFT JOIN DISC DISC ON DT.PROJECTION_MASTER_SID = DISC.PROJECTION_MASTER_SID
	AND DT.YEAR = DISC.YEAR
	AND DT.PERIOD = DISC.PERIOD
LEFT JOIN PPA PPA ON DT.PROJECTION_MASTER_SID = PPA.PROJECTION_MASTER_SID
	AND DT.YEAR = PPA.YEAR
                      ...
                      --SECOND TO LAST PROJECTION_DETAILS_SID(PULL FROM MAIN TABLE) START
                            ...
                            SELECT COALESCE(a.PROJECTION_MASTER_SID, SALES.PROJECTION_MASTER_SID) PROJECTION_ID,
                                   ...
                                   Isnull(CONTRACT_UNITS_PROJECTED, 0) AS  CONTRACT_UNITS_PROJECTED ,
                                    Isnull(DISC.CONTRACT_DISCOUNT_ACTUALS
                                                    + Isnull(PPA.PPA_DISCOUNT_ACTUALS, 0), 0) AS TOTAL_DISCOUNT ,
                                    Isnull(DISC.CONTRACT_DISCOUNT_PROJECTED
                                                              + Isnull(PPA.PPA_DISCOUNT_PROJECTED, 0), 0) AS TOTAL_DISCOUNT_PROJECTED ,
                                    ( Isnull(( DISC.CONTRACT_DISCOUNT_ACTUALS
                                   ...
                                   ( Isnull(SALES.CONTRACT_SALES_ACTUALS - ( Isnull(DISC.CONTRACT_DISCOUNT_ACTUALS, 0)
                                                                                             + Isnull(PPA.PPA_DISCOUNT_ACTUALS, 0)  ), 0)) AS NET_SALES ,
                                    Isnull(SALES.CONTRACT_SALES_PROJECTED - ( DISC.CONTRACT_DISCOUNT_PROJECTED
                                                                                                       + Isnull(PPA.PPA_DISCOUNT_PROJECTED, 0) ), 0) AS NET_SALES_PROJECTED , 
                                    Isnull(SALES.COGS_ACTUALS, 0) AS COGS_ACTUALS ,
                                    Isnull(SALES.COGS_PROJECTED, 0) AS COGS_PROJECTED ,
                                   (  Isnull((SALES.CONTRACT_SALES_ACTUALS - ( DISC.CONTRACT_DISCOUNT_ACTUALS
                                                                                                       + Isnull(PPA.PPA_DISCOUNT_ACTUALS, 0) ) ) - ( SALES.COGS_ACTUALS ) , 0) )AS NET_PROFIT_ACTUAL ,
                                    (  Isnull((SALES.CONTRACT_SALES_PROJECTED - ( DISC.CONTRACT_DISCOUNT_PROJECTED
                                                                                                            + Isnull(PPA.PPA_DISCOUNT_PROJECTED, 0) ) ) - ( SALES.COGS_PROJECTED ),0)) AS NET_PROFIT_PROJECTED ,
                                   ...
                                   ( COALESCE(((SALES.CONTRACT_SALES_ACTUALS - ( DISC.CONTRACT_DISCOUNT_ACTUALS
                                                                                                                               + Isnull(PPA.PPA_DISCOUNT_ACTUALS, 0) ) )) / NULLIF(A.GTS_SALES_ACTUALS, 0), 0) * 100 ) AS NET_SALES_OF_EX_FACTORY_ACTUALS ,
                                    ( COALESCE((( SALES.CONTRACT_SALES_PROJECTED - ( DISC.CONTRACT_DISCOUNT_PROJECTED
                                                                                                                                   + Isnull(PPA.PPA_DISCOUNT_PROJECTED, 0) ) )) / NULLIF(A.GTS_SALES_PROJECTED, 0), 0) * 100 ) AS NET_SALES_OF_EX_FACTORY_PROJECTED ,
                                    COALESCE((( DISC.CONTRACT_DISCOUNT_ACTUALS
                                                                                + Isnull(PPA.PPA_DISCOUNT_ACTUALS, 0) )) / NULLIF(A.GTS_SALES_ACTUALS, 0), 0) * 100 AS DISCOUNT_OF_EX_FACTORY_ACTUALS ,
                                  COALESCE((( DISC.CONTRACT_DISCOUNT_PROJECTED
                                                                                  + Isnull(PPA.PPA_DISCOUNT_PROJECTED, 0) )) / NULLIF(A.GTS_SALES_PROJECTED, 0), 0) * 100 AS  DISCOUNT_OF_EX_FACTORY_PROJECTED
                            ,NET_EX_FACTORY_SALES_ACTUALS=isnull(((ISNULL(A.GTS_SALES_ACTUALS, 0))-( DISC.CONTRACT_DISCOUNT_ACTUALS+ ISNULL(PPA.PPA_DISCOUNT_ACTUALS, 0))),0) -----------cel-386
							,NET_EX_FACTORY_SALES_PROJECTED =isnull(ISNULL(A.GTS_SALES_PROJECTED, 0)-(DISC.CONTRACT_DISCOUNT_PROJECTED + ISNULL(PPA.PPA_DISCOUNT_PROJECTED, 0)),0)-----------cel-386
