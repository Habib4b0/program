...
( SELECT g.ITEM_MASTER_SID,
       p.PERIOD_SID,
       A.ACTUAL_GTS_SALES,
       coalesce(a.FORECAST_GTS_SALES,A.ACTUAL_GTS_SALES)  FORECAST_GTS_SALES
...
-----------------Exfactory Logic
    IF Object_id('TEMPDB..#TEMP_EXFACTORY') IS NOT NULL
          ...
          SELECT DISTINCT 
                          ...
                          PERIOD_SID,
                          ACTUAL_GTS_SALES,
                          coalesce(FORECAST_GTS_SALES,ACTUAL_GTS_SALES) FORECAST_GTS_SALES
...
TOT_PROJECTION_SALES  NUMERIC(22,6),SALES_INCLUSION BIT)
')
...
SELECT 0,
	...
	a.period,
	TOT_ACTUAL_SALES,
	TOT_PROJECTION_SALES,SALES_INCLUSION
FROM (
	SELECT PERIOD,
		YEAR,SALES_INCLUSION
		sum(iif(INDICATOR = 0, sales, 0)) TOT_ACTUAL_SALES,
		sum(iif(INDICATOR = 1, sales, 0)) TOT_PROJECTION_SALES
	FROM ', @CUSTOM_TABLE_SALES, '
	GROUP BY PERIOD,
		YEAR,SALES_INCLUSION
	) a
	')
...
set @sql=concat('IF OBJECT_ID(''',@CURRENT_TOTAL_EXFACTORY,''') IS NOT NULL
DROP TABLE ',@CURRENT_TOTAL_EXFACTORY,'

CREATE TABLE ',@CURRENT_TOTAL_EXFACTORY,'
(
CFF_MASTER_SID INT,
YEAR SMALLINT,
PERIOD SMALLINT,
...
EXEC Sp_executesql @sql
SET @SQL = CONCAT ('
insert into ',@CURRENT_TOTAL_EXFACTORY,'
	SELECT 0,YEAR,PERIOD,		
		sum(ACTUAL_GTS_SALES) TOT_EXFACTORY_SALES#ACT,
...
AS (
	SELECT DISTINCT COMPANY_MASTER_SID
		...
		,RS.RS_CONTRACT_SID
		,RS.RS_MODEL_SID
	FROM CCP_DETAILS CD JOIN (select distinct RS_CONTRACT_SID,CCP_DETAILS_SID from ',@CUSTOM_CCP_SALES,' where CUST_VIEW_MASTER_SID= ', @CUSTOM_VIEW_MASTER_SID, ') C ON C.CCP_DETAILS_SID=CD.CCP_DETAILS_SID
...
,ISNULL(SUM(ACD.DISCOUNT_AMOUNT),0) AS ACCRUAL_DISCOUNT,
       ...
       0 INDICATOR
      FROM (select distinct RS_CONTRACT_SID,CCP_DETAILS_SID from ',@CUSTOM_CCP_SALES,' where CUST_VIEW_MASTER_SID= ', @CUSTOM_VIEW_MASTER_SID, '  ) C
       INNER JOIN #DEDUCTION_INCLUSION DM
       ON DM.CCP_DETAILS_SID = C.CCP_DETAILS_SID
       AND (DM.RS_CONTRACT_SID=C.RS_CONTRACT_SID OR  C.RS_CONTRACT_SID=0)
             CROSS JOIN (SELECT PERIOD_SID,
                                period,
                                YEAR
...
ACD.PERIOD=A.PERIOD AND ACD.YEAR=A.YEAR
              WHERE  EXISTS (SELECT 1 FROM ', @CCP_HIERARCHY, ' CCP WHERE CCP.CCP_DETAILS_SID=C.CCP_DETAILS_SID)
...
LEFT JOIN NM_DISCOUNT_PROJECTION PTD  
       ON C.CCP_DETAILS_SID = PTD.CCP_DETAILS_SID
              AND DM.PROJECTION_MASTER_SID = PTD.PROJECTION_MASTER_SID
...
SET @SQL_DED =CONCAT('INSERT INTO #DEDUCTION_INCLUSION_PRIOR(PROJECTION_MASTER_SID,
                      ...
                      WHERE  EXISTS (SELECT 1
                                     ...
                                     WHERE  (RD.RS_CONTRACT_SID = RS.RS_CONTRACT_SID or rd.RS_CONTRACT_SID=0)
...
AS (
	...
	FROM CCP_DETAILS CD JOIN (select distinct RS_CONTRACT_SID,CCP_DETAILS_SID,ROWID from ',@CUSTOM_CCP_SALES,' where CUST_VIEW_MASTER_SID= ', @CUSTOM_VIEW_MASTER_SID, ' ) C ON C.CCP_DETAILS_SID=CD.CCP_DETAILS_SID
                  join ', @CCP_HIERARCHY, ' ch
                     on  ch.CCP_DETAILS_SID = c.CCP_DETAILS_SID 
	JOIN NM_DISCOUNT_PROJ_MASTER RS ON RS.CCP_DETAILS_SID = ch.CCP_DETAILS_SID
    and (RS.RS_CONTRACT_SID = C.RS_CONTRACT_SID or c.RS_CONTRACT_SID =0)
...
SET @SQL_APP =  @SQL_APP + ' GROUP  BY PROJECTION_MASTER_SID,
	...
	SET @SQL = CONCAT ('
insert into ',@CURRENT_TOTAL_SALES,'(CFF_MASTER_SID,year,period,TOT_PROJECTION_SALES)
SELECT a.PROJECTION_MASTER_SID,
	a.year,
	a.period,
	TOT_PROJECTION_SALES
FROM (
	SELECT PROJECTION_MASTER_SID,
		A.PERIOD,
		A.YEAR,
		Sum(PROJECTION_SALES) AS TOT_PROJECTION_SALES
	FROM (
		SELECT PT.PROJECTION_MASTER_SID,
			...
			Sum(PROJECTION_SALES) AS PROJECTION_SALES
		FROM #PRIOR_DATA_TABLE PT
		INNER JOIN #SALES_INCLUSION SI ON SI.PROJECTION_DETAILS_SID = PT.PROJECTION_DETAILS_SID
		LEFT JOIN NM_SALES_PROJECTION NSP1 ON NSP1.PROJECTION_MASTER_SID = PT.PROJECTION_MASTER_SID
			AND NSP1.CCP_DETAILS_SID = PT.CCP_DETAILS_SID
			AND NSP1.PERIOD_SID = PT.PERIOD_SID
		) ab
	GROUP BY PROJECTION_MASTER_SID,
		PERIOD,
		YEAR
