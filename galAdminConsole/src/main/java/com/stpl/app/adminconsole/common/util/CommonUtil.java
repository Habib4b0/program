...
public final class CommonUtil {
    ...
    public static ExtFilterTable getFormattedTable(final ExtFilterTable table, final Object... formatCols) {
        LOGGER.info("Entering getFormattedTable with p1:table ,p2:formatCols length" + formatCols.length);
        for (int i = 0; i < formatCols.length; i++) {
            table.setColumnAlignment(formatCols[i], ExtFilterTable.Align.CENTER);
        }
        LOGGER.info("getFormattedTable Returning Table ");
    ...
    public static NativeSelect getDropDown(final NativeSelect select, final String listName) throws SystemException {
        ...
        LOGGER.info("Entering getDropDown P1:select and P2:listName=" + listName);
        final DynamicQuery cfpDynamicQuery = DynamicQueryFactoryUtil.forClass(HelperTable.class);

        cfpDynamicQuery.add(RestrictionsFactoryUtil.like("listName", listName));
        ...
        LOGGER.info("getDropDown RETURN NativeSelect select");
        return select;
    }
    ...
    public static ComboBox getComboBox(final ComboBox select, final String listName) throws SystemException {
        ...
        select.select(0);
        LOGGER.info("getDropDown RETURN NativeSelect select" + select.size());
    ...
    public static ComboBox getFileTypeComboBox(final ComboBox select, final String listName, List<HelperDTO> fileTypeList) throws SystemException {
        ...
        cfpDynamicQuery.add(RestrictionsFactoryUtil.like("listName", listName));
        cfpDynamicQuery.add(RestrictionsFactoryUtil.ne("description", ConstantsUtils.INVENTORY_WITHDRAWAL));
        cfpDynamicQuery.addOrder(OrderFactoryUtil.asc("description"));
        final List<HelperTable> list = hierarchyBuilderLogicDAO.getHelperTableList(cfpDynamicQuery);
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
    ...
    public static Map<String, String> getCreatedByUser() throws SystemException {
        ...
        LOGGER.info("Entering getCreatedByUser()");
        ...
        LOGGER.info("getCreatedByUser return HashMap<String, String> userMap size" + userMap.size());
        return userMap;
    }
     ...
     * @throws PortalException the portal exception
     */
    public static int getIDFromHelper(final String listName) throws SystemException, PortalException {

        LOGGER.info("Entering getIDFromHelper() --->> " + listName);
        int helperId = 0;
        ...
        if (listName != null && !listName.isEmpty()) {
            dynamicQuery.add(RestrictionsFactoryUtil.like("description", listName));
        }
        ...
        LOGGER.info("getIDFromHelper return ID  " + helperId);
        return helperId;
    }
    ...
    public static NativeSelect getBrandDropDown(final NativeSelect select) throws SystemException {
        ...
        LOGGER.info("getBrandDropDown RETURN NativeSelect select");
        return select;
    }
    ...
    public static int getIDFromHelper(final String description, final String listName) throws SystemException, PortalException {
        ...
        LOGGER.info("Entering getIDFromHelper()");
        int helperId = 0;
        final DynamicQuery dynamicQuery = DynamicQueryFactoryUtil.forClass(HelperTable.class);
        final ProjectionList projList = ProjectionFactoryUtil.projectionList();
        projList.add(ProjectionFactoryUtil.property("helperTableSid"));
        if (!description.isEmpty() && description != null) {
            dynamicQuery.add(RestrictionsFactoryUtil.like("description", description));
        }
        if (!listName.isEmpty() && listName != null) {
            dynamicQuery.add(RestrictionsFactoryUtil.like("listName", listName));
        }
        dynamicQuery.setProjection(ProjectionFactoryUtil.distinct(projList));
        final List ids = helperTableDAO.getHelperID(dynamicQuery);
        for (int i = 0; i < ids.size(); i++) {
    ...
    private void getPermissionAndRemoveComponent(Object cssLayout, String labelStr, String fieldStr, Map<String, AppPermission> fieldHM,
        ...
        try {
            if (fieldStr != null) {
                if (com.stpl.app.util.ConstantsUtils.ADD.equals(mode)) {
                    appPermission = ((AppPermission) fieldHM.get(fieldStr)).isAddFlag();
                } else if (com.stpl.app.util.ConstantsUtils.EDIT.equals(mode)) {
                    appPermission = ((AppPermission) fieldHM.get(fieldStr)).isEditFlag();
                } else if (com.stpl.app.util.ConstantsUtils.VIEW.equals(mode)) {
        ...
        if (mode != null) {
            for (int i = 0; i < obj.length; i++) {
                ...
                if (fieldHM.get(str) != null) {
                    ...
                    if ((com.stpl.app.util.ConstantsUtils.EDIT).equals(mode)) {
                        appPerm = appPermission.isEditFlag();
                    }
                    if ((com.stpl.app.util.ConstantsUtils.VIEW).equals(mode)) {
    ...
    public static TableResultCustom modifySearchTableResult(final Object[] obj, final String[] header, final Map<String, AppPermission> fieldHM) throws SystemException, PortalException, Exception {
            ...
            for (int j = 0; j < layout.getComponentCount(); j++) {
                ...
                if (((layout.getComponentCount() >= 1) && (layout.getComponent(j).getClass().isInstance(new VerticalLayout()))) || layout.getComponent(j).getClass().isInstance(new HorizontalLayout()) || layout.getComponent(j).getClass().isInstance(new GridLayout())) {
                    ...
                    if (layout.getComponent(i) != null && layout.getComponent(i).getClass().isInstance(new Label())) {
                        ...
                        if (labelStr.equals(((Label) layout.getComponent(i)).getValue())) {
                            layout.removeComponent(layout.getComponent(i));
                        }
                    }
                    if (layout.getComponent(i) != null && layout.getComponent(i).getId() != null && layout.getComponent(i).getId().equals(fieldStr)) {
