...
public class NetSalesFormulaLookup extends Window {
     ...
     * @throws Exception
     */
    public NetSalesFormulaLookup(CustomTextField netSalesNoField) throws Exception {
    ...
    private void configureFields() throws Exception  {  
        ...
        netSalesFormulaId.setMaxLength(38);
        netSalesFormulaId.addValidator(new StringLengthValidator(" Net Sales Formula Id Should be less than 38 characters", 0, 38, true));

        netSalesFormulaNo.setImmediate(true);
        netSalesFormulaNo.setMaxLength(50);
        netSalesFormulaNo.addValidator(new StringLengthValidator(" Net Sales Formula No Should be less than 50 characters", 0, 50, true));

        netSalesFormulaName.setImmediate(true);
        netSalesFormulaName.setMaxLength(100);
        netSalesFormulaName.addValidator(new StringLengthValidator(" Net Sales Formula Name Should be less than 38 characters", 0, 100, true));
...
//        excelExport.setStyleName("link");
    ...
    private void configureBinder() {
        binder = new ErrorfulFieldGroup(new BeanItem<>(netSalesDTO));
    ...
    */
    ...
    public void searchLogic(Button.ClickEvent event){
        try {   
             ...
             {
            binder.commit();
            tableLogic.setSearchData(netSalesDTO, binder);            
