package com.stpl.app.cff.security;
...
public StplSecurityDAO getDto() {
    ...
    public Map<String, AppPermission> getBusinessFunctionPermission(final String userId,final String moduleName) throws SystemException, PortalException {
        LOGGER.debug("getBusinessFunctionPermission= {}", userId);
    ...
    public Map<String, AppPermission> listToAppPermissionMap(final List permissionList,final int type) {
      ...
      //its altered to avoid MissingBreakInSwitch
        ...
        if(type == Constants.ZERO || type == NumericConstants.ONE){
            for (; counter < listSize; counter++) {
                final Object[] obj = (Object[]) permissionList.get(counter);
                final String propertyName = String.valueOf(obj[0]).trim();
                final String accessModule = String.valueOf(obj[1]);
        ...
        if(type == Constants.ZERO || type ==NumericConstants.ONE || type ==NumericConstants.TWO){
            for (; counter < listSize; counter++) {
                final Object[] obj = (Object[]) permissionList.get(counter);
                final String propertyName = String.valueOf(obj[0]).trim();
    ...
    public static Map<Integer, String> getUserName() throws SystemException {
        ...
        return getUserMap();
         ...
         public Map<String, AppPermission> getFieldOrColumnPermission(final String userId, final String moduleName ,final boolean column) throws SystemException, PortalException {
