...
public class FetchData {
    ...
    public static List<Row> fetchResultAsRow(PagedTableConfig tableConfig,String query, Object... params) {
//		if (params.length == 1) {
        logger.info(" fetchResultAsRow query" + query);
        query = replaceParameters(params, query);
        QueryRunner queryRunner = new QueryRunner();
        ResultSetHandler<List<Row>> resultSetHandler = getResultSetHandler(tableConfig);
        Connection conn = getDbConnection();
        try {
    ...
    private static ResultSetHandler<List<Row>> getResultSetHandler(PagedTableConfig tableConfig) {
        return (ResultSet rs) -> {
            ...
            while (rs.next()) {
                ...
                Row row = new Row();
                row.setRecordHeader(tableConfig.getVisibleColumns());
