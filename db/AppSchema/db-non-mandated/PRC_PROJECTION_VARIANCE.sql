...
AS (
	...
	FROM #TEMP_CCP A
	JOIN '+ @D_MASTER_TABLE +' RS ON RS.CCP_DETAILS_SID = A.CCP_DETAILS_SID
...
'
	...
	,TOTAL_DISCOUNT_ACCRUAL
               ...
               EXEC (
				...
				,NET_EX_FACTORY_SALES_ACTUALS=((CASE WHEN ''' + @CURRENT_DATE + ''' > DATEFROMPARTS( DT.YEAR, DT.PERIOD, 01) THEN ISNULL(GTS.GTS_SALES_ACTUALS, 0)  ELSE NULL END )-(CASE WHEN ''' + @CURRENT_DATE + ''' > DATEFROMPARTS( DT.YEAR, DT.PERIOD, 01) THEN ISNULL(DISC.CONTRACT_DISCOUNT_ACTUAL, 0)
                                              + ISNULL(PPA.PPA_DISCOUNT_ACTUAL, 0)  ELSE NULL END))
							-----------cel-386
			   ...
			   ,NET_EX_FACTORY_SALES_OF_EX_FACTORY_SALES_ACTUALS=ISNULL(((CASE WHEN ''' + @CURRENT_DATE + ''' > DATEFROMPARTS( DT.YEAR, DT.PERIOD, 01) THEN ISNULL(GTS.GTS_SALES_ACTUALS, 0)  ELSE NULL END )-(CASE WHEN ''' + @CURRENT_DATE + ''' > DATEFROMPARTS( DT.YEAR, DT.PERIOD, 01) THEN ISNULL(DISC.CONTRACT_DISCOUNT_ACTUAL, 0)
                                              + ISNULL(PPA.PPA_DISCOUNT_ACTUAL, 0)  ELSE NULL END))/nullif(CASE WHEN ''' + @CURRENT_DATE + ''' > DATEFROMPARTS( DT.YEAR, DT.PERIOD, 01) THEN ISNULL(GTS.GTS_SALES_ACTUALS, 0)  ELSE NULL END ,0),0)*100
											  -----------cel-386
					 
					 ,NET_EX_FACTORY_SALES_OF_EX_FACTORY_SALES_PROJECTED=ISNULL((ISNULL(GTS.GTS_SALES_PROJECTED, 0)-(ISNULL(DISC.CONTRACT_DISCOUNT_PROJECTED, 0) + ISNULL(PPA.PPA_DISCOUNT_PROJECTED, 0)))/NULLIF(GTS.GTS_SALES_PROJECTED, 0),0)*100
					 					 -----------cel-386
                   ...
                   LEFT JOIN (SELECT CONTRACT_DISCOUNT_PROJECTED = PROJECTION_SALES,
                           ...
                           ' NDPM
                                     ...
                                     INNER JOIN #PERIOD P
											 ...
											 LEFT JOIN #ACCRUAL_DISCOUNT AD ON NDPM.CCP_DETAILS_SID = AD.CCP_DETAILS_SID
                                                AND NDPM.RS_CONTRACT_SID = AD.RS_CONTRACT_SID
												AND NAD.PERIOD_SID=AD.PERIOD_SID
												AND C.HIERARCHY_NO=AD.HIERARCHY_NO
												AND C.LEVEL_NAME=AD.LEVEL_NAME
                           ...
                           ''' = ''PROGRAM'' THEN CAST(NAD.RS_CONTRACT_SID AS VARCHAR(20))
                                     ...
                                     [YEAR],
                                     C.HIERARCHY_NO,
                                     C.LEVEL_NAME)ACT FULL JOIN(SELECT PROJECTION_SALES=SUM(PROJECTION_SALES),
                                                                  PERIOD,
                                     [YEAR],
                                     C.HIERARCHY_NO,
                                     C.LEVEL_NAME,SUM(ISNULL(DISCOUNT_AMOUNT,0)) ACCRUAL_DISCOUNT_PROJ
                           ...
                           ' NDPM
									 ...
									 LEFT JOIN #ACCRUAL_DISCOUNT AD ON NDPM.CCP_DETAILS_SID = AD.CCP_DETAILS_SID
                                                AND NDPM.RS_CONTRACT_SID = AD.RS_CONTRACT_SID
												AND NDP.PERIOD_SID=AD.PERIOD_SID
												AND C.HIERARCHY_NO=AD.HIERARCHY_NO
												AND C.LEVEL_NAME=AD.LEVEL_NAME
                              WHERE  EXISTS (SELECT 1
                                                 FROM   #DISCOUNT_INFO A
...
AS (
	...
	JOIN NM_DISCOUNT_PROJECTION RS ON RS.PROJECTION_DETAILS_SID = A.PROJECTION_DETAILS_SID
	JOIN PERIOD P ON P.PERIOD_SID BETWEEN @START_PERIOD_SID
			AND @END_PERIOD_SID
...
GROUP BY PROJECTION_MASTER_SID
	...
	,ACCRUAL_PERIOD_END_DATE
        ...
        ,LEVEL_NAME 
				...
				,NET_EX_FACTORY_SALES_ACTUALS=ISNULL(GTS.GTS_SALES_ACTUALS, 0)-(ISNULL(DISC.CONTRACT_DISCOUNT_ACTUALS, 0)
                                              + (case when ISNULL(SALES.CONTRACT_SALES_ACTUALS, 0) = 0 then 0 else ISNULL(PPA.PPA_DISCOUNT_ACTUALS, 0) end))
				   ,NET_EX_FACTORY_SALES_PROJECTED=ISNULL(GTS.GTS_SALES_PROJECTED, 0)-(ISNULL(DISC.CONTRACT_DISCOUNT_PROJECTED, 0)
                                              + (case when ISNULL(SALES.CONTRACT_SALES_PROJECTED, 0) = 0 then 0 else ISNULL(PPA.PPA_DISCOUNT_PROJECTED, 0) end))
				   ,NET_EX_FACTORY_SALES_OF_EX_FACTORY_SALES_ACTUALS=ISNULL((ISNULL(GTS.GTS_SALES_ACTUALS, 0)-(ISNULL(DISC.CONTRACT_DISCOUNT_ACTUALS, 0)
                                              + (case when ISNULL(SALES.CONTRACT_SALES_ACTUALS, 0) = 0 then 0 else ISNULL(PPA.PPA_DISCOUNT_ACTUALS, 0) end)))/NULLIF(GTS.GTS_SALES_ACTUALS, 0),0)*100 					  
				   ,NET_EX_FACTORY_SALES_OF_EX_FACTORY_SALES_PROJECTED=	ISNULL((ISNULL(GTS.GTS_SALES_PROJECTED, 0)-(ISNULL(DISC.CONTRACT_DISCOUNT_PROJECTED, 0)
                                              + (case when ISNULL(SALES.CONTRACT_SALES_PROJECTED, 0) = 0 then 0 else ISNULL(PPA.PPA_DISCOUNT_PROJECTED, 0) end)))/NULLIF(GTS.GTS_SALES_PROJECTED, 0),0)*100			   
                  ,ACCRUAL_DISCOUNT AS TOTAL_DISCOUNT_ACCRUAL
				  FROM   #PRIOR_DATA_TABLE PDT
                         ...
                         LEFT JOIN (SELECT CONTRACT_DISCOUNT_PROJECTED =PROJECTION_SALES,
                                    ...
                                    FROM (SELECT ACTUAL_SALES=SUM(ACTUAL_SALES),   PERIOD,
                                           ...
                                           PD.PROJECTION_MASTER_SID,
                                           PP.HIERARCHY_NO,
                                           PP.LEVEL_NAME,SUM(ISNULL(DISCOUNT_AMOUNT,0)) ACCRUAL_DISCOUNT_ACTUAL FROM NM_ACTUAL_DISCOUNT NDP
                                           INNER JOIN PROJECTION_DETAILS PD
                                           ...
                                           INNER JOIN #PERIOD P
                                                   ON P.PERIOD_SID = NDP.PERIOD_SID
                                                                                       ...
                                                                                       FROM   #PROJECTION_MASTER PM
                                                                                       WHERE  ID <> 1)
											LEFT JOIN #PRIOR_ACCRUAL_DISCOUNT AD ON AD.PROJECTION_MASTER_SID=PP.PROJECTION_MASTER_SID
											AND AD.PROJECTION_DETAILS_SID=NDP.PROJECTION_DETAILS_SID
											AND AD.PERIOD_SID=NDP.PERIOD_SID
											AND AD.LEVEL_NAME=PP.LEVEL_NAME
											AND AD.HIERARCHY_NO=PP.HIERARCHY_NO
                                                                                     GROUP BY PERIOD,
                                           [YEAR],
                                           PD.PROJECTION_MASTER_SID,
                                           PP.HIERARCHY_NO,
                                           ...
                                           PD.PROJECTION_MASTER_SID,
                                           PP.HIERARCHY_NO,
                                           PP.LEVEL_NAME ,SUM(ISNULL(DISCOUNT_AMOUNT,0)) ACCRUAL_DISCOUNT_PROJ FROM NM_DISCOUNT_PROJECTION NDP
                                           INNER JOIN PROJECTION_DETAILS PD
                                                   ON PD.PROJECTION_DETAILS_SID = NDP.PROJECTION_DETAILS_SID
                                                                                     INNER JOIN #CURRENT_CPP_COMP_PRIOR_CPP CC
                                                                        ON CC.CCP_DETAILS_SID=PD.CCP_DETAILS_SID
              ...
              BEGIN
                     SET @SQL += 'EX_FACTORY_SALES_ACCRUALS_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN EX_FACTORY_SALES_ACCRUALS END), 
								  ...
								  TOTAL_DISCOUNT_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN TOTAL_DISCOUNT_ACCRUAL END),
...
ACCRUAL_DISCOUNT_ACTUAL					 
          ...
          SELECT CONTRACT_SALES_ACTUALS = SUM(ACTUAL_SALES),
                   ...
                   rs.LEVEL_NAME,
                   P.PERIOD,
                   P.YEAR,sum(ISNULL(DISCOUNT_AMOUNT, 0)) ACCRUAL_DISCOUNT_ACTUAL
		...
		CREATE TABLE #DISCOUNT_DATA (
			...
			,LEVEL_NAME VARCHAR(100)
			,PERIOD VARCHAR(50)
			,[YEAR] INT,ACCRUAL_DISCOUNT_PROJ NUMERIC(22, 6)
			)
		...
		SET @D_SQL = CONCAT (
            ...
            SELECT CONTRACT_SALES_PROJECTED = SUM(PROJECTION_SALES),
                   ...
                   rs.LEVEL_NAME,
                   P.PERIOD,
                   P.YEAR,sum(ISNULL(DISCOUNT_AMOUNT, 0)) ACCRUAL_DISCOUNT_PROJ
		...
		SELECT FD.YEAR
			...
			,FD.HIERARCHY_NO
			,FD.LEVEL_NAME
			,COALESCE(ACCRUAL_DISCOUNT_ACTUAL,ACCRUAL_DISCOUNT_PROJ) AS ACCRUAL_DISCOUNT
		...
		SELECT DISTINCT @FIRST_PROJ_SID AS PROJECTION_MASTER_SID
			...
			,P.HIERARCHY_NO
			,P.LEVEL_NAME
			,0.00000 AS DISCOUNT_AMOUNT_ACCRUAL
		FROM #CURRENT_PPA P
...
AS (
	SELECT DISTINCT COMPANY_MASTER_SID
		...
		,LEVEL_NAME
	FROM CCP_DETAILS A
	JOIN PROJECTION_DETAILS PD ON PD.CCP_DETAILS_SID=A.CCP_DETAILS_SID
	INNER JOIN #DPRIOR_PROJECTIONS DP ON DP.PROJECTION_MASTER_SID=PD.PROJECTION_MASTER_SID
	AND DP.PROJECTION_DETAILS_SID=PD.PROJECTION_DETAILS_SID
	JOIN NM_DISCOUNT_PROJ_MASTER RS ON RS.PROJECTION_DETAILS_SID = PD.PROJECTION_DETAILS_SID
	JOIN PERIOD P ON P.PERIOD_SID BETWEEN @START_PERIOD_SID
			AND @END_PERIOD_SID
	...
	,LEVEL_NAME
		...
		IF Object_id('TEMPDB..#DPRIOR_SALES_DATA') IS NOT NULL
			...
			LEFT JOIN NM_ACTUAL_DISCOUNT NPS ON S.PROJECTION_DETAILS_SID = NPS.PROJECTION_DETAILS_SID
				AND P.PERIOD_SID = NPS.PERIOD_SID 
				LEFT JOIN #PRIOR_ACCRUAL_DISCOUNT AD ON AD.PROJECTION_MASTER_SID=S.PROJECTION_MASTER_SID
			...
			AND AD.LEVEL_NAME=S.LEVEL_NAME
				GROUP BY S.PROJECTION_MASTER_SID,S.HIERARCHY_NO,S.LEVEL_NAME,DISCOUNT,PERIOD
				,[YEAR]
		...
		END
              ...
              BEGIN
                     SET @SQL2 += 'DISCOUNT_AMOUNT_ACCRUAL_' + Cast(@LOOP_CNTR1 AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR1 AS VARCHAR(10)) + ' THEN DISCOUNT_AMOUNT_ACCRUAL END),
