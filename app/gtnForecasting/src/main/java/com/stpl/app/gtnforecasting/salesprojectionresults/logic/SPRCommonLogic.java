...
public class SPRCommonLogic {
    ...
    public static int getLevelListCount(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, boolean isFilter, boolean isGroupFilter, String levelName) {
        int count = 0;
        String query = getLevelListQuery(projectionId, hierarchyIndicator, levelNo, hierarchyNo, StringUtils.EMPTY, StringUtils.EMPTY, isFilter, false, true, 0, 0, false, false, 0, isGroupFilter, levelName);
        List<Object> list = (List<Object>) executeSelectQuery(query, null, null);
        if (list != null && !list.isEmpty()) {
    ...
    public static String getCustomCCPQueryForPR(ProjectionSelectionDTO projSelDTO) {
        ...
        String ccpQuery = " SELECT distinct HLD" + projSelDTO.getHierarchyIndicator() + ".RELATIONSHIP_LEVEL_SID,HLD" + projSelDTO.getHierarchyIndicator() + ".HIERARCHY_NO, CCPMAP" + projSelDTO.getHierarchyIndicator() + ".CCP_DETAILS_SID FROM \n"
                + " (SELECT RLD.RELATIONSHIP_LEVEL_VALUES, RLD.HIERARCHY_NO, CCP.CCP_DETAILS_SID \n"
                + " FROM RELATIONSHIP_LEVEL_DEFINITION  RLD \n"
                + " JOIN  CCP_MAP CCP ON RLD.RELATIONSHIP_LEVEL_SID=CCP.RELATIONSHIP_LEVEL_SID \n"
    ...
    public static List<Leveldto> getAllHierarchyLevels(int startLevelNo, int projectionId, String hierarchyIndicator) {
        ...
        try {
            String query = getAllHierarchyLevelsQuery(startLevelNo, projectionId, hierarchyIndicator, StringUtils.EMPTY, 0, 0);
            List<Object> list = (List<Object>) executeSelectQuery(query, null, null);
            if (list != null && !list.isEmpty()) {
                for (Object list1 : list) {
                    final Object[] obj = (Object[]) list1;
    ...
    public static String getAllHierarchyLevelsQuery(int startLevelNo, int projectionId, String hierarchyIndicator) {
        ...
        finalQuery += getAllHierarchyLevelsQuery(startLevelNo, projectionId, hierarchyIndicator, StringUtils.EMPTY, 0, 0);
        return finalQuery;
    }
    ...
    public static String getHierarchyLevelsQuery(int projectionId, String hierarchyIndicator, int levelNo, String userGroup, int userId, int sessionId) {
        ...
        customSql = mainSelect + Constant.FROM_SMALL + selectClause1 + Constant.FROM_SMALL + joinQuery1 + " " + selectClause2 + Constant.FROM_SMALL + joinQuery2 + " " + mainJoin;
        return customSql;
    ...
    public static int getLevelIndex(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, String findHierarchyNo, boolean isCustom, int customId) {
        int index = 0;
        String levelIndexQuery = getLevelIndexQuery(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, findHierarchyNo, isCustom, customId, StringUtils.EMPTY, 0, 0);
    ...
    public static List<Object> getParentLevelNoAndHierarchyNo(int levelNo, String hierarchyNo) {
        ...
        if (StringUtils.countMatches(hierarchyNo, ".") > 1) {
            levelNo--;
            hierarchyNo = hierarchyNo.substring(0, StringUtils.ordinalIndexOf(hierarchyNo, ".", StringUtils.countMatches(hierarchyNo, ".") - 1) + 1);
        } else {
            levelNo = 0;
    ...
    public static boolean editButtonValidation(ComboBox customDdlb, List<CustomViewMaster> customViewList) {
        ...
        AbstractNotificationUtils.getErrorNotification("Edit a View", "You cannot edit a view that you did not create. Please select another view, or create a new one");
        return false;
    }
    ...
    public void saveCHSPRSelection(Map<String, Object> projectionSelectionDTO, int projectionId) {
        ...
        try {
            ...
            if (list != null && list.isEmpty()) {
                commonLogic.saveSelection(projectionSelectionDTO, projectionId, String.valueOf(projectionSelectionDTO.get(Constant.SCREEN_NAME)), Constant.SAVE, "CH_PROJECTION_SELECTION");
            } else {
                commonLogic.saveSelection(projectionSelectionDTO, projectionId, String.valueOf(projectionSelectionDTO.get(Constant.SCREEN_NAME)), Constant.UPDATE, "CH_PROJECTION_SELECTION");
            }
        } catch (SystemException ex) {
            LOGGER.debug(ex);
        } catch (PortalException ex) {
    ...
    public static String getHierarchyLevelsQueryForSales(int projectionId, String hierarchyIndicator, int levelNo, int hierarchyLevelDefId, int treeLevelNo) {
        ...
        String selectClause = "select"
                + " LEVEL_NO, "
                + treeLevelNo + " as TREE_LEVEL_NO, "
                + "'" + hierarchyIndicator + Constant.AS_HIERARCHY_INDICATOR_COMMA
                + " LEVEL_NAME,"
                + " RELATIONSHIP_LEVEL_VALUES,"
                + " PARENT_NODE,"
                + " HIERARCHY_NO";
        String customSql = selectClause + " from RELATIONSHIP_LEVEL_DEFINITION where RELATIONSHIP_LEVEL_SID in (" + selectClause1 + ")"
    ...
    public static String getHierarchyTreeQuery(int projectionId, String hierarchyIndicator) {
        String selectClause = "select distinct RLD.LEVEL_NO,RLD.LEVEL_NO, "
    ...
    public static int getLevelListCount(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, boolean isFilter, boolean isCustom, int customId) {
        int count = 0;
        String query = getLevelListQuery(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, isFilter, false, true, 0, 0, false, isCustom, customId, false, StringUtils.EMPTY);
        List<Object> list = (List<Object>) executeSelectQuery(query, null, null);
        if (list != null && !list.isEmpty()) {
            Object ob = list.get(0);
            count = Integer.valueOf(String.valueOf(ob));
    ...
    public static int getLevelIndex(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, String findHierarchyNo, boolean isCustom, int customId, String userGroup, int userId, int sessionId) {
        int index = 0;
        try {
            String levelIndexQuery = getLevelIndexQuery(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, findHierarchyNo, isCustom, customId, userGroup, userId, sessionId);
            List<Object> list = (List<Object>) executeSelectQuery(levelIndexQuery, null, null);
            if (list != null && !list.isEmpty()) {
                Object ob = list.get(0);
                index = Integer.valueOf(String.valueOf(ob));
    ...
    public static String getLevelIndexQuery(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, String findHierarchyNo, boolean isCustom, int customId, String userGroup, int userId, int sessionId) {
        String levelListQuery = getLevelListQuery(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, false, false, false, 0, 0, false, isCustom, customId, userGroup, userId, sessionId, false, StringUtils.EMPTY);
    ...
    private static String getGroupQuery(int projectionId, int sessionId, int userId, String table) {
        String query = "SELECT DISTINCT M.USER_GROUP FROM " + table + " M"
                ...
                + " WHERE RLD.LEVEL_NAME = 'Trading Partner' AND M.USER_GROUP IS NOT NULL AND PD.PROJECTION_MASTER_SID ='" + projectionId + "' "
    ...
    public void saveReturnsSPSelection(Map map, int projectionID, String screenName) {
        ...
        try {
            ...
            if (list.isEmpty()) {
                ...
                commonLogic.saveSelection(map, projectionID, screenName, Constant.UPDATE, "RETURNS_PROJECTION_SELECTION");
            }
        } catch (SystemException ex) {
            LOGGER.debug(ex);
        } catch (PortalException ex) {
