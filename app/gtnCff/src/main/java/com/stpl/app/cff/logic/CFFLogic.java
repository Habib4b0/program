...
public class CFFLogic {
    ...
    public int saveCFFMaster(final DataSelectionDTO dataSelectionDTO, boolean isUpdate, int projectionIdValue,SessionDTO sessionDTO) {
        ...
        String query = "INSERT INTO CFF_MASTER (CFF_ELIGIBLE_DATE,CFF_TYPE, CFF_NAME, ACTIVE_FROM_DATE, ACTIVE_TO_DATE, CFF_OFFICIAL, CUSTOMER_HIERARCHY_SID, CUSTOMER_HIERARCHY_LEVEL,\n"
                ...
                + "    CREATED_DATE, MODIFIED_BY, MODIFIED_DATE,BUSINESS_UNIT, PROJECTION_CUST_VERSION, PROJECTION_PROD_VERSION, PROJECTION_DED_VERSION @CUSTOM @DEDUCTION_ADDITION ) VALUES (@CFFELIGDATE,'@CFF_TYPE','@CFF_NAME',@ACTIVE_FROM_DATE, @ACTIVE_TO_DATE, 0,\n"
                ...
                + "    '@PROD_RELATIONSHIP_BUILDER_SID', 'A', '@CREATED_BY', '@CREATED_DATE', '@MODIFIED_BY', '@MODIFIED_DATE','@BUSINESS_UNIT',@PROJCUSTVERSION, @PROJPRODVERSION, @PROJDEDVERSION ,@CUSTOM_VW_MASTR_SALES @DED_ADD_VALUES ) ";
        ...
        query = query.replace("@PROJDEDVERSION", String.valueOf(versionNoList.get(0)));
        ...
        query = query.replace(",@CUSTOM_VW_MASTR_SALES",  dataSelectionDTO.getCustomViewMasterSid()!=0? ","+dataSelectionDTO.getCustomViewMasterSid():StringUtils.EMPTY);
        query = query.replace("@CUSTOM", dataSelectionDTO.getCustomViewMasterSid() != 0 ? ",CUSTOM_VW_MASTR_SALES" : StringUtils.EMPTY);
    ...
    public List getDeductionVersionNoList(Object relationShipBuilderSid) {
		...
		return CommonQueryUtils.getAppData(input, "getDedRelationshipVersionNo", null);
	}
    ...
    public void ccpHierarchyInsert(final GtnSmallHashMap tempTableNames, DataSelectionDTO dsDTO, final List<Leveldto> customerSelection, final List<Leveldto> productSelection, final String topLevelName, final boolean isDataSelectionTab) {
        ...
        if (contractList.isEmpty()) {
            ccpHierarchyQuery[0] = getCCPValues(dsDTO.getCustRelationshipBuilderSid(),dsDTO.getCustomerRelationShipVersionNo(), logic.formInqueryStringValue(hierarchyNoList, StringConstantsUtil.HIERARCHY_NO), "GET_CONTRACT_LEVEL");
        } else {
            ccpHierarchyQuery[0] = formQueryWithUnionAll(contractList);
        }
        if (customerList.isEmpty()) {
            ccpHierarchyQuery[1] = getCCPValues(dsDTO.getCustRelationshipBuilderSid(),dsDTO.getCustomerRelationShipVersionNo(), logic.formInqueryStringValue(hierarchyNoList, StringConstantsUtil.HIERARCHY_NO), "GET_CUSTOMER_LEVEL");
        } else {
        ...
        if (productList.isEmpty()) {
            ccpHierarchyQuery[NumericConstants.TWO] = getCCPValues(dsDTO.getProdRelationshipBuilderSid(),dsDTO.getProductRelationShipVersionNo(), logic.formInqueryStringValue(hierarchyNoList, StringConstantsUtil.HIERARCHY_NO), "GET_PRODUCT_LEVEL");
        } else {
    ...
    public void callDeductionCCPHierarchyInsertion(SessionDTO session, final GtnSmallHashMap tempTableNames, final boolean isDataSelectionTab) {
        ...
        builder.replace(builder.indexOf(StringConstantsUtil.RELATION_VER), StringConstantsUtil.RELATION_VER.length() + builder.lastIndexOf(StringConstantsUtil.RELATION_VER), String.valueOf(session.getDeductionRelationVersion()));
        HelperTableLocalServiceUtil.executeUpdateQuery(QueryUtil.replaceTableNames(builder.toString(), tempTableNames));
    }
     ...
     * @return
     */
    private String getCCPValues(String value,int versionNo, String formInqueryStringValue, final String queryName) {
        ...
        builder.replace(builder.indexOf(StringConstantsUtil.RELATION_VER), StringConstantsUtil.RELATION_VER.length() + builder.lastIndexOf(StringConstantsUtil.RELATION_VER), String.valueOf(versionNo));
    ...
    public void loadCustomViewValues(ComboBox customViewDdlb,Map<String,String> customViewInput,boolean isReset){
    if (customViewInput.get(ConstantsUtil.CUSTOMER_SID_LITERAL) != null && customViewInput.get(ConstantsUtil.CUSTVER) != null && customViewInput.size()>3) {
            String sqlQuery = SQlUtil.getQuery("loadCustomRelationValues");
            sqlQuery = sqlQuery.replace("@CUSTSID", customViewInput.get(ConstantsUtil.CUSTOMER_SID_LITERAL)).replace("@PRODSID", customViewInput.get(ConstantsUtil.PROD_SID_LITERAL))
                    .replace("@CUSTVER", customViewInput.get(ConstantsUtil.CUSTVER)).replace("@PRODVER", customViewInput.get(ConstantsUtil.PRODVER));
            List<Object[]> queryList = HelperTableLocalServiceUtil.executeSelectQuery(sqlQuery);

            for (Object[] objects : queryList) {
                customViewDdlb.addItem(objects[0]);
                customViewDdlb.setItemCaption(objects[0], String.valueOf(objects[1]));
            }
            if (!queryList.isEmpty()  && isReset) {
                customViewDdlb.setValue(queryList.get(0)[0]);
            }
        }
    }
    
    public List<Future> loadSalesTempTableInThread(SessionDTO session,boolean isDataSelection){
        List<Future> futureListSales=new ArrayList<>();
        futureListSales.add(service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.SALES, CUSTOMER_CONSTANT);
                Object[] orderedArgs={session.getProjectionId() + (session.getPriorProjectionId().isEmpty()?ConstantsUtils.EMPTY:STRING_COMMA + session.getPriorProjectionId()),session.getUserId(),session.getSessionId(),session.getStatusName(),session.getFrequency(),session.getCustomViewMasterSid(), Constants.SALES,session.getDeductionName(),"C",isDataSelection?EACH:session.getDiscountUom(),null};
       ...
       futureListSales.add(service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.SALES, PRODUCT_CONSTANT);
                Object[] orderedArgs={session.getProjectionId() + (session.getPriorProjectionId().isEmpty()?ConstantsUtils.EMPTY:STRING_COMMA + session.getPriorProjectionId()),session.getUserId(),session.getSessionId(),session.getStatusName(),session.getFrequency(),session.getCustomViewMasterSid(), Constants.SALES,session.getDeductionName(),"P",isDataSelection?EACH:session.getDiscountUom(),null};
                  CommonLogic.callProcedureUpdate(Constants.PRC_CFF_VIEW_POPULATION, orderedArgs);
            }
        }));
        futureListSales.add(service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.SALES, CUSTOM_CONSTANT);
                Object[] orderedArgs = {session.getProjectionId() + (session.getPriorProjectionId().isEmpty() ? ConstantsUtils.EMPTY : STRING_COMMA + session.getPriorProjectionId()), session.getUserId(), session.getSessionId(), session.getStatusName(), session.getFrequency(), session.getCustomViewMasterSid(), Constants.SALES, session.getDeductionName(), "U", isDataSelection ? EACH : session.getDiscountUom(), null};
                CommonLogic.callProcedureUpdate(Constants.PRC_CFF_VIEW_POPULATION, orderedArgs);
            }
        }));
        return futureListSales;
    ...
    public void loadDiscountCustomTempTableInThread(SessionDTO session,boolean isDataSelectionDiscount){
        service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.DISCOUNT, CUSTOM_CONSTANT);
    ...
    public List<Future> loadDiscountTempTableInThread(SessionDTO session,boolean isDataSelectionDiscount){
        List<Future> futureListDiscount=new ArrayList<>();
        futureListDiscount.add(service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.DISCOUNT, CUSTOMER_CONSTANT);
        ...
        futureListDiscount.add(service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.DISCOUNT, PRODUCT_CONSTANT);
                Object[] orderedArgs={session.getProjectionId() + (session.getPriorProjectionId().isEmpty()?ConstantsUtils.EMPTY:STRING_COMMA + session.getPriorProjectionId()),session.getUserId(),session.getSessionId(),session.getStatusName(),session.getFrequency(),session.getCustomViewMasterSid(), Constants.DISCOUNT,session.getDeductionName(),"P", isDataSelectionDiscount?EACH:session.getDiscountUom(),null};
                  CommonLogic.callProcedureUpdate(Constants.PRC_CFF_VIEW_POPULATION, orderedArgs);
            }
        }));
        return futureListDiscount;
    }
    ...
    public void callCFFHierarachyDetailsProcedure(SessionDTO session) {
        LOGGER.info("callCFFHierarachyDetailsProcedure");
            ...
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session,Constants.CFF_EXCEL, CUSTOMER_CONSTANT);
       ...
       service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session,Constants.CFF_EXCEL, PRODUCT_CONSTANT);
        ...
        service.submitRunnable(new Runnable() {
            @Override
            public void run() {
                CommonLogic.updateStatusForProcedure(Constants.RUNNING_STATUS, session, Constants.CFF_EXCEL, CUSTOM_CONSTANT);
