IF EXISTS (SELECT 'X'
           ...
           WHERE  ROUTINE_NAME = 'PRC_FE_PROJ_DET_ITEM_TRANSFER_SALES'
                  AND ROUTINE_SCHEMA = 'DBO')
  BEGIN
      DROP PROCEDURE [DBO].[PRC_FE_PROJ_DET_ITEM_TRANSFER_SALES]
  END

GO

CREATE PROCEDURE [dbo].[PRC_FE_PROJ_DET_ITEM_TRANSFER_SALES] (@SESSION_ID INT)
AS
  BEGIN
      SET NOCOUNT ON
      ...
      BEGIN TRY
          ...
          FROM   PERIOD
          WHERE  MONTH = Month(@ITEM_END_DATE)
                 AND YEAR = Year(@ITEM_END_DATE)
          ...
          SELECT @FROM_CONTRACT_MASTER_SID                              AS SOURCE_CONTRACT,
                 CONVERT (INT, LEFT(TOKEN, Charindex('-', TOKEN) - 1))  AS SOURCE_ITEM,
                 @TO_CONTRACT_MASTER_SID                                AS MAP_CONTRACT,
                 CONVERT (INT, RIGHT(TOKEN, Charindex('-', TOKEN) - 1)) AS MAPPED_ITEM
          ...
          FROM   @SOURCE S
          JOIN   @MAP M ON S.COMPANY_MASTER_SID = M.COMPANY_MASTER_SID
          WHERE  M.ITEM_MASTER_SID IN (SELECT TD.MAPPED_ITEM
                                       FROM   #TEMP_MAP_COMPANY TD
                                       WHERE  TD.SOURCE_ITEM = S.ITEM_MASTER_SID)
