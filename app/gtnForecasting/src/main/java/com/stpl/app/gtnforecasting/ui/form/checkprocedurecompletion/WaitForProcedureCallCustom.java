 ...
 */
public class WaitForProcedureCallCustom implements WaitForCustomerProcedureCompletion {

    private final SessionDTO session;
    private ThreadPool waitCustomThreadPooll = ThreadPool.getInstance();

    public WaitForProcedureCallCustom(SessionDTO session) {
    ...
    public void waitforStatusTableCOmpletion() {
        session.addFutureMap(Constant.CUSTOMER_VIEW_DISCOUNT_POPULATION_CALL,
				new Future[] {waitCustomThreadPooll.submitRunnable(createRunnable(Constant.PRC_VIEWS_CALL,
    ...
    private Runnable createRunnable(final Object... inputs){
        return new Runnable() {
            ...
            @Override
            public void run() {
                                CommonUtil.getInstance().waitsForOtherThreadsToComplete(session.getFutureValue("Check_Custom"));
