...
public class NationalAssumptionsForm extends CustomComponent {
	...
	public NationalAssumptionsForm(DataSelectionDTO dtoValue, OptionGroup mode, SessionDTO sessionDTO)
			throws SystemException, PortalException {
		LOGGER.info("NationalAssumptionsForm Constructor initiated ");
		setCompositionRoot(
				Clara.create(getClass().getResourceAsStream("/nationalassumption/NationalAssumptionsForm.xml"), this));
		this.sessionDTO = sessionDTO;
		if (!sessionDTO.getCurrentTableNames().isEmpty()) {

			com.stpl.app.gtnforecasting.logic.CommonLogic.dropDynamicTables(sessionDTO.getUserId(),
		...
		this.additionalInformation = new AdditionalInformationForm(Constant.NATIONAL_ASSUMPTION_SCREEN_NAME,
				sessionDTO.getProjectionId(), String.valueOf(mode.getValue()));
	...
	/**
	 * This method is used to initialize the components and to configure the fields.
	 */
	private void init() {
	...
	protected void configureFields() {
		...
		try {
			final Map<String, AppPermission> tabItemHM = stplSecurity.getBusinessFunctionPermission(userId,
					NATIONAL_ASSUMPTIONS.getConstant() + "," + "All Tabs");
			...
			if ((lastPosition == 0 && dsFlag && !"View".equalsIgnoreCase(getmode)) && (dataSelection.isChanged())) {
				...
				new AbstractNotificationUtils() {
					...
					public void noMethod() {
						...
						dsFlag = true;
						/*
						 * Relaod the Default Value(Previously stored in DB) for Available & Selected
						 * Table in Data Selection Tab
						 */
	...
	void closeBtnLogic(String closeMsg) {
		...
		}.getConfirmationMessage("Close Confirmation", closeMsg);
	...
	private void setTabSecurity() throws PortalException, SystemException {
		...
		Map<String, AppPermission> functionPsHM = stplSecurity.getBusinessTabPermission(userId,
				Constant.NATIONAL_ASSUMPTIONS_SCREEN);
		...
		if (functionPsHM.get(FunctionNameUtil.NA_NATIONAL_ASUMPTIONS_TAB) != null
				&& !((AppPermission) functionPsHM.get(FunctionNameUtil.NA_NATIONAL_ASUMPTIONS_TAB)).isTabFlag()) {
		...
		if (functionPsHM.get(FunctionNameUtil.MEDICAID_URA_TAB) != null
				&& !((AppPermission) functionPsHM.get(FunctionNameUtil.MEDICAID_URA_TAB)).isTabFlag()) {
			tabSheet.getTab(NumericConstants.TWO).setVisible(BooleanConstant.getFalseFlag());
		}
		if (functionPsHM.get(FunctionNameUtil.FCP_RESULTS_TAB) != null
				&& !((AppPermission) functionPsHM.get(FunctionNameUtil.FCP_RESULTS_TAB)).isTabFlag()) {
			tabSheet.getTab(NumericConstants.THREE).setVisible(BooleanConstant.getFalseFlag());
		}
		if (functionPsHM.get(FunctionNameUtil.PHS_RESULTS_TAB) != null
				&& !((AppPermission) functionPsHM.get(FunctionNameUtil.PHS_RESULTS_TAB)).isTabFlag()) {
			tabSheet.getTab(NumericConstants.FOUR).setVisible(BooleanConstant.getFalseFlag());
		}
