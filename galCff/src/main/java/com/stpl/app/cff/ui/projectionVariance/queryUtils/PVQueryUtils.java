...
public class PVQueryUtils {
    ...
    public String getProjectionResultsDiscountsQuery(PVSelectionDTO projSelDTO, String orderBy) {
        ...
        if (projSelDTO.getDiscountLevel().equals("Program Category")) {
            LOGGER.info("projSelDTO.getDiscountNameList() " + projSelDTO.getDiscountNameList().size() + " projSelDTO.isIsTotal() " + projSelDTO.isIsTotal());
            if (projSelDTO.getDiscountNameList() != null && !projSelDTO.getDiscountNameList().isEmpty()) {
                if (!projSelDTO.isIsTotal()) {
                    discountTypeColumnName = " B.PRICE_GROUP_TYPE AS DISCOUNTS";
                    groupBy += ", " + " B.PRICE_GROUP_TYPE";
    ...
    public String getProjectionResultsTotalDiscountDolQuery(PVSelectionDTO projSelDTO) {
        String selectClause = " select ";
        String customQuery = "";
        List<String> list = getCommonSelectWhereOrderGroupByClause("TODIS", "PPA", projSelDTO.getFrequencyDivision(), "on");
        selectClause += list.get(0);
        String finalWhereCond = list.get(1);
        ...
        customQuery = selectClause + " from (" + totalDiscountQuery + ") TODIS FULL OUTER JOIN (" + ppaQuery + ") PPA " + finalWhereCond;
        return customQuery;
    }
