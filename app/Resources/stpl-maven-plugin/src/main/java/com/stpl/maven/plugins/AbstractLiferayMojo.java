/**
 * Copyright (c) 2000-2013 stpl, Inc. All rights reserved.
 *
...
public abstract class AbstractLiferayMojo extends AbstractMojo {
		...
		try {
			...
			if (getPortalMajorVersion() < PORTAL_VERSION_6_1) {
				throw new MojoExecutionException(
					"stpl versions below 6.1.0 are not supported");
			}
	...
	protected void executeTool(
		...
		try {
			System.setProperty(
				"external-properties",
				"com/stpl/portal/tools/dependencies" +
	...
	protected float getPortalMajorVersion() {
		...
		Matcher matcher = _majorVersionPattern.matcher(stplVersion);
		...
		if ((appServerLibGlobalDir != null) && appServerLibGlobalDir.exists()) {
			...
			try {
				globalClassLoader.loadClass(
					"com.stpl.portal.kernel.util.ReleaseInfo");
			}
			catch (ClassNotFoundException cnfe) {
				Dependency portalServiceDependency = createDependency(
					"com.stpl.portal", "portal-service", stplVersion, "",
					"jar");
	...
	protected void initPortalProperties() throws Exception {
		if (((appServerPortalDir == null) || !appServerPortalDir.exists()) &&
			Validator.isNotNull(stplVersion)) {
			...
			Dependency dependency = createDependency(
				"com.stpl.portal", "portal-web", stplVersion, "", "war");
	...
	protected void initUtils() throws Exception {
		...
		Class<?> clazz = classLoader.loadClass(
			"com.stpl.portal.util.EntityResolver");
	...
	/**
	 * @parameter expression="${stplVersion}"
	 */
	protected String stplVersion;
	...
	/**
	 * @parameter default-value="${project.build.directory}/stpl-work"
	 * @required
	 */
