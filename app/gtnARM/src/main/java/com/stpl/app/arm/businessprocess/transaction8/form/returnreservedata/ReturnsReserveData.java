...
public class ReturnsReserveData extends VerticalLayout {
    ...
    private void configureWorkFlow() {
        if (selection.getSessionDTO().isWorkFlow()) {
            try {
                loadDetails();
                originalSaleLimiter.setValue(selection.getOriginalSaleLimiterVal().isEmpty() ? null : ARMUtils.getInstance().getmmddyyyy().parse(selection.getOriginalSaleLimiterVal()));
