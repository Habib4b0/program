...
public class DashboardSearchDAO implements BeanDAO<ContractMember> {
    ...
    public List<ContractMember> find(final BeanSearchCriteria criteria, final int startIndex, final int offset, final List<OrderByColumn> columns) {
        ...
        try {
            LOGGER.info("Enters getAlSearchList method with parameters startIndex=" + startIndex + ", offset=" + offset + ", columns size=" + columns.size());
            if (contractList != null) {
            	contractList=logic.getComponentSearch(leftSearchBinder,component, columns, criteria, startIndex, offset, true);
                LOGGER.info("End of  getAlSearchList method with contractList size=" + contractList.size());
            }
        } catch (Exception ex) {
          final   String errorMsg = ErrorCodeUtil.getErrorMessage(ex);
            LOGGER.error(errorMsg);
