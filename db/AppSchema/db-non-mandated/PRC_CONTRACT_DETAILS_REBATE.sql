...
AS
  BEGIN
      ...
      BEGIN TRY
          ...
          DECLARE @CURRENT_QUARTER INT =(SELECT Min(PERIOD_SID)
            FROM   PERIOD
            WHERE  PERIOD_DATE = Dateadd(QQ, Datediff(QQ, 0, Getdate()), 0))------------cel-394
     ...
     ITEM_PRICING_QUALIFIER_NAME VARCHAR(50)
          ...
          (
                ...
                SELECT CCP_DETAILS_SID,
                        ...
                        ITEM_PRICING_QUALIFIER_SID  ,
                        RETURN_RATE_SID ,                             
               CASE  WHEN CALCULATION_LEVEL LIKE ''%SINGLE%'' THEN   
 ...
 ROW_NUMBER()
          ...
          CREATE TABLE #TIER_INFO
            (
               ...
               ITEM_PRICING_QUALIFIER_NAME VARCHAR(50),
               FORMULA_TYPE                VARCHAR(1000),
               ADJUSTED_COMPLEX_FORMULA    VARCHAR(8000)
            )
...
,'','')) udf
          ...
          SELECT IM.ITEM_MASTER_SID,
                 ...
                 ( Isnull(Sum(CASE
                              ...
                              END)
                            OVER(
                              PARTITION BY IM.ITEM_MASTER_SID, QUARTER, YEAR ) / NULLIF(Sum(CASE
                                                                                              WHEN P.PERIOD_SID < @CURRENT_QUARTER THEN ACTUAL_GTS_UNITS
                                                                                              ELSE FORECAST_GTS_UNITS
                                                                                            END)
                                                                                          OVER(
                                                                                            PARTITION BY IM.ITEM_MASTER_SID, QUARTER, YEAR ), 0), 0) )                      SALES_WEIGHTEDWAC_NORMAL,
                 ( Isnull(Sum(CASE
                                WHEN P.PERIOD_SID < @CURRENT_QUARTER THEN ACTUAL_GTS_SALES
                                ELSE FORECAST_GTS_SALES
                              END)
                            OVER(
                              PARTITION BY IM.ITEM_MASTER_SID, QUARTER, YEAR ) / NULLIF(Sum(CASE
                                                                                            ...
                                                                                            END)
                                                                                          OVER(
                                                                                            PARTITION BY IM.ITEM_MASTER_SID, QUARTER, YEAR ), 0), 0) ) / NULLIF(IM.UPPS, 0) AS SALES_WEIGHTEDWAC_UPPS
...
CREATE TABLE ', @NATIONAL_INFO, '
  ...
  ')
        ...
        ')
          ...
          EXEC Sp_executesql
            @SQL,
            N'@ITEM_UDT UDT_ITEM READONLY,@PROJECTION_START_PERIOD_SID INT,@PROJ_END_PERIOD_SID INT,@ITEM_UOM VARCHAR(50),
                             @ACTUAL_START_PERIOD_SID INT,@QUALIFIER VARCHAR(MAX),@CURRENT_QUARTER INT',
            @ITEM_UDT=@ITEM_UDT,
            @PROJECTION_START_PERIOD_SID=@PROJECTION_START_PERIOD_SID,
...
CREATE TABLE ', @RETURNS_INFO, '
  (
     ITEM_MASTER_SID INT,
     PERIOD_SID      INT,
     RETURN_RATE  NUMERIC(22,6),COMPLEX_FORMULA VARCHAR(1000) ,FORMULA_TYPE varchar(50)
  ) 
  ')
          ...
          SET @SQL =Concat('
                      ...
                      ( ITEM_MASTER_SID,
                        PERIOD_SID,
					    RETURN_RATE,COMPLEX_FORMULA,FORMULA_TYPE )
						 SELECT 
                          DISTINCT  TI.ITEM_MASTER_SID,
                           P.PERIOD_SID,
                           RATE AS RETURN_RATE,
						   replace(replace(replace(CASE WHEN ti.ADJUSTED_COMPLEX_FORMULA 	 LIKE ''%$%'' THEN REPLACE (ti.ADJUSTED_COMPLEX_FORMULA,''$'',''PROJECTION_UNITS * '') ELSE REPLACE (ti.ADJUSTED_COMPLEX_FORMULA,''%'','' * (PROJECTION_SALES/100.00)'') END ,''[Returns Rate]'',''[RETURN_RATE]''),''['',''''),'']'','''')
						   ,COALESCE(FORMULA_TYPE,''COMPLEX'') FORMULA_TYPE
                           FROM  #TIER_INFO TI
						   JOIN ITEM_MASTER IM ON IM.ITEM_MASTER_SID=TI.ITEM_MASTER_SID                           
                           cross JOIN( select * from #PERIOD P
                           where PERIOD_SID BETWEEN  @ACTUAL_START_PERIOD_SID AND @PROJ_END_PERIOD_SID ) p
						   LEFT JOIN RETURN_RATE_FORECAST RRF ON  IM.ITEM_ID=RRF.ITEM_ID
						   AND IM.ITEM_MASTER_SID=RRF.ITEM_MASTER_SID AND RETURN_RATE_SID IS NOT NULL 
						   and  PERIOD_DATE=DATEFROMPARTS(FORECAST_YEAR,FORECAST_MONTH,01)
						   where RETURN_RATE_SID IS NOT NULL  or ti.ADJUSTED_COMPLEX_FORMULA is not null')
