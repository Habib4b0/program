...
public class Summary extends CustomComponent {
    ...
    private CustomTreeContainer<DiscountDTO> resultBean = new CustomTreeContainer<DiscountDTO>(DiscountDTO.class);
    ...
    public void configureFields() {
        LOGGER.info("Entering summary configure Fields with" + dto.getRsSystemId());
    ...
    public void configureSummaryTable() {
        ...
        for (Object propertyId : summaryResultsTable.getVisibleColumns()) {
            summaryResultsTable.setColumnWidth(propertyId, 165);
        }
        ...
        if (!Constants.ZEROSTRING.equals(dto.getRsSystemId()) && !Constants.NULL.equals(dto.getRsSystemId()) && !StringUtils.EMPTY.equals(dto.getRsSystemId())) {
            infoLogic.loadSetData(removeDiscount.getUserId(), removeDiscount.getSessionId(), new ArrayList<String>(), "summary");
        }
    ...
    public void configureDiscountTable() {
        ...
        resultBean = new CustomTreeContainer<DiscountDTO>(DiscountDTO.class);
        ...
        discountTable.setSplitPosition(400, Unit.PIXELS);
    ...
    private void loadResultTable() {
        LOGGER.info("Entering loadResultTable with " + dto.getRsSystemId());
        ...
        String rsContract = commonUtils.CollectionToString(dto.getRebateList(), true);
        removeDiscountDto.setRsContractSid(dto.getRsSystemId());
        if (projDetails.size() > 0) {
            if (dto.getRsSystemId() != null && !dto.getRsSystemId().equals(Constants.NULL) && !dto.getRsSystemId().equals(StringUtils.EMPTY) && !dto.getRsSystemId().equals(Constants.ZEROSTRING)) {
                tableLogic.setData(removeDiscountDto);
            }
        }
        LOGGER.info("Ended loadResultTable ");
    }
