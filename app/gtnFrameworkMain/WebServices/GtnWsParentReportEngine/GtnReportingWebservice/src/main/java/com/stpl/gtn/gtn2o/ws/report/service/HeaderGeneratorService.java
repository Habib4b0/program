...
public class HeaderGeneratorService {
	...
	public GtnWsPagedTreeTableResponse getReportRightTableColumns(GtnForecastBean gtnForecastBean) {
		...
		// String[] comparisonBasisHeader = new String[]{"Actuals", "Accruals", "Current
		// Projection",
		...
		// String[] variablesHeader = new String[]{"Ex-Factory Sales", "Gross Contract
		// Sales % of Ex-Factory", "Gross Contract Sales", "Contract Units",
		// "Contract Sales % of Total Contract Sales", "Deduction $", "Deduction %",
		// "RPU", "Deduction % of Ex-Factory", "Net Contract Sales",
		// "Net Contract Sales % of Ex-Factory", "Net Ex-Factory Sales", "Net Ex-Factory
		// Sales % of Total Ex-Factory",
		...
		// "grossContractSalesPerExFactory", "contractSales", "contractUnits",
		// "contractSalesPerTotalContractSales",
		// "discountAmount", "discountPercent", "rpu", "deductionPerExfactory",
		// "netContractSales", "netContractSalesPerExfactory", "netExfactorySales",
		...
		// "PerChange", "Volume", "Rate", "ChangeInChange"};
		// String[] variableCategoryHeader = new String[]{"Value", "Variance", "%
		// Change", "Volume", "Rate", "Change in Change"};
	...
	private List<Object[]> getPeriodColumnHeader(List<Object[]> periods) {
        ...
        public GtnWsPagedTableResponse getVariableBreakdownHeaderColumns(GtnUIFrameworkWebserviceRequest gtnUIFrameworkWebserviceRequest)
			throws GtnFrameworkGeneralException {

		GtnWsPagedTableResponse tableHeaderDTO = new GtnWsPagedTableResponse();
		
                List variableBreakdown=gtnUIFrameworkWebserviceRequest.getGtnWsReportRequest().getDataSelectionBean().getVariableBreakdownHeaderLoadList();
               ...
               if(fromPeriod.startsWith("Q")){
                    ...
                    tableHeaderDTO=getHeaderBasedOnFrequency(frequency, quarterToDateForFromPeriod, quarterToDateForToPeriod, tableHeaderDTO,fromPeriod);
                }
                    ...
                    tableHeaderDTO=getHeaderBasedOnFrequency(frequency, semiAnnualToDateForFromPeriod, semiAnnualToDateForToPeriod, tableHeaderDTO,fromPeriod);
                ...
                tableHeaderDTO = getHeaderBasedOnFrequency(frequency, yearToDateForFromPeriod, yearToDateForToPeriod, tableHeaderDTO,fromPeriod);
            } else {
                ...
                tableHeaderDTO = getHeaderBasedOnFrequency(frequency, monthToDateForFromPeriod, monthToDateForToPeriod, tableHeaderDTO,fromPeriod);
    ...
    private GtnWsPagedTableResponse getHeaderBasedOnFrequency(String frequency, List<Integer> quarterToDateForFromPeriod, List<Integer> quarterToDateForToPeriod, GtnWsPagedTableResponse tableHeaderDTO, String fromPeriod) {
        if(frequency.startsWith("Ann")){
            ...
            for (int i = startYear; i <= endYear; i++) {
                ...
                for (int j = quarterStarting; j <= quarterEnding; j++) {
                    tableHeaderDTO.addSingleColumn("q" + j + String.valueOf(i) + "quarter", "Q" + j + " " + String.valueOf(i), String.class);
                }
            }
        }
            ...
            for(int i=startYear;i<=endYear;i++){
                ...
                if (i == endYear) {
                  ...
                  for(int j=semiAnnualStarting;j<=semiAnnualEnding;j++){
                        tableHeaderDTO.addSingleColumn("s"+j+String.valueOf(i)+"semiAnnual", "S"+j+" "+String.valueOf(i), String.class);
                }
            }
        }
        else{
            ...
            for(int i=startYear;i<=endYear;i++){
                ...
                if (i == endYear) {
                  ...
                  for(int j=monthStarting;j<=monthEnding;j++){
                        tableHeaderDTO.addSingleColumn(getMonthFromYear(j)+"month"+String.valueOf(i), getMonthFromYear(j)+" "+String.valueOf(i), String.class);
                }
            }
        }
        return tableHeaderDTO;
