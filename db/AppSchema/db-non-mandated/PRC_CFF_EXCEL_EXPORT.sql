...
SET @SQL_OUT=CONCAT('
	...
	A.CFF_MASTER_SID,SUM(A.COGS_VALUE) COGS_VALUE from #HIERARCHY_NO HR join ', @sales_table ,' A on  a.HIERARCHY_NO like hr.HIERARCHY_NO+''%''
	JOIN #PERIOD P ON P.PERIOD=A.PERIOD AND P.YEAR=A.YEAR
	AND A.INDICATOR=0 AND A.FILTER_CCP=1
	AND (SALES_INCLUSION = @SALES_INCLUSION  OR @SALES_INCLUSION IS NULL)
	...
	LEFT JOIN (SELECT  HR.HIERARCHY_NO,ND.PERIOD,ND.YEAR,SUM(ND.SALES) SALES,SUM(ND.UNITS) AS UNITS,
	ND.CFF_MASTER_SID,SUM(ND.ACCRUALS) AS ACCRUALS,SUM(ND.DISCOUNT) AS DISCOUNT  FROM #HIERARCHY_NO HR join  ' , @discount_table ,' ND
	on ND.HIERARCHY_NO like HR.HIERARCHY_NO+''%'' AND ND.INDICATOR=0 AND ND.DISCOUNT_TYPE=0 and ND.FILTER_CCP=1
	 AND (ND.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY ND.PERIOD,ND.YEAR,ND.CFF_MASTER_SID,HR.HIERARCHY_NO ) ND
	ON ND.PERIOD = S.PERIOD AND S.YEAR = ND.YEAR AND S.CFF_MASTER_SID = ND.CFF_MASTER_SID AND S.HIERARCHY_NO=ND.HIERARCHY_NO 
	LEFT JOIN (SELECT HR.HIERARCHY_NO,MD.PERIOD,MD.YEAR,SUM(MD.SALES) SALES,SUM(MD.UNITS) AS UNITS,
	MD.CFF_MASTER_SID,SUM(MD.ACCRUALS) AS ACCRUALS,SUM(MD.DISCOUNT) AS DISCOUNT FROM #HIERARCHY_NO HR  JOIN  ' , @discount_table ,' MD
	on MD.HIERARCHY_NO like HR.HIERARCHY_NO+''%'' AND MD.INDICATOR=0 AND MD.DISCOUNT_TYPE=1 and MD.FILTER_CCP=1
	 AND (MD.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY MD.PERIOD,MD.YEAR,MD.CFF_MASTER_SID,HR.HIERARCHY_NO ) MD
	ON MD.PERIOD = S.PERIOD AND S.YEAR = MD.YEAR AND S.CFF_MASTER_SID = MD.CFF_MASTER_SID AND S.HIERARCHY_NO = MD.HIERARCHY_NO 
	LEFT JOIN (SELECT HR.HIERARCHY_NO,SPD.PERIOD,SPD.YEAR,SUM(SPD.SALES) SALES,SUM(SPD.UNITS) AS UNITS,
	SPD.CFF_MASTER_SID,SUM(SPD.ACCRUALS) AS ACCRUALS,SUM(SPD.DISCOUNT) AS DISCOUNT FROM  #HIERARCHY_NO HR join ' , @discount_table ,' SPD
	on SPD.HIERARCHY_NO like HR.HIERARCHY_NO+''%'' AND SPD.INDICATOR=0 AND SPD.DISCOUNT_TYPE=2 AND SPD.FILTER_CCP=1
	 AND (SPD.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY SPD.PERIOD,SPD.YEAR,SPD.CFF_MASTER_SID,HR.HIERARCHY_NO ) SPD
	...
	AND A.INDICATOR=1 AND A.FILTER_CCP=1
	AND (SALES_INCLUSION = @SALES_INCLUSION  OR @SALES_INCLUSION IS NULL)
	group by A.PERIOD,A.YEAR,A.CFF_MASTER_SID, HR.HIERARCHY_NO
	) S
	...
	LEFT JOIN (SELECT HR.HIERARCHY_NO,ND.PERIOD,ND.YEAR,SUM(ND.SALES) SALES,SUM(ND.UNITS) AS UNITS,
	ND.CFF_MASTER_SID,SUM(ND.ACCRUALS) AS ACCRUALS,SUM(ND.DISCOUNT) AS DISCOUNT FROM  #HIERARCHY_NO HR JOIN ' , @discount_table ,' ND on  ND.HIERARCHY_NO like HR.HIERARCHY_NO+''%''
	 AND ND.INDICATOR=1 AND ND.DISCOUNT_TYPE=0 AND ND.FILTER_CCP=1
	 AND (ND.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY ND.PERIOD,ND.YEAR,ND.CFF_MASTER_SID, HR.HIERARCHY_NO ) ND
	ON ND.PERIOD = S.PERIOD AND S.YEAR = ND.YEAR AND S.CFF_MASTER_SID = ND.CFF_MASTER_SID AND S.HIERARCHY_NO=ND.HIERARCHY_NO 
	LEFT JOIN (SELECT HR.HIERARCHY_NO,MD.PERIOD,MD.YEAR,SUM(MD.SALES) SALES,SUM(MD.UNITS) AS UNITS,
	MD.CFF_MASTER_SID,SUM(MD.ACCRUALS) AS ACCRUALS,SUM(MD.DISCOUNT) AS DISCOUNT FROM #HIERARCHY_NO HR JOIN ' , @discount_table ,' MD on 
	 MD.HIERARCHY_NO like HR.HIERARCHY_NO+''%'' AND MD.INDICATOR=1 AND MD.DISCOUNT_TYPE=1 AND MD.FILTER_CCP=1
	 AND (MD.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY MD.PERIOD,MD.YEAR,MD.CFF_MASTER_SID, HR.HIERARCHY_NO ) MD
	ON MD.PERIOD = S.PERIOD AND S.YEAR = MD.YEAR AND S.CFF_MASTER_SID = MD.CFF_MASTER_SID AND S.HIERARCHY_NO = MD.HIERARCHY_NO 
	LEFT JOIN (SELECT HR.HIERARCHY_NO,SPD.PERIOD,SPD.YEAR,SUM(SPD.SALES) SALES,SUM(SPD.UNITS) AS UNITS,
	SPD.CFF_MASTER_SID,SUM(SPD.ACCRUALS) AS ACCRUALS,SUM(SPD.DISCOUNT) AS DISCOUNT FROM  #HIERARCHY_NO HR JOIN ' , @discount_table ,' SPD
	on SPD.HIERARCHY_NO like HR.HIERARCHY_NO+''%'' AND SPD.INDICATOR=1 AND SPD.DISCOUNT_TYPE=2 AND SPD.FILTER_CCP=1
	 AND (SPD.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION  OR @DEDUCTION_INCLUSION IS NULL)
	 GROUP BY SPD.PERIOD,SPD.YEAR,SPD.CFF_MASTER_SID, HR.HIERARCHY_NO ) SPD
	ON SPD.PERIOD = S.PERIOD AND S.YEAR = SPD.YEAR AND S.CFF_MASTER_SID = SPD.CFF_MASTER_SID AND S.HIERARCHY_NO = SPD.HIERARCHY_NO
	GROUP BY S.CFF_MASTER_SID,
