...
SELECT SUM(IIF(( DEDUCTION_INCLUSION = CONVERT(CHAR(1), @DEDUCTION_INCLUSION) OR @DEDUCTION_INCLUSION IS NULL ), ISNULL(NSP.ACTUAL_SALES,0), NULL)) AS ACTUAL_SALES
...
SELECT Sum(IIF((DEDUCTION_INCLUSION= CONVERT (CHAR(1),@DEDUCTION_INCLUSION) OR @DEDUCTION_INCLUSION IS NULL),ISNULL(NSP.PROJECTION_SALES,0),NULL)) AS PROJECTION_SALES
...
;WITH PPA_SALES
AS (
	SELECT SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_SALES,0), NULL)) PROJECTION_SALES
		,SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) PROJECTION_UNITS
		,P.PERIOD
		,P.YEAR
		,CD.PPA_DISCOUNT
...
;WITH PPA_SALES
AS (
	SELECT SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_SALES,0), NULL)) PROJECTION_SALES
		,SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) PROJECTION_UNITS
		,P.PERIOD
		,P.YEAR
		,CD.PPA_DISCOUNT
...
SELECT SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_SALES,0), NULL)) ACTUAL_SALES
	,SUM(IIF(( SALES_INCLUSION = CONVERT(CHAR(1), @SALES_INCLUSION) OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) ACTUAL_UNITS
	,P.PERIOD
	,P.YEAR
	,SELECTED_LEVEL
FROM ' + @SALES_ACTUAL_TABLE + ' NSP
...
SELECT PD.PROJECTION_MASTER_SID
	,SUM(IIF(( DI.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL ), ISNULL(ACTUAL_SALES,0), NULL)) AS ACTUAL_SALES
	,SUM(DISCOUNT_AMOUNT) ACCRUAL_DISCOUNT_ACTUAL
	,P.PERIOD
	,P.YEAR
	,SELECTED_LEVEL
...
SELECT pd.PROJECTION_MASTER_SID
	,SUM(IIF(( DI.DEDUCTION_INCLUSION = @DEDUCTION_INCLUSION OR @DEDUCTION_INCLUSION IS NULL ), ISNULL(PROJECTION_SALES,0), NULL)) AS PROJECTION_SALES
	,P.PERIOD
	,P.YEAR
	,SELECTED_LEVEL
	,SUM(DISCOUNT_AMOUNT) ACCRUAL_DISCOUNT_PROJ
...
WITH PPA_SALES
AS (
	SELECT SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_SALES,0), NULL)) PROJECTION_SALES
		,SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) PROJECTION_UNITS
		,P.PERIOD
		,P.YEAR
		,PPA_DISCOUNT
		,PD.PROJECTION_MASTER_SID
...
AS (
	SELECT pd.PROJECTION_MASTER_SID
		,SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_SALES,0), NULL)) PROJECTION_SALES
		,SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) PROJECTION_UNITS
		,P.PERIOD
		,P.YEAR
		,PPA_DISCOUNT
...
SELECT SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_SALES,0), NULL)) ACTUAL_SALES
	,SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(ACTUAL_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) ACTUAL_UNITS
	,P.PERIOD
	,P.YEAR
	,b.SELECTED_LEVEL
	,pd.PROJECTION_MASTER_SID
...
SELECT SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_SALES,0), NULL)) PROJECTION_SALES
	,SUM(IIF(( SALES_INCLUSION = @SALES_INCLUSION OR @SALES_INCLUSION IS NULL ), ISNULL(PROJECTION_UNITS,0), NULL) *  COALESCE(NULLIF(UOM_VALUE, 0),1) ) PROJECTION_UNITS
	,P.PERIOD
	,P.YEAR
	,b.SELECTED_LEVEL
...
SELECT DISTINCT RD.PROJECTION_MASTER_SID
	...
	,TOTAL_DISCOUNT_ACCRUAL
FROM #RESULT_DETAILS_INFO RD
JOIN RS_CONTRACT RC ON  RC.RS_CONTRACT_SID=RD.RS_NAME   WHERE PROJECTION_MASTER_SID<>@FIRST_PROJ_SID
ORDER BY RS_NAME
	,RD.YEAR
	,RD.PERIOD
	,RS_CONTRACT_SID
...
SELECT DISTINCT PROJECTION_MASTER_SID
	...
	,RS_CONTRACT_SID
	,TOTAL_DISCOUNT_ACCRUAL
FROM #RESULT_DETAILS_INFO  RD WHERE PROJECTION_MASTER_SID<>@FIRST_PROJ_SID
ORDER BY RS_NAME
	,RD.YEAR
	,RD.PERIOD
	,RS_CONTRACT_SID
	...
	DECLARE @SQL1 NVARCHAR(MAX)
		...
		IF @LOOP_CNTR=1
		BEGIN
			SET @SQL1 += 'CONTRACT_DISCOUNT_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN IIF(CONTRACT_DISCOUNT_ACCRUAL=0,NULL,CONTRACT_DISCOUNT_ACCRUAL) END), 
						   CONTRACT_DISCOUNT_ACTUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = CASE WHEN '''+CAST(@CURRENT_DATE AS VARCHAR(10))+''' > DATEFROMPARTS(YEAR,PERIOD,01) THEN SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN CONTRACT_DISCOUNT_ACTUAL END) ELSE NULL END, 
								  CONTRACT_DISCOUNT_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN CONTRACT_DISCOUNT_PROJECTED END),0), 
								  TOTAL_DISCOUNT_ACCRUAL_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_ACCRUAL_PERCENTAGE END),
								  TOTAL_DISCOUNT_ACTUAL_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' =CASE WHEN '''+CAST(@CURRENT_DATE AS VARCHAR(10))+''' > DATEFROMPARTS(YEAR,PERIOD,01) THEN SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_ACTUAL_PERCENTAGE END) ELSE NULL END,  
                                  TOTAL_DISCOUNT_PROJECTED_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_PROJECTED_PERCENTAGE END),0),
                                   TOTAL_RPU_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_RPU_ACCRUAL END),
                                   TOTAL_RPU_ACTUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' =CASE WHEN '''+CAST(@CURRENT_DATE AS VARCHAR(10))+''' > DATEFROMPARTS(YEAR,PERIOD,01) THEN SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 
				'THEN TOTAL_RPU_ACTUAL END) ELSE NULL END,
                                   TOTAL_RPU_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_RPU_PROJECTED END),0),
                                   DISCOUNT_OF_EX_FACTORY_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_ACCRUAL END),
                                   DISCOUNT_OF_EX_FACTORY_ACTUALS_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' =CASE WHEN '''+CAST(@CURRENT_DATE AS VARCHAR(10))+''' > DATEFROMPARTS(YEAR,PERIOD,01) THEN SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_ACTUALS END) ELSE NULL END,
                                   DISCOUNT_OF_EX_FACTORY_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_PROJECTED END),0),'
			END 
			ELSE 
			BEGIN
			SET @SQL1 += 'CONTRACT_DISCOUNT_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN CONTRACT_DISCOUNT_ACCRUAL END), 
						   CONTRACT_DISCOUNT_ACTUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN CONTRACT_DISCOUNT_ACTUAL END),0), 
								  CONTRACT_DISCOUNT_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN CONTRACT_DISCOUNT_PROJECTED END),0), 
								  TOTAL_DISCOUNT_ACCRUAL_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_ACCRUAL_PERCENTAGE END),
								  TOTAL_DISCOUNT_ACTUAL_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_ACTUAL_PERCENTAGE END),0),      
                                  TOTAL_DISCOUNT_PROJECTED_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_DISCOUNT_PROJECTED_PERCENTAGE END),0),
                                   TOTAL_RPU_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_RPU_ACCRUAL END),
                                   TOTAL_RPU_ACTUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 
					'THEN TOTAL_RPU_ACTUAL END),0),
                                   TOTAL_RPU_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN TOTAL_RPU_PROJECTED END),0),
								     DISCOUNT_OF_EX_FACTORY_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_ACCRUAL END),
                                   DISCOUNT_OF_EX_FACTORY_ACTUALS_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_ACTUALS END),0) ,
                                   DISCOUNT_OF_EX_FACTORY_PROJECTED_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = ISNULL(SUM(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + 'THEN DISCOUNT_OF_EX_FACTORY_PROJECTED END),0),'
				END
	...
	SET @SQL1 = LEFT(@SQL1, Len(@SQL1) - 1)
				IF (@PROGRAM_TYPE = 'Program'   or @PROGRAM_TYPE = 'Schedule Id') 
		BEGIN
