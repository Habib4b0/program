 ...
 * @author Mahesh.James
 */
public class GtnUiFrameworkCustomerADDAllButtonAction implements GtnUIFrameWorkAction ,GtnUIFrameworkDynamicClass{
	...
	public void doAction(String componentId, GtnUIFrameWorkActionConfig gtnUIFrameWorkActionConfig)
		...
		GtnUIFrameworkComponentData componentData = GtnUIFrameworkGlobalUI
				.getVaadinComponentData("dcCustomerSelectedResultTable");
		...
		GtnWsRecordBean gtnWsRecordBean = GtnUIFrameworkGlobalUI
				.getVaadinBaseComponent("dcCustomerAvailablesearchResultTable").getValueFromPagedDataTable();

		GtnWsDeductionCalendarBean deductionCalendarBean = new GtnWsDeductionCalendarBean();

		if ("dcCustomerAvailableTablegtnAddAll01".equals(componentId)) {
			deductionCalendarBean.setIsAddAll(true);

		} else {
			List<Integer> idList = new ArrayList<>();
			idList.add(Integer.valueOf(String.valueOf(gtnWsRecordBean.getProperties().get(27))));
	...
	public GtnUIFrameWorkAction createInstance() {
		return new GtnUiFrameworkCustomerADDAllButtonAction();
	}
	...
	public void insertData(GtnWsDeductionCalendarBean deductionCalendarBean) {
		...
		new GtnUIFrameworkWebServiceClient().callGtnWebServiceUrl(
				"/" + GtnWsCDRContants.DC_SERVICE + GtnWsCDRContants.DC_COMPANY_ADD_SERVICE, updateRequest,
