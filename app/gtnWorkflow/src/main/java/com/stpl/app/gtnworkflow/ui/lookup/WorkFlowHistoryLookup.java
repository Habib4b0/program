...
public class WorkFlowHistoryLookup extends Window {
    ...
    BeanItemContainer<WorkFlowHistoryLookupDTO> wfHistoryLookupitemBean = new BeanItemContainer<WorkFlowHistoryLookupDTO>(WorkFlowHistoryLookupDTO.class);
    ...
    public WorkFlowHistoryLookup(InboxDashboardDTO inboxDashBoardDTOHidden) {
        ...
        filePath = new File(basepath + File.separator + "Documents" + File.separator + StringUtils.EMPTY);
        init();
    }
    ...
    public void configureFields() {
        ...
        if (inboxDashBoardDTOHidden.getWorkflowId().startsWith(WorkflowConstants.getComWorkflowParameter()) || inboxDashBoardDTOHidden.getWorkflowId().startsWith(WorkflowConstants.getGovWorkflowParameter())) {
        ...
        if ("ARM".equals(businessProcess.getValue())) {
            ...
            if (!CommonUtils.CONTRACT_MANAGEMENT.equals(businessProcess.getValue())) {
                List resultList = workFlowLogic.getDetailsForHistory(inboxDashBoardDTOHidden.getWorkflowMasterSystemId());
                if (resultList != null) {
                    getWorkFlowHistory(resultList, businessProcess.getValue());
        }
            }
            ...
            public void historyitemClickLogic(ItemClickEvent event) {
                ...
                if (event.isDoubleClick()) {
                    ...
                    if (historydto.getFileName() != null && !historydto.getFileName().isEmpty()) {
                        String sql = "SELECT DD.FILE_NAME,DD.UPLOADED_DATE,DD.UPLOADED_BY,DD.FILE_TYPE FROM DOC_DETAILS DD WHERE DOC_DETAILS_ID IN (" + historydto.getFileName() + ")";
                        List<Object[]> list = HelperTableLocalServiceUtil.executeSelectQuery(sql);

                        for (int i = 0; i < list.size(); i++) {
                            Object[] obj = list.get(i);
                            dto.setDocumentName(obj[0] != null && !String.valueOf(obj[0]).isEmpty() ? String.valueOf(obj[0]) : StringUtils.EMPTY);
                            dto.setDateAdded(obj[1] != null ? String.valueOf((Date) obj[1]) : null);
                            dto.setUserName(obj[NumericConstants.TWO] != null && !String.valueOf(obj[NumericConstants.TWO]).isEmpty() ? String.valueOf(obj[NumericConstants.TWO]) : StringUtils.EMPTY);
        ...
        table.addItemClickListener(new ItemClickEvent.ItemClickListener() {
            ...
            public void attachmentitemClickLogic(ItemClickEvent event) {
                try {
                    ...
                    if (event.isDoubleClick()) {
                        File uploadedFile = new File(notesdto.getDocumentFullPath());
    ...
    private void createWorkSheet() {
        ...
        try {
            final long recordCount = workFlowLogic.workFlowHistorySearchCount(workFlowMasterSystemId, tableLogic.getFilters());
            ExcelExportforBB.createWorkSheet(results.getColumnHeaders(), recordCount, this, getUI(), "WorkFlow_History");
