...
public class PromoteTpSearchDao implements BeanDAO<PromoteTpToChDto> {
    ...
    public int count(BeanSearchCriteria sc) {
        int count = 0;
        try {
            count = logic.searchCompanyCount(promoteTpToChDto, sc);
        } catch (Exception ex) {

            LOGGER.error(ex.getMessage());
        }
        LOGGER.info("count in PromoteTpSearchDao---->" + count);
        return count;
    }
    ...
    public List<PromoteTpToChDto> find(BeanSearchCriteria sc, int startIndex, int offset, List<OrderByColumn> orderByColumns) {
        ...
        try {
            resultList = logic.searchCompaniesLazy(promoteTpToChDto, startIndex, offset, sc, orderByColumns);
        } catch (Exception ex) {
