 ...
 */
package org.stpl.addon.calendar;
...
import org.stpl.addon.calendar.shared.DateValueChangeRpc;

@SuppressWarnings("serial")
public class SimpleCalendar extends AbstractField<Object> {

    public DateValueChangeRpc valueChangeRpc = new DateValueChangeRpc() {
     ...
     */
    @Override
    public SimpleCalendarState getState(boolean markAsDirty) {
...
//                super.setValue(newValue);
                getState().setDate=Arrays.asList((Date) newValue);
//                getState().setDate;
            }
...
//                            "Date is outside of start or end date");
//                }
                getState().setDate=(List <Date>) newValue;
            }
     ...
     *
     * @param multiselect Should the user be able to select several dates
     */
    ...
    public boolean isDateDisabled(Date date) {
        ...
        if (getDisabledWeekdays().length > 0) {
        ...
        if (getDisabledMonthlyDates().length > 0) {
    ...
    public boolean isDateInRange(Date date) {
        Date Beforeday =getStartDate();
        Beforeday.setDate(getStartDate().getDate()-1);
        if (getStartDate() != null && date != null
                && date.before(Beforeday)) {
            return false;
        }
        Date Afterday =getEndDate();
         Afterday.setDate(getStartDate().getDate()+1);
        if (getEndDate() != null && date != null && date.after(Afterday)) {
            return false;
        }
        return true;
    }
     ...
     * 1 and 31.
     *
     * @param days The days of the month which should be disabled each month
     */
     ...
     *
     * @param dates Dates which are disabled
     */
     ...
     *
     * @param start The first date in the panel
     */
     ...
     *
     * @param end The last date the panel should display
     */
