...
public class GtnWsTreeService {
		...
		for (Object[] objects : customViewInput) {
			...
			if (!nextCCPChildNode.isEmpty()) {
				for (GtnWsReportEngineTreeNode gtnWsTreeNode : nextCCPChildNode) {
					...
					case "C":
						addingInputNodeToParentNodeRecursive(customer, gtnWsTreeNode, objects, index, initialLevelNo,
								"C", ccpResult);
						break;
					case "P":
						addingInputNodeToParentNodeRecursive(product, gtnWsTreeNode, objects, index, initialLevelNo,
								"P", ccpResult);
						break;
					case "D":
	...
	private void addingInputNodeToParentNodeRecursive(GtnWsReportEngineTreeNode treeNode,
			GtnWsReportEngineTreeNode rootNode, Object[] objects, int index, int initialLevelNo, String indicator,
		...
		if (treeNode.getLevelNumber() != 0 && currentLevelNo == treeNode.getLevelNumber()) {
			addingChildNodeToParentNodeRecursive(ccpResult, indicator, rootNode, index, initialLevelNo,
					customeViewLevelNo, treeNode);
		} else {
			for (GtnWsReportEngineTreeNode childNode : treeNode.getChildren()) {
				addingInputNodeToParentNodeRecursive(childNode, rootNode, objects, index, initialLevelNo, indicator,
						ccpResult);
			}
		}
	}

	private void addCCPDiscountJoinNode(GtnWsReportEngineTreeNode rootNode, Object[] objects, int index,
			int initialLevelNo, List<Object[]> ccpResult, List<Object[]> deductionHierarchy) {
		int currentLevelNo = (int) objects[0];
		int customeViewLevelNo = (int) objects[3];
		...
		for (GtnWsReportEngineTreeNode gtnWsTreeNode : deductionTree) {
			addingChildNodeToParentNodeRecursive(ccpResult, "D", rootNode, index, initialLevelNo, customeViewLevelNo,
					gtnWsTreeNode);
		}

	}

	private void addingChildNodeToParentNodeRecursive(List<Object[]> ccpResult, String indicator,
			GtnWsReportEngineTreeNode rootNode, int index, int initialLevelNo, int customeViewLevelNo,
			GtnWsReportEngineTreeNode treeNode) {
		int ccpIndex = "D".equals(indicator) ? 3 : "C".equals(indicator) ? 1 : 2;
		for (Object[] objects : ccpResult) {
			if (objects[ccpIndex] != null
					&& (objects[ccpIndex].toString().startsWith(treeNode.getHierarchyNo())
							|| ("D".equals(indicator) && objects[ccpIndex] != null && treeNode.getRsIds() != null
									&& treeNode.getRsIds().contains(Integer.parseInt(objects[ccpIndex].toString()))))
					&& checkingParentNodeRecursively(rootNode, objects)) {
	...
	private boolean checkingParentNodeRecursively(GtnWsReportEngineTreeNode rootNode, Object[] objects) {
		...
		if ("D".equals(rootNode.getIndicator())) {
			if (rootNode.getParent() == null && objects[ccpIndex] != null && rootNode.getRsIds() != null
					&& rootNode.getRsIds().contains(Integer.parseInt(objects[ccpIndex].toString()))
					&& rootNode.getCcpIds() != null
					&& rootNode.getCcpIds().contains(Integer.parseInt(objects[0].toString()))) {
				returnFlag = true;
			} else if (objects[ccpIndex] != null && rootNode.getRsIds() != null
					&& rootNode.getRsIds().contains(Integer.parseInt(objects[ccpIndex].toString()))
					&& rootNode.getCcpIds() != null
					&& rootNode.getCcpIds().contains(Integer.parseInt(objects[0].toString()))) {
