...
import com.stpl.sso.autologin.config.StplSpnegoConfigReader;
...
final class SpnegoFilterConfig { // NOPMD
    ...
    private SpnegoFilterConfig(final FilterConfig config) throws FileNotFoundException, URISyntaxException {
        ...
        setLogLevel(StplSpnegoConfigReader.getInstance().getPropertyBean().getLogLevel());

        // check if exists
        assert loginConfExists(StplSpnegoConfigReader.getInstance().getPropertyBean().getLoginConf());

        // specify krb5 conf as a System property
        if (null == StplSpnegoConfigReader.getInstance().getPropertyBean().getKrbConf()) {
            throw new IllegalArgumentException(
                    SpnegoFilterConfig.MISSING_PROPERTY + Constants.KRB5_CONF);
        } else {
            System.setProperty("java.security.krb5.conf", StplSpnegoConfigReader.getInstance().getPropertyBean().getKrbConf());
        }

        // specify login conf as a System property
        if (null == StplSpnegoConfigReader.getInstance().getPropertyBean().getLoginConf()) {
            throw new IllegalArgumentException(
                    SpnegoFilterConfig.MISSING_PROPERTY + Constants.LOGIN_CONF);
        } else {
            System.setProperty("java.security.auth.login.config", StplSpnegoConfigReader.getInstance().getPropertyBean().getLoginConf());
        }

        // check if exists and no options specified
        doClientModule(StplSpnegoConfigReader.getInstance().getPropertyBean().getCliendModule());

        // determine if all req. met to use keyTab
        doServerModule(StplSpnegoConfigReader.getInstance().getPropertyBean().getServerModule());

        // if username/password provided, don't use key tab     
        this.setUsernamePassword(StplSpnegoConfigReader.getInstance().getPropertyBean().getUserName(), StplSpnegoConfigReader.getInstance().getPropertyBean().getPassWord());
        
        // determine if we should support Basic Authentication
        setBasicSupport(StplSpnegoConfigReader.getInstance().getPropertyBean().isAllowBasic(), StplSpnegoConfigReader.getInstance().getPropertyBean().isAllowUnsecureBasic());

        // determine if we should Basic Auth prompt if rec. NTLM token
        setNtlmSupport(StplSpnegoConfigReader.getInstance().getPropertyBean().isPromtNtlm());

        // requests from localhost will not be authenticated against the KDC 
        this.allowLocalhost
                = StplSpnegoConfigReader.getInstance().getPropertyBean().isAllowLocalHost();
