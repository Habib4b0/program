...
import com.stpl.gtn.gtn2o.ui.framework.component.table.pagedtreetable.GtnUIFrameworkPagedTreeTableColumnListener;
...
//        pagedTreeTableConfig.setQueryBean(queryBean);
		...
		try {
			...
			configureTableHeaders(pagedTreeGrid, tableConfig);
...
//		resultsTable.addStyleNames(ExtFilteringTableConstant.VALO_THEME_EXTFILTERING_TABLE);
	...
	protected void configureTableHeaders(PagedTreeGrid pagedTreeGrid,
			GtnUIFrameworkPagedTreeTableConfig tableConfig) {
...
//		rightTable.setColumnHeaders(rightHeaderList.toArray(new String[rightHeaderList.size()]));
		if (tableConfig.isDoubleHeaderVisible()) {
                          tableConfig.getLeftTableDoubleHeaderVisibleHeaders().addAll(tableConfig.getRightTableDoubleVisibleHeaders());
                tableConfig.getLeftTableDoubleHeaderVisibleColumns().addAll(tableConfig.getRightTableDoubleHeaderVisibleColumns());
