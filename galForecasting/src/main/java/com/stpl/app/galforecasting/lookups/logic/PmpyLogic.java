 ...
 */
package com.stpl.app.galforecasting.lookups.logic;
...
public class PmpyLogic {
     ...
     */
    public Object tradingPartnerLookUp(String tpNo, String tpName, Object contractHolder, int start, int offset, final List<SortByColumn> sortByColumns, final Set<Container.Filter> filterSet, boolean isCount) throws SystemException, Exception {
    ...
    List getPmpyData(List input, String queryName, final int index, final int next, final List<SortByColumn> sortByColumns, final Set<Container.Filter> filterSet, boolean isCount, Map<String, String> parameters) {
        LOGGER.info("Inside getPmpyData queryName=" + queryName);
        ...
        LOGGER.info("End of getPmpyData");
        return list;
    }
    ...
    public List loadCustomer(int customerHierarchyId, int productHierarchyId) throws PortalException, Exception {
    ...
    public List loadContract(int customerHierarchyId, int productHierarchyId, int customerSid) throws PortalException, Exception {
    ...
    public List loadProduct(int customerHierarchyId, int productHierarchyId, int customerSid, int contractSid) throws PortalException, Exception {
    ...
    public List convertProductResults(List<Object[]> list) {
        ...
        for (Object[] obj : list) {
            ...
            pmpyDTO.setProductName(String.valueOf(obj[2]));
    ...
    public List<MPmpyDTO> convertPMPYResults(List<Object[]> list, String frequency, boolean isExcelExport) {
        ...
        for (Object[] obj : list) {
            ...
            if (frequency.contains(Constants.FrequencyConstants.QUARTERLY.getConstant())) {
                key = Constant.Q + obj[5] + StringUtils.EMPTY + obj[6];
            } else if (frequency.contains(Constants.FrequencyConstants.SEMI_ANNUAL.getConstant())) {
                key = Constant.S + obj[5] + StringUtils.EMPTY + obj[6];
            } else if (frequency.contains(Constants.FrequencyConstants.ANNUAL.getConstant())) {
                key = StringUtils.EMPTY + obj[5];
            } else if (frequency.contains(Constants.FrequencyConstants.MONTHLY.getConstant())) {
                String monthName = HeaderUtils.getMonthForInt(Integer.valueOf(String.valueOf(obj[5])));
                key = String.valueOf(monthName + String.valueOf(obj[6])).toUpperCase();
            }
            ...
            if (StringUtils.isNotEmpty(key) && StringUtils.isNotBlank(key)) {
                ...
                if (StringUtils.EMPTY.equals(String.valueOf((obj[2]))) || Constant.NULL.equals(String.valueOf((obj[2])))) {
                    pmpyProjectionSalesDTO.addStringProperties(key, "$0");
                } else {
                    String tempValue = SalesUtils.valueToCurrency(String.valueOf((obj[2])), doubleDecimalFormat);
                    pmpyProjectionSalesDTO.addStringProperties(key, tempValue);
                }

                if (StringUtils.EMPTY.equals(String.valueOf((obj[3]))) || Constant.NULL.equals(String.valueOf((obj[3])))) {
                    pmpyProjectionUnitsDTO.addStringProperties(key, DASH);
                } else {
                    String tempValue = noDecimalFormat.format(Double.valueOf(String.valueOf(obj[3])));
                    pmpyProjectionUnitsDTO.addStringProperties(key, tempValue);
                }

                if (StringUtils.EMPTY.equals(String.valueOf((obj[4]))) || Constant.NULL.equals(String.valueOf((obj[4])))) {
                    pmpyLivesDTO.addStringProperties(key, DASH);
                } else {
                    String tempValue = noDecimalFormat.format(Double.valueOf(String.valueOf(obj[4])));
        ...
        resultList.add(2, pmpyActualUnitsDTO);
        resultList.add(3, pmpyProjectionUnitsDTO);
        resultList.add(4, pmpyLivesDTO);
    ...
    private List callNmPmpyProcedure(Object inputs[]) throws SystemException, SQLException {
        ...
        try {
            ...
            LOGGER.info("Entering callNmPmpyProcedure  ::::");
    ...
    public static Boolean pmpyUpdate(List input, String queryName) {
        LOGGER.info("Inside pmpy Update ");
        StringBuilder sql = new StringBuilder();
        try {
        ...
        LOGGER.info("End of pmpy Update ");
