...
-------------------- snap_cfp_eligibility_date ---------------------
 IF EXISTS (
		SELECT 1
		FROM INFORMATION_SCHEMA.tables
		WHERE TABLE_NAME = 'SNAP_CFP_2018_SEMI_ANNUAL_1'
		)
BEGIN
	IF NOT EXISTS (
			SELECT 1
	...
	BEGIN
		DROP TABLE SNAP_CFP_2018_SEMI_ANNUAL_1
	END
END
  GO
...
-------------------- snap_CONTRACT_ELIGIBLE_DATE ---------------------  
 IF EXISTS (
		SELECT 1
		FROM INFORMATION_SCHEMA.tables
		WHERE TABLE_NAME = 'SNAP_CONTRACT_HEADER_2018_SEMI_ANNUAL_1'
		)
BEGIN
	IF NOT EXISTS (
			SELECT 1
	...
	BEGIN
		DROP TABLE SNAP_CONTRACT_HEADER_2018_SEMI_ANNUAL_1
	END
END
  --GO
