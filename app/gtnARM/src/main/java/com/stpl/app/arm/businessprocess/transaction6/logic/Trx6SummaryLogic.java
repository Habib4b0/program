...
public class Trx6SummaryLogic<T extends AdjustmentDTO> extends AbstractPipelineSummaryLogic<T> {
    ...
    public static final Logger LOGGERFORSUMMARYLOGIC = Logger.getLogger(Trx6SummaryLogic.class);
    ...
    protected int getSummaryCount(List<Object> inputs, Criteria criteria) {
        ...
        try {
            ...
            if (isView) {
                resultList = QueryUtils.getItemData(inputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "getTrx6_Summary_generate_Inventory_Count", null, null, CommonConstant.ARM_INFLATION_INVENTORY_ADJ, null, null);
            } else {
                resultList = QueryUtils.getItemData(inputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "getTrx6_Summary_generate_Inventory_Count", null, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INFLATION_INVENTORY_ADJ), null, null);
            }
    ...
    protected DataResult<T> getSummaryData(List<Object> inputs, Criteria criteria, TreeMap<String, Integer> masterSids) {
        ...
        try {
            ...
            List<Object[]> data;
            if (isView) {
                data = QueryUtils.getItemData(inputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "getTrx6_Summary_generate_Inventory_select", null, "getTrx6_Summary_generate_Inventory_select_orderBy", CommonConstant.ARM_INFLATION_INVENTORY_ADJ, null, "WHERE RANK_VIEW BETWEEN ? AND ?");
            } else {
                data = QueryUtils.getItemData(inputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "getTrx6_Summary_generate_Inventory_select", null, "getTrx6_Summary_generate_Inventory_select_orderBy", criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INFLATION_INVENTORY_ADJ), null, "WHERE RANK_VIEW BETWEEN ? AND ?");
            }
            ...
            if (totalFlag) {
                ...
                totalInputs.remove(totalInputs.size() - 1);
                if (isView) {
                    totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_TRX6Inventory_total_data", null, null, CommonConstant.ARM_INFLATION_INVENTORY_ADJ, null, null);
                } else {
                    totaldata = QueryUtils.getItemData(totalInputs, CommonConstant.TRX6_SUMMARY_GENERATE_INVENTORY_EDIT, "Summary_TRX6Inventory_total_data", null, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_INFLATION_INVENTORY_ADJ), null, null);
                }
        ...
        try {
            ...
            for (int j = 0; j < data.size(); j++) {
                ...
                if (!brand.equals(lastBrand)) {
                    ...
                    resultList.add(dto);
                    if (!isTotal) {
                        index = indexMap.get(get[0].toString().replace(" ", org.apache.commons.lang.StringUtils.EMPTY)) * (hasOverride ? NumericConstants.SIX : NumericConstants.FIVE);
                    }
                }
                if (dto != null) {
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.TWO] == null ? StringUtils.EMPTY : DataFormatConverter.INDICATOR_DOLLAR + zeroDecimals.format(Double.valueOf(get[NumericConstants.TWO].toString().trim())));
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.THREE] == null ? StringUtils.EMPTY : DataFormatConverter.INDICATOR_DOLLAR + zeroDecimals.format(Double.valueOf(get[NumericConstants.THREE].toString().trim())));
                    ...
                    dto.addStringProperties(variables.get(index++), get[NumericConstants.FIVE] == null ? StringUtils.EMPTY : DataFormatConverter.INDICATOR_DOLLAR + zeroDecimals.format(Double.valueOf(get[NumericConstants.FIVE].toString().trim())));
