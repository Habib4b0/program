...
//                sessionDTO.setMode(ConstantsUtils.EDIT);
            ...
            notesLogic.saveUploadedInformation(availableUploadedInformation, "DEDUCTION_CALENDAR_MASTER", sessionDTO.getSystemId());
            sessionDTO.setSystemId(result.getDeductionCalendarMasterSid());
        ...
        try {
            ...
            try {
                for (Object obj : list) {
                    ...
                    record.setDeductionCalendarname(String.valueOf(result[2]));
                    record.setDeductionCalendardesc(String.valueOf(result[3]));
                    if (result[4] != null) {
                        record.setCategory((ConstantsUtils.SELECT_ONE).equals(String.valueOf(result[4])) ? StringUtils.EMPTY : String.valueOf(result[4]));
                    } else {
                        record.setCategory(String.valueOf(result[4]));
                    }
                    record.setDcCreatedBy(userMap.get(Integer.valueOf(String.valueOf(result[5]))));
                    record.setDcCreationDate(String.valueOf(result[6]).equals(ConstantsUtils.NULL) ? StringUtils.EMPTY : parseDateLogic(result[6]));
                    record.setDcModifiedBy(userMap.get(Integer.valueOf(String.valueOf(result[7]))));
                    record.setDcModifiedDate(String.valueOf(result[8]).equals(ConstantsUtils.NULL) ? StringUtils.EMPTY : parseDateLogic(result[8]));
    ...
    public void deleteTempDeductionDetails(SessionDTO session) {
        String query = CustomSQLUtil.get("deleteDeductionDetails");
    ...
    public void sameToTemp(SessionDTO session, int detailSid) {
        String query = CustomSQLUtil.get("insertToTempForDetails");
        query = query.replace("?DCID", String.valueOf(detailSid));
        query = query.replace("?UID", session.getUserId());
        query = query.replace("?SID", "'" + session.getUiSessionId() + "'");
        HelperTableLocalServiceUtil.executeUpdateQuery(query);
    }

    public DeductionCalendarDTO getDeductionCalendarById(int masterSid, DeductionCalendarDTO dto) {
        LOGGER.info("Entering getDeductionCalendarById with " + masterSid);
        try {
    ...
    public void saveDeductionDetails(SessionDTO sessionDTO) {
        String query = CustomSQLUtil.get("Add".equals(sessionDTO.getMode()) ? "savedeductiondetails" : "updatedeductiondetails");
