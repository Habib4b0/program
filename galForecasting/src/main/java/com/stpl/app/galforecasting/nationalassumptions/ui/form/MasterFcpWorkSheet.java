...
import com.stpl.app.galforecasting.nationalassumptions.logic.tablelogic.MasterFcpWorkSheetTableLogic;
...
import static com.stpl.app.galforecasting.nationalassumptions.util.Constants.LabelConstants.MASTER_FCP_WORKSHEET_LOOKUP;
import static com.stpl.app.galforecasting.nationalassumptions.util.Constants.LabelConstants.NATIONAL_ASSUMPTIONS;
import static com.stpl.app.galforecasting.nationalassumptions.util.Constants.LabelConstants.NOTES_DATE;
...
public class MasterFcpWorkSheet extends Window {
     ...
     * @param projSelection
     */
    public MasterFcpWorkSheet(ProjectionSelectionDTO projSelection) {
        super("Master FCP Worksheet");
        LOGGER.info("MasterFcpWorkSheet Constructor initiated ");
        ...
        init();
        LOGGER.info("MasterFcpWorkSheet Constructor Ended ");
    }
    ...
    public void init() {
        ...
        setWidth(68f, Sizeable.Unit.PERCENTAGE);
        setHeight(70f, Sizeable.Unit.PERCENTAGE);
        configureFields();
    ...
    private void configureFields() {
        ...
        try {
            if (ANNUALLY.getConstant().equals(projectionDTO.getFrequency())) {
                ...
                projectionDTO.setHistoryNum(projectionDTO.getHistoryNum() * 4);
            }
    ...
    public void submit(Button.ClickEvent event) {
        if (submitMsg) {
            new AbstractNotificationUtils() {
                ...
                public void yesMethod() {
                    try {
                        queryUtil.updateAdjustment(projectionDTO.getNdcSid().getId(), "getFcpAdjSumbitUpdate");
                        submitFlag = true;
                        submitMsg = false;
                    } catch (Exception ex) {
        ...
        try {
            ...
            if (formatFlag) {
                ...
                if (!fssAdjustedValues.isEmpty()) {
                    queryUtil.saveNotes(fssAdjustedValues, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId(), "QFSS");
                    adjustFlag = true;
                    fssAdjustedValues.clear();
                }
                if (!fssEditedNotes.isEmpty()) {
                    queryUtil.saveNotes(fssEditedNotes, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId(), "QFSS");
                    notesFlag = true;
                    fssEditedNotes.clear();
                }
                if (!ovrAdjustedValues.isEmpty()) {
                    queryUtil.saveNotes(ovrAdjustedValues, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId(), "QNON-FAMP");
                    adjustFlag = true;
                    ovrAdjustedValues.clear();
                }
                if (!ovrEditedNotes.isEmpty()) {
                    queryUtil.saveNotes(ovrEditedNotes, projectionDTO.getProjectionId(), projectionDTO.getNdcSid().getId(), "QNON-FAMP");
                    notesFlag = true;
    ...
    private void loadResultTable(int levelNo, String hierarchyNo, boolean flag) {
        try {
            ...
            tableLogic.setProjectionResultsData(projectionDTO, levelNo, hierarchyNo, flag);
    ...
    public void excelBtn(final Button.ClickEvent event) {
        ...
        ExcelExport exp = new ExcelExport(new ExtCustomTableHolder(exceltable), "Master FCP Worksheet", "Master FCP Worksheet", "Master FCP Worksheet.xls", false);
        exp.export();
    ...
    private void loadExcelResultTable() {
        ...
        List<TableDTO> resultList = fcpLogic.getFcpWorksheet(projectionDTO);
    ...
    public void addLowerLevelsForExport(TableDTO id) {
        ...
        List<TableDTO> resultList = fcpLogic.getFcpWorksheetChild(projectionDTO, id.getItemMasterSid());
    ...
    private void callAdjustmentProcedure() {
        ...
        try {
            ...
            medLogic.workSheetSetupCook(projectionDTO.getNdcSid().getId(), priceType, "FCP WORKSHEET", StringUtils.EMPTY);
        } catch (Exception ex) {
            LOGGER.error(ex.getMessage());
        }
    }
    ...
    public void closeLogic() {
        try {
            queryUtil.updateAdjustment(projectionDTO.getNdcSid().getId(), "getFcpAdjCloseUpdate");
        } catch (Exception ex) {
            LOGGER.error(ex.getMessage());
        }
    }
