...
import static com.stpl.app.arm.AbstractForms.AbstractFilter.DBDate;
...
public class Trx7ExclusionDetailsLogic {
    ...
    public static final Logger LOGGER = Logger.getLogger(Trx7ExclusionDetailsLogic.class);
    ...
    public int LoadView_RateConfig(DataSelectionDTO dataSelectionDTO) {
        try {
            int returnValue = 0;
            String query = SQlUtil.getQuery("LoadViewQuery");
            query = query.replace("@ADJUSTMENT_TYPE", "" + dataSelectionDTO.getAdjustmentId());
            query = query.replace("@GL_COMPANY_MASTER_SID", "" + dataSelectionDTO.getCompanyMasterSid());
            query = query.replace("@BU_COMPANY_MASTER_SID", "" + dataSelectionDTO.getBu_companyMasterSid());
            List rawList = QueryUtils.executeSelect(query);
            return returnValue = rawList == null || rawList.isEmpty() || rawList.get(0) == null ? 0 : Integer.parseInt(String.valueOf(rawList.get(0)));
        } catch (Exception e) {
            LOGGER.error(e);
            return 0;
        }
    ...
    public ViewLookupDTO LoadView_ViewName(int viewMasterSid) {
        ViewLookupDTO dtoValue = new ViewLookupDTO();
        try {

            String query = SQlUtil.getQuery("LoadViewNameQuery");
            query = query.replace("@VIEWID", "" + viewMasterSid);
            List<Object[]> rawList = QueryUtils.executeSelect(query);
            if (rawList == null || rawList.isEmpty()) {
                return dtoValue;
            }
            if (!rawList.isEmpty()) {
                for (int i = 0; i < rawList.size(); i++) {
                    ...
                    dtoValue.setViewSid(viewMasterSid + "");
                }
            }
            return dtoValue;
        } catch (Exception e) {
