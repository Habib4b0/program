...
public abstract class AbstractComparisonLookup extends AbstractLookup {
     ...
     * Builds the lookup
     *
     * @param results Search results table
     * @param selectedProjection Selected projection table
     * @return
     */
    protected VerticalLayout buildComparisonLookup(final ExtFilterTable results, final ExtFilterTable selectedProjection, int currentProjId, final List<ComparisonLookupDTO> selectedList) {
    ...
    private VerticalLayout addComparisonLookup() {
        VerticalLayout comparisonLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
    ...
    private VerticalLayout addProjectionSearchSection() {
        VerticalLayout searchSectionLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
    ...
    private Panel addSearchCriteria() {
        ...
        GridLayout searchLayout = new GridLayout(6, 3);
        searchLayout.setSpacing(true);

        Panel createdDatePanel = UiUtils.addCommonPanel(CREATED_DATE.getConstant());
        HorizontalLayout fromLayout = (HorizontalLayout) UiUtils.getLayout(HorizontalLayout.class);
        HorizontalLayout toLayout = (HorizontalLayout) UiUtils.getLayout(HorizontalLayout.class);
        VerticalLayout createdDateLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
        initializeSearchCriteria();
        loadWorkStatus();

        searchLayout.addComponent(UiUtils.makeLabel(WORKFLOW_STATUS.getConstant(),154f,Unit.PIXELS));
        searchLayout.addComponent(workflowStatus);
        searchLayout.addComponent(UiUtils.makeLabel(LEVEL_MARKET_TYPE.getConstant(),116f,Unit.PIXELS));
        searchLayout.addComponent(marketType);
        searchLayout.addComponent(UiUtils.makeLabel(BRAND.getConstant(),87f,Unit.PIXELS));
        searchLayout.addComponent(brand);
        searchLayout.addComponent(UiUtils.makeLabel(PROJECTION_NAME.getConstant(),154f,Unit.PIXELS));
        searchLayout.addComponent(projectionName);
        searchLayout.addComponent(UiUtils.makeLabel(LEVEL_CONTRACT_HOLDER.getConstant(),116f,Unit.PIXELS));
        searchLayout.addComponent(contractHolder);
        searchLayout.addComponent(UiUtils.makeLabel(NDC_NO.getConstant(),87f,Unit.PIXELS));
        searchLayout.addComponent(ndcNo);
        searchLayout.addComponent(UiUtils.makeLabel(PROJECTION_DESCRIPTION.getConstant(),165f,Unit.PIXELS));
        searchLayout.addComponent(projectionDescription);
        searchLayout.addComponent(UiUtils.makeLabel(CONTRACT.getConstant(),116f,Unit.PIXELS));
        searchLayout.addComponent(contract);
        searchLayout.addComponent(UiUtils.makeLabel(NDC_NAME.getConstant(),87f,Unit.PIXELS));
        searchLayout.addComponent(ndcName);
        fromLayout.addComponent(UiUtils.makeLabel(FROM.getConstant(),35f,Unit.PIXELS));
        fromLayout.addComponent(createdDateFrom);
        toLayout.addComponent(UiUtils.makeLabel(TO.getConstant(),35f,Unit.PIXELS));
    ...
    private VerticalLayout addResultsSection() {
        VerticalLayout resultSectionLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
    ...
    private VerticalLayout addSelectedProjections() {
        VerticalLayout selectedProjectionsLayout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
        ...
        selectedProjection.setPageLength(7);
        ...
        search.addClickListener(new Button.ClickListener() {
            public void buttonClick(Button.ClickEvent event) {
                ...
                try {
                    ...
                    if (workflowStatus.getValue() != null && !Constant.NULL.equalsIgnoreCase(workFlowState) && !StringUtils.EMPTY.equalsIgnoreCase(workFlowState)) {
                    ...
                    for (int j = 0; j < selectedResultsBean.size(); j++) {
                        ...
                        String resultString = logic.getComparisonSearch(
                                workFlowState, marketTypeValue, brandValue, projecName,
                                chValue, ndcNoValue, ndcNameValue, projDesc, contractVal, fromDate, toDate, notSearchProjId);
    ...
    protected Button getResetCriteriaButton(final Component... components) {
        ...
        reset.addClickListener(new Button.ClickListener() {
            public void buttonClick(Button.ClickEvent event) {
                MessageBox.showPlain(Icon.QUESTION, "Confirm Reset", "Are you sure you want to reset the page to default values?"
                    ...
                    public void buttonClicked(final ButtonId buttonId) {
                        if (buttonId.name().equals(Constant.YES)) {
                            UiUtils.componentResetLogic(workflowStatus, marketType, brand, projectionName, contractHolder, ndcNo, ndcName, projectionDescription, contract, createdDateFrom, createdDateTo);
                        }
                    }
    ...
    protected void removeItemsButtonClick(final Button.ClickEvent event) {
        ...
        final java.util.Set<ComparisonLookupDTO> itemMasterDetailsList = (java.util.Set<ComparisonLookupDTO>) selectedProjection.getValue();
