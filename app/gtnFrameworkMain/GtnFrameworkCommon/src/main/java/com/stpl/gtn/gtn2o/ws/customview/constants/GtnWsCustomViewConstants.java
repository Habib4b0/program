/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.stpl.gtn.gtn2o.ws.customview.constants;

/**
 *
 * @author Lokeshwari.Kumarasam
 */
public class GtnWsCustomViewConstants {
    private GtnWsCustomViewConstants() {
        super();
    }
    public static final String GTN_CUSTOM_VIEW_SERVICE = "/GtnCustomViewService";
    public static final String GET_CUSTOM_VIEW_TABLE_DATA = "/getCustomViewTableData";
    public static final String GET_DEDUCTION_HIERARCHY_TABLE_DATA = "/getDeductionHierarchyTableData";
    public static final String CHECK_CUSTOM_VIEW_SAVE = "/getCustomViewSaveData";
    public static final String CUSTOM_VIEW_SAVE_LOGIC = "/getCustomViewSaveLogic";
    public static final String CUSTOM_VIEW_GET_TREE_DATA = "/getCustomViewGetTreeData";
}
