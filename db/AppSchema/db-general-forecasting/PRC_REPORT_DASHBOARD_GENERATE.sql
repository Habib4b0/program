...
ELSE
  CREATE TABLE #PROJECTION_MASTER
    (
       ID                    INT IDENTITY(1, 1),
       PROJECTION_MASTER_SID INT,type bit
    )

SET @SQL=CONCAT('
INSERT INTO #PROJECTION_MASTER(PROJECTION_MASTER_SID,[type])

SELECT 0 AS PROJECTION_MASTER_SID ,0
UNION 
SELECT DISTINCT PROJECTION_MASTER_SID,[type] FROM ',@PRIOR_TABLE,'')
EXEC SP_EXECUTESQL @SQL
...
HIERARCHY_NO VARCHAR(8000),
RS_CONTRACT_SID INT,cff_master_Sid int
--,BUSINESS_UNIT INT
PRIMARY KEY (
PROJECTION_MASTER_SID,
...
pd.PROJECTION_DETAILS_SID,cfd.CFF_MASTER_SID
FROM #PROJECTION_MASTER A
join CFF_DETAILS cfd on a.PROJECTION_MASTER_SID=cfd.CFF_MASTER_SID
AND ID <> 1 and A.type=1
INNER JOIN PROJECTION_DETAILS PD
ON cfd.PROJECTION_MASTER_SID = PD.PROJECTION_MASTER_SID
and PD.CCP_DETAILS_SID = cfd.CCP_DETAILS_SID
INNER JOIN CCP_DETAILS CD
ON PD.CCP_DETAILS_SID = CD.CCP_DETAILS_SID and CD.CCP_DETAILS_SID = cfd.CCP_DETAILS_SID 
JOIN  #CCP_DETAILS_TEMP TC
ON cfd.CCP_DETAILS_SID = TC.CCP_DETAILS_SID
WHERE LEVEL_NO BETWEEN @CUSTOM_LEVEL_NO-1 AND @CUSTOM_LEVEL_NO+1
...
FROM (
SELECT HIERARCHY_NO,
CFF_MASTER_SID PROJECTION_MASTER_SID,
A.CCP_DETAILS_SID,
ITEM_MASTER_SID,
SALES_INCLUSION--,a.CFF_MASTER_SID
...
where pm.PROJECTION_MASTER_SID<>0
')
...
FROM (
SELECT HIERARCHY_NO,
cff_master_Sid as PROJECTION_MASTER_SID,
ITEM_MASTER_SID
FROM #PRIOR_TEMP_CCP 
GROUP BY HIERARCHY_NO,
cff_master_Sid,
...
FROM (SELECT HIERARCHY_NO,
		...
		A.RS_CONTRACT_SID,
		 DEDUCTION_INCLUSION
	FROM #PRIOR_TEMP_CCP A
	join ',@CUSTOM_TABLE_DISCOUNT,' d on d.CCP_DETAILS_SID=a.CCP_DETAILS_SID
	and d.RS_CONTRACT_SID=a.RS_CONTRACT_SID
	GROUP BY HIERARCHY_NO,
		cff_master_Sid,
		A.CCP_DETAILS_SID,
		A.RS_CONTRACT_SID,DEDUCTION_INCLUSION
) A
cross join (select distinct period ,YEAR from #PERIOD 
where PERIOD_SID BETWEEN @START_PERIOD_SID AND @END_PERIOD_SID) P
...
COALESCE(PROJECTION_ACCURAL_DISCOUNT,ACTUAL_ACCURAL_DISCOUNT) ACCURAL_DISCOUNT
FROM #DATA_TABLE A
JOIN ( SELECT HIERARCHY_NO,cff_master_Sid PROJECTION_MASTER_SID FROM #PRIOR_TEMP_CCP GROUP BY HIERARCHY_NO,cff_master_Sid )E ON A.HIERARCHY_NO=E.HIERARCHY_NO
