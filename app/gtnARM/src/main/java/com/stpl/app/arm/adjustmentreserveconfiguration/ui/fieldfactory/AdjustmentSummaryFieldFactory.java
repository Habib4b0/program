...
import com.stpl.app.arm.adjustmentreserveconfiguration.logic.ComboBoxLoadLogic;
...
public class AdjustmentSummaryFieldFactory implements TableFieldFactory {
     ...
     * The Constant LOGGER.
     */
    private static final Logger LOGGER = Logger.getLogger(AdjustmentSummaryFieldFactory.class);

    AdjustmentSummaryConfigLogic logic = AdjustmentSummaryConfigLogic.getInstance();
    ComboBoxLoadLogic comboLogic = new ComboBoxLoadLogic();
                ...
                try {
                    ...
                    if (ARMUtils.getAdjustmentSummaryAccountMap().containsKey(propertyId)) {
                        ...
                        if (selection.isIsViewMode()) {
                            custom.setReadOnly(true);
                        } else {
                            custom.setReadOnly(false);
                        }
                    }
                } catch (Exception ex) {
    ...
    CustomMenuBar.SubMenuCloseListener submenuCloseListener = new CustomMenuBar.SubMenuCloseListener() {
        @Override
