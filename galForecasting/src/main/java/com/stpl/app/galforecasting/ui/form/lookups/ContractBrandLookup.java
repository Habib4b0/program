...
public class ContractBrandLookup extends Window {
     ...
     * Container for contractResults table.
     */
    private BeanItemContainer<ContractBrandDTO> nmBrandContainer;
    ...
    public ContractBrandLookup(final String windowName, final SessionDTO sessionDTO, final String type, final String hierarchyNo, String screenName) {
        ...
        LOGGER.info("ContractBrandLookup Constructor initiated ");
    ...
    private void configureFields() {
        LOGGER.info("Entering Configure Fields");
        try {
            ...
            contractResultsTable.setPageLength(5);
            brandResultsTable.setPageLength(5);
            contractResultsTable.setSizeFull();
            brandResultsTable.setSizeFull();

            if (Constant.CONTRACT_SMALL.equalsIgnoreCase(lookupType)) {
                LOGGER.info("Entering the Configuration For Contract");
                ...
                LOGGER.info("Ending the Configuration For Contract");
            } else {
                LOGGER.info("Entering the Configuration For Contract");
    ...
    protected void nonmandatedContractSearchLogic() {
        ...
        if (StringUtils.isEmpty(getContractHolder().getValue()) && StringUtils.isEmpty(getCustomerId().getValue())) {
            com.stpl.app.galforecasting.utils.AbstractNotificationUtils.getErrorNotification("No Search Value Entered", "There is no search value entered. Please enter a search value and try again.");
            ...
            try {
                ...
                if (tpResult.isEmpty()) {
                    com.stpl.app.galforecasting.utils.AbstractNotificationUtils.getInfoNotification("No Results", "There were no results found that match the entered search criteria. \nPlease try again.");
     ...
     * @throws Exception
     */
    public void mandatedContractSearchLogic() throws Exception {
        LOGGER.info("Entering the Mandated Contract Search Logic");
        ...
        LOGGER.info("Ending the Mandated Contract Search Logic");
    }
     ...
     * @throws Exception
     */
    public void loadCustomerDDLB(ComboBox customerDDLB) throws SystemException, Exception {
     ...
     * @throws Exception
     */
    public void brandSearchLogic() throws Exception {
        LOGGER.info("Entering the Brand Search Logic");
        ...
        LOGGER.info("Ending the Brand Search Logic");
    }
    ...
    public void importButtonLogic() throws SQLException, SystemException {
        LOGGER.info("Entering Import Logic Button Listener");
        ...
        if (screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_MANDATED)) {
            ...
            if (selected != null) {
                ...
                if (contrtactDto != null) {
                    inputs[2] = contrtactDto.getContractSid();

                }
                inputs[4] = session.getProjectionId();
                inputs[5] = session.getSessionId();
                inputs[6] = session.getUserId();
                logic.callAlternateHistoryProcedure(inputs);
                close();
                contractBrandDTO = contrtactDto;
            } else {
                com.stpl.app.galforecasting.utils.AbstractNotificationUtils.getErrorNotification("Error", "There are no selected results.\n Please select a record and try again.");
            }
        }
    ...
    private void configureBasedOnScreenName() {
        LOGGER.info("Entering Configure Based On ScreenName");
        ...
        LOGGER.info("Ending Configure Based On ScreenName");
    }
