...
public class CommonQueryUtils {
     ...
     * @return 
     */
    public String getCCPSQLQuery(ProjectionSelectionDTO projSelDTO) {
       String ccpQuery = SQlUtil.getQuery("getCCPTempTableQuery");
        if (projSelDTO.isIsCustomHierarchy()) {
            ccpQuery += getCustomCCPQuery(projSelDTO);
        } else {
            ccpQuery += getGeneralCCPQuery(projSelDTO);
    ...
    public String getCustomCCPQuery(ProjectionSelectionDTO projSelDTO) {
        ...
        if (!(projSelDTO.isIsCustomHierarchy() && "P".equalsIgnoreCase(projSelDTO.getCustomViewIndicator()))) {
        ccpQuery += "   JOIN "
        ...
        if (!(projSelDTO.isIsCustomHierarchy() && "C".equalsIgnoreCase(projSelDTO.getCustomViewIndicator()))) {
        ccpQuery += "  JOIN "
                + " (SELECT RLD2.HIERARCHY_NO,RLD2.RELATIONSHIP_LEVEL_SID, CVD.LEVEL_NO FROM dbo.CUSTOM_VIEW_DETAILS CVD \n"
                + " JOIN dbo.HIERARCHY_LEVEL_DEFINITION HLD ON CVD.HIERARCHY_ID=HLD.HIERARCHY_LEVEL_DEFINITION_SID \n"
        ...
        ccpQuery += " JOIN PROJECTION_DETAILS PD ON PD.CCP_DETAILS_SID = CCPMAPC.CCP_DETAILS_SID AND\n"
    ...
    public String getGeneralCCPQuery(ProjectionSelectionDTO projSelDTO) {
        ...
        String ccpQuery = "SELECT  HLD.RELATIONSHIP_LEVEL_SID,CCPMAP.PROJECTION_DETAILS_SID,CCPMAP.CCP_DETAILS_SID, HLD.HIERARCHY_NO from\n"
                            ...
                            + " AND PD.PROJECTION_MASTER_SID=" + projSelDTO.getProjectionId() + " ) CCPMAP join\n"
                            ...
                            + "  WHERE RLD1.HIERARCHY_NO like '"+projSelDTO.getHierarchyNo()+"%' AND RLD1.LEVEL_NO=" + projSelDTO.getTreeLevelNo() + " ) HLD\n"
                            + "\n"
