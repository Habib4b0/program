...
AS (
	...
	FROM (
		SELECT dt.HIERARCHY_NO
			,dt.PERIOD
	        ,dt.YEAR
			,IIF(@SALES_INCLUSION IS NULL , ISNULL(ACTUAL_SALES,0), NULL) ACTUAL_SALES
			,IIF(@SALES_INCLUSION IS NULL , ISNULL(ACTUAL_UNITS,0), NULL)   ACTUAL_UNITS
			,IIF(@SALES_INCLUSION IS NULL , ISNULL(PROJECTION_SALES,0), NULL) PROJECTION_SALES
			,IIF(@SALES_INCLUSION IS NULL , ISNULL(PROJECTION_UNITS,0), NULL) PROJECTION_UNITS
			--,DT.ITEM_MASTER_SID
