...
public class SearchLogic {
   ...
   public List fetchDataFromDB(Object input[], int start, int end, boolean isCount, Set<Container.Filter> filterSet, List<SortByColumn> sortByList,boolean isExcelExport) {
       query = Constants.EMPTY;
       String tableName = String.valueOf(input[NumericConstants.TWO]).replaceAll(Constants.SPACE, Constants.UNDERSCORE);
       String typeQuery = tableName.equals(Constants.INDEX_FRAGMENTATION_STATISTICS)
               ? Constants.EMPTY : " AND TYPE='" + input[NumericConstants.ZERO] + "' ";
       String dateCondition = String.format(!DateUtils.isSameDay((Date) input[NumericConstants.THREE], new Date())
               ? "FORMAT( INSERTED_DATE, 'yyyy-MM-dd HH' )+':00:00' =   '%s'" : "INSERTED_DATE = '%s'", input[NumericConstants.ONE]);
       String dbCondtion = String.valueOf(input[NumericConstants.FOUR]).isEmpty() || tableName.equals(Constants.INDEX_FRAGMENTATION_STATISTICS)
               ? Constants.EMPTY : String.format("AND DATABASE_NAME LIKE '%s'", String.valueOf(input[NumericConstants.FOUR]));
       if (isCount) {
   ...
   public List fetchDataFromDBForJOb(Object input[], int start, int end, boolean isCount, Set<Container.Filter> filterSet, List<SortByColumn> sortByList,boolean isExcelExport ) {
    ...
    public List<Object[]> fetchCPUIODataFromDB(String type, Date selecteDate) {
        ...
        String dateCondition = String.format(!DateUtils.isSameDay(selecteDate, new Date()) ? "FORMAT( INSERTED_DATE, 'yyyy-MM-dd HH' )+':00:00' =   '%s'" : "INSERTED_DATE = '%s'", sqlDate);
        query = String.format("select ISNULL(DATABASE_NAME,'RESOURCE DATABASE'),\"VALUE\" from  dbo.CPU_IO_USAGE_PER_DB where %s and \"TYPE\"='%s'  ORDER BY VALUE desc", dateCondition, type);
        List<Object[]> resultList = HelperTableLocalServiceUtil.executeSelectQuery(query);
        return resultList;
    }
    ...
    Button createButtonLink(final String text_query) {
        link = new Button();
        link.setCaption(text_query.length() < NumericConstants.FIFTY ? text_query : text_query.substring(0, NumericConstants.FORTY_NINE) + Constants.Continous_Dots);
    ...
    private String getSortByQuery(List<SortByColumn> sortByList, String tableName) {
        if (sortByList!=null && !sortByList.isEmpty()) {
            return String.format("ORDER BY %s %s ", sortByList.get(NumericConstants.ZERO).getName(),  sortByList.get(NumericConstants.ZERO).getType());
        }
