...
IF NOT EXISTS (SELECT 1
               FROM   INFORMATION_SCHEMA.COLUMNS
               WHERE  TABLE_NAME = 'CFF_MASTER'
                      AND COLUMN_NAME = 'CFF_ELIGIBLE_DATE'
                      AND TABLE_SCHEMA = 'DBO')
  BEGIN
      ALTER TABLE CFF_MASTER
        ADD  CFF_ELIGIBLE_DATE DATETIME 
  END
...
IF EXISTS (
              ...
              WHERE CUSTOMER_HIER_VERSION_NO IS NULL
              )
BEGIN
       UPDATE PM
       SET CUSTOMER_HIER_VERSION_NO = RB.HIERARCHY_VERSION
       FROM PROJECTION_MASTER PM
       JOIN RELATIONSHIP_BUILDER RB ON RB.RELATIONSHIP_BUILDER_SID = PM.CUST_RELATIONSHIP_BUILDER_SID
END
GO
...
IF EXISTS (
              SELECT 1
              FROM PROJECTION_MASTER
              WHERE PRODUCT_HIER_VERSION_NO IS NULL
              )
BEGIN
       UPDATE PM
       SET PRODUCT_HIER_VERSION_NO = RB.HIERARCHY_VERSION
       FROM PROJECTION_MASTER PM
       JOIN RELATIONSHIP_BUILDER RB ON RB.RELATIONSHIP_BUILDER_SID = PM.PROD_RELATIONSHIP_BUILDER_SID
END
GO
...
IF  EXISTS (SELECT 1
FROM SYS.STATS
WHERE OBJECT_NAME(OBJECT_ID) = 'CFF_DETAILS'
	AND AUTO_CREATED = 0
   AND NAME='SALES_INCLUSION')
  BEGIN
      DROP STATISTICS CFF_DETAILS.SALES_INCLUSION
  END
GO
...
IF  EXISTS (SELECT 1
  ...
  BEGIN
      ALTER TABLE CFF_DETAILS
        ALTER COLUMN SALES_INCLUSION VARCHAR(10)
  END
GO
...
IF  EXISTS (SELECT 1
FROM SYS.STATS
WHERE OBJECT_NAME(OBJECT_ID) = 'CFF_DEDUCTION_INCLUSION'
	AND AUTO_CREATED = 0
   AND NAME='DEDUCTION_INCLUSION')
  BEGIN
      DROP STATISTICS CFF_DEDUCTION_INCLUSION.DEDUCTION_INCLUSION
  END
GO
...
IF  EXISTS (SELECT 1
  ...
  BEGIN
      ALTER TABLE CFF_DEDUCTION_INCLUSION
        ALTER COLUMN DEDUCTION_INCLUSION VARCHAR(10)
  END
GO
