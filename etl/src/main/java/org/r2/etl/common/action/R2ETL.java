...
public class R2ETL {
    ...
    public void runJob(final String filename) throws BPIETLException {
        ...
        try {
            ...
            final String path = logpath + FilenameUtils.getName("") + FilePathUtil.DATABASE_CONFIGURATION_FILE_NAME;
            final String procedurePath = logpath + FilePathUtil.PROCEDURE_CONFIGURATION_FILE_NAME;
            File file1 = new File(logpath, FilenameUtils.getName(FilePathUtil.DATABASE_CONFIGURATION_FILE_NAME));
            if (file1.exists()) {
                dbResouce.load(new FileInputStream(file1));
            ...
            final String bpiPassword = Encr.decryptPassword(dbResouce.getProperty("BPI_PASSWORD"));
            final String bpipassphrase = Encr.decryptPassword(dbResouce.getProperty("BPI_PASSPHRASE"));
            final String stagingPassword = Encr.decryptPassword(dbResouce.getProperty("STAGING_PASSWORD"));
            final String sourcePassword = Encr.decryptPassword(dbResouce.getProperty("BPI_SOURCE_PASSWORD"));

            final String sysPassword = Encr.decryptPassword(dbResouce.getProperty(BPI_SYS_PSW));
            ...
            if (index != -1) {
                ...
                fileArr[1] = fileNameExtension[0].substring(index + 1, fileNameExtension[0].length());
            } else {
            ...
            FilePathUtil.setLatestServerName(serverName.split("/")[2]);
            LOGGER.info("latestServerName = " + FilePathUtil.getLatestServerName());
            ...
            job.setVariable("VIEW_OFFSET", dbResouce.getProperty("VIEW_OFFSET"));
            ...
            if (job.getErrors() > ERRORCOUNT) {
                ...
                LOGGER.info(fileNameExtension[1].toLowerCase(Locale.ENGLISH) + " executed successfully ");
