...
public class SummaryTableLogic extends PageTreeTableLogic {
    ...
    public Object configureContainer(Object object, Container container) {
        ...
        ((CustomTreeContainer<AbstractSummaryDTO>) container).addBean(itemId);
        if (itemId != null && itemId.getParent() == 1) {
            ((CustomTreeContainer<AbstractSummaryDTO>) container).setChildrenAllowed(itemId, true);
        } else {
            ((CustomTreeContainer<AbstractSummaryDTO>) container).setChildrenAllowed(itemId, false);
        }
        return itemId;
    }
    ...
    public Map<Integer, Object> loadData(int start, int offset) {
        ...
        if (isSales) {
            list = logic.getConfiguredSalesTabResults(getLastParent(), start, offset, tabSelectionDTO, false);
        } else {
            list = logic.getConfiguredRebateTabResults(getLastParent(), start, offset, tabSelectionDTO, false);
        }
        int i = start;
