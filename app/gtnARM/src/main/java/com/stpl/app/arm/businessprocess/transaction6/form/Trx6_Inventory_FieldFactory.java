 ...
 * @author Srithar.Raju
 */
public class Trx6_Inventory_FieldFactory implements TableFieldFactory {

    private final Trx6_InventoryLogic logic;
    ...
    DataFormatConverter CUR_THREE = new DataFormatConverter("#,##0.00", "");
    DataFormatConverter CUR_THREE_DOLLAR = new DataFormatConverter("#,##0.00", DataFormatConverter.INDICATOR_DOLLAR);


    public Trx6_Inventory_FieldFactory(Trx6_InventoryLogic logic,AbstractSelectionDTO selection) {
        this.logic = logic;
    ...
    public Field<?> createField(Container container, Object itemId, Object propertyId, Component uiContext) {
        ...
        if (!dto.getChildrenAllowed() && dto.getLevelNo() == NumericConstants.THREE && (propertyId.toString().contains(ARMUtils.Trx6_Variables.BASELINE_PRICE_OVERRIDE.getColumn()) 
            ...
            if(propertyId.toString().contains(ARMUtils.Trx6_Variables.ADJUSTED_PRICE_OVERRIDE.getColumn()) || propertyId.toString().contains(ARMUtils.Trx6_Variables.BASELINE_PRICE_OVERRIDE.getColumn())){
              priceoverride.setConverter(CUR_THREE_DOLLAR);
        ...
        public void valueChange(Property.ValueChangeEvent event) {
            try {
                ...
                if (propertyId.contains(ARMUtils.Trx6_Variables.BASELINE_PRICE_OVERRIDE.getColumn())) {
                    input.add(ARMUtils.getDB_ColumnMap_For_Inventoy_Trx6().get(ARMUtils.Trx6_Variables.BASELINE_PRICE_OVERRIDE.getColumn()));
                } else if (propertyId.contains(ARMUtils.Trx6_Variables.ADJUSTED_PRICE_OVERRIDE.getColumn())) {
                    input.add(ARMUtils.getDB_ColumnMap_For_Inventoy_Trx6().get(ARMUtils.Trx6_Variables.ADJUSTED_PRICE_OVERRIDE.getColumn()));
                } else if (propertyId.contains(ARMUtils.Trx6_Variables.INFLATION_FACTOR.getColumn())) {
                    input.add(ARMUtils.getDB_ColumnMap_For_Inventoy_Trx6().get(ARMUtils.Trx6_Variables.INFLATION_FACTOR.getColumn()));
                }
