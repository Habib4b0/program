...
public class ContractInformation extends CustomComponent {
    ...
    public ContractInformation(final CustomFieldGroup binder , final String mode) throws PortalException, SystemException{
	...
	try{
        ...
        priceEscalationClause.addValidator(new StringLengthValidator("Price Escalation Clause should be less than 50 characters",Constants.ZERO, NumericConstants.FIFTY, true));
    ...
    public class DateValidatorProposed extends AbstractValidator {
        ...
        protected boolean isValidValue(final Object value) {
            ...
            if (proposedStartDate.getValue() != null && proposedEndDate.getValue() != null) {
                final boolean validValue = proposedStartDate.getValue().compareTo(proposedEndDate.getValue()) <= Constants.ZERO;
                LOGGER.debug("isValidValue method ends with validValue=" + validValue);
                return validValue;
            }
            LOGGER.debug("isValidValue method ends with validValue == true");
            return true;
        }
    ...
    public class DateValidatorOriginal extends AbstractValidator {
        ...
        protected boolean isValidValue(final Object value) {
            LOGGER.debug("Entering isValidValue method ");
            if (originalStartDate.getValue() != null && originalEndDate.getValue() != null) {
                final boolean isvalueValid = originalStartDate.getValue().compareTo(originalEndDate.getValue()) <= Constants.ZERO;
                LOGGER.debug("isValidValue method ends with isvalueValid= " + isvalueValid);
                return isvalueValid;
            }
