		...
		if (profileObj instanceof WorkflowProfile) {
			...
			if ("RELATIONSHIP_BUILDER_OUTBOUND".equalsIgnoreCase(profile.getProcessName())
				...
				try {
					...
					arpProcess.lock();
					new ProcessSchedulerLogic().schedulerInsertForArp(ConstantsUtils.NUMBER_ONE,
							ConstantsUtils.NUMBER_ONE);
					logic.runJob(getFtpBundleValue(), profile.getScriptName());
					logic.updateLastRun(profile.getProcessSid(), true);
					arpProcess.unlock();
				} catch (Exception ex) {
			...
			} else if (ConstantsUtils.ADJ_RESERVER_DETAIL_INTERFACE.equals(profile.getProcessName())
					|| ConstantsUtils.ADJ_GTN_DETAIL_INTERFACE.equals(profile.getProcessName())) {
				try {
					ArmSchedulerSynchronizer adjOutboundProcess = ArmSchedulerSynchronizer
							.getInstance(AUTOMATIC_SCHEDULER);
					adjOutboundProcess.lock();
					new ProcessSchedulerLogic().updateCheckRecord(profile.getProcessName(), ConstantsUtils.NUMBER_ONE,
							ConstantsUtils.NUMBER_ONE);
					logic.runJob(getFtpBundleValue(), profile.getScriptName());
					logic.updateLastRun(profile.getProcessSid(), true);
