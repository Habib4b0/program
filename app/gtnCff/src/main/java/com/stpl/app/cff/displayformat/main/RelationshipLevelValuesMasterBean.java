...
public class RelationshipLevelValuesMasterBean {
    ...
    List<RelationshipLevelValuesBean> queryList = new ArrayList<>();
    ...
    public RelationshipLevelValuesMasterBean(List<Object[]> tempList, String relationshipBuilderSid, String hierarchyNoType) {
    ...
    public String getFinalQuery() {
        for (int i = 0; i < queryList.size(); i++) {
            ...
            finalQry.append(query.getQuery().replace(DEFAULT_QUESTION, generateDefaultSelect(query.getNoOfSelectFormed())));
        }
    ...
    private RelationshipLevelValuesBean getCustomisedQuery(Object[] tempListObject) {
        ...
        customSql = customSql.replace("?DISPLAYFORMATCOLUMN", getDisplayFormatColumn(masterBean.getDisplayFormatList(), String.valueOf(tempListObject[0]), bean));
    ...
    private RelationshipLevelValuesBean getDeductionCustomisedQuery(Object[] tempListObject) {
        ...
        if (isUDC) {
            customSql = customSql.replace(FIELD_VALUE, " RS.RS_CONTRACT_SID=U.MASTER_SID AND U.MASTER_TYPE='RS_CONTRACT' ");
        } else if (isRSID) {
        ...
        if (isUDC) {
            customSql = customSql.replace("?UDCJOIN", " JOIN UDCS U ON U." + tempListObject[NumericConstants.ONE] + "=RLD.RELATIONSHIP_LEVEL_VALUES ");
            customSql = customSql.replace("?HELPERTABLEJOIN", " JOIN HELPER_TABLE HT ON HT.HELPER_TABLE_SID=U." + tempListObject[NumericConstants.ONE]);
            ...
            customSql = customSql.replace("?REBATEJOIN", isHelperTableJoin ? " JOIN HELPER_TABLE HT ON HT.HELPER_TABLE_SID=RS." + tempListObject[NumericConstants.ONE] : StringUtils.EMPTY);
            customSql = customSql.replace("?HELPERTABLEJOIN", StringUtils.EMPTY);
            customSql = customSql.replace("?UDCJOIN", StringUtils.EMPTY);
            customSql = customSql.replace("?HTDESCRIPTION", isHelperTableJoin ? ConstantsUtil.HT_DESCRIPTION : "RS.RS_ID");
            customSql = customSql.replace("?DEDGROUPBY", isHelperTableJoin ? ConstantsUtil.HT_DESCRIPTION : "RS.RS_ID");
        }
        customSql = customSql.replace("?DISPLAYFORMATCOLUMN", getDisplayFormatColumnRS(masterBean.getDisplayFormatList(), String.valueOf(tempListObject[1]), bean));
