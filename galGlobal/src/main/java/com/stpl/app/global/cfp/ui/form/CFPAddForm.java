...
public final class CFPAddForm extends StplCustomComponent {
    ...
    public void init() throws SystemException, PortalException {
        LOGGER.info("Entering CFPAddForm init");
    ...
    public void configureFields() {
        LOGGER.info("Entering inside ConfigureFields");
                ...
                try {
                    ...
                    if (companyIdList.size() > 0) {
                        ...
                        if (companyIdList.size() > 1) {
                            ...
                            binder.getErrorDisplay().setError("Start Date required for Company No " + companyNo + " in Companies tab");
                            return;
                        }
                    }
                    ...
                    if (companyIdList.size() > 0) {
                        ...
                        if (companyIdList.size() > 1) {
                            ...
                            binder.getErrorDisplay().setError("Status required for Company No " + companyNo + " in Companies tab");
                            return;
                        }
                    }
                    ...
                    if (companyIdList.size() > 0) {
                        ...
                        if (companyIdList.size() > 1) {
                            ...
                            binder.getErrorDisplay().setError("CFP Start Date is less than CFP End date for Company No " + companyNo + " in Companies tab");
                            return;
                        }
                    }
                    ...
                    if (companyIdList.size() > 0) {
                        ...
                        if (companyIdList.size() > 1) {
                            ...
                            binder.getErrorDisplay().setError("Please enter different Start Date.Since selected Trading Partner and Start Date Combination already exists for Company No " + companyNo + " in Companies tab");
                            return;
                        }
                    }
    ...
    private void backButton() {
        ...
        backBtn.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                if (sessionDTO.getMode().equals(ConstantsUtils.VIEW_BTN)) {
                    ...
                    MessageBox.showPlain(Icon.QUESTION, commonutil.getHeaderMessage(), commonutil.getBackMessage(), new MessageBoxListener() {
                        ...
                        public void buttonClicked(ButtonId buttonId) {
                            if (buttonId.name().equals(ConstantsUtils.YES)) {
                                LOGGER.info("Entering CFPAddActionButtonLayout  BACK  method ");
                                try {
                                    CommonLazyUtilDTO lazyUtilDTO = new CommonLazyUtilDTO();
                                    lazyUtilDTO.setUserId(String.valueOf(VaadinSession.getCurrent().getAttribute(ConstantsUtils.USER_ID)));
                                    lazyUtilDTO.setSessionId(sessionDTO.getUiSessionId());
                ...
                LOGGER.info("Ending CFPAddActionButtonLayout BACK  method ");
            }
        });
    ...
    private void deleteButton() {
        ...
        deleteBtn.addClickListener(new ClickListener() {
            ...
            public void buttonClick(final ClickEvent event) {
                ...
                binder.getFields();
                LOGGER.info("Entering CFPEditActionButtonLayout DELETE  method ");
                ...
                try {
                    ...
                    cfpDynamicQuery.add(RestrictionsFactoryUtil.eq("parentCfpId",
                            cfpId));
                    cfpDynamicQuery.add(RestrictionsFactoryUtil.ne("inboundStatus", "D"));
                    ...
                    if (!cfpDetailsList.isEmpty()) {
                        final MessageBox msg = MessageBox.showPlain(Icon.WARN, "Cannot Delete", "Company Family Plan cannot be deleted, associated as parent to another Company Family Plan", new MessageBoxListener() {
                            /**
                             * The method is triggered when a button of the message box is
                             * pressed .
                             *
                    ...
                    if (cfpDetailsList.isEmpty()) {                        
                        ...
                        List<CfpContract> cfpContractList = new ArrayList<>();
                            ...
                            if (cfpContractList != null && !cfpContractList.isEmpty() && cfpContractList.size() > 0) {
                                final MessageBox msg = MessageBox.showPlain(Icon.WARN, "Cannot Delete", "Company Family Plan cannot be deleted as it is associated with Contract", new MessageBoxListener() {
                    ...
                    LOGGER.info("Ending CFPEditActionButtonLayout DELETE  method ");
                } catch (SystemException ex) {
                    final String errorMsg = ErrorCodeUtil.getErrorMessage(ex);
                    LOGGER.error(errorMsg);
                    final MessageBox msg = MessageBox.showPlain(Icon.WARN, ErrorCodeUtil.getEC(ErrorCodes.ERROR_CODE_1001), errorMsg, new MessageBoxListener() {
