...
public class MSupplementalDiscountProjection extends ForecastDiscountProjection {
    ...
    private CustomTreeContainer<DiscountProjectionDTO> excelResultBean = new CustomTreeContainer<DiscountProjectionDTO>(DiscountProjectionDTO.class);
    /**
    ...
    public MSupplementalDiscountProjection(SessionDTO session, String screenName) throws Exception {
    ...
    public void configureFieldsForSupplemental() {
        ...
        value.addValidator(new StringLengthValidator("Value should be less than 38 characters", 0, 38, true));
    ...
    public void generateBtnClickLogic(Boolean isGenerate) {
        LOGGER.info("Generate button click listener started");
        firstGenerated = true;
    ...
    public boolean isProjectionSelection() {
        ...
        if (selected) {
            ...
            projectionDTO.setHistoryNum(12);
    ...
    private void configureResultTable() {
        ...
        resultBeanContainer = new CustomTreeContainer<DiscountProjectionDTO>(DiscountProjectionDTO.class);
        tableLogic.setTreeNodeMultiClick(false);
        ...
        if (rightTable.getVisibleColumns().equals("Methodology")) {
            rightTable.setColumnAlignment("Methodology", ExtCustomTable.Align.LEFT);
        }
        ...
        leftTable.setFilterFieldVisible(Constant.GROUP, true);
        ...
        rightTable.setDoubleHeaderColumnWidth("actualdiscount", 160);
        rightTable.setColumnWidth("actualdiscount", 160);

        rightTable.setDoubleHeaderColumnWidth("contractEndDate", 160);
        rightTable.setColumnWidth("contractEndDate", 160);
        leftTable.setSortEnabled(false);
                ...
                if (String.valueOf(propertyId).equals(Constant.CHECK)) {
                    ...
                    check.addClickListener(new ExtCustomCheckBox.ClickListener() {
                        public void click(ExtCustomCheckBox.ClickEvent event) {
                            periodTableId.getLeftFreezeAsTable().setRefresh(false);
                            ...
                            queryList = supplementalDiscountProjectionLogic.getUpdateRecord(itemDto.getParentProjectionDetailIdList(), session.getUserId(), session.getSessionId());
                            checkCustomerFlag = queryList.size() == itemDto.getParentProjectionDetailIdList().size();
                            ...
                            if (itemDto.getSupplementalLevelNo() == 5) {
                                if (queryList != null && queryList.size() > 0) {
                                    ...
                                    for (Object queryList1 : queryList) {
                                        ...
                                        if (ob[1] != null && itemDto.getProjectionDetailIdList().contains(ob[1].toString())) {
                                            ndcCount++;
                                        }
                                    }
                                    checkBrandFlag = ndcCount == itemDto.getProjectionDetailIdList().size();
                                }
                                if (checkCustomerFlag) {
                                    ...
                                    for (Map.Entry<String, Object> entry : mapList.entrySet()) {
                                        ...
                                        if (!itemDtoExpanded.getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                                                && !itemDtoExpanded.getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                                            if (itemDto.getProjectionDetailIdList().containsAll(itemDtoExpanded.getProjectionDetailIdList())) {
                                ...
                                if (itemDto.getSupplementalLevelNo() == 4) {
                                    if (checkCustomerFlag) {
                                        getCustomerLevelCheckUpdateForBrandChange(tableLogic, check.getValue(), itemDto);
                            ...
                            try {
                                leftTable.setColumnCheckBox(Constant.CHECK, true, listStr.size() != 1 ? false : (listStr.get(0) == 1));
                                periodTableId.getLeftFreezeAsTable().setRefresh(false);
                            } catch (Exception e) {
                                LOGGER.error(e);
                            }
                    ...
                    } else if (getBeanFromId(itemId).getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                            || getBeanFromId(itemId).getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                        check.setValue(false);
                        check.setEnabled(false);
                    }
                    return check;
            ...
            public Field<?> createField(final Container container,
                ...
                if (!ACTION_VIEW.getConstant().equals(session.getAction()) && !String.valueOf(propertyId).contains("disable") && (String.valueOf(propertyId).contains("Access")
                    ...
                    if (String.valueOf(propertyId).contains("Access")) {
                        comboBox.setWidth(120, UNITS_PERCENTAGE);
                    ...
                    textField.addBlurListener(new FieldEvents.BlurListener() {
                        ...
                        public void blur(FieldEvents.BlurEvent event) {
                            ...
                            LOGGER.info(" group blur Value" + String.valueOf(container.getContainerProperty(itemId, propertyId).getValue()).trim());
                        }
                    });
                    ...
                    parityLookUp.addClickListener(new CustomTextField.ClickListener() {
                        ...
                        public void click(CustomTextField.ClickEvent event) {
                            ...
                            if (parityCheckFlag) {
                                session.setProjectionDetailsSid(getBeanFromId(itemId).getProjectionDetailsSid());
                    ...
                    check.setImmediate(true);
                    check.setValue(false);
                    check.setWidth(100, UNITS_PERCENTAGE);
                    check.addClickListener(new ExtCustomCheckBox.ClickListener() {
                        public void click(ExtCustomCheckBox.ClickEvent event) {
                    ...
                    formulaNo.addClickListener(new CustomTextField.ClickListener() {
                        ...
                        public void click(final CustomTextField.ClickEvent event) {
                            ...
                            try {
                                ...
                                final MethodologyLookUp formulaLookup = new MethodologyLookUp();
    ...
    public void saveFromTableField(Object itemId, Object propertyId, String value, boolean flag, FreezePagedTreeTable periodTableId) {
        ...
        dto.setPropertyName(period[2]);
        if (projectionDTO.isIsFilter()) {
            dto.setParentProjectionDetailIdList(dto.getProjectionDetailIdList());
        }
    ...
    private void getProductLevelCheckUpdate(SupplementalTableLogic tableLogic, boolean value, DiscountProjectionDTO checkDto) {
        ...
        for (Object obj : dtoList) {
            ...
            if (suppDto.getSupplementalLevelNo() == 5 && checkDto.getProjectionDetailIdList().containsAll(suppDto.getProjectionDetailIdList())) {
                if (!suppDto.getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                        && !suppDto.getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                    suppDto.addBooleanProperties(Constant.CHECK, value);
    ...
    private void getCustomerLevelCheckUpdate(SupplementalTableLogic tableLogic, boolean value, DiscountProjectionDTO checkDto) {
        ...
        for (String tableTreeLevelNo : tableLogic.getAllLevels()) {
            ...
            if (itemId != null) {
                ...
                if (!dto.getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                        && !dto.getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                    if (checkDto.getProjectionDetailIdList().containsAll(dto.getProjectionDetailIdList())) {
    ...
    private void uncheckUpperLevel(SupplementalTableLogic tableLogic, boolean value, DiscountProjectionDTO checkDto, int[] levelNo) {
        ...
        for (String tableTreeLevelNo : tableLogic.getAllLevels()) {
            ...
            if (itemId != null) {
                ...
                if (!dto.getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                        && !dto.getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                    if ((dto.getSupplementalLevelNo() == levelNo[0] || dto.getSupplementalLevelNo() == levelNo[1]) && dto.getProjectionDetailIdList().containsAll(checkDto.getProjectionDetailIdList())) {
                        dto.addBooleanProperties(Constant.CHECK, value);
                        if (flag) {
                            tableLogic.getContainerDataSource().getContainerProperty(itemId, Constant.CHECK).setValue(value);
                        }
    ...
    private void getCustomerLevelCheckUpdateForBrandChange(SupplementalTableLogic tableLogic, boolean value, DiscountProjectionDTO checkDto) {
        ...
        for (String tableTreeLevelNo : tableLogic.getAllLevels()) {
            ...
            if (itemId != null) {
                ...
                if (!dto.getLevelName().equalsIgnoreCase(Constant.CONTRACT_SMALL)
                        && !dto.getLevelName().equalsIgnoreCase("Therapeutic Class")) {
                    if (dto.getProjectionDetailIdList().containsAll(checkDto.getProjectionDetailIdList())) {
                        dto.addBooleanProperties(Constant.CHECK, value);
                        if (flag) {
                            tableLogic.getContainerDataSource().getContainerProperty(itemId, Constant.CHECK).setValue(value);
                        }
    ...
    private Set<String> getCheckedRecordsHierarchyNoForMethodolgy() {
        ...
        List<List<String>> relatedProjectionDetailsId = new ArrayList<List<String>>();
        ...
        for (String tableTreeLevelNo : tableLogic.getAllLevels()) {
            ...
            if (itemId != null) {
                ...
                if ((Boolean) dto.getPropertyValue(Constant.CHECK) && dto.getSupplementalLevelNo() == 1) {
                    ...
                    relatedCustomerId.add(dto.getProjectionDetailIdList());
                } else if ((Boolean) dto.getPropertyValue(Constant.CHECK) && dto.getSupplementalLevelNo() == 5) {
                    relatedProjectionDetailsId.add(dto.getParentProjectionDetailIdList());
                    finalHirarechyNo.add(tableTreeLevelNo);
                }
            }
        }
        LOGGER.info("relatedCustomerId.size()  " + relatedCustomerId.size());
        LOGGER.info("relatedProjectionDetailsId.size()  " + relatedProjectionDetailsId.size());
        getRelatedProjectionDetails(relatedCustomerId, finalHirarechyNo, 5);
        getRelatedProjectionDetails(relatedProjectionDetailsId, finalHirarechyNo, 1);
    ...
    public Set<String> getRelatedProjectionDetails(List<List<String>> relatedProjectionDetailsId, Set<String> finalHirarechyNo, int levelNo) {
        for (List<String> relatedCustomerId1 : relatedProjectionDetailsId) {
            for (String tableTreeLevelNo : tableLogic.getAllLevels()) {
                Object itemId = tableLogic.getcurrentTreeData(tableTreeLevelNo);
                if (itemId == null) {
                    itemId = tableLogic.getExpandedTreeValues(tableTreeLevelNo);
                ...
                if (itemId != null) {
                    DiscountProjectionDTO dto = (DiscountProjectionDTO) itemId;
                    if (dto.getSupplementalLevelNo() == 1 && relatedCustomerId1.containsAll(dto.getProjectionDetailIdList())) {
                        finalHirarechyNo.add(tableTreeLevelNo);
                    }
                }
            }
    ...
    private void expandCollapseLevelOption(boolean isExpand, Object value) {
        ...
        int levelNo = Integer.valueOf(levelNoString) == 4 ? 3 : Integer.valueOf(levelNoString);
        if (!isExpand) {
            levelNo--;
        }
        tableLogic.clearAll();
    ...
    private void configureExcelResultTable() {
        excelResultBean = new CustomTreeContainer<DiscountProjectionDTO>(DiscountProjectionDTO.class);

        excelResultBean.setColumnProperties(fullHeader.getProperties());
        exportPeriodViewTable = new ExtFilterTreeTable();
        excelTableLayout.addComponent(exportPeriodViewTable);
    ...
    public void populateBtnClickLogic() {
        ...
        if (fieldDdlb.getValue() == null) {
            ...
            String suppLevelName = StringUtils.EMPTY;
            int i = 0;
            for (DiscountProjectionDTO dto2 : resultBeanContainer.getItemIds()) {

                if ((Boolean) dto2.getPropertyValue(Constant.CHECK)) {
                    if (dto2.getSupplementalLevelNo() == 5) {
                        if (i != 0) {
                            suppLevelName += ",";
                        }

                        suppLevelName += dto2.getGroup();
            ...
            final int startQuater = Integer.valueOf(startPeriod.getValue().toString().charAt(1) - 48);

            final int startYear = Integer.valueOf(startPeriod.getValue().toString().substring(3, 7));
            int endQuater = 0;
            int endYear = 0;
            ...
            if (columnName.equalsIgnoreCase(ACCESS.getConstant()) || columnName.equalsIgnoreCase(METHODOLOGY.getConstant())) {
                ...
                String[] finalPeriodList = new String[2];
                ...
                if (returnList.get(1).isEmpty()) {
                    ...
                    int quatValue = 0;
                    List<List> emptyQuarList = returnList.get(2);
                    ...
                    List<String> ndcValue = returnList.get(3);
            ...
            if (!flag) {
                ...
                for (DiscountProjectionDTO dto : resultBeanContainer.getItemIds()) {
                    if ((Boolean) dto.getPropertyValue(Constant.CHECK)) {
                        if (dto.getSupplementalLevelNo() == 5) {
                            checkedDto.add(dto);
                        } else if (dto.getSupplementalLevelNo() == 1 && String.valueOf(fieldDdlb.getValue()).equals(METHODOLOGY.getConstant())) {
                            checkedDto.add(dto);
                        } else if (dto.getSupplementalLevelNo() == 4 && String.valueOf(fieldDdlb.getValue()).equals(ACCESS.getConstant())) {
                            checkedDto.add(dto);
                        }
                    }
                }
    ...
    protected void excelExportClickLogic() {
        ...
        tableLogic.setRefresh(false);
        loadExcelResultTable(2, StringUtils.EMPTY);
        tableLogic.setRefresh(true);
        exportPeriodViewTable.setRefresh(Boolean.TRUE);
        ForecastUI.EXCEL_CLOSE=true;
        ExcelExport exp = new ExcelExport(new ExtCustomTableHolder(exportPeriodViewTable), Constant.SUPPLEMENTAL_DISCOUNT, Constant.SUPPLEMENTAL_DISCOUNT, "Supplemental Discount Projection.xls", false);
        exp.export();
        excelTableLayout.removeComponent(exportPeriodViewTable);
    }

    @Override
    protected void levelFilterValueChangeLogic(Property.ValueChangeEvent event) {
        LOGGER.info("levelFilterDdlbChangeOption inititated " + levelFilterDdlb.getValue());
        tableLogic.setRefresh(false);
    ...
    public void saveSDP() {
        ...
        if (success) {
            ...
            try {
                Object[] orderedArgs = {session.getProjectionId(), session.getMarketType(), session.getUserId(), session.getSessionId()};
                CommonLogic.callProcedure(Constant.SUPPLEMENTAL_INSERT_PRC, orderedArgs);
                Map map = new HashMap();
                map.put(Constant.VARIABLES, variablesForMandated.getValue());
                commonLogic.saveProjectionSelectionMandatedDiscountProjection(map, session.getProjectionId(), "Supplemental Discount Projection");
                supplementalDiscountProjectionLogic.supplementalSave(session);
