...
public class SPRCommonLogic {
    ...
    public static Leveldto getCustomizedHierarchyView(Object[] obj, boolean isHierarchy) {
        ...
        if (isHierarchy) {
            dto.setHierarchyId(Integer.valueOf(String.valueOf(obj[3])));
        } else {
            dto.setRelationshipLevelValue(String.valueOf(obj[2]));
            dto.setHierarchyNo(String.valueOf(obj[3]));
            dto.setRelationshipLevelName(String.valueOf(DataSelectionForm.getRelationLevelValues().get(dto.getHierarchyNo())));
        }
        return dto;
    }
     ...
     * @param screenName
     */
    public void saveMandatedSRPSelection(Map map, int projectionID, String screenName) throws PortalException, Exception {
    ...
    public static List<Leveldto> getAllHierarchyLevelForSales(int startLevelNo, int projectionId, List<Leveldto> hierarchy) {
        List<Leveldto> newLevelList = new ArrayList<Leveldto>();

        try {
            String query = getAllHierarchyLevelsQueryForSales(startLevelNo, projectionId, hierarchy, false);

            List<Object> list = (List<Object>) executeSelectQuery(query, null, null);
            if (list != null && !list.isEmpty()) {
                for (Object list1 : list) {
                    final Object[] obj = (Object[]) list1;
                    Leveldto dto = getCustomizedViewForSales(obj, false);
                    newLevelList.add(dto);
                }
            }
        } catch (Exception ex) {
    ...
    public static String getAllHierarchyLevelsQueryForSales(int startLevelNo, int projectionId, List<Leveldto> hierarchy, boolean firstCall) {
        String finalQuery = StringUtils.EMPTY;

        Leveldto lvlDto = getNextLevel(startLevelNo, hierarchy);

        if (lvlDto != null) {
            if (firstCall) {
                finalQuery += " Union ";
            }
            finalQuery += getHierarchyLevelsQueryForSales(projectionId, lvlDto.getHierarchyIndicator(), lvlDto.getLevelNo(), lvlDto.getHierarchyId(), startLevelNo);

            finalQuery += getAllHierarchyLevelsQueryForSales(startLevelNo + 1, projectionId, hierarchy, true);
        } else {
            finalQuery += " order by  TREE_LEVEL_NO,LEVEL_NO,RELATIONSHIP_LEVEL_VALUES";
        }
        return finalQuery;
    }
     ...
     * @return list
     */
    public static List<Leveldto> getCustomerHierarchy(int projectionId) {
        return getHierarchy(projectionId, Constant.INDICATOR_LOGIC_CUSTOMER_HIERARCHY);
    }
     ...
     * @return list
     */
    public static List<Leveldto> getProductHierarchy(int projectionId) {
        return getHierarchy(projectionId, Constant.INDICATOR_LOGIC_PRODUCT_HIERARCHY);
    }
        ...
        if (list != null && !list.isEmpty()) {
            for (Object list1 : list) {
                ...
                if (levelName.equals(obj[0])) {
                    ...
                    returnValues[1] = String.valueOf(obj[2]);
                }
    ...
    public static String getLevelListQueryForPR(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, boolean isFilter, boolean isExpand, boolean isCount, int start, int offset, boolean isLimit, boolean isCustom, int customId, String userGroup, int userId, int sessionId, boolean filterDdlb, String levelName) {

        boolean customLevelLimitfFlag = true;
        String customSql = StringUtils.EMPTY;
        if (isCustom) {
            String hierarchyIndicatorQuery = "select HIERARCHY_INDICATOR from dbo.CUSTOM_VIEW_DETAILS where CUSTOM_VIEW_MASTER_SID=" + customId + " and LEVEL_NO=" + levelNo;
            List<Object> list = (List<Object>) executeSelectQuery(hierarchyIndicatorQuery, null, null);
            if (list != null && !list.isEmpty()) {
                ...
                customLevelLimitfFlag = false;
                hierarchyIndicator = StringUtils.EMPTY;
            }
        }
        if (customLevelLimitfFlag) {
            String hierarchyNo1 = StringUtils.EMPTY;
            String whereCond = " ";
            if (hierarchyNo != null) {
                if (!hierarchyNo.equals(StringUtils.EMPTY)) {
    ...
    public static int getHelperTableSID(String listName, String description) throws Exception {
        SalesProjectionDAO dao = new SalesProjectionDAOImpl();
    ...
    public static List<Leveldto> getConditionalLevelListChannel(int projectionId, int start, int offset, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, boolean isFilter, boolean isExpand, boolean isCustom, int customId, String userGroup, int userId, int sessionId, String custRelSid, String prodRelSid, boolean isCount, boolean isLimit) {
        List<Leveldto> listValue = new ArrayList<Leveldto>();
        try {
            String query = getLevelListQueryChannel(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, isFilter, isExpand, false, start, offset, true, isCustom, customId, userGroup, userId, sessionId, custRelSid, prodRelSid);
    ...
    public static String getLevelListQueryChannel(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, boolean isFilter, boolean isExpand, boolean isCount, int start, int offset, boolean isLimit, boolean isCustom, int customId, String userGroup, int userId, int sessionId, String custRelSid, String prodRelSid) {
        if (isCustom) {
        ...
        if (isCustom) {
            ...
            String customViewQuery = getCustomViewLevelListQueryChannel(projectionId, customId, hierarchyIndicator, levelNo, productHierarchyNo, customerHierarchyNo, custRelSid, prodRelSid);
            customSql += " from " + customViewQuery;
        } else {
    ...
    public static String getCustomViewLevelListQueryChannel(int projectionId, int customId, String hierarchyIndicator, int levelNo, String productHierarchyNo, String customerHierarchyNo, String custRelSid, String prodRelSid) {
        customerHierarchyNo += Constant.PERCENT;
        productHierarchyNo += Constant.PERCENT;
        String customerLevelNo = Constant.PERCENT;
        String productLevelNo = Constant.PERCENT;
        ...
        String customViewQuery = "(SELECT RLD.RELATIONSHIP_LEVEL_VALUES, RLD.HIERARCHY_NO, CCP.CCP_DETAILS_SID "
                + " FROM RELATIONSHIP_LEVEL_DEFINITION RLD "
                + " JOIN CCP_MAP CCP ON RLD.RELATIONSHIP_LEVEL_SID=CCP.RELATIONSHIP_LEVEL_SID AND RLD.RELATIONSHIP_BUILDER_SID=" + custRelSid + "\n"
                + " JOIN PROJECTION_DETAILS PD ON PD.CCP_DETAILS_SID=CCP.CCP_DETAILS_SID AND PD.PROJECTION_MASTER_SID=" + projectionId
                + " ) CCPMAPC"
                + " JOIN"
                + " (SELECT RLD.RELATIONSHIP_LEVEL_VALUES, RLD.HIERARCHY_NO, CCP.CCP_DETAILS_SID "
                + " FROM RELATIONSHIP_LEVEL_DEFINITION RLD "
                + " JOIN CCP_MAP CCP ON RLD.RELATIONSHIP_LEVEL_SID=CCP.RELATIONSHIP_LEVEL_SID AND RLD.RELATIONSHIP_BUILDER_SID=" + prodRelSid + "\n"
                + " JOIN PROJECTION_DETAILS PD ON PD.CCP_DETAILS_SID=CCP.CCP_DETAILS_SID AND PD.PROJECTION_MASTER_SID=" + projectionId
                + " ) CCPMAPP "
                + " ON CCPMAPC.CCP_DETAILS_SID=CCPMAPP.CCP_DETAILS_SID"
                + " JOIN "
                ...
                + " CVD.CUSTOM_VIEW_MASTER_SID=" + customId + " AND CVD.LEVEL_NO  like '" + customerLevelNo + "'"
                + " JOIN dbo.HIERARCHY_LEVEL_DEFINITION HLD ON CVD.HIERARCHY_ID=HLD.HIERARCHY_LEVEL_DEFINITION_SID"
                + " JOIN RELATIONSHIP_LEVEL_DEFINITION RLD2 ON HLD.HIERARCHY_LEVEL_DEFINITION_SID=RLD2.HIERARCHY_LEVEL_DEFINITION_SID AND RLD2.RELATIONSHIP_BUILDER_SID=" + custRelSid + "\n"
                + " JOIN PROJECTION_CUST_HIERARCHY PCH2 ON PCH2.RELATIONSHIP_LEVEL_SID=RLD2.RELATIONSHIP_LEVEL_SID AND PCH2.PROJECTION_MASTER_SID=" + projectionId
                + " WHERE RLD2.HIERARCHY_NO like '" + customerHierarchyNo + "') HLDC ON CCPMAPC.HIERARCHY_NO like HLDC.HIERARCHY_NO+'%'"
                + " JOIN "
                + " (SELECT distinct RLD2.HIERARCHY_NO,RLD2.RELATIONSHIP_LEVEL_SID, CVD.LEVEL_NO as TREE_LEVEL_NO, RLD2.LEVEL_NO,RLD2.RELATIONSHIP_LEVEL_VALUES,RLD2.PARENT_NODE,RLD2.LEVEL_NAME FROM dbo.CUSTOM_VIEW_DETAILS CVD "
                ...
                + " CVD.CUSTOM_VIEW_MASTER_SID=" + customId + " AND CVD.LEVEL_NO like '" + productLevelNo + "'"
                + " JOIN dbo.HIERARCHY_LEVEL_DEFINITION HLD ON CVD.HIERARCHY_ID=HLD.HIERARCHY_LEVEL_DEFINITION_SID"
                + " JOIN RELATIONSHIP_LEVEL_DEFINITION RLD2 ON HLD.HIERARCHY_LEVEL_DEFINITION_SID=RLD2.HIERARCHY_LEVEL_DEFINITION_SID AND RLD2.RELATIONSHIP_BUILDER_SID=" + prodRelSid + "\n"
                + " JOIN PROJECTION_PROD_HIERARCHY PCH2 ON PCH2.RELATIONSHIP_LEVEL_SID=RLD2.RELATIONSHIP_LEVEL_SID AND PCH2.PROJECTION_MASTER_SID=" + projectionId
                + " WHERE RLD2.HIERARCHY_NO like '" + productHierarchyNo + "') HLDP ON CCPMAPP.HIERARCHY_NO like HLDP.HIERARCHY_NO+'%'";
        return customViewQuery;
    }

    public static String getCCPWhereConditionQueryChannel(String projectionDetails, String CCP) {
        String ccpWhereCond = " and " + CCP + ".CCP_DETAILS_SID=" + projectionDetails + ".CCP_DETAILS_SID ";
        return ccpWhereCond;
    }

    public static String getCCPQueryChannel(ProjectionSelectionDTO projSelDTO) {
        String ccpQuery = getCCPTempTableQueryChannel();
        if (projSelDTO.isIsCustomHierarchy()) {
            ccpQuery += getCustomCCPQueryChannel(projSelDTO);
        } else {
            ccpQuery += getGeneralCCPQueryChannel(projSelDTO);
        }

        return ccpQuery;
    }

    public static String getCCPTempTableQueryChannel() {
        String tableQuery = "DECLARE @CCP TABLE\n"
                + "  (\n"
                + "     RELATIONSHIP_LEVEL_SID INT,\n"
                + "     CCP_DETAILS_SID        INT,\n"
    ...
    public static String getCustomCCPQueryChannel(ProjectionSelectionDTO projSelDTO) {
        String customerHierarchyNo = projSelDTO.getCustomerHierarchyNo() + Constant.PERCENT;
        String productHierarchyNo = projSelDTO.getProductHierarchyNo() + Constant.PERCENT;
        String customerLevelNo = Constant.PERCENT;
        String productLevelNo = Constant.PERCENT;
    ...
    public static String getGeneralCCPQueryChannel(ProjectionSelectionDTO projSelDTO) {
        ...
        String viewtable = getViewTableNameChannel(projSelDTO.getHierarchyIndicator());
    ...
    public static String getViewTableNameChannel(String hierarchyIndicator) {
        String viewtable = StringUtils.EMPTY;
        if (Constant.INDICATOR_LOGIC_CUSTOMER_HIERARCHY.equals(hierarchyIndicator)) {
            viewtable = "PROJECTION_CUST_HIERARCHY";
        } else if (Constant.INDICATOR_LOGIC_PRODUCT_HIERARCHY.equals(hierarchyIndicator)) {
    ...
    public static int getLevelListCountChannel(int projectionId, String hierarchyIndicator, int levelNo, String hierarchyNo, String productHierarchyNo, String customerHierarchyNo, boolean isFilter, boolean isCustom, int customId, String userGroup, int userId, int sessionId, String custRelSid, String prodRelSid) {
        int count = 0;
        try {

            String query = getLevelListQueryChannel(projectionId, hierarchyIndicator, levelNo, hierarchyNo, productHierarchyNo, customerHierarchyNo, isFilter, false, true, 0, 0, false, isCustom, customId, userGroup, userId, sessionId, custRelSid, prodRelSid);
            List<Object> list = (List<Object>) executeSelectQuery(query, null, null);
            if (list != null && !list.isEmpty()) {
                Object ob = list.get(0);
                count = Integer.valueOf(String.valueOf(ob));
    ...
    public static List<Object> getLevelNoAndHierarchyNoChannel(Object value) {
        List<Object> levelHierarchy = new ArrayList<Object>();
        String selectedId = DASH;
        if (value != null) {
            if (!SELECT_ONE.equals(String.valueOf(value))) {
