...
import static com.stpl.app.arm.businessprocess.transaction7.logic.Trx7ExclusionDetailsLogic.LOGGER;
...
public class RRSummaryLogic<T extends AdjustmentDTO> extends AbstractPipelineSummaryLogic<T> {
    ...
    protected DataResult<T> getSummaryData(List<Object> inputs, Criteria criteria, TreeMap<String, Integer> masterSids) {
        ...
        List<Object[]> data;
        if (isView) {
            data = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_RR_GENERATE_EDIT, CommonConstant.SUMMARY_RR_GENERATE_DATA, null, "Summary_RR_generate_data_orderby", CommonConstant.ARM_RETURN_RESERVE, null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        } else {
            data = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_RR_GENERATE_EDIT, CommonConstant.SUMMARY_RR_GENERATE_DATA, null, "Summary_RR_generate_data_orderby", criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_RETURN_RESERVE), null, "WHERE RANK_VIEW BETWEEN ? AND ?");
        }
    ...
    protected int getSummaryCount(List<Object> inputs, Criteria criteria) {
        ...
        boolean isView = criteria.getSelectionDto().getSessionDTO().getAction().equals(ARMUtils.VIEW_SMALL);
        if (isView) {
            resultList = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_RR_GENERATE_EDIT, "Summary_RR_generate_count", null, null, CommonConstant.ARM_RETURN_RESERVE, null, null);
        } else {
            resultList = QueryUtils.getItemData(inputs, CommonConstant.SUMMARY_RR_GENERATE_EDIT, "Summary_RR_generate_count", null, null, criteria.getSelectionDto().getSessionDTO().getCurrentTableNames().get(CommonConstant.ST_ARM_RETURN_RESERVE), null, null);
        }
        ...
        if (dto instanceof AdjustmentDTO) {
            ...
            if (selection.getSummarylevelFilterNo() == 0) {
                viewType = getView(selection.getSummarydeductionLevelDes(), selection.getSummaryviewType());
                    ...
                    viewType = ARMUtils.getReturnsDemandSummaryLevelsinglePeriod().get(selection.getSummarylevelFilterNo());
                
            } else if (selection.getSummaryviewType().equals(CommonConstant.DEDUCTION_CONTRACT)) {

                viewType = ARMUtils.getSummaryLevel().get(selection.getSummarylevelFilterNo());

            } else {
                String view = selection.getSummaryviewType();
                viewType = ARMUtils.getReturnsLevelAndLevelFilter(view).get(selection.getSummarylevelFilterNo());
            }
        }
