...
public abstract class ForecastProjectionResults extends CustomComponent {
    ...
    protected void resetButtonLogic() {
        new AbstractNotificationUtils() {
            ...
            public void yesMethod() {
                ...
                salesOrUnitsOpg.setValue(SALES.getConstant());
                actualOrProjectionsOpg.setValue(Constants.LabelConstants.ACTUALS);
                periodOrderOpg.setValue(ASCENDING);
    ...
    protected void loadLevelAndFilterValue() {
        ...
        if (projectionSelectionDTO.isIsCustomHierarchy()) {
           LOGGER.info("Customer Hierarchy Selection");
            ...
            hierarchy = CommonLogic.getCustomTree(customId);
             ...
             LOGGER.info("Customer C Hierarchy Selection");
    ...
    public void newHierarchyBtn(Button.ClickEvent event) {
        final CustomTreeBuild customTree = new CustomTreeBuild("Add", sessionDTO);
    ...
    public void editHierarchyBtn(Button.ClickEvent event) {
        if (CommonLogic.editButtonValidation(customDdlb, customViewList)) {
            final CustomTreeBuild customTree = new CustomTreeBuild("Edit", sessionDTO, customId);
            customTree.addCloseListener(new Window.CloseListener() {
                @Override
                public void windowClose(Window.CloseEvent e) {
                    customIdToSelect = customTree.getCustomId();
...
//        h.addComponent(exceltable);
//        Panel p = new Panel();
//        p.setContent(h);
//        tableVerticalLayout.addComponent(p);
        ...
        System.out.println("==excel visible column=================>>>>"+Arrays.toString(fullHeader.getSingleColumns().toArray()));
    ...
    public void loadOnEdit() throws SystemException, Exception {
