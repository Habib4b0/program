...
public abstract class Summary extends CustomComponent {
    ...
    public CustomTreeContainer<AbstractSummaryDTO> resultBean = new CustomTreeContainer<AbstractSummaryDTO>(AbstractSummaryDTO.class);
    /**
    ...
    public CustomTreeContainer<AbstractSummaryDTO> excelResultBean = new CustomTreeContainer<AbstractSummaryDTO>(AbstractSummaryDTO.class);
    ...
    void ConfigureSalesSummaryTable() {
        ...
        deletetableLogic.setPageLength(10);
    ...
    public void configureExcelResultTable() {
        excelResultBean = new CustomTreeContainer<AbstractSummaryDTO>(AbstractSummaryDTO.class);
    ...
    public void loadExcelResultTable() {
        ...
        List<AbstractSummaryDTO> resultList = logic.getConfiguredSalesTabResults(new Object(), 0, 10000000, selectionDTO, true);
    ...
    public void addLowerLevelsForExport(Object id) {
        List<AbstractSummaryDTO> resultList = logic.getConfiguredSalesTabResults(id, 0, 10000000, selectionDTO, true);
        loadDataToContainer(resultList, id);
    }

    private void setSecurityMap() throws Exception {
        if (selection.getButtonMode().equals(ConstantsUtil.ADD)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_ADD, SecurityConstants.SUMMARY_SALES);
        } else if (selection.getButtonMode().equals(ConstantsUtil.EDIT)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_UPDATE, SecurityConstants.SUMMARY_SALES);
        } else if (selection.getButtonMode().equals(ConstantsUtil.DELETE)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_REMOVE, SecurityConstants.SUMMARY_SALES);
        } else if (selection.getButtonMode().equals(ConstantsUtil.TRANSFER)) {
            functionHM = stplSecurity.getBusinessFunctionPermission(userId, SecurityConstants.ITEM_TRANSFER, SecurityConstants.SUMMARY_SALES);
        }
    }
    ...
    private void historyChangeLogic() {
        if (history.getValue() != null) {
            ...
            resultBean.setColumnProperties(rightDTO.getProperties());
            deletetableLogic.setTreeNodeMultiClick(false);
            deletetableLogic.setPageLength(15);
            deletetableLogic.sinkItemPerPageWithPageLength(false);
            deletetableLogic.setContainerDataSource(resultBean);
            leftTable = deletedetailtable.getLeftFreezeAsTable();
            rightTable = deletedetailtable.getRightFreezeAsTable();
