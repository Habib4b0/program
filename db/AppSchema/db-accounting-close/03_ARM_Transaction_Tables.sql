...
BEGIN
	ALTER TABLE ARM_DEMAND_RF_TRUE_UP_SUMMARY ADD TEMP_OVERRIDE NUMERIC(22,6)  NULL
END
GO
...
------------------Adjustment details Excel tables Starts here
IF NOT EXISTS (SELECT 'X' 
               FROM   INFORMATION_SCHEMA.TABLES 
               WHERE  TABLE_NAME = 'ARM_ADJ_GTN_DETAIL_EXCEL' 
                      AND TABLE_SCHEMA = 'DBO') 
  BEGIN 
  
  
      CREATE TABLE [DBO].ARM_ADJ_GTN_DETAIL_EXCEL 
        ( 
		   ARM_ADJ_GTN_DETAIL_EXCEL_SID INT IDENTITY(1,1) NOT NULL,
           ADJUSTMENT_TYPE  VARCHAR(50) NOT NULL,
		   DEDUCTION_TYPE VARCHAR(50),
		   GL_MONTH TINYINT,
		   GL_YEAR SMALLINT,
		   GL_STRING VARCHAR(1000),
...
IF NOT EXISTS(SELECT 1 
              FROM   SYS.KEY_CONSTRAINTS 
              WHERE  OBJECT_NAME(PARENT_OBJECT_ID) = 'ARM_ADJ_GTN_DETAIL_EXCEL' 
                     AND SCHEMA_NAME(SCHEMA_ID) = 'DBO' 
                     AND NAME = 'PK_ARM_ADJ_GTN_DETAIL_EXCEL_CNT_CM_IM_RS_ADJ_ACC_GL_BU_CAL_PERIOD' 
                     AND TYPE = 'PK') 
  BEGIN 
      ALTER TABLE ARM_ADJ_GTN_DETAIL_EXCEL 
        ADD CONSTRAINT PK_ARM_ADJ_GTN_DETAIL_EXCEL_CNT_CM_IM_RS_ADJ_ACC_GL_BU_CAL_PERIOD PRIMARY KEY(CONTRACT_ID,COMPANY_ID,ITEM_ID,DEDUCTION_ID,ADJUSTMENT_TYPE,ACCOUNT,GL_COMPANY_ID,BUSINESS_UNIT_ID,CALCULATION_PERIOD)
  END 
...
IF NOT EXISTS (SELECT 'X' 
               FROM   INFORMATION_SCHEMA.TABLES 
               WHERE  TABLE_NAME = 'ARM_ADJ_RES_DETAIL_EXCEL' 
                      AND TABLE_SCHEMA = 'DBO') 
  BEGIN 
  
  
      CREATE TABLE [DBO].ARM_ADJ_RES_DETAIL_EXCEL
        ( 
		ARM_ADJ_RES_DETAIL_EXCEL_SID INT IDENTITY(1,1) NOT NULL,
		ADJUSTMENT_TYPE  VARCHAR(50),
		BALANCE_TYPE  VARCHAR(50),
		[DATABASE] VARCHAR(50),
	    [DATA_ACCESS_SET]  VARCHAR(50),
		...
		JOURNAL_DESCRIPTION VARCHAR(50), 
		REVERSE_JOURNAL VARCHAR(50), 
		REVERSAL_PERIOD_DATE INT, 
		COMPANY VARCHAR(50), 
		DIVISION VARCHAR(50), 
		...
		END 
		GO
------------------Adjustment details Excel tables Ends here
