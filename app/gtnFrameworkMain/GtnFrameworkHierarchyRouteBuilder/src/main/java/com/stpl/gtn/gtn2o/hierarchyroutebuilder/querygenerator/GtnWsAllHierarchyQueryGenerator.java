...
public class GtnWsAllHierarchyQueryGenerator {

	private GtnFrameworkHierarchyService gtnHierarchyServiceBuilder;
	private GtnFrameworkEntityMasterBean gtnFrameworkEntityMasterBean;

	public GtnWsAllHierarchyQueryGenerator() {
		super();
	}
	...
	public void getLinkedValueQuery(HierarchyLevelDefinitionBean destinationHierarchyBean,
		...
		final GtnFrameworkSelectColumnRelationBean destinationkeyBean = gtnFrameworkEntityMasterBean
		...
		final List<GtnFrameworkSelectColumnRelationBean> keyListBeanList = getKeyListBean(
	...
	private List<GtnFrameworkSelectColumnRelationBean> getKeyListBean(int levelNo,
			List<HierarchyLevelDefinitionBean> hierarchyList) {
		final List<GtnFrameworkSelectColumnRelationBean> keyListBeanList = new ArrayList<>();
		for (int i = 0; i < levelNo - 1; i++) {
			...
			if (!GtnFrameworkWebserviceConstant.USER_DEFINED.equals(hierarchyBean.getLevelValueReference())) {
				final GtnFrameworkSelectColumnRelationBean keyListBean = gtnFrameworkEntityMasterBean
