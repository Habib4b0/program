...
public class DiscountQueryBuilder {
        ...
        try {
            ...
            if ("Group".equals(selectedField)) {
                ...
                if ("Discount Rate".equals(selectedField) || "RPU".equals(selectedField)) {
                 ...
                 int freqDivision = projectionSelection.getFrequencyDivision();
            String semiOrAnnualFreq = freqDivision == 2 ? "S" : "A";
            String monthOrQuarter = freqDivision == 4 ? "Q" : semiOrAnnualFreq;
            String freq = (freqDivision == 12 ? "M" : monthOrQuarter);
            startPeriod = getPeriodSid(freq + startFreq + " " + startYear, projectionSelection.getFrequency(), "Min");
    ...
    public List getDiscountPrograms(SessionDTO session, String programType) {
        ...
        try {
            ...
            return HelperTableLocalServiceUtil.executeSelectQuery(QueryUtil.replaceTableNames(sb.toString(), session.getCurrentTableNames()));
        } catch (Exception e) {
            LOGGER.error(e.getMessage());
    ...
    public List getDiscountProjectionCustom(final boolean isProgram, final String frequency, final List<String> discountList,
        ...
        String queryBuilder = SQlUtil.getQuery("discountGenerateCustomView");
    ...
    public List getDiscountProjectionLastLevel(final String frequency, final List<String> discountList,
            final SessionDTO session, final String hierarchyNo, final String hierarchyIndicator, final int levelNo, final int treeLevelNo,
    ...
    StringBuilder framedInQueryStringBuilder = new StringBuilder();
        ...
        return "SELECT DISTINCT DEDUCTION_HIERARCHY_NO FROM ST_NM_DISCOUNT_PROJ_MASTER  WHERE RS_CONTRACT_SID in ("+framedInQueryString+")";
    ...
    private String insertAvailableHierarchyNo(SessionDTO sessionDTO, String hierarchyNo, String hierarchyIndicator, int levelNo, boolean isCustomHierarchy,
        ...
        if (isCustomHierarchy) {
            sql = sql.replace(Constant.QUESTION_HIERARCHY_NO_VALUES, getSelectedHierarchyCustom(sessionDTO, hierarchyNo, hierarchyIndicator, levelNo,false));
        } else {
    ...
    private String insertAvailableHierarchyNoForCount(SessionDTO sessionDTO, String hierarchyNo, String hierarchyIndicator, int levelNo, boolean isCustomHierarchy,
        ...
        if (isCustomHierarchy) {
            sqlQuery = sqlQuery.replace(Constant.QUESTION_HIERARCHY_NO_VALUES, getSelectedHierarchyCustom(sessionDTO, hierarchyNo, hierarchyIndicator, levelNo,false));
        } else {
    ...
    public String getHierarchyJoinQuery(final SessionDTO sessionDTO, final int customId, final int treeLevelNo, final boolean isCustom, final String hierarchyIndicator, final String customerHierarchyNo, final String productHierarchyNo, final String deductionHierarchyNo, final String userGroup) {
        ...
        return getHierarchyJoinQuery(isCustom, customerHierarchyNo, productHierarchyNo, deductionHierarchyNo, currentHierarchyIndicator, userGroup,sessionDTO);
    }
