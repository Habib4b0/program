...
public class ItemQueries {
    ...
    private static String getQuery(StringBuilder query, Object myDTO) throws Exception {
        while (true) {
            int index = query.indexOf("${");
            if (index != -1) {
                String variable = query.substring(index + 2, query.indexOf("}"));
                ...
                if (filter instanceof SimpleStringFilter) {
                    ...
                    if (operation.EQUAL.toString().equals(operation.name())) {
                        if (((Integer) stringFilter.getValue()) == 0) {
                            parameters.put(stringFilter.getPropertyId() + "~=", String.valueOf(stringFilter.getValue()) + "--" + Constants.ZEROSTRING);
                        } else {
                            int val = (Integer) stringFilter.getValue();
                            parameters.put(stringFilter.getPropertyId() + "~=", String.valueOf(val) + "--" + "=");
                        }
                    }
                    if (operation.GREATER.toString().equals(operation.name())) {
                        int val = (Integer) stringFilter.getValue();
                        if (val < 0) {
                            parameters.put(stringFilter.getPropertyId() + "~>", String.valueOf(val) + "--" + ">0");
                        } else {
                            parameters.put(stringFilter.getPropertyId() + "~>", String.valueOf(val) + "--" + ">");
                        }
                    }
        ...
        if (filters != null) {
            for (Container.Filter filter : filters) {
                if (filter instanceof SimpleStringFilter) {
                    ...
                    Compare stringFilter = (Compare) filter;
                    String filterValue = String.valueOf(stringFilter.getValue());
                    if ("statusDescription".equals(stringFilter.getPropertyId())) {
                        if (filterValue != null && !filterValue.equals(Constants.NULL)) {
                            try {
                    ...
                    if ("contractType".equals(stringFilter.getPropertyId())) {
                        if (filterValue != null && !filterValue.equals(Constants.NULL)) {
                            try {
