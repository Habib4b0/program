...
public class FileSelectionTableLogic extends PageTableLogic {
    ...
    public int getCount() {
        int count =0;
        if(iscount){
        count = Integer.parseInt(String.valueOf(logic.getFileName(true,sessionDTO, getFilters())));
        }
        return count;
    }
    ...
    public List loadData(int start, int offset) {
        list=(List<FileSelectionDTO>)logic.getFileName(false,sessionDTO, getFilters());
        LOGGER.info("getCount"+list.size());
        return list;
    }
    ...
    public void setSearchData(SessionDTO sessionDTO){   
        this.iscount=true;
