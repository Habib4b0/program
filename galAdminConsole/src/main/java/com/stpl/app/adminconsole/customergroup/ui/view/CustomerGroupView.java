...
public class CustomerGroupView extends VerticalLayout implements View {
     ...
     */
    public CustomerGroupView(final SessionDTO sessionDTO) throws SystemException, PortalException, Exception {
        super();
        ...
        LOGGER.info("enter CustomerGroupView constructor");
        setSpacing(true);
        addComponent(customerGroupInfo);
        setStyleName("bootstrap");
        LOGGER.info("CustomerGroupView constructor Ended");
    }
    ...
    public void enter(final ViewChangeListener.ViewChangeEvent event) {
        try {
            ...
            if (companyGroupSystemId == ConstantsUtils.ZERO_NUM) {
                ...
                List<CustomerDetailsDTO> availableCompanyList = new ArrayList<CustomerDetailsDTO>();
                if (ConstantsUtils.LOWERCASE_EDIT.equalsIgnoreCase(pageName) || ConstantsUtils.VIEW.equalsIgnoreCase(pageName) || ConstantsUtils.LOWERCASE_COPY.equalsIgnoreCase(pageName)) {
                    ...
                    if (!StringUtils.isEmpty(customerGroupDTO.getSearchCriteria())) {
                        List availableList = new CustomerGroupLogic().getAvailableSearchResults(customerGroupDTO.getSearchCriteria());

                        if (availableList.size() > 0) {
                            availableCompanyList = new CustomerGroupLogic().getCustomizedCustomerResults(availableList, new HashMap());
                        }
                    }

                    flag = true;
                }
                if (!availableCompanyList.isEmpty()) {
                    availableResultsBean.addAll(availableCompanyList);
                }
