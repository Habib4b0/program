...
public class SaveViewPopup extends AbstractSaveViewPopup {
    ...
    public void saveView(final List<Leveldto> selectedCustomersList, final List<Leveldto> selectedProductsList,
            final String actionFlag, int projectionIdValue) throws SystemException, PortalException, Exception {
        try {
            ...
            if ("Update".equalsIgnoreCase(actionFlag)) {
                projectionIdValue = cffLogic.saveCFFMaster(dataselectionDtoToSave, screenName, Boolean.TRUE, projectionIdValue);
                ...
                projectionIdValue = cffLogic.saveCFFMaster(dataselectionDtoToSave, screenName, Boolean.FALSE, projectionIdValue);
                dsLogic.saveCustomerHierarchyLogic(selectedCustomersList, customerListEndSids, projectionIdValue, null, "save");
                dsLogic.saveProductHierarchyLogic(selectedProductsList, productListEndSids, projectionIdValue, null, "save");
            ...
            if (projectionIdValue != 0) {
                ...
                if (viewIdValue != 0) {
                    if (!Constants.FAIL.equals(String.valueOf(saveViewDTO.getViewId()))) {
    ...
    protected void btnUpdateLogic() {
        ...
        if (!dataselectionDtoToSave.getViewType().equals("")) {
            if (dataselectionDtoToSave.getViewType().equals("public")) {
                if (!sessionDTO.getUserId().equals(viewDTO.getCreatedUserid())) {
                    non_creatoralert++;
                }
            }
    ...
    private void deleteViewRecord(String viewName, String viewType) {
        ...
        if (viewType.equals(Constants.LabelConstants.PUBLIC)) {
            input.add(Constants.LabelConstants.PRIVATE);
        } else {
            input.add(Constants.LabelConstants.PUBLIC);
        }
        input.add(viewName);
        CommonQueryUtils.updateAppData(input, "Delete View Master");
    }
