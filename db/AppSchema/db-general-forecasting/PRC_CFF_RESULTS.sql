...
SET @SQL_OUT=CONCAT('
	...
	ISNULL((ISNULL(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0)) / NULLIF(SUM(S.SALES), 0), 0) * 100 AS TOTAL_DISCOUNT_PERCENTAGE,
	...
	ISNULL(SUM(S.SALES), 0) - ISNULL(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) AS NET_SALES,
	...
	ISNULL((ISNULL(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0)) / NULLIF(SUM(S.UNITS), 0), 0) AS TOTAL_RPU,
	ISNULL(SUM(S.COGS_VALUE), 0) AS COGS_VALUE,
	ISNULL(((isnull(SUM(S.SALES), 0) - (ISNULL(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0))) - (ISNULL(SUM(S.COGS_VALUE), 0))), 0) AS NET_PROFIT,
	...
	coalesce(((ISNULL(isnull(SUM(S.SALES), 0) - (ISNULL(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0)), 0)) / NULLIF(SUM(pfd.EX_FACTORY_SALES), 0)) * 100, 0) AS NET_SALES_OF_EX_FACTORY,
	isnull(((isnull(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0)) / NULLIF(SUM(pfd.EX_FACTORY_SALES), 0)) * 100, 0) AS DISCOUNT_OF_EX_FACTORY,
	ISNULL(SUM(PFD.EX_FACTORY_SALES), 0) - ISNULL((isnull(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0)), 0) AS NET_EXFACTORY_SALES,
	ISNULL(((ISNULL(SUM(PFD.EX_FACTORY_SALES), 0) - ISNULL((isnull(SUM(ND.DISCOUNT), 0) + ISNULL(SUM(SPD.DISCOUNT), 0) + ISNULL(SUM(MD.DISCOUNT), 0)), 0)) / NULLIF(SUM(PFD.EX_FACTORY_SALES), 0)), 0) * 100 AS NET_EXFACTORY_SALES_PERCENT,
	ISNULL(SUM(ND.ACCRUALS), 0) TOTAL_DISCOUNT_ACCRUAL
		, 0 AS INDICATOR
	FROM #PRODUCT_FILE_DATA PFD
	INNER JOIN #CFF_PROJECTION_MASTER CFM ON PFD.CFF_MASTER_SID=CFM.CFF_MASTER_SID AND PFD.CFF_MASTER_SID = ', @FIRST_PROJ_SID ,'
	AND PFD.INDICATOR=0
	LEFT JOIN (select  ''',@HIERARCHY_NO,''' HIERARCHY_NO,A.PERIOD,A.YEAR,sum(A.SALES) SALES ,sum(A.UNITS) UNITS,
	A.CFF_MASTER_SID,SUM(A.COGS_VALUE) COGS_VALUE from ', @sales_table ,' A
	JOIN (select year,period from #PERIOD  where period_sid < ',@END_SALES_SID,' group by year,period) P ON P.PERIOD=A.PERIOD AND P.YEAR=A.YEAR
	...
	ON CPFD.PERIOD = S.PERIOD AND S.YEAR = CPFD.YEAR AND PFD.CFF_MASTER_SID = CPFD.CFF_MASTER_SID 
	...
	ON ND.PERIOD = S.PERIOD AND S.YEAR = ND.YEAR AND PFD.CFF_MASTER_SID = ND.CFF_MASTER_SID AND S.HIERARCHY_NO=ND.HIERARCHY_NO 
	...
	ON MD.PERIOD = S.PERIOD AND S.YEAR = MD.YEAR AND PFD.CFF_MASTER_SID = MD.CFF_MASTER_SID AND S.HIERARCHY_NO = MD.HIERARCHY_NO 
	...
	ON SPD.PERIOD = S.PERIOD AND S.YEAR = SPD.YEAR AND PFD.CFF_MASTER_SID = SPD.CFF_MASTER_SID AND S.HIERARCHY_NO = SPD.HIERARCHY_NO
	...
	LEFT JOIN (select  ''',@HIERARCHY_NO,''' HIERARCHY_NO,A.PERIOD,A.YEAR,sum(A.SALES) SALES ,sum(A.UNITS) UNITS,
	A.CFF_MASTER_SID,SUM(A.COGS_VALUE) COGS_VALUE from ', @sales_table ,' A
	JOIN (select year,period from #PERIOD group by year,period) P ON P.PERIOD=A.PERIOD AND P.YEAR=A.YEAR
	where a.HIERARCHY_NO like ''',@HIERARCHY_NO,'''+''%''
	AND A.INDICATOR=1
	AND (SALES_INCLUSION = @SALES_INCLUSION  OR @SALES_INCLUSION IS NULL) AND A.FILTER_CCP = 1
	group by A.PERIOD,A.YEAR,A.CFF_MASTER_SID
