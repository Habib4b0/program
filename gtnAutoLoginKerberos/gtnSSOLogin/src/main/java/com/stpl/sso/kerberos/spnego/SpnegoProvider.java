...
/**
 * This is a Utility Class that can be used for finer grained control over
 * message integrity, confidentiality and mutual authentication.
 *
 * <p>
 * This Class is exposed for developers who want to implement a custom HTTP
 * client.
 * </p>
 *
 * <p>
 * Take a look at the {@link SpnegoHttpURLConnection} class and the
 * {@link SpnegoHttpFilter} class before attempting to implement your own HTTP
 * client.
 * </p>
 *
 * <p>
 * For more example usage, see the documentation at
...
public final class SpnegoProvider {
    ...
    /**
     * Returns the {@link SpnegoAuthScheme} mechanism used to authenticate the
     * request.
     *
     * <p>
     * This method may return null in which case you must check the HTTP Status
     * Code to determine if additional processing is required.
     * <br />
     * For example, if req. did not contain the SpnegoConstants.AUTHZ_HEADER,
     * the HTTP Status Code SC_UNAUTHORIZED will be set and the client must send
     * authentication information on the next request.
     * </p>
     *
    ...
    static SpnegoAuthScheme negotiate(
            final HttpServletRequest req, final SpnegoHttpServletResponse resp, final boolean basicSupported, final boolean promptIfNtlm, final String realm) throws IOException {
        ...
        final PrivilegedExceptionAction<GSSCredential> action
            ...
            public GSSCredential run() throws GSSException {
                return MANAGER.createCredential(
                        null, GSSCredential.DEFAULT_LIFETIME, SpnegoProvider.SPNEGO_OID, GSSCredential.INITIATE_ONLY);
            }
        };
    ...
    /**
     * Returns a GSSContext to be used by custom clients to set data integrity
     * requirements, confidentiality and if mutual authentication is required.
     *
     * @param creds credentials of the person to be authenticated
     * @param url HTTP address of server (used for constructing a
     * {@link GSSName}).
     * @return GSSContext
     * @throws GSSException
     * @throws PrivilegedActionException
    ...
    static GSSCredential getServerCredential(final Subject subject)
        ...
        final PrivilegedExceptionAction<GSSCredential> action
            ...
            public GSSCredential run() throws GSSException {
                return MANAGER.createCredential(
                        null, GSSCredential.INDEFINITE_LIFETIME, SpnegoProvider.SPNEGO_OID, GSSCredential.ACCEPT_ONLY);
            }
        };
        return Subject.doAs(subject, action);
    }
    ...
    /**
     * Returns the {@link GSSName} constructed out of the passed-in URL object.
     *
