...
public class DiscountSelection extends Window {
    ...
    private VerticalLayout addToContent() {
        final VerticalLayout content = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
    ...
    private HorizontalLayout addButtons() {
        final HorizontalLayout layout = (HorizontalLayout) UiUtils.getLayout(HorizontalLayout.class);
    ...
    private VerticalLayout addTable() {
        final VerticalLayout layout = (VerticalLayout) UiUtils.getLayout(VerticalLayout.class);
        layout.setMargin(false);
    ...
    private void configureFields() {
                        ...
                        public void valueChange(Property.ValueChangeEvent event) {
                            if ((Boolean) event.getProperty().getValue()) {
                                ...
                                if (checkCount > NumericConstants.FIVE) {
                                    ...
                                    AbstractNotificationUtils.getErrorNotification("Cannot select the discount", "You can select a maximum of 5 discounts");
                                }
                            } else {
                                checkCount--;
                            }
            ...
            public void buttonClick(final Button.ClickEvent event) {
                ...
                for (DiscountSelectionDTO discountDto : discountSelectionBean.getItemIds()) {
                    if (discountDto.isCheckRecord()) {
                        ...
                        } else if (discountDto.getDiscountRSlist() != null && !discountDto.getDiscountRSlist().isEmpty()) {
                ...
                if (selectedDiscounts != null && !selectedDiscounts.isEmpty()) {
                    ...
                    AbstractNotificationUtils.getErrorNotification("No Values Selected", "No Discounts have been selected");
                }
        ...
        btnClose.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(final Button.ClickEvent event) {
                ...
                }.getConfirmationMessage("Confirm Close", "Closing the popup will not submit the selected discounts back to the discount projection screen. Are you sure you want to continue?");
