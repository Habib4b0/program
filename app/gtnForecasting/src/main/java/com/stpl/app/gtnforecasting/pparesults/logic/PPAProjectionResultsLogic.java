...
public class PPAProjectionResultsLogic {
	...
	private List getCustomizedList(List<Object[]> list, ProjectionSelectionDTO selection) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			freq = 0;
			yearInt = 1;
			int actulOrProjection = str[NumericConstants.SIX] != null
					? Integer.parseInt(str[NumericConstants.SIX].toString())
					: -1;
			String quater = null;
			String year = null;
			...
			if (header != null && actulOrProjection == 1) {
				...
				discountDoller.addStringProperties(header,
						(str[discountDollerNo] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
				...
				unitVolume.addStringProperties(header,
						(str[unitVolumeNo] == null) ? CommonUtils.UNITVOLUME.format(Double.valueOf(Constant.DASH))
								: isProj ? CommonUtils.UNITVOLUME.format(0.0)
										: CommonUtils.UNITVOLUME.format(Double.valueOf(str[unitVolumeNo].toString())));
				totalDiscount.addStringProperties(header,
						(str[TOTAL_DISCOUNT_NO] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
								: isProj ? CommonUtils.MONEY.format(0.0)
			...
			if (header != null && actulOrProjection == 0) {
				discountDoller.addStringProperties(header,
						(str[discountDollerNo] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
								: CommonUtils.MONEY.format(Double.valueOf(str[discountDollerNo].toString())));
				discountPercent.addStringProperties(header,
						(str[discountPercentNo] == null)
								? CommonUtils.PERCENT_FORMAT.format(Double.valueOf(Constant.DASH))
								: CommonUtils.PERCENT_FORMAT.format(Double.valueOf(str[discountPercentNo].toString())));
				unitVolume.addStringProperties(header,
						(str[unitVolumeNo] == null) ? CommonUtils.UNITVOLUME.format(Double.valueOf(Constant.DASH))
	...
	private String isColumn(ProjectionSelectionDTO selection, String quater, String year, String caption,
			String frequency) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (tempList.contains(constant)) {
				...
				if (actulOrProjection == 0) {
					dto.setDiscountPerUnitActuals(
							(str[discountDollerNo] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
									: CommonUtils.MONEY.format(Double.valueOf(str[discountDollerNo].toString())));
					dto.setDiscountPercentActuals((str[discountPercentNo] == null)
							? CommonUtils.PERCENT_FORMAT.format(Double.valueOf(Constant.DASH))
							: CommonUtils.PERCENT_FORMAT.format(Double.valueOf(str[discountPercentNo].toString())));
					dto.setUnitVolumeActuals(
							(str[unitVolumeNo] == null) ? CommonUtils.UNITVOLUME.format(Double.valueOf(Constant.DASH))
									: CommonUtils.UNITVOLUME.format(Double.valueOf(str[unitVolumeNo].toString())));
					dto.setTotalDiscountActuals(
							(str[TOTAL_DISCOUNT_NO] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
									: CommonUtils.MONEY.format(Double.valueOf(str[TOTAL_DISCOUNT_NO].toString())));
				} else if (actulOrProjection == 1) {
					dto.setDiscountPerUnitProjections(
							(str[discountDollerNo] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
									: CommonUtils.MONEY.format(Double.valueOf(str[discountDollerNo].toString())));
					dto.setDiscountPercentProjections((str[discountPercentNo] == null)
							? CommonUtils.PERCENT_FORMAT.format(Double.valueOf(Constant.DASH))
							: CommonUtils.PERCENT_FORMAT.format(Double.valueOf(str[discountPercentNo].toString())));
					dto.setUnitVolumeProjections(
							(str[unitVolumeNo] == null) ? CommonUtils.UNITVOLUME.format(Double.valueOf(Constant.DASH))
									: CommonUtils.UNITVOLUME.format(Double.valueOf(str[unitVolumeNo].toString())));
					dto.setTotalDiscountProjections(
							(str[TOTAL_DISCOUNT_NO] == null) ? CommonUtils.MONEY.format(Double.valueOf(Constant.DASH))
									: CommonUtils.MONEY.format(Double.valueOf(str[TOTAL_DISCOUNT_NO].toString())));
				}
			}
	...
	private List<PPAProjectionResultsDTO> setDefaultValue(List<PPAProjectionResultsDTO> list1,
			List<String> visibleColumn) {
	...
	public int getConfiguredPPAProjectionResultsCount(Object parentId, ProjectionSelectionDTO projSelDTO,
			...
			if (parentId instanceof PPAProjectionResultsDTO) {
				...
				if (Constant.INDICATOR_LOGIC_CUSTOMER_HIERARCHY.equalsIgnoreCase(parentDto.getHirarechyIndicater())) {
					...
					projSelDTO.setProductHierarchyNo(parentDto.getProductHierarchyNo());
						...
						.equalsIgnoreCase(parentDto.getHirarechyIndicater())) {
	...
	public int getProjectionResultsCount(ProjectionSelectionDTO projSelDTO, boolean isLevelCount) {
		...
		if (!projSelDTO.getGroup().startsWith(Constant.ALL) && !projSelDTO.getGroup().contains(Constant.PPA)) {
	...
	public List<PPAProjectionResultsDTO> getConfiguredPPAProjectionResults1(Object parentId, int start, int offset,
			ProjectionSelectionDTO selection, SessionDTO session) {
		...
		if (!selection.isIsFilter() || (parentId instanceof PPAProjectionResultsDTO)) {
			...
			if (parentId instanceof PPAProjectionResultsDTO) {
				...
				if (!parentDto.getGroup().startsWith(Constant.ALL) && !parentDto.getGroup().contains(Constant.PPA)
						&& !selection.isIsCustomHierarchy()) {
					if (Constant.INDICATOR_LOGIC_CUSTOMER_HIERARCHY
							.equalsIgnoreCase(parentDto.getHirarechyIndicater())) {
	...
	public List<PPAProjectionResultsDTO> getConfiguredPPAProjectionResultsTotal(int start, int offset,
			ProjectionSelectionDTO projSelDTO, SessionDTO session) {
	 ...
	 * @return
	 */
	public PPAProjectionResultsDTO configureDetailsInDTO(ProjectionSelectionDTO projSelDTO, String hierarchyNo,
	...
	public List<PPAProjectionResultsDTO> getPPAProjectionResultsTotal(int start, int offset,
			ProjectionSelectionDTO selection, SessionDTO session) {
	...
	public List<PPAProjectionResultsDTO> getPPAProjectionResults1(int start, int offset,
			ProjectionSelectionDTO selection, SessionDTO session) {
		int neededRecord;
		neededRecord = offset;
		int started = start;
		int maxRecord = 0;
		if (!selection.getGroup().startsWith(Constant.ALL) && !selection.getGroup().contains(Constant.PPA)) {
			if (selection.getPivotView().equals(Constant.PERIOD)) {
				maxRecord = NumericConstants.FOUR;
			} else {
		...
		if ((!selection.isIsProjectionTotal()) && ((neededRecord > 0)
				&& (!selection.getGroup().startsWith(Constant.ALL) && !selection.getGroup().contains(Constant.PPA)))) {
			List<PPAProjectionResultsDTO> resultList = new ArrayList<>();
			if (selection.getPivotView().contains(Constant.PERIOD)) {
				try {
		...
		if (neededRecord > 0 && !selection.isIsFilter()) {
			if ((selection.getTreeLevelNo() + 1) == selection.getTpLevel()
					&& ((selection.isIsCustomHierarchy())
							|| (!selection.getHierarchyIndicator().equals(Constant.INDICATOR_LOGIC_PRODUCT_HIERARCHY)))
					&& !selection.getGroup().startsWith(Constant.ALL) && !selection.getGroup().contains(Constant.PPA)) {
				PPAProjectionResultsDTO dto = new PPAProjectionResultsDTO();
				...
				int maxLevelNo = selection.getHierarchyIndicator().equals(Constant.INDICATOR_LOGIC_CUSTOMER_HIERARCHY)
						? session.getMaximumCustomerLevel()
						: session.getMaximumProductLevel();
				if (selection.getTreeLevelNo() <= maxLevelNo) {
					List<PPAProjectionResultsDTO> nextLevelValueList = configureLevels(resultStart, neededRecord,
							selection);
	 ...
	 * @return
	 */
	public int getPPADetailsDDLBCount(String filter, final String ddlbType, final int projectionId,
	...
	public List<HelperDTO> getPPADetailsDDLBResult(int startIndex, int end, String filter, HelperDTO dto,
		...
		if (filter != null && !(Constant.NULL.equalsIgnoreCase(String.valueOf(filter)))
				&& !StringUtils.EMPTY.equals(filter)) {
			switch (ddlbType) {
			case Constant.CONTRACT:
	...
	public int getPPAItemCount(PPADetailsDTO ppaDetailsDTO, final String filterText, String ddlbtype) {
		...
		if (ppaDetailsDTO.getPpaHelperDTO().getItemMasterSysId() != 0) {
			query = query.replace(Constant.ITEMSID_AT,
					StringUtils.EMPTY + ppaDetailsDTO.getPpaHelperDTO().getItemMasterSysId());
		} else {
		...
		if (filterText != null && !(Constant.NULL.equalsIgnoreCase(String.valueOf(filterText)))
				&& !StringUtils.EMPTY.equals(filterText)) {
			switch (ddlbtype) {
	...
	public List<PPAHelperDTO> getPPAItemDDLBResult(int startIndex, int end, PPADetailsDTO ppaDetailsDTO,
			String ddlbtype, final String filterText) {
		LOGGER.debug("Inside Item Load= {}, END INDEX= {}, ppaDetailsDTO= {} ", startIndex, end,
				ppaDetailsDTO.getPpaHelperDTO().getItemMasterSysId());
		String query = StringUtils.EMPTY;
		String searchFilter;
		...
		if (ppaDetailsDTO.getPpaHelperDTO().getItemMasterSysId() != 0) {
			query = query.replace(Constant.ITEMSID_AT,
					String.valueOf(ppaDetailsDTO.getPpaHelperDTO().getItemMasterSysId()));
		} else {
			query = query.replace("AND IM.ITEM_MASTER_SID NOT IN (@itemsid)", StringUtils.EMPTY);
		}
		if (filterText != null && !(Constant.NULL.equalsIgnoreCase(String.valueOf(filterText)))
		...
		for (final Iterator<Object[]> iterator = returnList.iterator(); iterator.hasNext();) {
			...
			helperTable.setItemName(
					value[NumericConstants.TWO] != null ? value[NumericConstants.TWO].toString() : StringUtils.EMPTY);
	 ...
	 * @return
	 */
	public Object loadPPADetails(PPADetailsDTO ppaDetailsDTO, SessionDTO sessionDTO, boolean isCount, int start,
			int offset, List<SortByColumn> sortByColumns) {
		...
		List resultList = (List) HelperTableLocalServiceUtil
				.executeSelectQuery(QueryUtil.replaceTableNames(query, sessionDTO.getCurrentTableNames()));
		if (isCount) {
		...
		if (list != null && !list.isEmpty()) {
			...
			for (Object[] object : list) {
				try {
					...
					detailsDTO.setPeriod((object[0] != null ? String.valueOf(object[0]) : StringUtils.EMPTY) + ' '
							+ (object[1] != null ? CommonUtils.getMonthName(Integer.parseInt(String.valueOf(object[1])))
									: StringUtils.EMPTY));
					detailsDTO.setPriceProtectionPriceType(
							object[NumericConstants.TWO] != null ? String.valueOf(object[NumericConstants.TWO])
									: StringUtils.EMPTY);
					detailsDTO.setMap(object[NumericConstants.FIVE] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.FIVE]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setTotalDeductions(object[NumericConstants.SIX] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.SIX]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setUnits(
							object[NumericConstants.SEVEN] != null
									? getFormatValue(ZERO_DECIMAL, String.valueOf(object[NumericConstants.SEVEN]),
											StringUtils.EMPTY)
									: StringUtils.EMPTY);
					detailsDTO.setDeductionPerUnit(object[NumericConstants.EIGHT] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.EIGHT]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setNetPrice(object[NumericConstants.NINE] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.NINE]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setNetMAP(object[NumericConstants.TEN] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.TEN]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setPriceProtectionAmountPerUnit(object[NumericConstants.ELEVEN] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.ELEVEN]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setPriceProtectionPercent(object[NumericConstants.TWELVE] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.TWELVE]), PERCENTAGE)
							: StringUtils.EMPTY);
					detailsDTO.setTotalPriceProtectionDeduction(object[NumericConstants.THIRTEEN] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.THIRTEEN]), PERCENTAGE)
							: StringUtils.EMPTY);
					detailsDTO.setNep(object[NumericConstants.FOURTEEN] != null
							? getFormatValue(TWO_DECIMAL, String.valueOf(object[NumericConstants.FOURTEEN]), CURRENCY)
							: StringUtils.EMPTY);
					detailsDTO.setNepFormula(
							object[NumericConstants.FIFTEEN] != null ? String.valueOf(object[NumericConstants.FIFTEEN])
									: StringUtils.EMPTY);
					detailsDTO.setPriceToleranceType(
							object[NumericConstants.SIXTEEN] != null ? String.valueOf(object[NumericConstants.SIXTEEN])
									: StringUtils.EMPTY);
					detailsDTO.setPriceTolerance(
							object[NumericConstants.SEVENTEEN] != null
									? getFormatValue(ZERO_DECIMAL, String.valueOf(object[NumericConstants.SEVENTEEN]),
											StringUtils.EMPTY)
									: StringUtils.EMPTY);
					detailsDTO.setPriceToleranceInterval(
							object[NumericConstants.EIGHTEEN] != null
									? getFormatValue(ZERO_DECIMAL, String.valueOf(object[NumericConstants.EIGHTEEN]),
											StringUtils.EMPTY)
									: StringUtils.EMPTY);
					detailsDTO.setPriceToleranceFrequency(object[NumericConstants.NINETEEN] != null
							? String.valueOf(object[NumericConstants.NINETEEN])
							: StringUtils.EMPTY);
					detailsDTO.setMaxIncrementalChange(
							object[NumericConstants.TWENTY] != null ? String.valueOf(object[NumericConstants.TWENTY])
									: StringUtils.EMPTY);
					detailsDTO.setResetEligible(object[NumericConstants.TWENTY_ONE] != null
							? String.valueOf(object[NumericConstants.TWENTY_ONE])
							: StringUtils.EMPTY);
					detailsDTO.setResetType(object[NumericConstants.TWENTY_TWO] != null
							? String.valueOf(object[NumericConstants.TWENTY_TWO])
							: StringUtils.EMPTY);
					if (object[NumericConstants.TWENTY_THREE] != null) {
						SimpleDateFormat fromUser = new SimpleDateFormat(DATE_FORMAT.getConstant());
						String formateddate = fromUser
								.format(myFormat.parse(String.valueOf(object[NumericConstants.TWENTY_THREE])));
						detailsDTO.setResetDate(formateddate);
					} else {
						detailsDTO.setResetDate(StringUtils.EMPTY);
					}

					detailsDTO.setResetInterval(object[NumericConstants.TWENTY_FOUR] != null
							? String.valueOf(object[NumericConstants.TWENTY_FOUR])
							: StringUtils.EMPTY);
					detailsDTO.setResetFrequency(object[NumericConstants.TWENTY_FIVE] != null
							? String.valueOf(object[NumericConstants.TWENTY_FIVE])
							: StringUtils.EMPTY);
					detailsDTO.setNetPriceType(object[NumericConstants.TWENTY_SIX] != null
							? String.valueOf(object[NumericConstants.TWENTY_SIX])
							: StringUtils.EMPTY);
					detailsDTO.setNetPriceTypeFormula(object[NumericConstants.TWENTY_SEVEN] != null
							? String.valueOf(object[NumericConstants.TWENTY_SEVEN])
							: StringUtils.EMPTY);
					detailsDTO.setRebateScheduleName(object[NumericConstants.TWENTY_EIGHT] != null
							? String.valueOf(object[NumericConstants.TWENTY_EIGHT])
							: StringUtils.EMPTY);
					...
					String[] wacPriceArray = calcWacPriceChange(String.valueOf(object[NumericConstants.THIRTY_TWO]),
							String.valueOf(object[NumericConstants.TWENTY_NINE]),
							String.valueOf(object[NumericConstants.ZERO]), frquencyValue);

					detailsDTO.setPrice(
							getFormatValue(TWO_DECIMAL, wacPriceArray[NumericConstants.ZERO], StringUtils.EMPTY));
					detailsDTO.setPriceChange(
							getFormatValue(TWO_DECIMAL, wacPriceArray[NumericConstants.ONE], PERCENTAGE));
	 ...
	 * @throws Exception
	 */
	public ExtTreeContainer<PPAProjectionResultsDTO> getLoadedExcelContainer(ProjectionSelectionDTO selection,
			ExtTreeContainer<PPAProjectionResultsDTO> container)
			throws IllegalAccessException, InvocationTargetException {
		List<PPAProjectionResultsDTO> totalList = ppaProjecetionResults(selection.isIsProjectionTotal(), selection,
				selection.getSessionDTO());
	...
	private List getExcelProcedureList(final ProjectionSelectionDTO selection) {
		...
		Object[] orderedArgs = { selection.getProjectionId(), selection.getUserId(),
				selection.getSessionDTO().getSessionId(), selection.getFrequency(), null, null, null, 1, null, null,
				selection.getCustomId() };
	 ...
	 * 
	 * @PROJECTION_MASTER_SID INT ,@USER_ID VARCHAR(100) ,@SESSION_ID VARCHAR(100)
	 * ,@NETTING BIT ,@VIOLATION_DATE BIT
	 */
	...
	Runnable createRunnable(final String procedureName, final Object[] orderedArgs,
			final ProjectionSelectionDTO selection) {
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				CommonUtil.getInstance().waitsForOtherThreadsToComplete(
						selection.getSessionDTO().getFutureValue(Constant.PPA_PROCEDURE_CALL));
	...
	String[] calcWacPriceChange(String rsId, String periodSid, String year, String frequencyValue) {
		...
		try {
			...
			int historyPeriod = Integer.parseInt(periodSid)
					- (currentfrquencyForWacReset + periodList.indexOf(periodSid));
			int baseperiod = Integer.parseInt(periodSid) - periodList.indexOf(periodSid);
			wac_price[NumericConstants.ONE] = calculateWacPriceChange(searchWacPrice(rsId, String.valueOf(baseperiod)),
					searchWacPrice(rsId, String.valueOf(historyPeriod)));
			return wac_price;
	...
	String[] fetchPeriod_Sids(String year, String frequencyValue) {
		...
		try {
			...
			for (int i = 0; i < periodTableList.size(); i++) {
				...
				if (String.valueOf(object[NumericConstants.ONE]).equals(year)
						&& (currentfrquencyForWacReset == NumericConstants.TWELVE
	...
	String calculateWacPriceChange(String current, String history) {
		try {
			Double finalValue = (Double.parseDouble(current) - Double.parseDouble(history))
					/ Double.parseDouble(history);
			return String.valueOf(finalValue.isInfinite() || finalValue.isNaN() ? Constants.ZERO
					: finalValue * NumericConstants.HUNDRED);
		} catch (NumberFormatException ex) {
			LOGGER.error(ex.getMessage());
	...
	String searchWacPrice(String rsId, String periodSId) {
		try {
			...
			for (i = NumericConstants.ZERO; i < wacPriceTableList.size(); i++) {
				...
				if (((String.valueOf(object[NumericConstants.ONE]).equals(rsId))
						&& String.valueOf(object[NumericConstants.TWO]).equals(String.valueOf(periodSId)))) {
