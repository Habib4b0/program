...
public class CFPCompanyAddition extends CustomComponent {
    ...
    public CFPCompanyAddition(final ErrorfulFieldGroup binder, CFPCompanyDTO cfpMaster, final SessionDTO sessionDTO,CFPCompanies cfpCompanies)  {
        this.binder = binder;
        ...
        try {
            ...
            final Map<String, AppPermission> functionCompanyHM = stplSecurity.getBusinessFunctionPermission(userId, UISecurityUtil.COMPANY_FAMILY_PLAN+ConstantsUtils.COMMA+TabNameUtil.CFP_COMPANY_ADDITION);
    ...
    /**
     * Initialize the CFPCompanyAddition UI and configure its fields.
     */
    private void init() {
            ...
            public void buttonClick(final Button.ClickEvent event) {
                try {
                    ...
                    if (searchFieldObject == null) {
                        final MessageBox msg = MessageBox.showPlain(Icon.ERROR, ConstantsUtils.ERROR, "Select the search field", new MessageBoxListener() {
                            /**
     ...
     */
    private void loadAvailbleTable() throws PortalException, SystemException {
         final StplSecurity stplSecurity = new StplSecurity();
            userId = String.valueOf(VaadinSession.getCurrent().getAttribute(ConstantsUtils.USER_ID));
            ...
            public void buttonClick(final Button.ClickEvent event) {
                try {
                    ...
                    for (CompanyMasterDTO company : companyDto) {
                        if (company != null && !cfpLogic.moveToRightDuplicateValidation(company, lazyUtilDTO)) {
    ...
    public void addSelectedTable() {
        try {
            LOGGER.debug("Entering addSelectedTable");
             final StplSecurity stplSecurity = new StplSecurity();
                userId = String.valueOf(VaadinSession.getCurrent().getAttribute(ConstantsUtils.USER_ID));
                final Map<String, AppPermission> fieldCfpHM = stplSecurity.getFieldOrColumnPermission(userId, UISecurityUtil.COMPANY_FAMILY_PLAN+ConstantsUtils.COMMA+TabNameUtil.CFP_COMPANY_ADDITION,false);
                List<Object> resultList = ifpLogic.getFieldsForSecurity(UISecurityUtil.COMPANY_FAMILY_PLAN, TabNameUtil.CFP_COMPANY_ADDITION);
            ...
            LOGGER.debug("Ending addSelectedTable");
        } catch (PortalException ex) {
            java.util.logging.Logger.getLogger(CFPCompanyAddition.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SystemException ex) {
            java.util.logging.Logger.getLogger(CFPCompanyAddition.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void addResultsTable() {
        try {
            LOGGER.debug("Entering CFPViewForm addSelectedTable");
           ...
           selectedItemResultLazyBean = new LazyBeanItemContainer(CFPCompanyDTO.class, new CompanyDetailsContainer(viewResultsTable, Boolean.TRUE,
            ...
            LOGGER.debug("Ending CFPViewForm addSelectedTable");
        } catch (PortalException ex) {
            java.util.logging.Logger.getLogger(CFPCompanyAddition.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SystemException ex) {
            java.util.logging.Logger.getLogger(CFPCompanyAddition.class.getName()).log(Level.SEVERE, null, ex);
