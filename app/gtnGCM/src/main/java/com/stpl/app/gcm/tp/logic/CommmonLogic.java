...
public class CommmonLogic {
    ...
    public List<IdDescriptionDTO> loadDdlbs(String fieldName) {
        ...
        List<IdDescriptionDTO> resultDdlbList = new ArrayList<>();
        IdDescriptionDTO idDescription = null;
        try {
            DynamicQuery dynamicListQuery = HelperTableLocalServiceUtil.dynamicQuery();
            final ProjectionList productDropDownProjectionList = ProjectionFactoryUtil.projectionList();
            productDropDownProjectionList.add(ProjectionFactoryUtil.property("helperTableSid"));
            productDropDownProjectionList.add(ProjectionFactoryUtil.property("description"));
            dynamicListQuery.add(RestrictionsFactoryUtil.eq("listName", fieldName));
            dynamicListQuery.setProjection(productDropDownProjectionList);
            resultDatalist = HelperTableLocalServiceUtil.dynamicQuery(dynamicListQuery);

            if (!resultDatalist.isEmpty()) {
                for (int i = 0; i < resultDatalist.size(); i++) {
                    Object[] obj = (Object[]) resultDatalist.get(i);
                    idDescription = new IdDescriptionDTO(Integer.parseInt(String.valueOf(obj[0])), String.valueOf(obj[1]));
                    resultDdlbList.add(idDescription);
                }
            }
    ...
    public String buildContractSearchQuery(ContractSelectionDTO conSelDTO, String userId, String sessionId, int start, int end, Set<Container.Filter> filters) throws ParseException {
        ...
        List<String> companyMasterContractSidsList;
        if (conSelDTO.getModuleName().equals(PROJECTION_DETAILS_TRANSFER.getConstant()) && conSelDTO.getScreenName().equals(TAB_TRANSFER_CONTRACT.getConstant())) {
            companyMasterContractSidsList = conSelDTO.getPhCompanyMasterSids();
        } else {
        ...
        if (filters != null) {
            for (Container.Filter inputDataFilter : filters) {
                if (inputDataFilter instanceof SimpleStringFilter) {
                    SimpleStringFilter stringQueryFilter = (SimpleStringFilter) inputDataFilter;
                    String filterString = "%" + stringQueryFilter.getFilterString() + "%";
                    dataParameters.put(FILTER + stringQueryFilter.getPropertyId(), filterString);
                } else if (inputDataFilter instanceof Between) {
                    Between betweenQueryFilter = (Between) inputDataFilter;
                    Date startValue = (Date) betweenQueryFilter.getStartValue();
                    Date endValue = (Date) betweenQueryFilter.getEndValue();
                    dataParameters.put(FILTER + betweenQueryFilter.getPropertyId() + StringConstantsUtil.FROM_FILTER, String.valueOf(startValue));
                    dataParameters.put(FILTER + betweenQueryFilter.getPropertyId() + StringConstantsUtil.TO_FILTER, String.valueOf(endValue));
                } else if (inputDataFilter instanceof Compare) {
                    Compare compareDate = (Compare) inputDataFilter;
                    Compare.Operation operationDate = compareDate.getOperation();
                    Date value = (Date) compareDate.getValue();
                    if (Compare.Operation.GREATER_OR_EQUAL.toString().equals(operationDate.name())) {
                        dataParameters.put(FILTER + compareDate.getPropertyId() + StringConstantsUtil.FROM_FILTER, String.valueOf(value));
                    } else {
                        dataParameters.put(FILTER + compareDate.getPropertyId() + StringConstantsUtil.TO_FILTER, String.valueOf(value));
                    }
                }
            }
        }
        ...
        if (dataParameters.containsKey(StringConstantsUtil.FILTERR_AR_CATEGORY)) {
            ...
            if (dataParameters.get(StringConstantsUtil.FILTERR_AR_CATEGORY) != null) {
    ...
    public int callStatusInsert(int status, ContractResultDTO dto, SessionDTO session, String contractType) {
        int count = 0;
        StringBuilder queryInputString = new StringBuilder("   ");
        queryInputString.append("INSERT INTO GCM_GLOBAL_DETAILS ");
        if (!contractType.equals(StringUtils.EMPTY) && !contractType.equals(Constants.NULL)) {
            queryInputString.append(" (STATUS,CONTRACT_MASTER_SID,CFP_CONTRACT_SID,IFP_CONTRACT_SID,RS_CONTRACT_SID,PS_CONTRACT_SID,USER_ID,SESSION_ID,SCREEN_NAME)  ");
        } else {
            queryInputString.append(" (STATUS,CONTRACT_MASTER_SID,CFP_CONTRACT_SID,IFP_CONTRACT_SID,RS_CONTRACT_SID,PS_CONTRACT_SID,USER_ID,SESSION_ID)  ");
        }

        queryInputString.append("VALUES(  ");
        queryInputString.append('\'').append(status).append('\'');
        queryInputString.append(',').append(!dto.getContractMasterSid().equals(StringUtils.EMPTY) ? dto.getContractMasterSid() : 0);
        queryInputString.append(',').append(!dto.getCfpContSid().equals(StringUtils.EMPTY) ? dto.getCfpContSid() : 0);
        queryInputString.append(',').append(!dto.getIfpContSid().equals(StringUtils.EMPTY) ? dto.getIfpContSid() : 0);
        queryInputString.append(',').append(!dto.getRsContSid().equals(StringUtils.EMPTY) ? dto.getRsContSid() : 0);
        queryInputString.append(',').append(!dto.getPsContSid().equals(StringUtils.EMPTY) ? dto.getPsContSid() : 0);
        queryInputString.append(",'").append(session.getUserId()).append('\'');
        queryInputString.append(",'").append(session.getSessionId()).append('\'');
        if (!contractType.equals(StringUtils.EMPTY) && !contractType.equals(Constants.NULL)) {
            queryInputString.append(",'").append(contractType).append('\'');
        }
        queryInputString.append("  )");
        count = HelperTableLocalServiceUtil.executeUpdateQueryCount(queryInputString.toString());
    ...
    public static String generateCustomerMappings(List<String> sourceCompanies, List<String> destinationCompanies) {
        ...
        for (int i = 0; i < sourceCompanies.size(); i++) {
            customerMapping.append(String.valueOf(sourceCompanies.get(i)) ).append( " - " ).append( String.valueOf(destinationCompanies.get(i)));
