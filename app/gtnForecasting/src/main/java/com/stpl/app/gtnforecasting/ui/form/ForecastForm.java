...
public class ForecastForm extends AbstractForm {
    ...
    protected void initializeTabs() {
        ...
        try {
            ...
            if (screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_NONMANDATED)) {
                ...
                tabSheet.addTab(salesProjectionResults, TAB_SALES_PROJECTION_RESULTS.getConstant(), null, NumericConstants.THREE);
                tabSheet.addTab(discountProjection, TAB_DISCOUNT_PROJECTION.getConstant(), null, NumericConstants.FOUR);
                tabSheet.addTab(discountProjectionResults, TAB_DISCOUNT_PROJECTION_RESULTS.getConstant(), null, NumericConstants.FIVE);
                tabSheet.addTab(ppaProjectionResults, TAB_PPA_PROJECTION_RESULTS.getConstant(), null, NumericConstants.SIX);
                tabSheet.addTab(nonmandatedprojectionResults, TAB_PROJECTION_RESULTS.getConstant(), null, NumericConstants.SEVEN);
    ...
    protected void btnPreviousLogic() {
        if (screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_MANDATED) || screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_NONMANDATED)) {
            ...
            if (isCommercialGovernment && lastPosition - i == 0) {
                 CommonUtil.getInstance().waitsForOtherThreadsToComplete(session.getFutureValue(Constant.DATA_SELECTION_TAB_LOAD,0));
            }
    ...
    private boolean canBeSubmitted(String screenName) {
        if(CommonUtils.BUSINESS_PROCESS_TYPE_NONMANDATED.equals(screenName) && nmSalesProjection.getSubmitFlag() == true  && discountProjection.getSubmitFlag() == true) {
            return true;
        } else if(CommonUtils.BUSINESS_PROCESS_TYPE_MANDATED.equals(screenName) && salesProjectionForMandated.getSubmitFlag() == true) {
            return true;
        } else if(CommonUtils.BUSINESS_PROCESS_TYPE_RETURNS.equals(screenName)) {
            return true;
        }
    ...
    private void saveProjection() {
        ...
        try {
            if (screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_NONMANDATED)) {
                ...
                Future discountListView = service.submit(CommonUtil.getInstance().createRunnable(Constant.DISCOUNT_LIST_VIEW_SAVE,discountProjection));
                checkRunningThreads(); 
		...
		additionalInformation.saveNotesInformation(session.getProjectionId(), CommonUtils.BUSINESS_PROCESS_TYPE_NONMANDATED);
                ...
                //Below code will wait for the temp to main insertion to get complete
    ...
    private void submitProjToWorkflow(Map<String, Object> params, final String notes, final String screenName, final List<NotesDTO> getUploadedData) {
            ...
            List<ForecastingRulesDTO> list = DSCalculationLogic.getProjectionValues(session.getProjectionId(), session.getUserId(), session.getSessionId(),screenName,session);
            try {
    ...
    private void setTabSecurity()  {
        ...
        if (screenName.equals(CommonUtils.BUSINESS_PROCESS_TYPE_MANDATED)) {
            ...
            if (functionPsHM.get(FunctionNameUtil.NM_DATA_ASSUMPTIONS_TAB) != null && !((AppPermission) functionPsHM.get(FunctionNameUtil.NM_DATA_ASSUMPTIONS_TAB)).isTabFlag()) {
                tabSheet.getTab(1).setVisible(Boolean.FALSE);
            }
    ...
    private void callInsertProcedureOnGenerate(final SessionDTO session, final String screenName) {
        dsLogic.callInsertProcedure(session.getProjectionId(), session.getUserId(), session.getSessionId(),
                CommonUtils.BUSINESS_PROCESS_TYPE_RETURNS.equalsIgnoreCase(screenName) ? SalesUtils.RETURNS_SALES_INSERT_PRO_NAME
                : SalesUtils.MANDATED_PRO_NAME);
    }
    ...
    public boolean checkMandatedDiscountAvailablity(SessionDTO session) {
     ...
     /**
     * To run the dataselection tab in separate thread to reduce the time in DS generate,edit and view
     * It will be same for Government, Commercial and Returns.
     *
     * @param form
     * @return
     */
    ...
    /**
     * Used to wait for the Data Selection thread(Once the dataselection tab is loaded screen will be visible to the user).
     */
    ...
    /**
     * Commercial configuration for all the mode... Add,Edit and View. All the thread should be added in session.addFutureMap(); 
     * So that we can check all the threads are completed when we save or submit.
     *
     * In Add mode
     *
     * ProjHierarchyInsert-->> will be triggered to insert the values in Projection_Cust and Projection_Prod tables. It will be 
     * checked when we save or submit and also in data selection tab load, since the data selection tab will be loaded based on the 
     * Projection_cust and Projection_Prod tables
     *
     * Data Selection tab -->> will be triggered to load the data selection tab and it will be checked if the user goes to Data selection tab.
     * ProjHierarchyInsert thread will be sent as input to this thread. Data selection tab will be loaded based on the ProjHierarchyInsert, so once
     * the ProjHierarchyInsert is complete, data selection tab load will be started
     *
     * PROJECTION_DETAILS_INSERT-->> will be triggered to insert the data to the projection_details table. Projection details query will be differ for
     * generate and data selection tab change(If the user change the customer and product selection inside the data selection tab).
     *
     * DISCOUNT_PROCEDURE_CALL -->> it will trigger the discount insert procedure call and it will be checked in save and submit. And also in
     * Discount projection tab load.
     *
     *
     * In Edit Mode
     *
     * Main to temp insert--> Main to temp copy is called. Sales projection insert alone will be triggered and wait for its get complete since sales
     * is the initial screen. all other screen(Discount and PPA) insert will be triggered after sales.
     *
     * Data Selection tab -->> will be triggered to load the data selection tab load and it will be checked if the user goes to Data selection tab.
     *
     * DISCOUNT_PROCEDURE_CALL -->> it will trigger the discount insert procedure call. Inside this thread, it will wait for the discount main to
     * temp copy get complete and it will be checked in save and submit. And also in Discount projection tab load.
     */
    private void commercialConfiguration() {
        switch (session.getAction().toLowerCase()) {
            case Constant.ADD_FULL_SMALL:
                ...
                session.addFutureMap(Constant.PROJ_HIERARCHY_INSERT, new Future[]{
                    //PROJECTION_CUST_HIERARCHY INSERT CALL
                    service.submit(CommonUtil.getInstance().createRunnable(Constant.CUST_HIERARCHY_INSERT, dataSelectionDTO.getProjectionId(), dataSelectionDTO.getSelectedCustomerRelationSid(), Boolean.FALSE)),
                    //PROJECTION_PROD_HIERARCHY INSERT CALL
                    service.submit(CommonUtil.getInstance().createRunnable(Constant.PROD_HIERARCHY_INSERT, dataSelectionDTO.getProjectionId(), dataSelectionDTO.getSelectedProductRelationSid(), Boolean.FALSE))});
                //To load the data selection tab once the PROJECTION_CUST AND PROJECTION_PROD GET EXCUTE
                session.addFutureMap(Constant.DATA_SELECTION_TAB_LOAD, new Future[]{service.submit(CommonUtil.getInstance().createRunnable(Constant.DATA_SELECTION_TAB_LOAD, data, session.getFutureValue(Constant.PROJ_HIERARCHY_INSERT)))});
                //To insert the Projection details table
                session.addFutureMap(Constant.PROJECTION_DETAILS_INSERT, new Future[]{service.submit(CommonUtil.getInstance().createRunnable(Constant.PROJECTION_DETAILS_INSERT, dataSelectionDTO.getProjectionId(), session.getCurrentTableNames(), Boolean.FALSE))});
                //Call sales Insert Procedure
                nmSalesInsertProcedure();
                ...
                //Call Discount Insert Procedure, inside this thread wait until the main to temp insert get complete. For this we passing CountDownLatch
                // --> Once the master procedure is excecuted then actual and discound procedure should be executed
                nmDiscountInsertProcedure();
      ...
      private void governmentConfiguration() {
        switch (session.getAction().toLowerCase()) {
            case Constant.ADD_FULL_SMALL:
                ...
                CommonUtil.getInstance().waitsForOtherThreadsToComplete(session.getFutureValue(Constant.SUPPLEMENTAL_INSERT_PRC));
                //Call supplement Insert Procedure
                mDiscountProcedure();
                break;
            case Constant.EDIT_SMALL:
    ...
    /**
     * This method is to check whether any thread is running or not
     * If it its running we will make it wait untill the thread get executed
     * 
 ...
 private void nmDiscountInsertProcedure() {
     ...
     /**
      * To call the PPA insert procedure and calculation procedure
      * calculation procedure will be triggered once the insert procedure is done
      * 
      */
     private void nmPPAProcedure() {
         Future ppaInsert = service.submit(CommonUtil.getInstance().createRunnableForPPAProcedureCall(SalesUtils.PRC_NM_PPA_INSERT, dataSelectionDTO.getProjectionId(), session,null));
         session.addFutureMap(Constant.PPA_PROCEDURE_CALL, new Future[]{ppaInsert});
     }
     private void nmPPAInitProcedure() {
         Future ppaInit = service.submit(CommonUtil.getInstance().createRunnableForPPAInitProcedure(SalesUtils.PRC_NM_PPA_PROJ_INIT, session));
      ...
      */
      private void supplementDiscountProcedure() {
         Future supplementDiscountInsert = service.submit(CommonUtil.getInstance().createRunnable(Constant.PROCEDURE_CALL, Constant.SUPPLEMENTAL_INSERT_PRC, session.getProjectionId(), session.getMarketTypeValue(), session.getUserId(), session.getSessionId()));
