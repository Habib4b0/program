 ...
 * @author santanukumar
 */
public class SelectedProductTableLogic extends PageTableLogic{
    SessionDTO sessionDTO;
    boolean generate=false;
    DataSelectionLogic dataSelectionLogic=new DataSelectionLogic();
    CustomerProductDTO binderDto =new CustomerProductDTO();
    public static final Logger LOGGER = Logger.getLogger(SelectedProductTableLogic.class);

    public SelectedProductTableLogic() {
    }
    ...
    public int getCount() {
        if (generate) {
            try {
                return dataSelectionLogic.getCommonCountForSelectedCP(binderDto,getFilters(),sessionDTO);
            } catch (Exception ex) {
               LOGGER.error(ex);
            }
        }
    ...
    public List loadData(int start, int offset) {
        ...
        try {
            resultList=dataSelectionLogic.getCommonResultForSelectedCP(binderDto,getFilters(),sessionDTO);
        } catch (Exception ex) {
            LOGGER.error(ex);
        }
         return resultList;
