...
public class SummarySalesProjectionTransfer extends VerticalLayout implements View {
    ...
    private CustomTreeContainer<SalesTabDTO> resultBean = new CustomTreeContainer<SalesTabDTO>(SalesTabDTO.class);
    /**
    ...
    private CustomTreeContainer<SalesTabDTO> excelResultBean = new CustomTreeContainer<SalesTabDTO>(SalesTabDTO.class);
    ...
    public void configureTable() {
        ...
        leftTable.setColumnWidth("levelValue", 400);
    ...
    public void loadResultTable(boolean isFirst) {
        ...
        if(!session.getNewProjectionId().equals(StringUtils.EMPTY)){
           
                String projectionId1=String.valueOf(session.getNewProjectionId()); 
            projId=Integer.valueOf(projectionId1);
        }
    ...
    private void configureExcelResultTable() {
        excelResultBean = new CustomTreeContainer<SalesTabDTO>(SalesTabDTO.class);
    ...
    private void loadExcelResultTable() {
        ...
        int projectionId = CommonLogic.getProjectionIdForSubmittedContract(session.getSessionId(), false);
        int count = tabLogic.getLevelCount(new Object(), selectionDTO, projectionId, true, session);
        List<SalesTabDTO> resultList = tabLogic.getConfiguredSalesTabResults(new Object(), 0, count, selectionDTO, true, session);
    ...
    public void addLowerLevelsForExport(Object id) {
        int projectionId = CommonLogic.getProjectionIdForSubmittedContract(session.getSessionId(), false);
        int count = tabLogic.getLevelCount(id, selectionDTO, projectionId, true, session);
        List<SalesTabDTO> resultList = tabLogic.getConfiguredSalesTabResults(id, 0, count, selectionDTO, true, session);
