...
public class CDRLogic {
     ...
     * LOGGER.info(queryBuilder.toString()); List<Object> masterData =
     * (List<Object>)
     ...
     * @return the dynamic query search
     */
    public int getCDRCount(final ErrorfulFieldGroup searchFields, final Set<Container.Filter> filterSet) throws Exception {
        int count = 0;
    ...
    public List<SearchResultsDTO> loadCDRResults(
        ...
        LOGGER.info(" Final Query ----- >>> " + String.valueOf(queryBuilder));
    ...
    private StringBuilder buildSearchQuery(ErrorfulFieldGroup searchFields, boolean isCount) {
        ...
        for (String fields : keys) {
            ...
            if ((ConstantsUtils.COMBO1.equals(fields) || ConstantsUtils.COMBO6.equals(fields)) && searchFields.getField(fields).getValue() != null && !ConstantUtil.SELECT_ONE.equals(searchFields.getField(fields).getValue().toString())) {
                queryBuilder.append(" AND ").append(criteria.get(fields)).append(" LIKE '").append(String.valueOf(((com.stpl.app.util.HelperDTO) searchFields.getField(fields).getValue()).getId())).append(ConstantsUtils.SINGLE_QUOTE);
    ...
    private List<SearchResultsDTO> getCustomizedSearchFormToDTO(List list) {
        ...
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                ...
                searchDto.setRuleNo(!ConstantsUtils.NULL.equals(String.valueOf(object[2])) && StringUtils.isNotBlank(String.valueOf(object[2])) ? String.valueOf(object[2]) : StringUtils.EMPTY);
                searchDto.setRuleName(!ConstantsUtils.NULL.equals(String.valueOf(object[3])) && StringUtils.isNotBlank(String.valueOf(object[3])) ? String.valueOf(object[3]) : StringUtils.EMPTY);
                searchDto.setCdrCreatedDate(object[5] != null && StringUtils.isNotBlank(String.valueOf(object[5])) && !ConstantsUtils.NULL.equals(String.valueOf(object[5])) ? (Date) object[5] : null);
                searchDto.setCdrModifiedDate(object[7] != null && StringUtils.isNotBlank(String.valueOf(object[7])) && !ConstantsUtils.NULL.equals(String.valueOf(object[7])) ? (Date) object[7] : null);
                searchDto.setRecordLockStatus(false);
                if (!ConstantsUtils.NULL.equals(String.valueOf(object[6])) && StringUtils.isNotBlank(String.valueOf(object[6]))) {
                    try {
                        User createdUser = (User) securityDto.getUserByUserId(Long.valueOf(String.valueOf(object[6])));
                ...
                if (!ConstantsUtils.NULL.equals(String.valueOf(object[8])) && StringUtils.isNotBlank(String.valueOf(object[8]))) {
                    try {
                        User createdUser = (User) securityDto.getUserByUserId(Long.valueOf(String.valueOf(object[8])));
                ...
                if (!ConstantsUtils.NULL.equals(String.valueOf(object[4])) && StringUtils.isNotBlank(String.valueOf(object[4])) && !ConstantsUtils.ZERO.equals(String.valueOf(object[4]))) {
                    searchDto.setRuleCategory(HelperListUtil.getInstance().getIdDescMap().get(Integer.valueOf(String.valueOf(object[4]).trim())));
            ...
            for (Container.Filter filter : filterSet) {
                if (filter instanceof SimpleStringFilter) {
                    ...
                    if (!"createdBy".equals(stringFilter.getPropertyId().toString()) && !"modifiedBy".equals(stringFilter.getPropertyId().toString())) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" LIKE '%").append(stringFilter.getFilterString()).append("%'");
                    } else {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" = '").append(stringFilter.getFilterString()).append("'");
                    }
                    ...
                    if (startValue != null) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(betweenFilter.getPropertyId().toString())).append(" >= '").append(dateFormat.format(startValue)).append("' ");
                    }
                    if (endValue != null) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(betweenFilter.getPropertyId().toString())).append(" <= '").append(dateFormat.format(endValue)).append("' ");
                    }
                    ...
                    if (Compare.Operation.GREATER_OR_EQUAL.toString().equals(operation.name())) {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" >= '").append(dateFormat.format(value)).append("' ");
                    } else {
                        stringBuilder.append(" AND ").append(constantProperties.getString(stringFilter.getPropertyId().toString())).append(" <= '").append(dateFormat.format(value)).append("' ");
                    }
                }
    ...
    public static List getSavedRuleDetails(final int systemId) throws Exception {
    ...
    public void saveOrUpdateNotesTab(List<NotesDTO> uploadedData, List<NotesDTO> removeDetailsList, int cdrModelSid) {
        ...
        try {
            LOGGER.info("removeDetailsList.size()" + removeDetailsList.size());
