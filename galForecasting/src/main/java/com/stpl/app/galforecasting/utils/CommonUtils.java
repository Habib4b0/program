...
import static com.stpl.app.galforecasting.ui.form.lookups.AltHistorySelection.getMonthMap;
import static com.stpl.app.galforecasting.utils.CommonUtils.getDateTime;
import static com.stpl.app.galforecasting.utils.CommonUtils.getEndMonth;
import static com.stpl.app.galforecasting.utils.CommonUtils.getPeriod;
import static com.stpl.app.galforecasting.utils.CommonUtils.getStartMonth;
...
public class CommonUtils {
    ...
    public static Calendar getCalendar() {
        ...
        LOGGER.info("Entering getCalendar method");

        // TMPSOLUTION - Hardcoded for temporary solution
        now.set(now.get(Calendar.YEAR), Calendar.JANUARY, 5);
        LOGGER.info("End of getCalendar method");
        return now;
    }
    ...
    public static int[] getQSPeriodDetails(ProjectionSelectionDTO projSelDTO, int freqDiv) {
        int[] ret = new int[7];
        ret[0] = freqDiv;
        ret[1] = getPeriod(projSelDTO.getForecastDTO().getHistoryStartMonth(), 12 / freqDiv);
        ...
        ret[5] = getPeriod(projSelDTO.getForecastDTO().getProjectionStartMonth() + 1, 12 / freqDiv);
...
//Added for mailNotification
    ...
    public static Map<String, String> getAllUsers() throws PortalException, SystemException {
        ...
        try {
            ...
            userList = null;
            LOGGER.error(ex.getMessage() + " in CommonUtils ~ getAllUsers");
        }
        return userMap;
    }
    ...
    public static Map<String, String> getAllUserIds() throws PortalException, SystemException {
        ...
        try {
            ...
            for (int loop = 0, limit = userList.size(); loop < limit; loop++) {
                Object array[] = (Object[]) userList.get(loop);
                userMap.put((String.valueOf(array[2]) + ", " + String.valueOf(array[1])), String.valueOf(array[0]));
            }
        } catch (Exception ex) {
            userList = null;
            LOGGER.error(ex.getMessage() + " in CommonUtils ~ getAllUsers");
    ...
    public static Map<String, String> getAllDiscounts() throws PortalException, SystemException {
        ...
        try {
            ...
            discountList = null;
            LOGGER.error(ex.getMessage() + " in CommonUtils ~ getAllDiscounts");
        }
        return userMap;
    }
    ...
    public static String getReturnsProperties(String key) {
        LOGGER.info("getReturnsProperties--> " + key);
    ...
    public String formDate(String value, String freq, boolean isFromDate) {
        LOGGER.info("formDate method Starts----->"+freq);
        ...
        if (freq.equals(Constant.QUARTERLY)) {
            ...
            int period = monthMap.get(StringUtils.capitalize(value.substring(value.length() - 7, value.length() - 4)));
