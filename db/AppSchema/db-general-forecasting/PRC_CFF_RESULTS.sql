...
WHERE CF.CFF_MASTER_SID = @FIRST_PROJ_SID
	...
	AND (
		...
		AND EXISTS (SELECT 1 FROM #PRIOR_RS_DATA A WHERE A.RS_CONTRACT_SID=RC.RS_CONTRACT_SID AND A.CCP_DETAILS_SID=CC.CCP_DETAILS_SID)
--------------------------PULLING DEDUCTION INCLUSION(GALUAT_377)  FOR CFF---------------  
IF NOT EXISTS (
		SELECT *
...
JOIN #DISCOUNT_INFORMATION DI ON NSP.PROJECTION_DETAILS_SID = DI.PROJECTION_DETAILS_SID
	AND NSP.RS_CONTRACT_SID = DI.RS_CONTRACT_SID
...
JOIN #DISCOUNT_INFORMATION DI ON NAD.PROJECTION_DETAILS_SID = DI.PROJECTION_DETAILS_SID
	AND NAD.RS_CONTRACT_SID = DI.RS_CONTRACT_SID
...
SELECT @FIRST_PROJ_SID
	...
	,ISNULL(PFD.EX_FACTORY_SALES_ACTUALS, 0)- ISNULL((CASE 
		WHEN @DEDUCTION_INC = 0
			THEN NULL
		ELSE ISNULL(ND.ACTUAL_SALES, 0) + ISNULL(PPA_ACTUAL_SALES, 0) + ISNULL(SUPPLEMENTAL_DISCOUNT_ACTUALS, 0) + ISNULL(MANDATED_DISCOUNT_ACTUALS, 0)
		END), 0)
	,ISNULL(PFD.EX_FACTORY_SALES_PROJ, 0)-ISNULL((CASE 
		WHEN @DEDUCTION_INC = 0
			THEN NULL
		ELSE ISNULL(ND.PROJECTION_SALES, 0) + ISNULL(PPA_DISCOUNT_PROJECTED, 0) + ISNULL(SUPPLEMENTAL_DISCOUNT_PROJECTED, 0) + ISNULL(MANDATED_DISCOUNT_PROJECTED, 0)
		END), 0)
	,ISNULL(((ISNULL(PFD.EX_FACTORY_SALES_ACTUALS, 0)- ISNULL((CASE 
		WHEN @DEDUCTION_INC = 0
			THEN NULL
		ELSE ISNULL(ND.ACTUAL_SALES, 0) + ISNULL(PPA_ACTUAL_SALES, 0) + ISNULL(SUPPLEMENTAL_DISCOUNT_ACTUALS, 0) + ISNULL(MANDATED_DISCOUNT_ACTUALS, 0)
		END), 0))/NULLIF(PFD.EX_FACTORY_SALES_ACTUALS, 0)),0)*100
	,ISNULL(((ISNULL(PFD.EX_FACTORY_SALES_PROJ, 0)-ISNULL((CASE 
		WHEN @DEDUCTION_INC = 0
			THEN NULL
		ELSE ISNULL(ND.PROJECTION_SALES, 0) + ISNULL(PPA_DISCOUNT_PROJECTED, 0) + ISNULL(SUPPLEMENTAL_DISCOUNT_PROJECTED, 0) + ISNULL(MANDATED_DISCOUNT_PROJECTED, 0)
		END), 0))/NULLIF(PFD.EX_FACTORY_SALES_PROJ, 0)),0)*100,
...
GROUP BY CFF_MASTER_SID
	...
	JOIN #PRIOR_DISC_INFORMATION PRDI ON C.PROJECTION_DETAILS_SID = PRDI.PROJECTION_DETAILS_SID
		...
		AND PRDI.RS_CONTRACT_SID = NSP.RS_CONTRACT_SID
	JOIN #PERIOD P ON P.PERIOD_SID = NSP.PERIOD_SID
	...
	JOIN #PRIOR_DISC_INFORMATION PRDI ON C.PROJECTION_DETAILS_SID = PRDI.PROJECTION_DETAILS_SID
		AND C.CFF_MASTER_SID = PRDI.CFF_MASTER_SID
		AND PRDI.RS_CONTRACT_SID = NAD.RS_CONTRACT_SID
	JOIN #PERIOD P ON P.PERIOD_SID = NAD.PERIOD_SID
...
SET @SQL += 'SUPPLEMENTAL_DISCOUNT_PROJECTED_PERCENTAGE_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN SUPPLEMENTAL_DISCOUNT_PROJECTED_PERCENTAGE END),
		...
		SET @SQL += 'COGS_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN 	COGS_ACCRUAL	END),
			 ...
			 ADJUSTED_DEMAND_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN 	ADJUSTED_DEMAND_ACCRUAL	END),
	         ...
	         IW_DETAILS_ACCRUAL_' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' = MAX(CASE WHEN ID = ' + Cast(@LOOP_CNTR AS VARCHAR(10)) + ' THEN 	IW_DETAILS_ACCRUAL	END),
