...
public class FileManagementTableLogic extends PageTableLogic {
    ...
    public int getCount() {
        ...
        try {
            if (isFirstLoad) {
                count = (Integer) searchLogic.getFileHistoryResults(fileType, country, 0, 0, this.getSortByColumns(), this.getFilters(), true);
            }
    ...
    public List loadData(int start, int offset) {
        ...
        if (isFirstLoad) {
            try {
                list = (List) searchLogic.getFileHistoryResults(fileType, country, start, offset, this.getSortByColumns(), this.getFilters(), false);
            } catch (Exception ex) {
                LOGGER.error(ex);
    ...
    public void configureSearchData(HelperDTO fileType, String country) {
        isFirstLoad = true; 
        isReset = false;
        this.clearAll();
