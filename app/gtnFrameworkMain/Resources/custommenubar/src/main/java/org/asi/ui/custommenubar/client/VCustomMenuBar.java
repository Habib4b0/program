 ...
 */
package org.asi.ui.custommenubar.client;
...
public class VCustomMenuBar extends SimpleFocusablePanel implements
        CloseHandler<PopupPanel>, KeyPressHandler, KeyDownHandler,
        FocusHandler, SubPartAware {

    // The hierarchy of VCustomMenuBar is a bit weird as VCustomMenuBar is the Paintable,
     ...
     * For internal use only. May be removed or replaced in the future.
     */
    public final VCustomMenuBar hostReference = this;
     ...
     * For internal use only. May be removed or replaced in the future.
     */
    public VCustomMenuBar collapsedRootItems;
    ...
    protected VCustomMenuBar visibleChildMenu;
    protected boolean menuVisible = false;
    protected VCustomMenuBar parentMenu;
    protected CustomMenuItem selected;
    protected VOverlay closePopup;
    protected CloseBar visibleCloseMenu;
//    protected CloseItem closeItem;
//    protected boolean isCloseMenuBar;
    protected boolean scrollable = false;
    protected int pageLength = 5;    
    protected int checkAllItem = 0;
    protected int checkedNo = 0;
    ...
    public VCustomMenuBar(boolean subMenu, VCustomMenuBar parentMenu) {

        items = new ArrayList<CustomMenuItem>();
        popup = null;
        closePopup = null;
        visibleChildMenu = null;
        visibleCloseMenu = null;
    ...
    public int getCheckAllItem() {
        return checkAllItem;
    }

    public void setCheckAllItem(int checkAllItem) {
...
//        DOM.appendChild(getContainerElement(), closeItem.getElement());
//    }
...
//        }
    ...
    public void onBrowserEvent(Event e) {
        ...
        if (DOM.eventGetType(e) == Event.ONLOAD) {
            VCustomMenuBar parent = getParentMenu();
            if (parent != null) {
                // The onload event for an image in a popup should be sent to
                // the parentItem, which owns the popup
                parent.iconLoaded();
            } else {
            ...
            switch (DOM.eventGetType(e)) {
                ...
                case Event.ONCLICK:
                    ...
                    if (subMenu) {
                        ...
                        VCustomMenuBar parent = parentMenu;
    ...
    public void itemClick(CustomMenuItem item) {
        if (item.isItemClickNotClosable()) {
            item.setTempSelected(item.isItemClickSelectable() && !item.isChecked());
            ...
            VCustomMenuBar itemParent = item.getParentMenu();
            VCustomMenuBar ob=itemParent;
            while (ob.uidlId == null || ob.client == null) {
                ob = ob.getParentMenu();
            }
        List<CustomMenuItem> menuItems = itemParent.getItems();
        int size = menuItems.size();
        boolean value =false;
        if (item.isCheckAll()) {
            for (CustomMenuItem siblingItem : menuItems) {
                if (!item.equals(siblingItem)) {
                    siblingItem.setTempSelected(siblingItem.isItemClickSelectable() && item.isChecked());
                    siblingItem.setChecked(item.isChecked());
                }
            }
        } else {
            int x = 0;
            value = itemParent.getCheckedNo() == (size - itemParent.getCheckAllItem());
            for (int i = 0; i < size && x < itemParent.getCheckAllItem(); i++) {
                CustomMenuItem siblingItem = menuItems.get(i);
                if (siblingItem.isCheckAll()) {
                    siblingItem.setTempSelected(value && siblingItem.isItemClickSelectable());
                    siblingItem.setChecked(value);
                    x++;
                }
            }
        }
        if (item.isItemClickable() && !item.isSeparator()) {
            if (ob.uidlId != null && ob.client != null) {
                ob.client.updateVariable(ob.uidlId, "chkValue", value, false);
                ob.client.updateVariable(ob.uidlId, "checkAllItem", itemParent.getCheckAllItem(), false);
                ob.client.updateVariable(ob.uidlId, "itemChecked", item.isChecked(), false);
                ob.client.updateVariable(ob.uidlId, "itemClickId", item.getId(), true);
            }
        }
    }
    ...
    public void itemOver(CustomMenuItem item) {
        if (visibleCloseMenu != item.getCloseMenu()
                && closePopup != null) {
            closePopup.hide();
        }
        if (item.isClosable() && !item.isSeparator()) {
            showCloseMenu(item);
        }
        if ((openRootOnHover || subMenu || menuVisible) && !item.isSeparator()) {
            if (item.isItemHoverSelectable()) {
                setSelected(item);
            }
    ...
    public void showCloseMenu(CustomMenuItem item) {
        if (item.getSubMenu() != null) {
            if (item.getSubMenu().closePopup != null) {
                item.getSubMenu().closePopup.removeFromParent();
                item.getSubMenu().closePopup.hide();
            }
        }
        if (getParentMenu() != null) {
            if (getParentMenu().closePopup != null) {
                getParentMenu().closePopup.removeFromParent();
                getParentMenu().closePopup.hide();
            }
        }
        if (closePopup != null) {
            closePopup.removeFromParent();
        ...
        if (client != null) {
            client.getVTooltip().connectHandlersToWidget(closePopup);
        }
        closePopup.setWidget(item.getCloseMenu());
        closePopup.addCloseHandler(this);
        closePopup.addAutoHidePartner(item.getElement());

        // at 0,0 because otherwise IE7 add extra scrollbars (#5547)
        int left = item.getAbsoluteLeft() + item.getOffsetWidth() - 5;
        int top = item.getAbsoluteTop() + 2;
        closePopup.setPopupPosition(left, top);
        closePopup.show();
        closePopup.setStyleName("v-closebar-popup");
        visibleCloseMenu = item.getCloseMenu();
    }

    public void hideCloseMenu(CustomMenuItem item) {
        if (visibleCloseMenu != null
                && !(visibleCloseMenu == item.getCloseMenu())) {
            closePopup.hide();
        }
    }
    ...
    private static class LazyCloser extends Timer {
        ...
        private VCustomMenuBar activeRoot;
        ...
        public static void prepare(VCustomMenuBar vMenuBar) {
            if (INSTANCE == null) {
    ...
    protected void showChildMenuAt(CustomMenuItem item, int top, int left) {
        ...
        // handling of overlay widgets (will direct queries to parentItem menu)
        if (parentMenu == null) {
            popup.setOwner(this);
        ...
        visibleChildMenu.setScrollable(visibleChildMenu.getParentMenu().isScrollable());
        visibleChildMenu.setPageLength(visibleChildMenu.getParentMenu().getPageLength());
        if (visibleChildMenu.isScrollable()) {
            int hieght = 0;

            for (CustomMenuItem items : visibleChildMenu.getItems()) {
                int hieght1 = items.getElement().getOffsetHeight();
                if (hieght1 > hieght) {
                    hieght = hieght1;
                }
            }
            visibleChildMenu.addStyleName(SCROLLABLE);
            hieght = hieght * visibleChildMenu.getPageLength();
            visibleChildMenu.getElement().getStyle().setHeight(hieght, Unit.PX);
        } else {
            visibleChildMenu.removeStyleName(SCROLLABLE);
            visibleChildMenu.getElement().getStyle().clearHeight();
        }
    ...
    public void onClose(CloseEvent<PopupPanel> event) {
        ...
        closePopup = null;
        if (uidlId != null && client != null) {
            client.updateVariable(uidlId, "closeChildEvent", true, true);
        }
    }
    ...
    /**
     * Recursively hide all parentItem menus
     */
    ...
    /**
     * Returns the parentItem menu of this menu, or null if this is the
     * top-level menu
     *
     * @return
     */
    public VCustomMenuBar getParentMenu() {
        return parentMenu;
    }

    /**
     * Set the parentItem menu of this menu
     *
     * @param parent
     */
    public void setParentMenu(VCustomMenuBar parent) {
        parentMenu = parent;
    }
    ...
    public static class CloseBar extends SimpleFocusablePanel {

        protected Element closeElement;
        CloseItem closeItem;
        CustomMenuItem item;

        public CloseBar(CustomMenuItem item) {
            this.item = item;
            closeElement = getElement();
            closeItem = new CloseItem();
            DOM.appendChild(getContainerElement(), closeItem.getElement());
            setStyleName("v-closebar");
            closeItem.setStyleName("v-closebar-closeitem");
            sinkEvents(Event.ONCLICK | Event.ONMOUSEOVER | Event.ONMOUSEOUT
                    | Event.ONLOAD);
        }
        ...
        public void onBrowserEvent(Event e) {
            ...
            if (DOM.eventGetType(e) == Event.ONCLICK) {
                VCustomMenuBar ob = item.getParentMenu();
                while (ob.uidlId == null || ob.client == null) {
                    ob = ob.getParentMenu();
                }
                if (ob.uidlId != null && ob.client != null) {
                    ob.client.updateVariable(ob.uidlId, "closeId", item.getId(), true);
                }
            }
        }
    }

    public static class CloseItem extends Widget {

        protected Element closeElement;

        public CloseItem() {

            closeElement = DOM.createSpan();
            setElement(closeElement);
    ...
    public static class CustomMenuItem extends Widget implements HasHTML {
        ...
        protected VCustomMenuBar subMenu = null;
        protected VCustomMenuBar parentMenu = null;
        ...
        protected boolean clickNotClosable = false;
        protected boolean checkAll = false;
        protected CloseBar closeMenu;
        protected int id;
        protected boolean clickSelectable = false;
        protected boolean hoverSelectable = true;        
        ...
        public void setTempSelected(boolean selected) {
            this.selected = selected;
        }
        ...
        public void setChecked(boolean checked) {
            boolean oldChk = this.checked;
            if (checkable && !isSeparator) {
                this.checked = checked;
            } else {
                this.checked = false;
            }
            if (oldChk != this.checked&&!isCheckAll()) {
                if (this.checked) {
                    getParentMenu().setCheckedNo(getParentMenu().getCheckedNo()+1);
                } else {
                    getParentMenu().setCheckedNo(getParentMenu().getCheckedNo()-1);
                }
            }
        ...
        public void setItemClickNotClosable(boolean clickNotClosable) {
            this.clickNotClosable = clickNotClosable;
        }

        public boolean isItemClickNotClosable() {
            return clickNotClosable;
        }
        ...
        public void setItemHoverSelectable(boolean hoverSelectable) {
            this.hoverSelectable = hoverSelectable;
        }

        public boolean isItemHoverSelectable() {
            return hoverSelectable;
        }

        public void setItemClickSelectable(boolean clickSelectable) {
            this.clickSelectable = clickSelectable;
        }
        ...
        public void setSubMenu(VCustomMenuBar subMenu) {
            this.subMenu = subMenu;
        }

        public VCustomMenuBar getSubMenu() {
            return subMenu;
        }

        public void setParentMenu(VCustomMenuBar parentMenu) {
        ...
        protected void updateStyleNames() {
            if (parentMenu == null) {
                // Style names depend on the parentItem menu's primary style name so
                // don't do updates until the item has a parentItem
                return;
            }
        ...
        public CloseBar getCloseMenu() {
            return closeMenu;
        }

        public void setCloseMenu(CloseBar closeMenu) {
        ...
        public void setHTML(String html) {
            ...
            // events are handled by the parentItem VCustomMenuBar.
        ...
        public void updateFromUIDL(UIDL uidl, ApplicationConnection client) {
            setId(uidl.getIntAttribute("id"));
            setSeparator(uidl.hasAttribute("separator"));
            setClosable(uidl.getBooleanAttribute("closable"));
            setItemClickable(uidl.getBooleanAttribute("itemclickable"));
            setItemClickNotClosable(uidl.getBooleanAttribute("itemClickNotClosable"));           
            setCheckAll(uidl.getBooleanAttribute("checkAll"));
            setItemClickSelectable(uidl.getBooleanAttribute("itemClickSelectable"));
            setItemHoverSelectable(uidl.getBooleanAttribute("itemHoverSelectable"));
            
            if (isCheckAll()) {
                getParentMenu().setCheckAllItem(getParentMenu().getCheckAllItem()+1);
            }
            setEnabled(!uidl
    ...
    public boolean handleNavigation(int keycode, boolean ctrl, boolean shift) {
        ...
        if (keycode == getNavigationLeftKey()) {
            if (getSelected() == null) {
                ...
                // Inside a sub menu, whose parentItem is a root menu item
                VCustomMenuBar root = getParentMenu();
    ...
    private void openMenuAndFocusFirstIfPossible(CustomMenuItem menuItem) {
        VCustomMenuBar subMenu = menuItem.getSubMenu();
        if (subMenu == null) {
            // No child menu? Nothing to do
            return;
        }

        VCustomMenuBar parentMenu = menuItem.getParentMenu();
     ...
     *
     * @param element Element used in search
