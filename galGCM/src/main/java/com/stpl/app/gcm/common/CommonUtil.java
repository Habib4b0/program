...
//    private static final ResourceBundle confirmationMessage = ResourceBundle.getBundle("properties.message");
    ...
    public static ComboBox loadComboBoxForGCM(final ComboBox select, String listName, boolean isFilter) throws Exception {
        ...
        try {
            if (obj != null && key != null && !Constants.NULL.equals(key) && obj instanceof TextField) {
                ...
                if (rules[0] != null && ValidationUtils.getMessage(rules[0]) != null && StringUtils.isNotEmpty(ValidationUtils.getMessage(rules[0]))) {
                    ...
                    tempObj.addValidator(new StringLengthValidator(ValidationUtils.getMessage(rules[1]), Integer.valueOf(temp[0]), Integer.valueOf(temp[1]), Boolean.valueOf(temp[2])));
                }
                if (rules[2] != null && ValidationUtils.getMessage(rules[2]) != null && StringUtils.isNotEmpty(ValidationUtils.getMessage(rules[2]))) {
                    tempObj.addValidator(new RegexpValidator(ValidationUtils.getMessage(rules[2]), ValidationUtils.getMessage(rules[3])));
                }
            }
        } catch (Exception e) {
