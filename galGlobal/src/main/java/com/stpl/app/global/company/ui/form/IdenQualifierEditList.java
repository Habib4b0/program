...
public final class IdenQualifierEditList extends StplWindow {
     ...
     */
    public IdenQualifierEditList(final ComboBox companyQualifierName) throws SystemException, PortalException, Exception {
    ...
    public void configureFields() {
        ...
        companyQualifier.addValidator(new StringLengthValidator(
                "Company Qualifier Should be less than 25 characters", 0, 25, true));
        ...
        companyCrtQualifierName.addValidator(new StringLengthValidator(
                "Company CRT Qualifier Name Should be less than 100 characters", 0, 100, true));
        
        effectiveDates.setPageLength(7);
     ...
     */
    public VerticalLayout addToContent() throws SystemException, PortalException, Exception {
    ...
    public VerticalLayout addToContent1() throws SystemException, PortalException, Exception {
        final VerticalLayout content = new VerticalLayout();
        final StplSecurity stplSecurity = new StplSecurity();
        final String userId = String.valueOf(VaadinSession.getCurrent().getAttribute(ConstantsUtils.USER_ID));
    ...
    private void addToTable() throws Exception {
        ...
        qualifierTable.setPageLength(7);
        ...
        qualifierTable.addItemClickListener(new ItemClickListener() {
            ...
            public void itemClick(final ItemClickEvent event) {
                final CompanyCrtIdentifierDTO iden = companyQualifierBean.getItem(event.getItemId()).getBean();
                binder.setItemDataSource(new BeanItem<CompanyCrtIdentifierDTO>(iden));
    ...
    public GridLayout addToForm(final Map<String, AppPermission> fieldCompanyHM) {
        final CustomGridLayout grid = new CustomGridLayout(2, 1);
        grid.setWidth(null);
    ...
    @UiHandler("QualifierSave")
            public void buttonClick(final ClickEvent event) {
                LOGGER.info("Entering inside ItemQualifierEditList SAVE  method ");
                try {
                    binder.commit();
            ...
            LOGGER.info("Ending ItemQualifierEditList SAVE  method ");
     ...
     */
        @UiHandler("QualifierDelete")
            public void deleteButtonClick(final ClickEvent event) {
                try {
                    LOGGER.info("Entering inside ItemQualifierEditList  DELETE  method ");
                    binder.commit();        
                    ...
                    LOGGER.info("Ending ItemQualifierEditList  DELETE  method ");
                    }
                } catch (FieldGroup.CommitException ex) {
                    LOGGER.error(ex);
                } catch (Exception ex) {
        ...
        @UiHandler("IdentifierRemove")
            public void removeButtonClick(final ClickEvent event) {
                MessageBox.showPlain(Icon.QUESTION, commmonUtil.getHeaderMessage(), commmonUtil.getResetMessage(), new MessageBoxListener() {
                    ...
                    public void buttonClicked(final ButtonId buttonId) {
                        if (buttonId.name().equals(ConstantsUtils.YES)) {
                            LOGGER.info("Entering inside ItemQualifierEditList RESET  method ");
                            binder.discard();
                            binder.setItemDataSource(new BeanItem<CompanyCrtIdentifierDTO>(new CompanyCrtIdentifierDTO()));
                            btnSave.setCaption(ConstantsUtils.SAVE);
                            Object selectValue = qualifierTable.getValue();
                            qualifierTable.unselect(selectValue);
                            LOGGER.info("Ending ItemQualifierEditList RESET  method ");
                        }
                    }
                }, ButtonId.YES, ButtonId.NO);
    ...
    private void setDefaultTableColumns() {
        ...
        qualifierTable.setColumnCollapsingAllowed(true);
        int browserWidth = Page.getCurrent().getBrowserWindowWidth();
        try {

            if (browserWidth > 360) {
                for (Object propertyId : qualifierTable.getVisibleColumns()) {
                    qualifierTable.setColumnCollapsed(propertyId, false);
                }
            } else if (browserWidth < 360) {
                Object[] visibleColumns = qualifierTable.getVisibleColumns();
                for (Object propertyId : visibleColumns) {
                    qualifierTable.setColumnCollapsed(propertyId, false);
                }
                qualifierTable.setColumnCollapsed(visibleColumns[1], true);
            }
        } catch (Exception ex) {
           LOGGER.error(ex);
        }
